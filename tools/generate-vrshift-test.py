...
parser = argparse.ArgumentParser(description='VRShift microkernel test generator')
...
def split_ukernel_name(name):
  match = re.fullmatch(r"xnn_s16_vrshift_ukernel__(.+)_x(\d+)", name)
...
def generate_test_cases(ukernel, channel_tile, isa):
  """Generates all tests cases for a VRShift micro-kernel.
  ...
  return xngen.preprocess(VRSHIFT_TEST_TEMPLATE, {
