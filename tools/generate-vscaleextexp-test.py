...
parser = argparse.ArgumentParser(
  description='Vector ScaleExtExp microkernel test generator')
parser.add_argument("-t", "--tester", metavar="TESTER", required=True,
                    choices=["VScaleExtExpMicrokernelTester"],
...
VSCALEEXTEXP_TEST_TEMPLATE = """\
#define XNN_UKERNEL_WITH_PARAMS(arch_flags, ukernel, element_tile, datatype, params_type, init_params) \
XNN_TEST_VSCALEEXTEXP_ELEMENT_EQ(ukernel,arch_flags, ${", ".join(TEST_ARGS)});  
XNN_TEST_VSCALEEXTEXP_ELEMENT_DIV(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
XNN_TEST_VSCALEEXTEXP_ELEMENT_LT(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
XNN_TEST_VSCALEEXTEXP_ELEMENT_GT(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
"""
...
def main(args):
  ...
  tester_header = {
  "VScaleExtExpMicrokernelTester": "vscaleextexp-microkernel-tester.h",
  }[tester]
...
""".format(specification=options.ukernel, generator=sys.argv[0])
  ...
  folder = datatype + "-" + ("vscaleextexp" if datatype.startswith("f") else op)
