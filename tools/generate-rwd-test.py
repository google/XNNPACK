...
def split_ukernel_name(name):
  match = re.match(r"xnn_(qs8|f16_f32acc|f32)_rwd(minmax|max|min|sum)?(_minmax)?(_(fp32|rndnu))?_ukernel_((\d+)p)?(\d+)x__(.+)_c(\d+)(_acc(\d+))?", name)
  ...
  return requantization_type, op_type, primary_tile, incremental_tile, channel_tile, arch, isa
...
def generate_test_cases(ukernel, op_type, init_fn, requantization_type, primary_tile,
                        incremental_tile, channel_tile, isa):
  """Generates all tests cases for a RWD micro-kernel.
...
""".format(specification=options.spec, generator=sys.argv[0])
    ...
    for ukernel_spec in spec_yaml:
      ...
      requantization_type, op_type, primary_tile, incremental_tile, channel_tile, arch, \
