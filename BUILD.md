...
XNNPACK uses Chromium's `depot_tools` to manage dependencies when building with GN.
...
[1] Building unit tests and benchmarks is not supported on Android just yet.

[2] Benchmarks and unit tests can be slow to start if `symbol_level = 2`.
