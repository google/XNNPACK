...
static void GEMMBenchmark(benchmark::State& state, xnn_f32_gemm_minmax_ukernel_function gemm, size_t mr, size_t nr,
  size_t kr, size_t sr, xnn_init_f32_minmax_params_fn init_params,
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  const size_t num_buffers = 1 + benchmark::utils::DivideRoundUp<size_t>(benchmark::utils::GetMaxCacheSize(),
  ...
  for (auto _ : state) {
    ...
    for (uint32_t m = 0; m < mc; m += mr) {
      ...
      gemm(mb, nc, kc * sizeof(float), a.data() + m * kc, kc * sizeof(float),
        w.data() + buffer_index * nc_stride * (kc_stride + 1), c.data() + (buffer_index * mc + m) * nc,
        nc * sizeof(float), nr * sizeof(float), &params);
    }
  }
  ...
  state.counters["FLOPS"] =
    benchmark::Counter(uint64_t(state.iterations()) * 2 * mc * nc * kc, benchmark::Counter::kIsRate);
}

static void PPMM1PBenchmark(benchmark::State& state, xnn_f32_ppmm_minmax_ukernel_function ppmm,
  xnn_x32_packx_ukernel_function packx, size_t mr, size_t nr, xnn_init_f32_minmax_params_fn init_params,
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  if (isa_check && !isa_check(state)) {
    return;
  ...
  for (auto _ : state) {
    ...
    for (uint32_t m = 0; m < mc; m += mr) {
      ...
      ppmm(mb, nc, kc * sizeof(float), reinterpret_cast<const float*>(t.data()),
        w.data() + nc_stride * buffer_index * (kc + 1), c.data() + (mc * buffer_index + m) * nc, nc * sizeof(float),
...
static void PPMM2PBenchmark(benchmark::State& state, xnn_f32_ppmm_minmax_ukernel_function ppmm,
  xnn_x32_packx_ukernel_function packx, size_t mr, size_t nr, xnn_init_f32_minmax_params_fn init_params,
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  if (isa_check && !isa_check(state)) {
  ...
  for (auto _ : state) {
    ...
    for (uint32_t m = 0; m < mc; m += mr) {
      const uint32_t mb = min(mc - m, mr);
      ppmm(mb, nc, kc * sizeof(float), reinterpret_cast<const float*>(t.data() + m * kc),
        w.data() + nc_stride * buffer_index * (kc + 1), c.data() + (mc * buffer_index + m) * nc, nc * sizeof(float),
        nr * sizeof(float), &params);
    }
  }
...
static void GEMMBenchmark(benchmark::State& state, xnn_jit_code_generator_function generator, size_t mr, size_t nr,
  size_t kr, size_t sr, xnn_init_f32_minmax_params_fn init_params,
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  GEMMBenchmark(state, reinterpret_cast<xnn_f32_gemm_minmax_ukernel_function>(code_buffer.code), mr, nr, kr, sr,
    init_params);
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x4__aarch32_vfp_ld64, 4, 4, 1, 1, xnn_init_f32_minmax_scalar_params,
...
#if XNN_ARCH_ARM && XNN_ENABLE_JIT && XNN_PLATFORM_JIT
...
#endif  // XNN_ARCH_ARM && XNN_ENABLE_JIT && XNN_PLATFORM_JIT

#if XNN_ARCH_ARM || XNN_ARCH_ARM64
static void f32_gemm_1x8__neon_lane_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__neon_lane_ld64, 1, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_4x8__neon_lane_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__neon_lane_ld64, 4, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_4x8__neon_lane_ld128(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__neon_lane_ld128, 4, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_5x8__neon_lane_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__neon_lane_ld64, 5, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_6x8__neon_lane_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8__neon_lane_ld64, 6, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_6x8__neon_lane_ld128(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8__neon_lane_ld128, 6, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_1x8__neonfma_dup_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__neonfma_dup_ld64, 1, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
static void f32_gemm_4x8__neonfma_dup_ld64(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__neonfma_dup_ld64, 4, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8__neonfma_dup_ld64, 6, 8, 1, 1, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8s4__neon, 1, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_1x8s4__neonfma(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8s4__neonfma, 1, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
static void f32_gemm_4x8s4__neon(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8s4__neon, 4, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_4x8s4__neonfma(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8s4__neonfma, 4, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
static void f32_gemm_6x8s4__neon(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8s4__neon, 6, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_6x8s4__neonfma(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8s4__neonfma, 6, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
static void f32_gemm_8x8s4__neon(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_8x8s4__neon, 8, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEON);
}
static void f32_gemm_8x8s4__neonfma(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_8x8s4__neonfma, 8, 8, 1, 4, xnn_init_f32_minmax_scalar_params,
    benchmark::utils::CheckNEONFMA);
}
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__fma3_broadcast, 1, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_4x8__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__fma3_broadcast, 4, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_5x8__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__fma3_broadcast, 5, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_6x8__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8__fma3_broadcast, 6, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_7x8__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_7x8__fma3_broadcast, 7, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_8x8__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_8x8__fma3_broadcast, 8, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_1x16__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x16__fma3_broadcast, 1, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_3x16__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x16__fma3_broadcast, 4, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_4x16__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x16__fma3_broadcast, 4, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_5x16__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x16__fma3_broadcast, 5, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}

static void f32_gemm_1x16s4__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x16s4__fma3_broadcast, 1, 16, 1, 4, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_3x16s4__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x16s4__fma3_broadcast, 4, 16, 1, 4, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_4x16s4__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x16s4__fma3_broadcast, 4, 16, 1, 4, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}
static void f32_gemm_5x16s4__fma3_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x16s4__fma3_broadcast, 5, 16, 1, 4, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckFMA3);
}

static void f32_gemm_1x8__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__avx_broadcast, 1, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_4x8__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__avx_broadcast, 4, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_5x8__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__avx_broadcast, 5, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_6x8__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8__avx_broadcast, 6, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_7x8__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_7x8__avx_broadcast, 7, 8, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_1x16__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x16__avx_broadcast, 1, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_3x16__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x16__avx_broadcast, 4, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_4x16__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x16__avx_broadcast, 4, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}
static void f32_gemm_5x16__avx_broadcast(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x16__avx_broadcast, 5, 16, 1, 1, xnn_init_f32_minmax_avx_params,
    benchmark::utils::CheckAVX);
}

static void f32_gemm_1x8__sse2_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__sse2_dup, 1, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_3x8__sse2_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8__sse2_dup, 3, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_4x8__sse2_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__sse2_dup, 4, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_5x8__sse2_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__sse2_dup, 5, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}

static void f32_gemm_1x8__sse_load1(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__sse_load1, 1, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_3x8__sse_load1(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8__sse_load1, 3, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_4x8__sse_load1(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__sse_load1, 4, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_5x8__sse_load1(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__sse_load1, 5, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}

static void f32_gemm_1x8__sse_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8__sse_dup, 1, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_3x8__sse_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8__sse_dup, 3, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_4x8__sse_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8__sse_dup, 4, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_5x8__sse_dup(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8__sse_dup, 5, 8, 1, 1, xnn_init_f32_minmax_sse_params);
}

static void f32_gemm_1x8s4__sse(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x8s4__sse, 1, 8, 1, 4, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_3x8s4__sse(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8s4__sse, 3, 8, 1, 4, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_4x8s4__sse(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8s4__sse, 4, 8, 1, 4, xnn_init_f32_minmax_sse_params);
}
static void f32_gemm_5x8s4__sse(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8s4__sse, 5, 8, 1, 4, xnn_init_f32_minmax_sse_params);
}
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8s4__wasmsimd_arm, 3, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_4x8s4__wasmsimd_arm(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8s4__wasmsimd_arm, 4, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_5x8s4__wasmsimd_arm(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8s4__wasmsimd_arm, 5, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_6x8s4__wasmsimd_arm(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8s4__wasmsimd_arm, 6, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_3x8s4__wasmsimd_x86(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_3x8s4__wasmsimd_x86, 3, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_4x8s4__wasmsimd_x86(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x8s4__wasmsimd_x86, 4, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_5x8s4__wasmsimd_x86(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_5x8s4__wasmsimd_x86, 5, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_6x8s4__wasmsimd_x86(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_6x8s4__wasmsimd_x86, 6, 8, 1, 4, xnn_init_f32_minmax_scalar_params);
}
...
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_1x4__scalar, 1, 4, 1, 1, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_2x4__scalar(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_2x4__scalar, 2, 4, 1, 1, xnn_init_f32_minmax_scalar_params);
}
static void f32_gemm_4x4__scalar(benchmark::State& state, const char* net)
{
  GEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x4__scalar, 4, 4, 1, 1, xnn_init_f32_minmax_scalar_params);
}
