...
static void xnnpack_tanh_f16(benchmark::State& state) {
  ...
  status = xnn_setup_tanh_nc_f16(tanh_op, input.data(), output.data());
  if (status != xnn_status_success) {
...
static void xnnpack_tanh_f32(benchmark::State& state) {
  ...
  status = xnn_setup_tanh_nc_f32(tanh_op, input.data(), output.data());
  if (status != xnn_status_success) {
    state.SkipWithError("failed to setup Tanh operator");
    return;
  }
...
static void xnnpack_tanh_qs8(benchmark::State& state) {
  ...
  status = xnn_setup_tanh_nc_qs8(tanh_op, input.data(), output.data());
  if (status != xnn_status_success) {
    state.SkipWithError("failed to setup Tanh operator");
    return;
  }
...
static void xnnpack_tanh_qu8(benchmark::State& state) {
  ...
  status = xnn_setup_tanh_nc_qu8(tanh_op, input.data(), output.data());
  if (status != xnn_status_success) {
    state.SkipWithError("failed to setup Tanh operator");
    return;
  }
...
static void tflite_tanh_f32(benchmark::State& state) {
  ...
  const flatbuffers::Offset<tflite::OperatorCode> operator_code =
      CreateOperatorCode(builder, tflite::BuiltinOperator_TANH);
  ...
  const flatbuffers::Offset<tflite::Model> model_buffer = tflite::CreateModel(builder,
      ...
      builder.CreateString("TanH model"),
