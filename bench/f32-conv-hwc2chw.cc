  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  xnnpack::Buffer<float> input(input_height * input_width * input_channels + XNN_EXTRA_BYTES / sizeof(float));
  std::generate(input.begin(), input.end(), std::ref(f32rng));
  xnnpack::Buffer<float> kernel(output_channels * kernel_size * kernel_size * input_channels);
  ...
  xnnpack::Buffer<float, XNN_ALLOCATION_ALIGNMENT> packed_weights(weights_elements *
                                                          num_buffers);
  ...
  static void f32_conv_hwc2chw_3x3s2p1c3x2v__rvv_1x1(benchmark::State& state, const char* net) {
    f32_conv_hwc2chw(state,
      xnn_f32_conv_hwc2chw_ukernel_3x3s2p1c3x2v__rvv_1x1,
      xnn_init_f32_minmax_scalar_params,
      2 * xnn_init_hardware_config()->vlenb / sizeof(float) /* output channel tile */);
  }
  static void f32_conv_hwc2chw_3x3s2p1c3x2v__rvv_2x1(benchmark::State& state, const char* net) {
    f32_conv_hwc2chw(state,
      xnn_f32_conv_hwc2chw_ukernel_3x3s2p1c3x2v__rvv_2x1,
      xnn_init_f32_minmax_scalar_params,
      2 * xnn_init_hardware_config()->vlenb / sizeof(float) /* output channel tile */);
  }
  static void f32_conv_hwc2chw_3x3s2p1c3x2v__rvv_2x2(benchmark::State& state, const char* net) {
    f32_conv_hwc2chw(state,
