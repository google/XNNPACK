...
xnn_subgraph_t FP32DepthwiseSeparable(size_t w, size_t h, size_t kw, size_t ci, size_t co, FP32DepthwiseSeparableWeights& weights) {
  xnn_status status;
  ...
  status = xnn_define_tensor_value(
    ...
    /*data=*/nullptr,
    v0, XNN_VALUE_FLAG_EXTERNAL_INPUT, &v0);
  if (status != xnn_status_success) {
  ...
  status = xnn_define_tensor_value(
    ...
    /*data=*/nullptr,
    v2, XNN_VALUE_FLAG_EXTERNAL_OUTPUT, &v2);
  if (status != xnn_status_success) {
    std::cerr << "failed to create tensor v2" << std::endl;
    return nullptr;
  }

  weights.w0 = std::vector<float>(kw * kw * ci + XNN_EXTRA_BYTES / sizeof(float));
  ...
  status = xnn_define_tensor_value(
    subgraph, xnn_datatype_fp32,
    w0_dims.size(), w0_dims.data(),
    /*data=*/weights.w0.data(),
    XNN_INVALID_VALUE_ID, /*flags=*/0, &w0);
  if (status != xnn_status_success) {
  ...
  status = xnn_define_tensor_value(
    subgraph, xnn_datatype_fp32,
    w1_dims.size(), w1_dims.data(),
    /*data=*/weights.w1.data(),
    XNN_INVALID_VALUE_ID, /*flags=*/0, &w1);
  if (status != xnn_status_success) {
  ...
  status = xnn_define_tensor_value(
    subgraph, xnn_datatype_fp32,
    w2_dims.size(), w2_dims.data(),
    /*data=*/weights.w2.data(),
    XNN_INVALID_VALUE_ID, /*flags=*/0, &w2);
  if (status != xnn_status_success) {
  ...
  status = xnn_define_tensor_value(
    subgraph, xnn_datatype_fp32,
    w3_dims.size(), w3_dims.data(),
    /*data=*/weights.w3.data(),
    XNN_INVALID_VALUE_ID, /*flags=*/0, &w3);
  if (status != xnn_status_success) {
