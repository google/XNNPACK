...
static void xnnpack_average_pooling_f32(benchmark::State& state,
  ...
  status = xnn_create_average_pooling2d_nhwc_f32(
      ...
      pooling_size, stride, stride, -std::numeric_limits<float>::infinity(),
  ...
  status = xnn_setup_average_pooling2d_nhwc_f32(pooling_op, input.data(),
                                                output.data());
  if (status != xnn_status_success) {
...
void tflite_average_pooling_f32(benchmark::State& state, const char* net) {
  ...
  flatbuffers::Offset<tflite::Pool2DOptions> pool2d_options =
      CreatePool2DOptions(
          builder, padding, stride /* stride_w */, stride /* stride_h */,
  ...
  const int32_t input_shape[4] = {
      ...
      static_cast<int32_t>(input_width), static_cast<int32_t>(channels)};
  const int32_t output_shape[4] = {
      static_cast<int32_t>(batch_size), static_cast<int32_t>(output_height),
      static_cast<int32_t>(output_width), static_cast<int32_t>(channels)};
  ...
  flatbuffers::Offset<tflite::Operator> op = CreateOperator(
      ...
      tflite::BuiltinOptions_Pool2DOptions, pool2d_options.Union());

  const int32_t graph_inputs[1] = {0};
  const int32_t graph_outputs[1] = {1};
  flatbuffers::Offset<tflite::SubGraph> subgraph =
      CreateSubGraph(builder, builder.CreateVector(tensors, 2),
...
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, imagenet, "ImageNet")
    ->Apply(ImageNet)
    ->UseRealTime();
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, shufflenet_v1_g1,
                  "ShuffleNet v1 (1 group)")
    ->Apply(ShuffleNetV1G1)
    ->UseRealTime();
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, shufflenet_v1_g2,
                  "ShuffleNet v1 (2 groups)")
    ->Apply(ShuffleNetV1G2)
    ->UseRealTime();
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, shufflenet_v1_g3,
                  "ShuffleNet v1 (3 groups)")
    ->Apply(ShuffleNetV1G3)
    ->UseRealTime();
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, shufflenet_v1_g4,
                  "ShuffleNet v1 (4 groups)")
    ->Apply(ShuffleNetV1G4)
    ->UseRealTime();
BENCHMARK_CAPTURE(xnnpack_average_pooling_f32, shufflenet_v1_g8,
                  "ShuffleNet v1 (8 groups)")
    ->Apply(ShuffleNetV1G8)
    ->UseRealTime();

#ifdef BENCHMARK_TENSORFLOW_LITE
BENCHMARK_CAPTURE(tflite_average_pooling_f32, imagenet, "ImageNet")
    ->Apply(ImageNet)
    ->UseRealTime();
BENCHMARK_CAPTURE(tflite_average_pooling_f32, shufflenet_v1_g1,
                  "ShuffleNet v1 (1 group)")
    ->Apply(ShuffleNetV1G1)
    ->UseRealTime();
BENCHMARK_CAPTURE(tflite_average_pooling_f32, shufflenet_v1_g2,
                  "ShuffleNet v1 (2 groups)")
    ->Apply(ShuffleNetV1G2)
    ->UseRealTime();
BENCHMARK_CAPTURE(tflite_average_pooling_f32, shufflenet_v1_g3,
                  "ShuffleNet v1 (3 groups)")
    ->Apply(ShuffleNetV1G3)
    ->UseRealTime();
BENCHMARK_CAPTURE(tflite_average_pooling_f32, shufflenet_v1_g4,
                  "ShuffleNet v1 (4 groups)")
    ->Apply(ShuffleNetV1G4)
    ->UseRealTime();
BENCHMARK_CAPTURE(tflite_average_pooling_f32, shufflenet_v1_g8,
                  "ShuffleNet v1 (8 groups)")
    ->Apply(ShuffleNetV1G8)
    ->UseRealTime();
#endif  // BENCHMARK_TENSORFLOW_LITE
