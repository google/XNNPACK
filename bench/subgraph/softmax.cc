...
xnn_subgraph_t FP32Softmax(size_t m, size_t n, size_t k, uint32_t norm_mask,
  ...
  if (use_softmax) {
    ...
    if (needs_transpose) {
      ...
      status = xnn_define_static_transpose(subgraph.get(), perm.size(),
                                           perm.data(), v0, transposed_v0,
                                           /*flags=*/0);
      if (status != xnn_status_success) {
    ...
    if (reshaped_dims.size() < dims.size()) {
      ...
      status = xnn_define_static_reshape(subgraph.get(), reshaped_dims.size(),
    ...
    // Compute the softmax.
    status = xnn_define_softmax(subgraph.get(), transposed_reshaped_v0,
    ...
    if (reshaped_dims.size() < dims.size()) {
      status = xnn_define_static_reshape(subgraph.get(), perm_dims.size(),
                                         perm_dims.data(),
    ...
    if (needs_transpose) {
      status = xnn_define_static_transpose(subgraph.get(), inv_perm.size(),
    ...
    status = xnn_define_static_reduce(
        subgraph.get(), xnn_reduce_max, reduction_axes.size(),
        reduction_axes.data(), v0, max_v0, /*flags=*/XNN_FLAG_KEEP_DIMS);
    if (status != xnn_status_success) {
    ...
    status = xnn_define_binary(subgraph.get(), xnn_binary_subtract, nullptr, v0,
    ...
    status = xnn_define_unary(subgraph.get(), xnn_unary_exp, nullptr,
                              v0_minus_max, exp_v0_minus_max, /*flags=*/0);
    if (status != xnn_status_success) {
    ...
    status = xnn_define_static_reduce(
        subgraph.get(), xnn_reduce_sum, reduction_axes.size(),
        reduction_axes.data(), exp_v0_minus_max, sum_exp_v0_minus_max,
    ...
    status = xnn_define_binary(subgraph.get(), xnn_binary_divide, nullptr, one,
    ...
    status = xnn_define_binary(subgraph.get(), xnn_binary_multiply, nullptr,
...
static void SoftmaxArguments(benchmark::Benchmark* b) {
