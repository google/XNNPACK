...
xnn_subgraph_t FP16TransformerBlock(size_t batch_size, size_t sequence_length,
  ...
  static std::array<xnn_float16, XNN_PAD_EXTRA_BYTES(1, xnn_float16)> w40_data;
  uint32_t w40 = XNN_INVALID_VALUE_ID;
  std::array<size_t, 1> w40_dims = {{1}};
  status = xnn_define_tensor_value(
      subgraph, xnn_datatype_fp16, w40_dims.size(), w40_dims.data(),
  ...
  alignas(16) static std::array<xnn_float16, XNN_PAD_EXTRA_BYTES(1, xnn_float16)> w41_data;
  uint32_t w41 = XNN_INVALID_VALUE_ID;
  std::array<size_t, 1> w41_dims = {{1}};
  status = xnn_define_tensor_value(
      subgraph, xnn_datatype_fp16, w41_dims.size(), w41_dims.data(),
  ...
  w42_data.resize(XNN_PAD_EXTRA_BYTES(head_dim * embedding_dim, float));
  ...
  w45_data.resize(
      XNN_PAD_EXTRA_BYTES(num_heads * head_dim * embedding_dim, float));
  ...
  static std::vector<xnn_float16> w50_data;
  ...
  status = xnn_define_tensor_value(
      subgraph, xnn_datatype_fp16, w50_dims.size(), w50_dims.data(),
  ...
  static std::vector<xnn_float16> w51_data;
  ...
  std::array<size_t, 5> w51_dims = {
      {batch_size, 1, 1, sequence_length, sequence_length}};
  status = xnn_define_tensor_value(
      subgraph, xnn_datatype_fp16, w51_dims.size(), w51_dims.data(),
  ...
  w57_data.resize(
      XNN_PAD_EXTRA_BYTES(embedding_dim * num_heads * head_dim, float));
  ...
  w58_data.resize(XNN_PAD_EXTRA_BYTES((hidden_dim / 2) * embedding_dim, float));
  ...
  w59_data.resize(XNN_PAD_EXTRA_BYTES((hidden_dim / 2) * embedding_dim, float));
  ...
  w60_data.resize(XNN_PAD_EXTRA_BYTES(embedding_dim * (hidden_dim / 2), float));
  ...
  std::generate(w40_data.begin(), w40_data.end(), std::ref(f16rng));
  std::generate(w41_data.begin(), w41_data.end(), std::ref(f16rng));
  std::generate(w42_data.begin(), w42_data.end(), std::ref(f32rng));
  std::generate(w45_data.begin(), w45_data.end(), std::ref(f32rng));
  std::generate(w50_data.begin(), w50_data.end(), std::ref(f16rng));
  std::generate(w51_data.begin(), w51_data.end(), std::ref(f16rng));
