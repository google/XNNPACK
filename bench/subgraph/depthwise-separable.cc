...
template <typename T>
struct DepthwiseSeparableWeights {
  std::vector<T> w0;
  std::vector<T> w1;
  std::vector<T> w2;
  std::vector<T> w3;
...
template <typename T>
xnn_subgraph_t DepthwiseSeparableImpl(size_t w, size_t h, size_t kw, size_t ci,
                                      size_t co, uint32_t op_flags,
                                      DepthwiseSeparableWeights<T>& weights) {
  xnn_status status;
  ...
  status = xnn_define_tensor_value(
      subgraph.get(), datatype, v2_dims.size(), v2_dims.data(),
  ...
  weights.w0 = std::vector<T>(kw * kw * ci + XNN_EXTRA_BYTES / sizeof(T));
  weights.w1 = std::vector<T>(ci + XNN_EXTRA_BYTES / sizeof(T));
  weights.w2 = std::vector<T>(co * ci + XNN_EXTRA_BYTES / sizeof(T));
  weights.w3 = std::vector<T>(co + XNN_EXTRA_BYTES / sizeof(T));
  ...
  status = xnn_define_tensor_value(
      subgraph.get(), datatype, w2_dims.size(), w2_dims.data(),
  ...
  status = xnn_define_tensor_value(
      subgraph.get(), datatype, w3_dims.size(), w3_dims.data(),
...
xnn_subgraph_t FP32DepthwiseSeparable(
    size_t w, size_t h, size_t kw, size_t ci, size_t co, uint32_t op_flags,
    DepthwiseSeparableWeights<float>& weights) {
  return DepthwiseSeparableImpl<float>(w, h, kw, ci, co, op_flags, weights);
}

xnn_subgraph_t FP16DepthwiseSeparable(
    size_t w, size_t h, size_t kw, size_t ci, size_t co, uint32_t op_flags,
    DepthwiseSeparableWeights<xnn_float16>& weights) {
  return DepthwiseSeparableImpl<xnn_float16>(w, h, kw, ci, co, op_flags,
                                             weights);
}
...
static void FP32DepthwiseSeparable(benchmark::State& state) {
  models::DepthwiseSeparableWeights<float> weights;
...
static void FP16DepthwiseSeparable(benchmark::State& state) {
  models::DepthwiseSeparableWeights<xnn_float16> weights;
  xnnpack::RunBenchmark(state, [&state, &weights]() {
    uint32_t op_flags = 0;
    if (state.range(5)) {
      op_flags |= XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER;
    }
    return models::FP16DepthwiseSeparable(state.range(0), state.range(1),
                                          state.range(2), state.range(3),
                                          state.range(4), op_flags, weights);
  });
}
...
BENCHMARK(FP16DepthwiseSeparable)
    ->Unit(benchmark::kMicrosecond)
    ->MeasureProcessCPUTime()
    ->UseRealTime()
    ->Apply(DepthwiseSeparableArguments);
