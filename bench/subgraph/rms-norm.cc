...
// `norm_mask`.
template <typename T>
xnn_subgraph_t RMSNormImpl(size_t m, size_t n, size_t k, uint32_t norm_mask) {
  xnn_status status;
  ...
  // 2. Compute input^2 in math_datatype (FP32)
  uint32_t input_sq = XNN_INVALID_VALUE_ID;
  status = xnn_define_tensor_value(
      subgraph.get(), math_datatype, dims.size(), dims.data(),
      /*data=*/nullptr, XNN_INVALID_VALUE_ID, 0, &input_sq);
  if (status != xnn_status_success) {
  ...
  // 3. Compute mean(input^2) in math_datatype (FP32)
  ...
  // 4. Compute inv_rms = rsqrt(mean_sq) in math_datatype (FP32)
  uint32_t inv_rms_math = XNN_INVALID_VALUE_ID;
  status = xnn_define_tensor_value(subgraph.get(), math_datatype,
                                   reduction_dims.size(), reduction_dims.data(),
                                   /*data=*/nullptr, XNN_INVALID_VALUE_ID, 0,
                                   &inv_rms_math);
  if (status != xnn_status_success) {
    std::cerr << "failed to create tensor inv_rms_math" << std::endl;
    return nullptr;
  }
  status = xnn_define_unary(subgraph.get(), xnn_unary_reciprocal_square_root,
                            nullptr, mean_sq, inv_rms_math, /*flags=*/0);
  if (status != xnn_status_success) {
  ...
  // 5. Convert inv_rms_math to datatype if necessary
  uint32_t inv_rms = inv_rms_math;
  if (datatype != math_datatype) {
    status = xnn_define_tensor_value(
        subgraph.get(), datatype, reduction_dims.size(), reduction_dims.data(),
        /*data=*/nullptr, XNN_INVALID_VALUE_ID, 0, &inv_rms);
    if (status != xnn_status_success) {
      std::cerr << "failed to create tensor inv_rms" << std::endl;
      return nullptr;
    }
    status = xnn_define_unary(subgraph.get(), xnn_unary_convert, nullptr,
                              inv_rms_math, inv_rms, /*flags=*/0);
    if (status != xnn_status_success) {
      std::cerr << "failed to create unary convert inv_rms->datatype"
                << std::endl;
      return nullptr;
    }
  }

  // 6. Compute output = input * inv_rms in datatype
...
xnn_subgraph_t FP32RMSNorm(size_t m, size_t n, size_t k, uint32_t norm_mask) {
  return RMSNormImpl<float>(m, n, k, norm_mask);
}

xnn_subgraph_t FP16RMSNorm(size_t m, size_t n, size_t k, uint32_t norm_mask) {
  return RMSNormImpl<xnn_float16>(m, n, k, norm_mask);
}
...
static void FP16RMSNorm(benchmark::State& state) {
  xnnpack::RunBenchmark(state, [&state]() {
    return models::FP16RMSNorm(state.range(0), state.range(1), state.range(2),
                               state.range(3));
  });
}
