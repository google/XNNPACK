...
static void Im2ColGEMMBenchmark(
    benchmark::State& state, xnn_f32_gemm_minmax_ukernel_fn f32_gemm,
  ...
  const size_t kc_stride =
      benchmark::utils::RoundUp<size_t>(group_input_channels, kr);

  xnnpack::Buffer<float> a(input_height * input_width * group_input_channels +
  ...
  xnn_pack_f32_gemm_goi_w(/*groups=*/1, group_output_channels,
                          group_input_channels * kernel_size, nr, kr, sr,
  ...
  xnnpack::Buffer<float> im2col_buffer(output_size * group_input_channels *
                                       kernel_size * group_output_channels);
  ...
  for (auto _ : state) {
    ...
    if (kernel_size != 1 || subsampling != 1) {
      xnn_im2col_conv2d(
          ...
          subsampling, dilation, dilation, input_width, padding_top,
          padding_left,
          group_input_channels * sizeof(float) /* input channels */,
          group_input_channels * sizeof(float) /* input stride */, a.data(),
          im2col_buffer.data());
    ...
    for (uint32_t m = 0; m < output_size; m += mr) {
      ...
      for (uint32_t n = 0; n < group_output_channels; n += nr) {
        ...
        f32_gemm(
            ...
            inputData + m * kernel_size * group_input_channels,
            kernel_size * group_input_channels * sizeof(float),
            w.data() +
                (buffer_index * nc_stride + n) * (kernel_size * kc_stride + 1),
            c.data() +
                (buffer_index * output_size + m) * group_output_channels + n,
            group_output_channels * sizeof(float), nr * sizeof(float), &params);
      }
    }
  }
...
static void f32_gemm_4x8__asm_aarch64_neonfma_cortex_a75_prfm(
  ...
  Im2ColGEMMBenchmark(
      state,
      xnn_f32_gemm_minmax_ukernel_4x8__asm_aarch64_neonfma_cortex_a75_prfm, 4,
      8, 1, 1, xnn_init_f32_minmax_scalar_params);
}
...
static void f32_gemm_2x4__scalar(benchmark::State& state, const char* net) {
  Im2ColGEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_2x4__scalar, 2, 4, 1,
                      1, xnn_init_f32_minmax_scalar_params);
}

static void f32_gemm_4x4__scalar(benchmark::State& state, const char* net) {
  Im2ColGEMMBenchmark(state, xnn_f32_gemm_minmax_ukernel_4x4__scalar, 4, 4, 1,
                      1, xnn_init_f32_minmax_scalar_params);
}
