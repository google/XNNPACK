...
static void f16_rmax(benchmark::State& state, xnn_f16_reduce_ukernel_fn rmax,
  ...
  xnn_float16 output;
  for (auto _ : state) {
    rmax(elements * sizeof(xnn_float16), input.data(), &output, &params);
  }
...
BENCHMARK_CAPTURE(f16_rmax, avx512skx_u16, xnn_f16_rmax_ukernel__avx512skx_u16,
...
BENCHMARK_CAPTURE(f16_rmax, f16c_u32, xnn_f16_rmax_ukernel__f16c_u32,
                  /*init_params=*/nullptr, benchmark::utils::CheckF16C)
    ->Apply(benchmark::utils::ReductionParameters<xnn_float16>)
    ->UseRealTime();
#endif

BENCHMARK_CAPTURE(f16_rmax, scalar_u1, xnn_f16_rmax_ukernel__scalar_u1)
    ->Apply(benchmark::utils::ReductionParameters<xnn_float16>)
    ->UseRealTime();
