...
class X64(base_architecture.BaseArchitecture):
  ...
  def input_output_register_setup(self, M):
    ...
    ret = ''
    if self.subtract_stack_ptr(M) != 0:
      ret += self.space() + """sub rsp, {subtract_stack_ptr}\n""".format(
          subtract_stack_ptr=self.subtract_stack_ptr(M)
      )
    # Write rsi & r10 if required to the stack.
    if M > self.max_M_before_spilling():
      ret += self.space() + '# Write rsi (a pointer) to the stack as we need the register.\n'
      ret += self.space() + 'mov [rsp + 128], rsi\n'
      ret += self.space() + '# Write r10 (c pointer) to the stack as we need the register.\n'
  ...
  def read_a_registers(self, M):
    ...
    if M <= self.max_M_before_spilling():
      return ''
    ret = self.space() + '# Read a pointers from stack into GP registers.\n'
