...
class Fma3(arch.X64):
  ...
  def clamp_min(self, max_reg, reg, prefix):
    return (
        self.space()
        + f'vminps  {prefix}{reg}, {prefix}{max_reg}, {prefix}{reg}\n'
    )

  def clamp_max(self, min_reg, reg, prefix):
    return (
        self.space()
  ...
  def store(
    ...
    for mr in range(0, M):
      asm_string += 'vmovups  [{c_reg}], y{ACC}\n'.format(
          ACC=acc_registers[mr], c_reg=cm_registers[mr]
      )
    ...
    sub {nc}, {n_step}
    jne {OUTER}
    jmp return""".format(n_step=N, nc=nc_reg, OUTER=labels[outer_label])
    asm_string += CHECK
    N = int(N / 2)
    if N * 2 > N_STEP:
      asm_string += """
      tail_{N}:
      test {nc_lo}, {N}
      jz tail_{N_2}\n""".format(N=N, N_2=int(N / 2), nc_lo=nc_lo)
      for mr in range(0, M):
        asm_string += 'vmovups  [{c_reg}], y{ACC}\n'.format(
            ACC=acc_registers[mr], c_reg=cm_registers[mr]
        )
      ...
      for mr in range(0, M):
        asm_string += 'vmovaps  y{ACC0}, y{ACC1}\n'.format(
            ACC0=acc_registers[mr], ACC1=acc_registers[mr + M * nr]
        )
      for mr in range(0, M):
