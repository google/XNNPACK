...
def generate_gemm_microkernel(M :int, N :int, isa :base_architecture.BaseArchitecture, output_file :str):
  ...
  asm_string += isa.outer_loop_prepare(
      M=M, N=num_horizontal_registers
  )

  # the outer loop label
  OUTER = isa.labels()[M]
  asm_string += f'\n{OUTER}: # Outer loop\n'
  asm_string += isa.zero_gp_reg(k_register)
  ...
  # loop counter
  asm_string += isa.cmp_and_jump(label='inner_loop')

  asm_string += isa.dequantize(
      M=M, N=num_horizontal_registers, W=w_ptr_reg
  )
