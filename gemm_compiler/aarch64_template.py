...
class Aarch64(base_architecture.BaseArchitecture):
  ...
  def am_registers(self):
    return [self.a_ptr_reg()] + ['x9', 'x10', 'x11', 'x12']
  ...
  def cm_registers(self):
    return [self.c_ptr_reg()] + ['x13', 'x14', 'x15', 'x19']
  ...
  def inner_loop(self, M, N):
    ...
    if 'before' in self.input_asm():
      asm_string += self.space() + self.input_asm()['before']
    for mr in range(0, M):
    ...
    if 'after' in self.input_asm():
      asm_string += self.space() + self.input_asm()['after']
  ...
  def input_output_register_setup(self, M):
    ...
    a_base_ptr = self.a_ptr_reg()
    c_base_ptr = self.c_ptr_reg()
    # setup a{0}->a{M-1} registers
    if M == 1:
      return ''
    asm_string = self.space() + '# Setup and alias a & c pointers.\n'
  ...
  def input_output_strides(self, M, registers, stride):
    ...
    ret = ''
    for mr in range(1, M):
      ret += self.space() + INPUT_OUTPUT_REGISTER_SETUP.format(
          M=mr,
          M_1=mr - 1,
          aM=registers[mr],
          aM_1=registers[mr - 1],
  ...
  def increment_ptr(self, ptr, step):
    return self.space() + f'add {ptr}, {ptr}, {step}\n'

  def zero_gp_reg(self, reg):
    return self.space() + f'eor {reg}, {reg}, {reg}\n'

  def copy_gp_register(self, src, dst):
    return self.space() + f'mov {dst}, {src}\n'

  def decrement_counter(self, reg, step):
    return self.space() + f'sub {reg}, {reg}, {step}\n'

  def cmp_and_jump(self, label):
    kc_register = self.kc_register()
    k_register = self.k_register()
    return (
        self.space()
        + """add {k_register}, {k_register}, 4
