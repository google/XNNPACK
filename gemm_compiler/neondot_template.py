...
class NeonDot(isa.NeonFma):
  ...
  def adjust_kc(self):
    ...
    ret = self.space() + '# Round kc up to channels.\n'
    ret += (
        self.space()
        + """add {kc_reg}, {kc_reg}, #{channels}
  ...
  def dequantize(self, M, N, W):
    ...
    for nr in range(0, N * M):
      ret += self.space() + 'scvtf v{ACC}.4s, v{ACC}.4s\n'.format(
          ACC=accumulators[nr]
      )
    ret += self.space() + '# Multiply by input scale.\n'
    for nr in range(0, N):
      for mr in range(0, M):
        ret += (
            self.space()
            + 'fmul v{ACC}.4s, v{ACC}.4s, v{zp_scale}.s[{pos}]\n'.format(
                ACC=accumulators[nr * M + mr],
                zp_scale=self.zp_scale(int(mr / 2)),
                pos=int((mr % 2) * 2) + 1,
            )
        )
    ret += self.space() + '# Load weights scale.\n'
    output_scale_pair = 'ldp q{W_SCALE_0}, q{W_SCALE_1}, [{W}, {offset}]\n'
    # output scales
    for nr in range(0, N, 2):
      ret += self.space() + output_scale_pair.format(
          W=W,
    ...
    ret += self.space() + 'add {W}, {W}, {increment}\n'.format(
    ...
    # for 4 results.
    ret += self.space() + "# Multiply by weight's scale.\n"
    for nr in range(0, N):
      for mr in range(0, M):
        ret += self.space() + 'fmul v{ACC}.4s, v{ACC}.4s, v{SCALE}.4s\n'.format(
            ACC=accumulators[nr * M + mr], SCALE=self.a_registers(nr)
        )
    ret += self.space() + '# Add bias.\n'
    for nr in range(0, N):
      for mr in range(0, M):
        ret += self.space() + 'fadd v{ACC}.4s, v{ACC}.4s, v{BIAS}.4s\n'.format(
