...
ynn_status define_blockwise_scale(ynn_subgraph_t subgraph, uint32_t scale_id,
  ...
  // 1. Transpose the last two dimensions: [N, num_blocks] -> [num_blocks, N].
  int32_t perm[2] = {1, 0};
  uint32_t transposed_id = YNN_INVALID_VALUE_ID;
  YNN_RETURN_IF_ERROR(ynn_define_static_transpose(subgraph, 2, perm, scale_id,
                                                  &transposed_id, 0));
...
ynn_status define_blockwise_dot(ynn_subgraph_t subgraph, uint32_t a_id,
  ...
  // Dynamic quantization of input A if scale is not provided.
  if (a_scale_id == YNN_INVALID_VALUE_ID) {
    ...
    YNN_RETURN_IF_ERROR(ynn_define_reduce(
        subgraph, ynn_reduce_min_max, 1, &reduce_axis, a_id,
        YNN_INVALID_VALUE_ID, &min_max_id, YNN_NODE_FLAG_KEEP_DIMS));

    YNN_RETURN_IF_ERROR(ynn_define_dynamic_quantization(
        subgraph, min_max_id, ynn_type_int8, &a_zero_point_id, &a_scale_id, 0));

    uint32_t quantized_a_id = YNN_INVALID_VALUE_ID;
    YNN_RETURN_IF_ERROR(ynn_define_quantize(subgraph, a_id, ynn_type_int8,
                                            a_zero_point_id, a_scale_id,
                                            &quantized_a_id, 0));
    a_id = quantized_a_id;
  }
  ...
  YNN_RETURN_IF_ERROR(ynn_define_split_dim(subgraph, -2, 2, split_dims, b_id,
                                           &b_inner_id, 0));
