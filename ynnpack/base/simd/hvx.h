...
#ifndef XNNPACK_YNNPACK_BASE_SIMD_HVX_H_
#define XNNPACK_YNNPACK_BASE_SIMD_HVX_H_
...
YNN_ALWAYS_INLINE s32x32 operator*=(s32x32& a, s32x32 b) {
  HVX_Vector result = Q6_Vw_vmpye_VwVuh(a.v, b.v);
  result = Q6_Vw_vmpyoacc_VwVwVh_s1_sat_shift(result, a.v, b.v);
  return s32x32{Q6_Vw_vasr_VwR(result, 1)};
}
...
YNN_ALWAYS_INLINE float horizontal_sum(f32x32 x) {
  // This converts qfloat to float at each step. We could do the reduction in
  // qfloats, but that risks loss of precision due to not renormalizing after
  // each add.
  x.v = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_VsfVsf(x.v, Q6_V_vror_VR(x.v, 64)));
  x.v = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_VsfVsf(x.v, Q6_V_vror_VR(x.v, 32)));
  x.v = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_VsfVsf(x.v, Q6_V_vror_VR(x.v, 16)));
  x.v = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_VsfVsf(x.v, Q6_V_vror_VR(x.v, 8)));
  x.v = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vadd_VsfVsf(x.v, Q6_V_vror_VR(x.v, 4)));
  return *(float*)&x.v;
}
...
#endif  // XNNPACK_YNNPACK_BASE_SIMD_HVX_H_
