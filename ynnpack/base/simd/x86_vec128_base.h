...
YNN_ALWAYS_INLINE std::tuple<u8x16, u8x16> interleave(
  ...
  __m128i check = _mm_and_si128(_mm_xor_si128(_mm_srli_epi16(x0.v, 4), x1.v),
                                _mm_set1_epi8(0x0f));
  return interleave(std::integral_constant<size_t, 8>{},
                    u8x16{_mm_xor_si128(x0.v, _mm_slli_epi16(check, 4))},
                    u8x16{_mm_xor_si128(x1.v, check)});
}
YNN_ALWAYS_INLINE std::tuple<u8x16, u8x16> interleave(
    std::integral_constant<size_t, 2>, u8x16 x0, u8x16 x1) {
  __m128i check = _mm_and_si128(_mm_xor_si128(_mm_srli_epi16(x0.v, 2), x1.v),
                                _mm_set1_epi8(0x33));
  return interleave(std::integral_constant<size_t, 4>{},
                    u8x16{_mm_xor_si128(x0.v, _mm_slli_epi16(check, 2))},
                    u8x16{_mm_xor_si128(x1.v, check)});
}
