...
static f32x16x8& operator+=(f32x16x8& a, bf16x32x4 b) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    __m512bh expanded_lo = reinterpret_cast<__m512bh>(
        _mm512_cvtepu16_epi32(lower_half));
    __m512bh expanded_hi = reinterpret_cast<__m512bh>(
        _mm512_cvtepu16_epi32(upper_half));

    a.v[2 * i + 0].v = _mm512_dpbf16_ps(a.v[2 * i + 0].v, expanded_lo, ones);
    a.v[2 * i + 1].v = _mm512_dpbf16_ps(a.v[2 * i + 1].v, expanded_hi, ones);
  }
...
static f32x16x8 reduce_add(
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    a.v[2 * i].v = _mm512_dpbf16_ps(a.v[2 * i].v, expanded_lo, expanded_lo);
    a.v[2 * i + 1].v = _mm512_dpbf16_ps(a.v[2 * i + 1].v, expanded_hi,
        expanded_hi);
  }
