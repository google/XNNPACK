...
TEST(GroupedDotTest, Basic) {
  ...
  dot_kernel kernel = get_dot_kernel(type, {}, {}, 0, false);
  ...
  size_t alignment = 16;
  std::vector<char> packed_b_raw(E * expert_packed_size + alignment, 0);
  void* ptr = packed_b_raw.data();
  size_t space = packed_b_raw.size();
  void* aligned_ptr = std::align(alignment, E * expert_packed_size, ptr, space);
  ASSERT_NE(aligned_ptr, nullptr);
  char* packed_b = static_cast<char*>(aligned_ptr);

  packer p(/*transpose=*/false, /*elem_size_bits=*/32, tile_k, aligned_n);

  for (size_t e = 0; e < E; ++e) {
    const float* B_e = input_b.data() + e * D_in * D_out;
    char* packed_B_e = packed_b + e * expert_packed_size;
  ...
  grouped_dot(E, expert_counts.data(), offsets.data(), input_a.data(), packed_b,
              ynn_output.data(), /*m_start=*/0, /*m_count=*/NK, /*n_start=*/0,
              /*n_count=*/D_out, D_in, D_out, D_in * sizeof(float),
              D_out * sizeof(float), packed_b_stride, expert_packed_size,
              kernel, aligned_n, 0, sizeof(float), sizeof(float));
...
TEST(GroupedDotTest, UnbalancedRouting) {
  std::mt19937 rng(42);
  std::uniform_real_distribution<float> dist(-1.0f, 1.0f);
  ...
  // 1. Generate routing: unbalanced (all tokens to expert 0).
  std::vector<int32_t> expert_indices(NK, 0);
...
TEST(GroupedDotTest, Slices) {
  ...
  // Evaluate in 4 separate slices of tokens.
  std::vector<float> ynn_output(NK * D_out, 0.0f);
  size_t slice_size = NK / 4;
  for (size_t m_start = 0; m_start < NK; m_start += slice_size) {
    size_t m_count = std::min(NK - m_start, slice_size);
    grouped_dot(E, expert_counts.data(), offsets.data(),
                input_a.data() + m_start * D_in, packed_b,
                ynn_output.data() + m_start * D_out, m_start, m_count,
                /*n_start=*/0, /*n_count=*/D_out, D_in, D_out,
                D_in * sizeof(float), D_out * sizeof(float), packed_b_stride,
                expert_packed_size, kernel, aligned_n, 0, sizeof(float),
                sizeof(float));
  }
