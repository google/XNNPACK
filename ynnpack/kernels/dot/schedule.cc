...
//    tile M and N. This eliminates loop overhead for small tasks.
...
//    streams past it.
//
// If `packed_b` and `transposed_a` are both true, we can stream A and B from L3
// cache. Since the working set is contiguous in both dimensions, the hardware
// prefetcher can effectively hide the latency of L3 cache.
span<dot_loop> schedule_dot(const cpu_info& cpu_info, size_t m, size_t n,
                            ...
                            bool packed_b, bool transposed_a,
                            dot_loop* storage) {
  ...
  auto make_innermost_loops = [&]() -> span<dot_loop> {
  ...
  size_t cache_capacity = cpu_info.cache_sizes.l2;
  if (packed_b && transposed_a) {
    // If both A and B are packed/contiguous, they can stream efficiently from
    // L3.
    cache_capacity = std::max(
        cache_capacity, cpu_info.cache_sizes.l3 / cpu_info.num_shared_l3_cores);
  }
