...
void dot(benchmark::State& state, uint64_t arch_flags, dot_kernel_fn kernel,
  ...
  Tensor<TA> a({m, transpose_a ? align_up(k, tile_k) : k});
  Tensor<TB> b({align_up(k, tile_k), align_up(n, tile_n)},
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < m; i += block_m) {
      ...
      kernel(m_i, n, 1, 1, k, a.stride(0) * sizeof(TA), 0, 0, &a(i, 0), 0, 0,
             b.stride(0) * sizeof(TB), b.base(), c.stride(0) * sizeof(TC),
             &c(i, 0), c.stride(0) * sizeof(TC), &c(i, 0));
    }
  }
