...
packer packer::with_tile_n(size_t new_tile_n) const {
  packer p = *this;
  p.tile_n = new_tile_n;
  if (!transpose_fn && tile_m == 1) {
    p.transpose_blocks_fn = get_tiled_transpose(elem_size_bits * new_tile_n);
    assert(p.transpose_blocks_fn);
  }
  return p;
}
