...
template <typename X>
void check_results(const unary_op_info& op, Tensor<int4x2> a, Tensor<X> x,
                   const quantization_params& a_quantization = {},
                   const quantization_params& x_quantization = {}) {
  Tensor<int8_t> a_unpacked(x.extents());
  for (const auto& i : EnumerateIndices(x.extents())) {
    std::vector<size_t> a_i = i;
    a_i.back() /= 2;
    a_unpacked(i) = a(a_i).get(i.back() % 2);
  }
  check_results(op, a_unpacked, x, a_quantization, x_quantization);
}

template <typename X>
void check_results(const unary_op_info& op, Tensor<int2x4> a, Tensor<X> x,
                   const quantization_params& a_quantization = {},
                   const quantization_params& x_quantization = {}) {
  Tensor<int8_t> a_unpacked(x.extents());
  for (const auto& i : EnumerateIndices(x.extents())) {
    std::vector<size_t> a_i = i;
    a_i.back() /= 4;
    a_unpacked(i) = a(a_i).get(i.back() % 4);
  }
  check_results(op, a_unpacked, x, a_quantization, x_quantization);
}

// These dummy overloads are required because the C++ compiler instantiates
// the test template matrix for every combination of input and output types.
// Since we only support converting *from* sub-byte types (not to them),
// these prevent compilation errors on the unsupported paths.
template <typename A>
void check_results(const unary_op_info& op, Tensor<A> a, Tensor<int4x2> x,
                   const quantization_params& a_quantization = {},
                   const quantization_params& x_quantization = {}) {
  YNN_UNREACHABLE;
}
...
template <typename A>
void check_results(const unary_op_info& op, Tensor<A> a, Tensor<int2x4> x,
                   const quantization_params& a_quantization = {},
                   const quantization_params& x_quantization = {}) {
  YNN_UNREACHABLE;
}

inline void check_results(const unary_op_info& op, Tensor<int4x2> a,
                          Tensor<int4x2> x,
                          const quantization_params& a_quantization = {},
                          const quantization_params& x_quantization = {}) {
  YNN_UNREACHABLE;
...
inline void check_results(const unary_op_info& op, Tensor<int2x4> a,
                          Tensor<int2x4> x,
                          const quantization_params& a_quantization = {},
                          const quantization_params& x_quantization = {}) {
  YNN_UNREACHABLE;
