...
std::tuple<slinky::expr, slinky::expr> choose_split_factors(
  ...
  auto impl = [elem_a, elem_b, elem_c](const slinky::call* op,
    ...
    // - Area: The number of elements in the output tile C. We bound it by
    //   min/max values to balance threading overhead against available
    //   parallelism.
    ...
    index_t max_area = 256 * 256;
    const index_t min_cost = 1024 * 64;
    index_t max_cost = std::max<index_t>(1024 * 64, ((m + n) / 64) * k);
    index_t target_footprint = 96 * 1024 * elem_a;
    ...
    // growing the larger dimension. We dynamically restrict `max_area` by the
    // cube of the major dimensions to prevent slicing into skinny, inefficient
    // slivers. For example, for a 4096x40 shape, the `max_area` is capped at
    // (4096 * 40 * 40) / 1024 = 6400 (instead of the default 65536).
    if (m >= 4 * n) {
      aspect_ratio = 16;  // Prioritize growing m
      if (m >= 8 * n) {
        max_area = std::min<index_t>(
            max_area, std::max<index_t>(4096, (m * n * n) / 1024));
      }
    } else if (n >= 4 * m) {
      aspect_ratio = 1;  // Prioritize growing n
      if (n >= 8 * m) {
        max_area = std::min<index_t>(
            max_area, std::max<index_t>(4096, (n * m * m) / 1024));
      }
    }

    if (aspect_ratio != 4) {
      // Highly asymmetric matrices (like 33280x40) suffer from extreme thread
      // starvation if chopped too finely. We boost `target_footprint` and
      // `max_cost` to coalesce tiny slivers into fewer, denser blocks.
      target_footprint *= 2;
      max_cost = std::max<index_t>(max_cost, 1024 * 128);
    }
    ...
    while (true) {
      ...
      if (split_n * split_m >= min_area) {
        bool footprint_met = footprint >= target_footprint;
        bool min_cost_met = cost >= min_cost;

        // We've reached the minimum tile size, should we stop?
        if ((footprint_met && min_cost_met) || cost >= max_cost ||
    ...
    // Clamp splits to exact bounds to eliminate Slinky step clamping overheads
    // for geometries that yield 1 task in a dimension.
