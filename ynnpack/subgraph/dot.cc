...
auto make_dot_impl(dot_type type, bool consistent_arithmetic, bool symmetric_b,
  ...
  return [type, kernel_flags, transposed_a, pack_b, num_k_dims](
    ...
    if (transposed_a) {
      ...
      assert(c_m.min() % tile_m == 0);
      a.slice(0, slinky::in_bounds{c_m.min() / tile_m});
    } else {
...
void define_transpose_a(ynn_subgraph& subgraph, ynn_node& node, index_t tile_m,
  ...
  node.create = [](const ynn_node& node, ynn_runtime& runtime) {
    ...
    slinky::expr stride = elem_size;
    output.buffer->dim(0).stride = stride;
    stride *= tile_k;
    output.buffer->dim(1).stride = stride;
    stride *= tile_m;
    output.buffer->dim(m_dim + 2).stride = stride;
    stride *= output.buffer->dim(m_dim + 2).extent();
    for (int d = 3; d <= m_dim + 1; ++d) {
      output.buffer->dim(d).stride = stride;
      stride *= output.buffer->dim(d).extent();
    }
    output.buffer->dim(2).stride = stride;
    stride *= output.buffer->dim(2).extent();
    for (size_t d = m_dim + 3; d < output.buffer->rank(); ++d) {
      if (slinky::prove_true(output.buffer->dim(d).is_broadcast())) continue;
      output.buffer->dim(d).stride = stride;
      stride *= output.buffer->dim(d).extent();
    }

    // Don't allow folding of dimensions we transpose.
    for (size_t d = 0; d < output.buffer->rank(); ++d) {
      if (slinky::prove_true(output.buffer->dim(d).is_broadcast())) continue;
      output.buffer->dim(d).fold_factor = slinky::dim::unfolded;
    }
...
ynn_status define_dot(ynn_subgraph& subgraph, size_t num_k_dims,
  ...
  node.create = [consistent_arithmetic, symmetric_b, pack_b, transpose_a,
    ...
    if (input_c.rank() >= 1) {
      c_bounds.push_back(make_broadcast_bounds(j, input_c.physical_extent(0),
                                               output.extent(0)));
    }
    ...
    for (size_t i = 1; i < output_dims.size(); ++i) {
      if (transpose_a) {
        if (i >= 2 && i + num_a_k_dims - 1 < input_a.rank()) {
          a_bounds.push_back(make_broadcast_bounds(
      ...
      if (i >= 2 && i + 2 + num_k_dims - 1 < packed_b.rank()) {
        b_bounds.push_back(make_broadcast_bounds(
            output_dims[i], packed_b.physical_extent(i + 2 + num_k_dims - 1),
            output.extent(i)));
      }
      if (i < input_c.rank()) {
        c_bounds.push_back(make_broadcast_bounds(
            output_dims[i], input_c.physical_extent(i), output.extent(i)));
      }
    }
