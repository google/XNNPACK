...
// We assume a baseline server CPU with 32 cores when calculating target
// concurrency.
constexpr index_t default_num_cpu_cores = 32;
...
std::tuple<index_t, index_t, index_t> choose_split_factors(
  ...
  // Calculate target concurrency dynamically based on the total number of MACs,
  // scaling from 32 tasks (for medium workloads) up to 128 tasks (for very
  // large workloads, up to 4 tasks per core).
  const index_t total_min_macs = min_macs_per_task * default_num_cpu_cores;
  const index_t num_tasks_per_core =
      std::min<index_t>(4, ceil_div(total_macs, total_min_macs));
  const index_t num_tasks = num_tasks_per_core * default_num_cpu_cores;
  ...
  while (true) {
    ...
    if (split_m * split_n >= area_threshold && footprint >= min_footprint) {
      ...
      if ((enforce_cost_ceiling && cost >= target_cost) ||
          ...
          (grid_concurrency <= default_num_cpu_cores &&
           total_macs >= total_min_macs)) {
        break;
      }
    }
