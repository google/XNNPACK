...
TEST(CloneSubgraphSubset, SimpleChain) {
  ...
  // Clone subgraph.
  auto cloned_subgraph = clone_subgraph_subset(original, a_id, b_id);
  ...
  for (const auto& val : cloned_subgraph->values) {
    if (val.is_external_input()) cloned_a_id = val.id;
    if (val.is_external_output()) cloned_b_id = val.id;
  }
  ...
  Runtime cloned_runtime(cloned_subgraph.get(), &scheduler);
...
TEST(CloneSubgraphSubset, Branching) {
  // Original subgraph:
  // a -> (abs) -> b ->
  //                    \ -> (add) -> c
  // d -> (negate) -> e ->
  //
  // Cloned subgraph (input a, output c):
  // a -> (abs) -> b ->
  //                    \ -> (add) -> c
  // d -> (negate) -> e ->
  const int kShapeSize = 10;
  ...
  // Clone subgraph.
  auto cloned_subgraph = clone_subgraph_subset(original, a_id, c_id);
  EXPECT_EQ(cloned_subgraph->nodes.size(), 3);

  uint32_t cloned_a_id = YNN_INVALID_VALUE_ID;
  uint32_t cloned_d_id = YNN_INVALID_VALUE_ID;
  uint32_t cloned_c_id = YNN_INVALID_VALUE_ID;
  for (const auto& val : cloned_subgraph->values) {
    if (val.is_external_output()) cloned_c_id = val.id;
  }
  for (const auto& node : cloned_subgraph->nodes) {
    if (auto* op = std::get_if<ynn_node::unary_elementwise>(&node.op)) {
      if (op->op == ynn_unary_abs) {
        cloned_a_id = node.inputs[0];
      } else if (op->op == ynn_unary_negate) {
        cloned_d_id = node.inputs[0];
      }
    }
  }

  EXPECT_NE(cloned_a_id, YNN_INVALID_VALUE_ID);
  EXPECT_NE(cloned_d_id, YNN_INVALID_VALUE_ID);
  ...
  std::vector<float> a_data(kShapeSize), d_data(kShapeSize);
  std::iota(a_data.begin(), a_data.end(), -5.0f);
  std::iota(d_data.begin(), d_data.end(), 10.0f);
  std::vector<float> c_data(kShapeSize), cloned_c_data(kShapeSize);

  orig_runtime.ReshapeExternalTensor({kShapeSize}, a_data.data(), a_id)
      .ReshapeExternalTensor({kShapeSize}, d_data.data(), d_id)
      .ReshapeRuntime()
      .SetupExternalTensor(c_data.data(), c_id)
      .InvokeRuntime();

  cloned_runtime.ReshapeExternalTensor({kShapeSize}, a_data.data(), cloned_a_id)
      .ReshapeExternalTensor({kShapeSize}, d_data.data(), cloned_d_id)
      .ReshapeRuntime()
...
TEST(CloneSubgraphSubset, MiddleCut) {
  ...
  // Clone subgraph.
  auto cloned_subgraph = clone_subgraph_subset(original, b_id, c_id);
  EXPECT_EQ(cloned_subgraph->nodes.size(), 1);
  const auto& node = cloned_subgraph->nodes[0];
  const auto* op = std::get_if<ynn_node::unary_elementwise>(&node.op);
  ASSERT_NE(op, nullptr);
  ...
  uint32_t cloned_c_id = YNN_INVALID_VALUE_ID;
  for (const auto& val : cloned_subgraph->values) {
    if (val.is_external_input()) cloned_b_id = val.id;
    if (val.is_external_output()) cloned_c_id = val.id;
  }
  EXPECT_NE(cloned_b_id, YNN_INVALID_VALUE_ID);
  EXPECT_NE(cloned_c_id, YNN_INVALID_VALUE_ID);
  ...
  Runtime orig_runtime(builder.GetSubgraph(), &scheduler);
  ASSERT_EQ(orig_runtime.Status(), ynn_status_success);
  Runtime subset_runtime(cloned_subgraph.get(), &scheduler);
...
TEST(CloneSubgraphSubset, QuantizationParams) {
  ...
  // Clone subgraph.
  auto cloned_subgraph = clone_subgraph_subset(subgraph, a_id, b_id);
  // Should have a, b and their quantization params.
  EXPECT_GE(cloned_subgraph->values.size(), 6);

  // Find A and B in new subgraph.
  uint32_t cloned_a_id = YNN_INVALID_VALUE_ID;
  uint32_t cloned_b_id = YNN_INVALID_VALUE_ID;
  ...
  for (const auto& val : cloned_subgraph->values) {
    ...
    if (val.is_external_input()) {
      // This should be A. Check if it has scale_id and zero_point_id set.
      if (val.scale_id != YNN_INVALID_VALUE_ID &&
          val.zero_point_id != YNN_INVALID_VALUE_ID) {
        cloned_a_id = val.id;
      }
    }
    if (val.is_external_output()) {
      // This should be B. Check if it has scale_id and zero_point_id set.
      if (val.scale_id != YNN_INVALID_VALUE_ID &&
          val.zero_point_id != YNN_INVALID_VALUE_ID) {
        cloned_b_id = val.id;
      }
...
TEST(CloneSubgraphSubset, DisconnectedInput) {
  ...
  // Clone subgraph.
  auto cloned_subgraph = clone_subgraph_subset(original, c_id, b_id);
