    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      for (size_t i = 0; i < a.size(); ++i) {
        ...
        if constexpr (std::is_signed<A>::value) {
          // If the input is signed, we need to add 128 to the index to convert
          // to a valid index value into the LUT.
          index = static_cast<int>(a.data()[i]) + 128;
        } else {
          index = static_cast<size_t>(a.data()[i]);
        }
