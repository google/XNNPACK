...
void BM_GroupedDot(benchmark::State& state) {
  ...
  shape.n = D_out;
  dot_kernel kernel =
      get_dot_kernel(type, shape, /*dot_packed_shape=*/{},
                     /*required_flags=*/0, /*transpose_a=*/false);
  ...
  if (aligned_ptr == nullptr) {
    state.SkipWithMessage("Failed to align packed_b");
    return;
  }
  ...
  for (auto _ : state) {
    grouped_dot(E, expert_counts.data(), offsets.data(), input_a.data(),
                packed_b, output.data(), /*m_start=*/0, /*m_count=*/NK,
...
void BenchmarkArgs(benchmark::Benchmark* b) {
