...
TEST(fusion, reduce_sum_of_gather_constant) {
  ...
  // onto the constant so it folds at build time, and, more importantly, the
  // weight gather is left with a single consumer so `rewrite_pack_b_gather`
  // can hoist its `pack_b` to init time instead of repacking every invocation.
  builder
