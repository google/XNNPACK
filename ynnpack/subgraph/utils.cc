...
bool has_producer(uint32_t val_id, const ynn_subgraph& subgraph,
                  const std::set<const ynn_node*>& relevant_nodes) {
  const ynn_node* producer = subgraph.get_producer(val_id);
  return producer && relevant_nodes.find(producer) != relevant_nodes.end();
}

bool has_multiple_inputs(const std::set<uint32_t>& relevant_values,
                         uint32_t input_id, const ynn_subgraph& subgraph,
                         const std::set<const ynn_node*>& relevant_nodes) {
  for (uint32_t val_id : relevant_values) {
    if (val_id == input_id) continue;
    if (subgraph.value(val_id).is_static()) continue;
    if (!has_producer(val_id, subgraph, relevant_nodes)) return true;
  }
  return false;
}
...
std::optional<ynn_subgraph> clone_subgraph_subset(const ynn_subgraph& subgraph,
  ...
  std::set<uint32_t> relevant_values;
  std::set<const ynn_node*> relevant_nodes;
  ...
  while (head < values_to_traverse.size()) {
    ...
    if (producer) {
      ...
      for (uint32_t producer_input_id : producer->inputs) {
        if (producer_input_id != YNN_INVALID_VALUE_ID) {
          add_value_for_traversal(producer_input_id);
        }
      }
    }
  }
