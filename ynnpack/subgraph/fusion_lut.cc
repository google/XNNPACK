...
std::unordered_set<uint32_t> get_variable_inputs(const ynn_subgraph& subgraph,
  ...
  if (is_elementwise_node(n)) {
    for (size_t i = 0; i < n.inputs.size(); ++i) {
      ...
      if (id != YNN_INVALID_VALUE_ID && !subgraph.value(id).is_static()) {
        // For unary elementwise operations, the second input is the parameters
        // tensor, which we don't want to fuse.
