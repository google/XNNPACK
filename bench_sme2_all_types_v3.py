...
def analyze(data, type_str):
    ...
    for b in data.get("benchmarks", []):
        ...
        if "error_occurred" in b and b["error_occurred"]:
            print(f" (Error: {b.get('error_message')})", end="")
            return None
...
def main():
    ...
    header = f"{'Type':<6} | {'Size':<6} | {'Orig(ms)':<10} | {'Opt(ms)':<10} | {'Speedup'}"
    print(f"\n{header}")
    print("-" * 60)
    
    for t in TYPES:
        for s in SIZES:
            type_filter = t if t == "fp32" else f"{t}_{t}_fp32"
            ...
            if res:
                ...
                print(f"{m_orig/1e6:<10.3f} | {m_opt/1e6:<10.3f} | {speedup:.3f}x")
            else:
                print(f"{'ERR':<10} | {'ERR':<10} | {'N/A'}")
