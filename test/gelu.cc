...
using GeluTestF32 = UnaryTest<float>;

TEST_F(GeluTestF32, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_gelu(subgraph, input_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_gelu);
...
TEST_F(GeluTestF32, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_gelu_nc_f32(/*flags=*/0, &op);
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_reshape_gelu_nc_f32(op, batch_size, channels, channels, channels,
                              /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success,
            xnn_setup_gelu_nc_f32(op, input.data(), operator_output.data()));
