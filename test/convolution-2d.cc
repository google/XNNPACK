...
TEST_F(ConvolutionTestQS8, transient_indirection_buffer)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  compute_convolution_qs8_reference_results(
      ...
      filter, accumulators, /*has_bias=*/true, bias);
  ...
  float output_scale =
      static_cast<double>(static_cast<uint32_t>(
          accumulated_max - accumulated_min)) / 255.0;
  int8_t output_zero_point = static_cast<int8_t>(std::max(
    std::min(
      lrint(-0.5 - 0.5 * static_cast<double>(
            accumulated_min + accumulated_max) / output_scale),
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qs8(
      ...
      quantized_output_max, /*flags=*/XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER,
      nullptr, nullptr, &op);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
        ...
        input_dims.size(), input_dims.data(), nullptr, /*external_id=*/0,
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_qint8, 0, kernel_scale, filter_dims.size(),
        filter_dims.data(), filter.data(), /*external_id=*/1, /*flags=*/0,
        &filter_id));

  uint32_t bias_id = XNN_INVALID_NODE_ID;
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_qint32, 0, kernel_scale, bias_dims.size(),
        bias_dims.data(), bias.data(), /*external_id=*/2, /*flags=*/0,
        &bias_id));
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
        ...
        output_dims.size(), output_dims.data(), nullptr, /*external_id=*/3,
  ...
  ASSERT_EQ(
    ...
    xnn_define_convolution_2d(
        ...
        input_padding_left, kernel_height, kernel_width, subsampling_height,
        subsampling_width, dilation_height, dilation_width, groups,
        group_input_channels, group_output_channels, output_min, output_max,
        input_id, filter_id, bias_id, output_id,
        /*flags=*/XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER));

  xnn_runtime_t runtime = nullptr;
  ASSERT_EQ(xnn_status_success, xnn_create_runtime_v3(
      subgraph, nullptr, nullptr, xnn_test_runtime_flags(), &runtime));
  ASSERT_NE(nullptr, runtime);
  ...
  ASSERT_EQ(xnn_status_success, xnn_setup_runtime(
      runtime, external.size(), external.data()));
  ASSERT_EQ(xnn_status_success, xnn_invoke_runtime(runtime));
...
TEST_F(ConvolutionTestQU8, transient_indirection_buffer)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  compute_convolution_qu8_reference_results(
      ...
      kernel_zero_point, input, filter, accumulators, /*has_bias=*/true, bias);
  ...
  const double output_scale =
      static_cast<double>(
        static_cast<uint32_t>(accumulated_max - accumulated_min)) / 255.0;
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_quint8, 0, kernel_scale, filter_dims.size(),
        filter_dims.data(), filter.data(), /*external_id=*/1, /*flags=*/0,
        &filter_id));
...
TEST_F(ConvolutionTestF16, transient_indirection_buffer)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_f16(
      ...
      /*flags=*/
      XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER | XNN_FLAG_FP32_STATIC_WEIGHTS,
      nullptr, nullptr, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_reshape_convolution2d_nhwc_f16(
        ...
        &workspace_alignment, /*output_height_out=*/nullptr,
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
        subgraph, xnn_datatype_fp16, input_dims.size(), input_dims.data(),
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
        subgraph, xnn_datatype_fp32, filter_dims.size(), filter_dims.data(),
        filter.data(), /*external_id=*/1, /*flags=*/0, &filter_id));

  uint32_t bias_id = XNN_INVALID_NODE_ID;
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
        subgraph, xnn_datatype_fp32, bias_dims.size(), bias_dims.data(),
        bias.data(), /*external_id=*/2, /*flags=*/0, &bias_id));
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
        ...
        nullptr, /*external_id=*/3, XNN_VALUE_FLAG_EXTERNAL_OUTPUT,
        &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);
  ASSERT_EQ(
    xnn_status_success,
