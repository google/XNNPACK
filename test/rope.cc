...
template <class T>
class RoPETestBase : public ::testing::Test {
 protected:
  RoPETestBase() {
    ...
    operator_output =
        xnnpack::Buffer<T>(batch_size * tokens * heads * channels);
...
TEST_F(RoPETestF16, define) {
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(subgraph, xnn_datatype_fp16, weights_dims.size(),
                              weights_dims.data(), weights.data(),
                              /*external_id=*/1, /*flags=*/0, &weights_id));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_tensor_value(
                ...
                /*external_id=*/2, /*flags=*/XNN_VALUE_FLAG_EXTERNAL_OUTPUT,
                &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);

  ASSERT_EQ(xnn_status_success,
            xnn_define_rope(subgraph, max_tokens, input_id, weights_id,
                            output_id, /*flags=*/0));
...
TEST_F(RoPETestF32, define) {
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(subgraph, xnn_datatype_fp32, weights_dims.size(),
                              weights_dims.data(), weights.data(),
                              /*external_id=*/1, /*flags=*/0, &weights_id));
...
TEST_F(RoPETestF16, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_rope_nthc_f16(op, batch_size, tokens, heads, channels,
                                      /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success,
            xnn_setup_rope_nthc_f16(op, input.data(), weights.data(),
...
TEST_F(RoPETestF32, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_rope_nthc_f32(op, batch_size, tokens, heads, channels,
                                      /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success,
            xnn_setup_rope_nthc_f32(op, input.data(), weights.data(),
                                    operator_output.data()));
