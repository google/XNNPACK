...
protected:
  EvenSplitNTest()
  {
    ...
    switch (num_outputs) {
      case 2:
        split_type = xnn_even_split2;
        break;
      case 3:
        split_type = xnn_even_split3;
        break;
      case 4:
        split_type = xnn_even_split4;
        break;
      default:
        if (num_outputs > 4) {
          split_type = xnn_even_splitn;
        } else {
          split_type = xnn_even_split_invalid;  // Handle invalid cases
        }
        break;
    }
  }
  ...
  size_t RandomNumOutputs() { return std::uniform_int_distribution<size_t>(1, 500)(rng); }
...
TEST_F(EvenSplitNTestQS8, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_even_split(subgraph, split_type, split_dim, input_id, num_outputs, output_ddds, /*flags=*/0));
...
TEST_F(EvenSplitNTestQU8, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_even_split(subgraph, split_type, axis, input_id, num_outputs, output_ddds, /*flags=*/0));
  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_even_split_n);
  ASSERT_EQ(node->params.even_split.axis, axis);
