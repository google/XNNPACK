...
#define XNN_CVT_UKERNEL_WITH_PARAMS(arch_flags, ukernel, batch_tile, vector_tile,                                                              \
  ...
  TEST(ukernel, output_saturation) { TestOutputSaturation<Convert, datatype_in, datatype_out>(arch_flags, batch_tile, ukernel, init_params); } \
  TEST(ukernel, output_overflow) { TestOutputOverflow<Convert, datatype_in, datatype_out>(arch_flags, batch_tile, ukernel, init_params); }
