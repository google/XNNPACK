...
using RemainderTestF32 = BinaryTest<float>;

TEST_F(RemainderTestF32, define) {
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_remainder(subgraph, 0, 0, input1_id, input2_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_remainder);
...
TEST_F(RemainderTestF32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_remainder_nd_f32(/*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_remainder_nd_f32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_remainder_nd_f32(op, input1.data(), input2.data(), operator_output.data()));
