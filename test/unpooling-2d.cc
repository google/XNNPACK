...
class Unpooling2DTestBase : public ::testing::Test {
 protected:
  Unpooling2DTestBase() {
    ...
    output_height = xnn_compute_unpooling_output_dimension(
        input_height, padding_top + padding_bottom, pooling_height);
    output_width = xnn_compute_unpooling_output_dimension(
        input_width, padding_left + padding_right, pooling_width);

    index_dist = std::uniform_int_distribution<uint32_t>(
    ...
    input_index =
        xnnpack::Buffer<T>(batch_size * input_height * input_width * channels);
...
TEST_F(Unpooling2DTestX32, define) {
  ...
  ASSERT_EQ(
      ...
      xnn_define_tensor_value(
          subgraph, xnn_datatype_fp32, input_value_dims.size(),
          input_value_dims.data(), nullptr,
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_tensor_value(
                subgraph, xnn_datatype_fp32, input_index_dims.size(),
                input_index_dims.data(), input_index.data(),
                XNN_INVALID_VALUE_ID, /*flags=*/0, &input_index_id));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_unpooling_2d(subgraph, padding_top, padding_right,
                                    padding_bottom, padding_left,
                                    pooling_height, pooling_width,
                                    input_value_id, input_index_id, output_id,
                                    /*flags=*/0));
...
TEST_F(Unpooling2DTestX32, matches_operator_api) {
  ...
  std::generate(input_index.begin(), input_index.end(),
                [&]() { return index_dist(rng); });
  std::generate(operator_output.begin(), operator_output.end(),
                [&]() { return u32dist(rng); });
  std::generate(subgraph_output.begin(), subgraph_output.end(),
                [&]() { return u32dist(rng); });
  ...
  const xnn_status status = xnn_create_unpooling2d_nhwc_x32(
      ...
      pooling_width, channels, channels, channels, /*flags=*/0, &op);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_setup_unpooling2d_nhwc_x32(op, input.data(), input_index.data(),
  ...
  std::array<xnn_external_value, 2> external = {
      xnn_external_value{input_value_id, input.data()},
...
TEST_F(Unpooling2DTestX32, reshape_output) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_external_value(runtime, 0, input_value_dims.size(),
                                       input_value_dims.data()));
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_external_value(runtime, 1, input_index_dims.size(),
                                       input_index_dims.data()));
  ...
  const size_t expected_height = xnn_compute_unpooling_output_dimension(
      input_value_dims[1], padding_top + padding_bottom, pooling_height);
  const size_t expected_width = xnn_compute_unpooling_output_dimension(
      input_value_dims[2], padding_left + padding_right, pooling_width);
