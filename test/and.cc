...
using ANDTestS32 = BinaryTest<int32_t>;

TEST_F(ANDTestS32, define) {
  ...
  ASSERT_EQ(node->type, xnn_node_type_and);
...
TEST_F(ANDTestS32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_and_nd_s32(/*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_and_nd_s32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_and_nd_s32(op, input1.data(), input2.data(), operator_output.data()));
