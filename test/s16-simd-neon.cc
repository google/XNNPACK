...
TEST_F(S16SimdNEONTest, SignComplement) {
  const xnn_simd_s16_t a = xnn_loadu_s16(inputs_.data());
...
TEST_F(S16SimdNEONTest, LoadTail) {
...
TEST_F(S16SimdNEONTest, StoreTail) {
  const xnn_simd_s16_t a = xnn_loadu_s16(inputs_.data());
  for (size_t num_elements = 1; num_elements < xnn_simd_size_s16;
      num_elements++) {
