...
using Subtract2TestF16 = BinaryTest<xnn_float16>;
...
TEST_F(Subtract2TestF16, matches_operator_api)
{
  ...
  // Call operator API.
  const xnn_status status = xnn_create_subtract_nd_f16(0, &op);
...
TEST_F(Subtract2TestF32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_subtract_nd_f32(0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);
