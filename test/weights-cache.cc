...
static void* cache_end(const struct xnn_internal_weights_cache* cache) {
  return reinterpret_cast<void*>(
      reinterpret_cast<uintptr_t>(cache->cache.weights.start) +
      cache->cache.weights.size);
}

static void write_weights(struct xnn_internal_weights_cache* cache,
                          const std::string& str) {
  ASSERT_NE(nullptr,
            xnn_internal_reserve_space_in_weights_cache(cache, str.length()));
...
TEST(WEIGHTS_CACHE, init_and_release) {
  ...
  EXPECT_EQ(xnn_status_success, xnn_internal_init_weights_cache_with_size(
...
TEST(WEIGHTS_CACHE, init_with_size_and_release) {
  ...
  EXPECT_EQ(xnn_status_success,
            xnn_internal_init_weights_cache_with_size(&cache, four_mb));
...
TEST(WEIGHTS_CACHE, get_or_insert) {
  ...
  ASSERT_EQ(0, xnn_internal_get_or_insert_weights_cache(
                   &cache, nullptr, cache.cache.weights.start, 4));
  ...
  write_weights(&cache, "1234");
  ASSERT_EQ(0, xnn_internal_get_or_insert_weights_cache(&cache, nullptr,
                                                        span2_weights, 4));
  ...
  ASSERT_EQ(4, xnn_internal_get_or_insert_weights_cache(&cache, nullptr,
                                                        span3_weights, 4));
  ASSERT_EQ(1, cache.cache.hits);
  ...
  for (size_t i = 0, expected_offset = 0; i < old_num_buckets; i++) {
    ...
    ASSERT_EQ(expected_offset,
              xnn_internal_get_or_insert_weights_cache(
                  &cache, nullptr, cache_end(&cache), s.length()));
...
TEST(WEIGHTS_MEMORY, allocate_and_release) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_allocate_weights_memory(&b, XNN_DEFAULT_WEIGHTS_BUFFER_SIZE));
...
TEST(WEIGHTS_MEMORY, grow) {
  ...
  std::string actual = std::string(static_cast<char*>(b.start),
                                   static_cast<char*>(b.start) + junk.length());
...
TEST(WEIGHTS_CACHE, finalize_empty) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_allocate_weights_memory(&b, initial_capacity));
...
#else
  // The actual capacity depends on page size, since it is aligned, just check
  // that it shrunk.
...
TEST(WEIGHTS_CACHE, write_many_cache_hits) {
  ...
  auto write = [&] {
    ...
    xnn_internal_get_or_insert_weights_cache(&cache, nullptr, cache_end(&cache),
                                             weights_size);
  };
...
TEST(WEIGHTS_CACHE, operations_on_finalized_cache_hard) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_internal_finalize_weights_cache(
                &cache, xnn_weights_cache_finalization_kind_hard));
  ...
  // We should not be able to insert into the weights cache, and also this
  // shouldn't timeout by unlocking a mutex which has not been locked (since
  // xnn_internal_reserve_space_in_weights_cache above failed).
...
TEST(WEIGHTS_CACHE, operations_on_finalized_cache_soft) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_internal_finalize_weights_cache(
                &cache, xnn_weights_cache_finalization_kind_soft));
  // Finalizing a finalized cache is an error.
  ASSERT_NE(xnn_status_success,
            xnn_internal_finalize_weights_cache(
                &cache, xnn_weights_cache_finalization_kind_soft));
  ...
  ASSERT_EQ(nullptr, xnn_internal_reserve_space_in_weights_cache(
                         &cache, cache.cache.weights.capacity + 1));
...
TEST(WEIGHTS_CACHE, insert_into_finalized_cache_soft) {
  ...
  // Inserting into a finalized cache is okay as long as cache memory has space
  // and it is a cache hit.
  ...
  ASSERT_EQ(XNN_CACHE_NOT_FOUND, xnn_internal_get_or_insert_weights_cache(
                                     &cache, nullptr, cached_weights, 4));

  // Not enough space in the finalized weights cache.
  std::string big_string(cache.cache.weights.capacity, '5');
  // Don't use write_weights here as it asserts
  // xnn_internal_reserve_space_in_weights_cache does not return nullptr.
  ASSERT_EQ(nullptr, xnn_internal_reserve_space_in_weights_cache(
