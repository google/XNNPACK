...
class ConvertOperatorTester {
 public:
  ...
  void TestF32toF16() const {
    ...
    auto rng = std::mt19937(random_device());
    auto f32rng =
        std::bind(std::uniform_real_distribution<float>(-1.0f, 1.0f), rng);
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        for (size_t c = 0; c < channels(); c++) {
          output_ref[i * channels() + c] =
              fp16_ieee_from_fp32_value(input[i * input_stride() + c]);
        }
      }
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_create_convert_nc_f32_f16(channels(), input_stride(),
                                              output_stride(), 0, &convert_op));
      ASSERT_NE(nullptr, convert_op);

      // Smart pointer to automatically delete convert op.
      std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)>
          auto_ceiling_op(convert_op, xnn_delete_operator);

      ASSERT_EQ(xnn_status_success,
                xnn_setup_convert_nc_f32_f16(convert_op, batch_size(),
                                             input.data(), output.data(),
                                             nullptr /* thread pool */));
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        for (size_t c = 0; c < channels(); c++) {
           ASSERT_EQ(output_ref[i * channels() + c] + 1, output[i *
           output_stride() + c])
