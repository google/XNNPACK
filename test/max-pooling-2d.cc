...
template <class T>
class MaxPooling2DTestBase : public ::testing::Test {
 protected:
  MaxPooling2DTestBase() {
    ...
    stride_height =
        std::uniform_int_distribution<uint32_t>(1, pooling_height)(rng);
    stride_width =
        std::uniform_int_distribution<uint32_t>(1, pooling_width)(rng);
    output_min = -std::numeric_limits<float>::infinity();
    output_max = std::numeric_limits<float>::infinity();
    output_height = xnn_compute_convolution_output_dimension(
        padding_top + input_height + padding_bottom, pooling_height,
        dilation_height, stride_height);
    output_width = xnn_compute_convolution_output_dimension(
        padding_left + input_width + padding_right, pooling_width,
    ...
    operator_output = xnnpack::Buffer<T>(XNN_EXTRA_BYTES / sizeof(T) +
                                         batch_size * output_height *
                                             output_width * channels);
    subgraph_output = xnnpack::Buffer<T>(XNN_EXTRA_BYTES / sizeof(T) +
                                         batch_size * output_height *
                                             output_width * channels);
  }
...
TEST_F(MaxPooling2DTestQS8, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_max_pooling_2d(
                subgraph, padding_top, padding_right, padding_bottom,
                padding_left, pooling_height, pooling_width, stride_height,
                stride_width, dilation_height, dilation_width, output_min,
                output_max, input_id, output_id, /*flags=*/0));
...
TEST_F(MaxPooling2DTestQS8, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_max_pooling2d_nhwc_s8(
      padding_top, padding_right, padding_bottom, padding_left, pooling_height,
      pooling_width, stride_height, stride_width, dilation_height,
      dilation_width, quantized_output_min, quantized_output_max, /*flags=*/0,
      &op);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_max_pooling2d_nhwc_s8(
                ...
                /*output_pixel_stride=*/channels, /*output_height_out=*/nullptr,
                /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_max_pooling2d_nhwc_s8(
  ...
  for (size_t i = 0; i < batch_size * output_height * output_width * channels;
       i++) {
...
TEST_F(MaxPooling2DTestQU8, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_max_pooling2d_nhwc_u8(
                ...
                /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_max_pooling2d_nhwc_u8(
                                    op, input.data(), operator_output.data()));
...
TEST_F(MaxPooling2DTestF16, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_max_pooling2d_nhwc_f16(
      padding_top, padding_right, padding_bottom, padding_left, pooling_height,
      pooling_width, stride_height, stride_width, dilation_height,
      dilation_width, output_min, output_max, /*flags=*/0, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_max_pooling2d_nhwc_f16(
                ...
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_max_pooling2d_nhwc_f16(
                                    op, input.data(), operator_output.data()));
...
TEST_F(MaxPooling2DTestF32, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_max_pooling2d_nhwc_f32(
                ...
                /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_max_pooling2d_nhwc_f32(
                                    op, input.data(), operator_output.data()));
...
TEST_F(MaxPooling2DTestF32, Reshape) {
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_max_pooling_2d(
          subgraph, /*input_padding_top=*/0, /*input_padding_right=*/0,
          /*input_padding_bottom=*/0, /*input_padding_left=*/0, pooling_height,
          pooling_width, stride_height, stride_width, dilation_height,
          dilation_width, output_min, output_max, input_id, output_id,
          /*flags=*/0));
  ASSERT_EQ(subgraph->num_nodes, 1);
  ...
  input_dims[3] += 1;
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_external_value(runtime, 0, input_dims.size(),
                                       input_dims.data()));
