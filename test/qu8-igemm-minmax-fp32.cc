...
std::vector<GemmTestParams> CreateTests1(
  ...
  std::vector<GemmTestParams> CreateTests2(
    ...
    std::string kb2s = std::to_string(k_block * 2);
    std::string akbs = std::to_string(adj_k_block);
    nr = nr * xnn_init_hardware_config()->vlenb / sizeof(uint8_t);
    std::string nrs = std::to_string(nr);
...
INSTANTIATE_TEST_SUITE_P(
...
#if XNN_ENABLE_RISCV_VECTOR && XNN_ARCH_RISCV
  INSTANTIATE_TEST_SUITE_P(
      QU8_IGEMM_MINMAX_FP32_1X1V__RVV, GemmTest,
      testing::ValuesIn(CreateTests2(
          ...
          /*planes=*/1,
          [](GemmMicrokernelTester& tester) {
            tester.Test(xnn_qu8_igemm_minmax_fp32_ukernel_1x1v__rvv,
                        xnn_init_qu8_conv_minmax_fp32_scalar_params,
                        xnn_pack_qu8_conv_goki_w,
                        xnn_qu8_requantize_fp32);
          },
  ...
  INSTANTIATE_TEST_SUITE_P(
      QU8_IGEMM_MINMAX_FP32_7X1V__RVV, GemmTest,
      testing::ValuesIn(CreateTests2(
          /*k_block=*/1,
          /*adj_k_block=*/1,
          /*mr=*/7, /*nr=*/1, /*kr=*/1, /*sr=*/1,
          ...
          /*planes=*/1,
          [](GemmMicrokernelTester& tester) {
            tester.Test(xnn_qu8_igemm_minmax_fp32_ukernel_7x1v__rvv,
                        xnn_init_qu8_conv_minmax_fp32_scalar_params,
                        xnn_pack_qu8_conv_goki_w,
                        xnn_qu8_requantize_fp32);
          },
