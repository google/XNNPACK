...
TEST(polyfill, _mm_dpbusd_epi32_madd_kzp2) {
#if defined(__SSSE3__)
  TEST_REQUIRES_ARCH_FLAGS(xnn_arch_x86_ssse3);
  ...
  for (uint32_t u8_val = 0; u8_val < 256; ++u8_val) {
    for (uint32_t u2_val = 0; u2_val < 4; ++u2_val) {
      ...
      __m128i vres = _mm_dpbusd_epi32_madd_kzp2(vacc, vu8, vu2);
...
TEST(polyfill, _mm_dpbusd_epi32_madd_kzp0) {
#if defined(__SSSE3__)
  TEST_REQUIRES_ARCH_FLAGS(xnn_arch_x86_ssse3);
  ...
  for (uint32_t u8_val = 0; u8_val < 256; ++u8_val) {
    for (uint32_t u2_val = 0; u2_val < 4; ++u2_val) {
      ...
      __m128i vres = _mm_dpbusd_epi32_madd_kzp0(vacc, vu8, vu2);

      int32_t res[4];
      _mm_storeu_si128(reinterpret_cast<__m128i*>(res), vres);

      // Reference formula: result = acc + 4 * (u8 * u2)
      int32_t expected = 100 + 4 * (static_cast<int32_t>(u8_val) *
                                    static_cast<int32_t>(u2_val));

      for (int i = 0; i < 4; ++i) {
        ASSERT_EQ(res[i], expected)
            << "u8=" << u8_val << ", u2=" << u2_val << ", index=" << i;
      }
    }
  }
#else
...
TEST(polyfill, _mm256_dpbusd_epi32_madd_kzp0) {
#if defined(__AVX2__)
  TEST_REQUIRES_ARCH_FLAGS(xnn_arch_x86_avx2);
  ...
  for (uint32_t u8_val = 0; u8_val < 256; ++u8_val) {
    for (uint32_t u2_val = 0; u2_val < 4; ++u2_val) {
      ...
      __m256i vres = _mm256_dpbusd_epi32_madd_kzp0(vacc, vu8, vu2);
