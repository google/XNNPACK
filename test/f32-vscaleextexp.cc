...
#include "src/xnnpack/vscaleextexp.h"  // IWYU pragma: keep
...
class VScaleExtExpMicrokernelTester {
 public:
  ...
  void Test(xnn_f32_vscaleextexp_ukernel_fn vscaleextexp) const {
    ...
    // Choose such range that expf(x[i]) overflows, but double-precision exp
    // doesn't overflow.
    auto f32rng = [&rng]() {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      vscaleextexp(elements() * sizeof(float), x.data(), y.data(),
      ...
      for (size_t i = 0; i < elements(); i++) {
        ASSERT_NEAR(y_ref[i], y[i], std::abs(y_ref[i]) * 1.0e-6)
            << "elements = " << elements()
            << ", scale:mantissa = " << scale_mantissa
            << ", scale:exponent = " << scale_exponent;
      }
    }
  }
