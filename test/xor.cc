...
using XORTestS32 = BinaryTest<int32_t>;

TEST_F(XORTestS32, define) {
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_xor(subgraph, input1_id, input2_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_xor);
...
TEST_F(XORTestS32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_xor_nd_s32(/*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_xor_nd_s32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_xor_nd_s32(op, input1.data(), input2.data(), operator_output.data()));
