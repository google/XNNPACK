...
class Tester {
 public:
  ...
  Tester& input_stride1(size_t value) {
  ...
  void Test(UKernelFn<Input, Output, Params> ukernel,
    ...
    for (size_t channels = this->channels().begin;
      ...
      for (size_t rows = this->rows().begin; rows < this->rows().end;
        ...
        xnnpack::Buffer<Input> input(rows * input_stride1 * k2 * k3 + channels,
        ...
        ukernel(channels, rows, k2, k3, input.data(),
                input_stride1 * sizeof(Input), input_stride2 * sizeof(Input),
                input_stride3 * sizeof(Input), zero.data(), output.data(),
                &params);

        const float tolerance = channels * rows * k2 * k3 * max_abs_value *
                                max_abs_value * scale * 2.0f *
...
INSTANTIATE_TEST_SUITE_P(
    rdsum2, Test, testing::ValuesIn(kernels),
