...
class SquareOperatorTester {
 public:
  inline SquareOperatorTester& channels(size_t channels) {
    assert(channels != 0);
  ...
  inline SquareOperatorTester& input_stride(size_t input_stride) {
  ...
  inline SquareOperatorTester& output_stride(size_t output_stride) {
  ...
  inline SquareOperatorTester& batch_size(size_t batch_size) {
    assert(batch_size != 0);
  ...
  inline SquareOperatorTester& iterations(size_t iterations) {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        for (size_t c = 0; c < channels(); c++) {
          const float value = fp16_ieee_to_fp32_value(input[i * input_stride() + c]);
          output_ref[i * channels() + c] = value * value;
        }
      }
      ...
      const xnn_status status = xnn_create_square_nc_f16(
        0, &square_op);
      if (status == xnn_status_unsupported_hardware) {
        GTEST_SKIP();
      }
      ASSERT_EQ(xnn_status_success, status);
      ASSERT_NE(nullptr, square_op);
      ...
      ASSERT_EQ(xnn_status_success, xnn_reshape_square_nc_f16(square_op, batch_size(),
  ...
  void TestF32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        for (size_t c = 0; c < channels(); c++) {
          const float value = input[i * input_stride() + c];
          output_ref[i * channels() + c] = value * value;
        }
      }
      ...
      ASSERT_EQ(xnn_status_success, xnn_reshape_square_nc_f32(square_op, batch_size(),
          channels(), input_stride(), output_stride(), /*threadpool=*/nullptr));
  ...
  void TestRunF32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      // Initialize and run Square operator.
      ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));

      ASSERT_EQ(xnn_status_success,
        xnn_run_square_nc_f32(
          channels(),
          input_stride(),
          output_stride(),
          batch_size(),
