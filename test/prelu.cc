...
using PreluTestQS8 = PreluTest<int8_t>;
using PreluTestQU8 = PreluTest<uint8_t>;
...
TEST_F(PreluTestQS8, define)
{
  ...
  const int32_t slope_zero_point = i8dist(rng);
  const float slope_scale = scale_dist(rng);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_qint8, input_zero_point, input_scale, input_dims.size(), input_dims.data(), nullptr, 0,
                          /*flags=*/XNN_VALUE_FLAG_EXTERNAL_INPUT, &input_id));
  ASSERT_NE(input_id, XNN_INVALID_NODE_ID);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_qint8, slope_zero_point, slope_scale, slope_dims.size(), slope_dims.data(), slope.data(), 1,
                          /*flags=*/0, &slope_id));
  ASSERT_NE(slope_id, XNN_INVALID_NODE_ID);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_qint8, output_zero_point, output_scale, input_dims.size(), input_dims.data(), nullptr, 2,
                          /*flags=*/XNN_VALUE_FLAG_EXTERNAL_OUTPUT, &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);
...
TEST_F(PreluTestQU8, define)
{
  ...
  const float input_scale = scale_dist(rng);
  const int32_t slope_zero_point = u8dist(rng);
  const float slope_scale = scale_dist(rng);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_quint8, input_zero_point, input_scale, input_dims.size(), input_dims.data(), nullptr, 0,
                          /*flags=*/XNN_VALUE_FLAG_EXTERNAL_INPUT, &input_id));
  ASSERT_NE(input_id, XNN_INVALID_NODE_ID);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_quint8, slope_zero_point, slope_scale, slope_dims.size(), slope_dims.data(), slope.data(), 1,
                          /*flags=*/0, &slope_id));
  ASSERT_NE(slope_id, XNN_INVALID_NODE_ID);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          subgraph, xnn_datatype_quint8, output_zero_point, output_scale, input_dims.size(), input_dims.data(), nullptr, 2,
                          /*flags=*/XNN_VALUE_FLAG_EXTERNAL_OUTPUT, &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);
...
TEST_F(PreluTestQS8, matches_operator_api)
{
  std::uniform_int_distribution<int32_t> i8dist(
    std::numeric_limits<int8_t>::min(),std::numeric_limits<int8_t>::max());
  std::uniform_real_distribution<float> scale_dist(0.25f, 0.50f);
  ...
  std::generate(slope.begin(), slope.end(), [&]() { return i8dist(rng); });
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status =
    xnn_create_prelu_nc_qs8(input_channels, slope_channels, input_channels, input_channels, slope.data(),
    input_zero_point, input_scale, slope_zero_point, slope_scale, output_zero_point, output_scale, /*flags=*/0, nullptr, nullptr, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_reshape_prelu_nc_qs8(op, batch_size, /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success,
    xnn_setup_prelu_nc_qs8(op, input.data(), operator_output.data()));
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_quantized_tensor_value(
      subgraph, xnn_datatype_qint8, slope_zero_point, slope_scale, slope_dims.size(), slope_dims.data(), slope.data(),
      /*external_id=*/1, /*flags=*/0, &slope_id));
  ASSERT_NE(slope_id, XNN_INVALID_NODE_ID);
...
TEST_F(PreluTestQU8, matches_operator_api)
{
  std::uniform_int_distribution<int32_t> u8dist(
    std::numeric_limits<uint8_t>::min(),std::numeric_limits<uint8_t>::max());
  std::uniform_real_distribution<float> scale_dist(0.25f, 0.50f);
  ...
  std::generate(slope.begin(), slope.end(), [&]() { return u8dist(rng); });
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status =
    xnn_create_prelu_nc_qu8(input_channels, slope_channels, input_channels, input_channels, slope.data(),
    input_zero_point, input_scale, slope_zero_point, slope_scale, output_zero_point, output_scale, /*flags=*/0, nullptr, nullptr, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_reshape_prelu_nc_qu8(op, batch_size, /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success,
    xnn_setup_prelu_nc_qu8(op, input.data(), operator_output.data()));
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_quantized_tensor_value(
      subgraph, xnn_datatype_quint8, slope_zero_point, slope_scale, slope_dims.size(), slope_dims.data(), slope.data(),
      /*external_id=*/1, /*flags=*/0, &slope_id));
  ASSERT_NE(slope_id, XNN_INVALID_NODE_ID);
