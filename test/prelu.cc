...
using PreluTestF16 = BinaryTest<xnn_float16>;
using PreluTestF32 = BinaryTest<float>;
...
TEST_F(PreluTestF16, define)
{
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(subgraph, xnn_datatype_fp16, input2_dims.size(),
  ...
  ASSERT_EQ(xnn_status_success, xnn_define_prelu(subgraph, input1_id, input2_id,
                                                 output_id, /*flags=*/0));
...
TEST_F(PreluTestF32, define)
{
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(subgraph, xnn_datatype_fp32, input2_dims.size(),
                              input2_dims.data(), nullptr,
                              /*external_id=*/0, /*flags=*/0, &input2_id));
  ASSERT_NE(input2_id, XNN_INVALID_NODE_ID);
...
TEST_F(PreluTestF16, matches_operator_api)
{
  ...
  const xnn_status status =
      xnn_create_prelu_nd_f16(output_min, output_max, 0, &op);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_prelu_nd_f16(op, input1_dims.size(), input1_dims.data(),
                                     input2_dims.size(), input2_dims.data(),
                                     /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success,
            xnn_setup_prelu_nd_f16(op, input1.data(), input2.data(),
...
TEST_F(PreluTestF32, matches_operator_api)
{
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_create_prelu_nd_f32(-std::numeric_limits<float>::infinity(),
                              std::numeric_limits<float>::infinity(), 0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(xnn_status_success,
            xnn_reshape_prelu_nd_f32(op, input1_dims.size(), input1_dims.data(),
                                     input2_dims.size(), input2_dims.data(),
                                     /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success,
            xnn_setup_prelu_nd_f32(op, input1.data(), input2.data(),
                                   operator_output.data()));
