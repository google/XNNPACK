  ...
  xnn_qu8_requantize_fn requantize) const
{
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    for (size_t i = 0; i < m(); i++) {
      for (size_t j = 0; j < n(); j++) {
        ASSERT_LE(uint32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), uint32_t(qmax()));
        ASSERT_GE(uint32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), uint32_t(qmin()));
        ASSERT_EQ(uint32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), uint32_t(c_ref[i * n() + j]))
  ...
  xnn_qs8_requantize_fn requantize) const
{
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    for (size_t i = 0; i < m(); i++) {
      for (size_t j = 0; j < n(); j++) {
        ASSERT_LE(int32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), int32_t(qmax()) - 0x80);
        ASSERT_GE(int32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), int32_t(qmin()) - 0x80);
        ASSERT_EQ(int32_t(c[i * cm_stride() + (j / nr()) * nr() + j % nr()]), int32_t(c_ref[i * n() + j]))
...
void GemmMicrokernelTester::Test(
    ...
    xnn_init_f32_minmax_params_fn init_params,
    xnn_pack_bf16_f32_gio_gemm_fn pack) const {
  if (a_stride() < k()) {
    return;
  }
  if (m() > mr()) {
  ...
  xnnpack::ReplicableRandomDevice rng;
  auto f32rng = std::bind(std::uniform_real_distribution<float>(0.5f, 1.0f),
                          std::ref(rng));
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    pack(/*g=*/1, n(), k(), nr(), kr(), sr(), n(), b.data(), bias.data(),
    ...
    for (size_t m_index = 0; m_index < m(); m_index++) {
      for (size_t n_index = 0; n_index < n(); n_index++) {
        ...
        for (size_t k_index = 0; k_index < k(); k_index++) {
          ...
          c_ref[m_index * n() + n_index] +=
              a[m_index * a_stride() + k_index] * b[k_index * n() + n_index];
        }
      }
    }
    ...
    const float accumulated_max =
        *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min =
        xnn_bfloat16(accumulated_min + (accumulated_max - accumulated_min) /
                                           255.0f * float(qmin()));
    const float c_max =
        xnn_bfloat16(accumulated_max - (accumulated_max - accumulated_min) /
                                           255.0f * float(255 - qmax()));
