...
void GemmMicrokernelTester::Test(xnn_qd8_f16_qc8w_igemm_ukernel_fn igemm,
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    init_params(&params, static_cast<xnn_float16>(c_min),
...
void GemmMicrokernelTester::Test(xnn_qd8_f16_qc4w_gemm_ukernel_fn gemm,
  ...
  {  // for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    init_params(&params, static_cast<xnn_float16>(c_min),
                static_cast<xnn_float16>(c_max), 8);
...
void GemmMicrokernelTester::Test(xnn_qd8_f16_qb4w_gemm_ukernel_fn gemm,
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    init_params(&params, static_cast<xnn_float16>(c_min),
                static_cast<xnn_float16>(c_max), 8, bl());
...
void GemmMicrokernelTester::Test(xnn_f16_gemm_minmax_ukernel_fn gemm_minmax,
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    const float accumulated_max =
        *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min =
        xnn_float16(accumulated_min + (accumulated_max - accumulated_min) /
                                          255.0f * float(qmin()));
    const float c_max =
        xnn_float16(accumulated_max - (accumulated_max - accumulated_min) /
                                          255.0f * float(255 - qmax()));
...
void GemmMicrokernelTester::Test(xnn_f16_igemm_minmax_ukernel_fn igemm_minmax,
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    const float c_min =
        xnn_float16(accumulated_min + (accumulated_max - accumulated_min) /
                                          255.0f * xnn_float16(qmin()));
    const float c_max =
        xnn_float16(accumulated_max - (accumulated_max - accumulated_min) /
                                          255.0f * xnn_float16(255 - qmax()));
    for (size_t m_index = 0; m_index < m(); m_index++) {
      for (size_t n_index = 0; n_index < n(); n_index++) {
        c_ref[m_index * n() + n_index] =
            std::min(c_ref[m_index * n() + n_index], c_max);
        c_ref[m_index * n() + n_index] =
            std::max(c_ref[m_index * n() + n_index], c_min);
      }
    }
...
void GemmMicrokernelTester::Test(xnn_f32_ppmm_minmax_ukernel_fn ppmm_minmax,
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    const float accumulated_max =
        *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min = accumulated_min + (accumulated_max - accumulated_min) /
                                              255.0f * float(qmin());
    const float c_max = accumulated_max - (accumulated_max - accumulated_min) /
                                              255.0f * float(255 - qmax());
...
void GemmMicrokernelTester::Test(
  ...
  for (size_t iteration = 0; iteration < kIterations; iteration++) {
    ...
    for (size_t m_index = 0; m_index < m(); m_index++) {
      for (size_t n_index = 0; n_index < n(); n_index++) {
        c_ref[m_index * n() + n_index] =
            std::max(std::min(c_ref[m_index * n() + n_index], double(c_max)),
                     double(c_min));
      }
    }
