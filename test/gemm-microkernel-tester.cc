...
void GemmMicrokernelTester::Test_PF16(
    xnn_packed_f16_lhs_igemm_ukernel_fn packed_igemm,
    xnn_init_f16_minmax_params_fn init_minmax_params,
    xnn_pack_lh_igemm_ukernel_fn pack_lh_for_igemm_fn,
    xnn_pack_lh_igemm_size_fn size_for_igemm_fn,
    xnn_pack_f16_igemm_fn pack_rhs)
    {
  ASSERT_LE(m(), mr());
  ...
  xnnpack::Buffer<xnn_float16> a_f16((mr() - 1) * a_stride() + k(), xnnpack::XnnExtraBytes,
                                         "input_f16");
  xnnpack::Buffer<xnn_float16> b_f16(n() * ks() * k(), /*extra_bytes=*/{0},
                                         "weights_f16");
  xnnpack::Buffer<xnn_float16, XNN_ALLOCATION_ALIGNMENT> b_packed_f16(ks() * packed_n() * packed_k() +
      packed_n() * (sizeof(int32_t) + sizeof(float)) / sizeof(int8_t), /*extra_bytes=*/{0},
                                         "weights_packed_f16");
  xnnpack::Buffer<xnn_float16> bias(n(), 0.0f);
  xnnpack::Buffer<xnn_float16> c((m() - 1) * cm_stride() + n(),
                                 /*extra_bytes=*/{0}, "c");
