...
class RDSumMicrokernelTester {
 public:
  RDSumMicrokernelTester& rows(LoopInfo rows) {
    this->rows_ = rows;
    return *this;
  }
  ...
  RDSumMicrokernelTester& channels(LoopInfo channels) {
  ...
  void Test(xnn_f16_f32acc_rdsum_ukernel_fn rdsum,
    ...
    for (size_t channels = this->channels().begin;
      ...
      for (size_t rows = this->rows().begin; rows < this->rows().end;
        ...
        // Call optimized micro-kernel.
        rdsum(rows, channels, input.data(), input_stride * sizeof(xnn_float16),
              zero.data(), output.data(), &params);

        // Verify results.
        for (size_t c = 0; c < channels; c++) {
          ASSERT_NEAR(output[c], output_ref[c],
                      std::abs(output_ref[c]) * 1.0e-5f)
              << "at position " << c << ", rows = " << rows
              << ", channels = " << channels;
        }
      }
 ...
 private:
  LoopInfo rows_{0, 1, 1};
  LoopInfo channels_{0, 1, 1};
