...
struct StencilParams {
  ...
  // they are "empty". Many randomly generated kernel parameters need the
  // empty reduction case to work. This helper detects these cases, so they can
  // be skipped.
  bool uses_identity(size_t input_extent, size_t output_extent) const {
...
inline std::ostream& operator<<(std::ostream& os, const StencilParams& params) {
  return os << "{size=" << params.size << ", dilation=" << params.dilation
...
// If `kernels[i]` is not `nullopt`, replace the corresponding dimension in `x`
// with two dimensions: a "spatial" dimension, and a "kernel" dimension. These
// dimensions "overlap" in memory, such that `result(i, di) = x(i + di)`. In
// this example, the kernel is a simple stride 1, but the function implements
// all kernel parameters.
template <typename T>
Tensor<T> make_stencil_dim(Tensor<T> x, size_t dim, const StencilParams& kernel) {
