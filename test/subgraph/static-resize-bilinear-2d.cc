...
TEST(ResizeBilinear, ReshapeOverflowInputElements) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  ...
  ASSERT_EQ(
      ...
      xnn_define_static_resize_bilinear_2d(
          subgraph, 4, 4, input_id, output_id, 0));
...
TEST(ResizeBilinear, ReshapeOverflowOutputSize) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  ...
  runtime->values[input_id].shape.num_dims = 4;
  runtime->values[input_id].shape.dim[0] = (SIZE_MAX / (4 * 4 * 2 * sizeof(float))) + 1;
  runtime->values[input_id].shape.dim[1] = 2;
  runtime->values[input_id].shape.dim[2] = 2;
  runtime->values[input_id].shape.dim[3] = 2;
