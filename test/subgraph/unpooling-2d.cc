    ...
    for (size_t y = 0; y < value.extent(1); y++) {
      for (size_t x = 0; x < value.extent(2); x++) {
        for (size_t c = 0; c < output.extent(3); c++) {
          ...
          const size_t oy = std::min<size_t>(std::max<size_t>(y * kh.size + dy, kh.padding_min) - kh.padding_min, output.extent(1) - 1);
          const size_t ox = std::min<size_t>(std::max<size_t>(x * kw.size + dx, kw.padding_min) - kw.padding_min, output.extent(2) - 1);
          output(i, oy, ox, c) = value(i, y, x, c);
        }
      }
    }
  }
...
void TestImpl() {
  ...
  for (int rep = 0; rep < 100; ++rep) {
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      // not necessary.
      if (kh.uses_identity(output_shape[1], input_shape[1]) ||
          kw.uses_identity(output_shape[2], input_shape[2])) {
        continue;
      }
