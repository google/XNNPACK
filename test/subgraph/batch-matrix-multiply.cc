...
void ReferenceImpl(Tensor<InputA> input_a, Tensor<InputB> input_b,
                   ...
                   const xnn_quantization_params& input_b_quantization,
                   Tensor<float> output) {
  ...
  for (size_t i = 0; i < output.extent(0); ++i) {
    for (size_t j = 0; j < output.extent(1); ++j) {
      ...
      for (size_t k = 0; k < input_a.extent(1); ++k) {
        ...
        float b = dequantize(input_b(k, j), input_b_quantization);
        output_ij += a * b;
      }
    }
  }
...
Tensor<float> ReferenceImpl(Tensor<InputA> input_a, Tensor<InputB> input_b,
  ...
  for (const auto& i : EnumerateIndices(output_batches)) {
    ...
    ReferenceImpl(a_i, b_i, input_a_quantization, input_b_quantization,
                  output_i);
  }
...
xnn_quantization_params quantization_for_range(float min, float max) {
  ...
  result.scale = (max - min) / (
      static_cast<float>(NumericLimits<T>::max()) -
...
xnn_quantization_params CalculateBatchMatrixMultiplyQuantizationParams(
    size_t reduction_size, xnn_quantization_params input_a_quantization,
    xnn_quantization_params input_b_quantization) {
  if (!xnn_datatype_is_quantized(xnn_datatype_of<Output>())) {
    return {0, 1.0f};
  }

  // Get the dequantized input_a and input_b ranges.
  const float input_a_min =
      dequantize(NumericLimits<Input>::min(), input_a_quantization);
  const float input_a_max =
      dequantize(NumericLimits<Input>::max(), input_a_quantization);
  const float input_b_min =
      dequantize(NumericLimits<Input>::min(), input_b_quantization);
  const float input_b_max =
      dequantize(NumericLimits<Input>::max(), input_b_quantization);

  // Find the range of the product of an input_a and an input_b value.
  std::array<float, 4> corners = {
      input_a_min * input_b_min,
      input_a_max * input_b_min,
      input_a_min * input_b_max,
      input_a_max * input_b_max,
  };
  auto input_a_input_b_minmax =
      std::minmax_element(corners.begin(), corners.end());

  const float output_min = *input_a_input_b_minmax.first * reduction_size;
  const float output_max = *input_a_input_b_minmax.second * reduction_size;
...
void TestDynamicB(uint64_t subgraph_flags = xnn_test_runtime_flags()) {
  ...
  xnn_quantization_params input_a_quantization =
  ...
  xnn_quantization_params output_quantization =
      CalculateBatchMatrixMultiplyQuantizationParams<Input, Output>(
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(1000))) {
    ...
    if (xnn_datatype_is_quantized(xnn_datatype_of<Input>())) {
    ...
    subgraph.AddBatchMatrixMultiply(input_a_id, input_b_id, output_id, flags);
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      size_t mn[] = {dim_dist(rng), dim_dist(rng), dim_dist(rng)};
      if (!is_k_big) {
      ...
      if (xnn_datatype_is_quantized(xnn_datatype_of<Input>())) {
        expected = ReferenceImpl(input_a, input_b, input_a_quantization,
                                 input_b_quantization, flags);
      } else {
        // There is no quantization in this case, make a dummy scale for b.
        Tensor<float> b_scales({1, 1});
        b_scales.fill(1.0f);
        broadcast_extent_1(b_scales);
        expected = ReferenceImpl(input_a, input_b, b_scales, flags);
      }
      ...
      if (xnn_datatype_is_quantized(xnn_datatype_of<Input>())) {
        ...
        for (const auto& i : EnumerateIndices(output.shape())) {
          ASSERT_NEAR(
              quantize<Output>(output(i), output_quantization),
...
void TestStaticB(xnn_datatype convert_to = xnn_datatype_invalid,
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(1000))) {
    ...
    const bool is_qs8_qs8 = std::is_same_v<InputA, InputB> &&
                            std::is_same_v<InputA, xnnpack::quantized<int8_t>>;
    ...
    // quantization.
    xnn_quantization_params output_quantization =
        CalculateBatchMatrixMultiplyQuantizationParams<InputA, Output>(
            k, input_a_quantization, input_b_quantization);
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      if (is_qs8_qs8) {
        const float tolerance = 1.0f;
        for (const auto& i : EnumerateIndices(output.shape())) {
          ASSERT_NEAR(quantize<Output>(output(i), output_quantization),
                      expected(i), tolerance)
              << "i=" << index_to_string(i)
              << ", a_shape=" << index_to_string(a_shape)
              << ", b_shape=" << index_to_string(input_b.shape())
