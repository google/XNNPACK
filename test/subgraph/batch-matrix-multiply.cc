...
void TestDynamicB(xnn_datatype convert_to = xnn_datatype_invalid,
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(1000))) {
    ...
    if (xnn_datatype_is_channelwise_quantized(xnn_datatype_of<InputB>())) {
      ...
      if (flags & XNN_FLAG_TRANSPOSE_B) {
        std::swap(channelwise_b_shape[input_b_rank - 1],
                  channelwise_b_shape[input_b_rank - 2]);
      }
...
TEST(BatchMatrixMultiply, ReshapeOverflowInputAElements) {
  ...
  uint32_t input_a_id = XNN_INVALID_VALUE_ID;
  const size_t input_a_dims[3] = {1, 2, 2};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(
          subgraph, xnn_datatype_fp32, 3, input_a_dims, nullptr,
          /*external_id=*/0, XNN_VALUE_FLAG_EXTERNAL_INPUT, &input_a_id));

  uint32_t input_b_id = XNN_INVALID_VALUE_ID;
  const size_t input_b_dims[3] = {1, 2, 2};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(
          subgraph, xnn_datatype_fp32, 3, input_b_dims, nullptr,
          /*external_id=*/1, XNN_VALUE_FLAG_EXTERNAL_INPUT, &input_b_id));

  uint32_t output_id = XNN_INVALID_VALUE_ID;
  const size_t output_dims[3] = {1, 2, 2};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(
  ...
  ASSERT_EQ(
      ...
      xnn_define_batch_matrix_multiply(
          subgraph, input_a_id, input_b_id, output_id, /*flags=*/0));
  ...
  runtime->values[input_a_id].shape.num_dims = 3;
  runtime->values[input_a_id].shape.dim[0] = SIZE_MAX;
  runtime->values[input_a_id].shape.dim[1] = 2;
  runtime->values[input_a_id].shape.dim[2] = 2;

  const enum xnn_status reshape_status = xnn_reshape_runtime(runtime);
  EXPECT_EQ(xnn_status_invalid_parameter, reshape_status);
}

TEST(BatchMatrixMultiply, ReshapeOverflowInputBElements) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  ...
  runtime->values[input_b_id].shape.num_dims = 3;
  runtime->values[input_b_id].shape.dim[0] = SIZE_MAX;
  runtime->values[input_b_id].shape.dim[1] = 2;
  runtime->values[input_b_id].shape.dim[2] = 2;

  const enum xnn_status reshape_status = xnn_reshape_runtime(runtime);
  EXPECT_EQ(xnn_status_invalid_parameter, reshape_status);
}

TEST(BatchMatrixMultiply, ReshapeOverflowOutputSize) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  ...
  uint32_t input_a_id = XNN_INVALID_VALUE_ID;
  const size_t input_a_dims[3] = {1, 1, 1};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(
          subgraph, xnn_datatype_fp32, 3, input_a_dims, nullptr,
  ...
  uint32_t input_b_id = XNN_INVALID_VALUE_ID;
  const size_t input_b_dims[3] = {1, 1, 1};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(
          subgraph, xnn_datatype_fp32, 3, input_b_dims, nullptr,
  ...
  runtime->values[input_a_id].shape.num_dims = 3;
  runtime->values[input_a_id].shape.dim[0] = (SIZE_MAX / 4) + 1;
  runtime->values[input_a_id].shape.dim[1] = 1;
  runtime->values[input_a_id].shape.dim[2] = 1;

  runtime->values[input_b_id].shape.num_dims = 3;
  runtime->values[input_b_id].shape.dim[0] = 1;
  runtime->values[input_b_id].shape.dim[1] = 1;
  runtime->values[input_b_id].shape.dim[2] = 1;
