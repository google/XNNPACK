...
class ConvolutionTestBase : public ::testing::Test {
 protected:
  ConvolutionTestBase() {
    ...
    filter = xnnpack::Buffer<KernelType>(groups * group_output_channels *
...
template <class InputType, class KernelType = InputType,
          class BiasType = InputType, class OutputType = InputType>
class QuantizedConvolutionTestBase
    : public ConvolutionTestBase<InputType, KernelType, BiasType, OutputType> {
 protected:
  QuantizedConvolutionTestBase() {
    ...
    w8dist = std::uniform_int_distribution<int32_t>(
        -std::numeric_limits<KernelType>::max(),
        std::numeric_limits<KernelType>::max());
...
using ConvolutionTestQD8F16QC8W =
    QuantizedConvolutionTestBase<float, int8_t, float, xnn_float16>;
using ConvolutionTestQD8F32QC8W =
    QuantizedConvolutionTestBase<float, int8_t, float, float>;
...
TEST_F(ConvolutionTestQD8F16QC8W, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value(
                ...
                filter.data(), /*external_id=*/1, /*flags=*/0, &kernel_id));
  ...
  ASSERT_EQ(
      ...
      xnn_define_convolution_2d(
          ...
          output_min, output_max, input_id, kernel_id, bias_id, output_id,
          /*flags=*/0));
...
TEST_F(ConvolutionTestQD8F16QC8W,
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_qd8_f16_qc8w(
                convolution_op, /*workspace=*/nullptr, operator_dq_data.data(),
                operator_output.data(), quantization_params.data()));
  ASSERT_EQ(xnn_status_success,
            xnn_run_operator(convolution_op, /*threadpool=*/nullptr));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_convolution_2d(
                subgraph, input_padding_top, input_padding_right,
                input_padding_bottom, input_padding_left, kernel_height,
                kernel_width, subsampling_height, subsampling_width,
                dilation_height, dilation_width, groups, group_input_channels,
                group_output_channels, output_min, output_max, dq_quantized_id,
...
TEST_F(ConvolutionTestQD8F32QC8W,
       internally_allocated_dynamic_quantization_parameters) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  xnn_subgraph_t subgraph = nullptr;
  ASSERT_EQ(xnn_status_success, xnn_create_subgraph(/*external_value_ids=*/4,
...
TEST_F(ConvolutionTestQU8, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_quantized_tensor_value(
                subgraph, xnn_datatype_quint8, 0, 1.0f, filter_dims.size(),
  ...
  ASSERT_EQ(
      ...
      xnn_define_convolution_2d(
          ...
          subsampling_height, subsampling_width, dilation_height,
          dilation_width, groups, group_input_channels, group_output_channels,
          output_min, output_max, input_id, filter_id, bias_id, output_id,
          /*flags=*/0));
  ...
  ASSERT_EQ(node->params.convolution_2d.input_padding_right,
            input_padding_right);
  ASSERT_EQ(node->params.convolution_2d.input_padding_bottom,
  ...
  ASSERT_EQ(node->params.convolution_2d.group_input_channels,
            group_input_channels);
  ASSERT_EQ(node->params.convolution_2d.group_output_channels,
...
TEST_F(ConvolutionTestQC8, matches_operator_api) {
  ...
  compute_convolution_qs8_reference_results(
      ...
      input_padding_left, kernel_height, kernel_width, subsampling_height,
      subsampling_width, dilation_height, dilation_width, groups,
      group_input_channels, group_output_channels, input_zero_point, input,
      filter, accumulators,
      /*has_bias=*/true, bias);
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qs8_qc8w(
      ...
      filter.data(), bias.data(), output_zero_point, output_scale,
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value(
                subgraph, xnn_datatype_qcint8, requantization_scales.data(),
                filter_dims.size(), 0, filter_dims.data(), filter.data(),
                /*external_id=*/1, /*flags=*/0, &filter_id));
...
TEST_F(ConvolutionTestQS8, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qs8(
      ...
      group_input_channels, group_output_channels,
      groups * group_input_channels, groups * group_output_channels,
      input_zero_point, input_scale, kernel_scale, filter.data(), bias.data(),
...
TEST_F(ConvolutionTestQU8, matches_operator_api) {
  ...
  compute_convolution_qu8_reference_results(
      ...
      input_padding_left, kernel_height, kernel_width, subsampling_height,
      subsampling_width, dilation_height, dilation_width, groups,
      group_input_channels, group_output_channels, input_zero_point,
...
TEST_F(ConvolutionTestF16, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_f16(
      ...
      group_input_channels, group_output_channels,
      groups * group_input_channels, groups * group_output_channels,
      filter.data(), bias.data(), output_min, output_max,
...
TEST_F(ConvolutionTestF32, transient_indirection_buffer) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_convolution2d_nhwc_f32(
                ...
                /*output_height_out=*/nullptr, /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  // workspace_size might be 0 if we hit the vmulcaddc path which does not
  // require any indirection buffers.
