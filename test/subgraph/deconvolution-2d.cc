...
template <class T, class KernelType = T, class BiasType = T>
class DeconvolutionTestBase : public ::testing::Test {
 protected:
  DeconvolutionTestBase() {
    ...
    output_height = xnn_compute_deconvolution_output_dimension(
        input_height, padding_top + padding_bottom, adjustment_height,
        kernel_height, dilation_height, upsampling_height);
    output_width = xnn_compute_deconvolution_output_dimension(
        input_width, padding_left + padding_right, adjustment_width,
        kernel_width, dilation_width, upsampling_width);

    input_dims = {
        {batch_size, input_height, input_width, groups * group_input_channels}};
    kernel_dims = {{groups * group_output_channels, kernel_height, kernel_width,
    ...
    kernel = xnnpack::Buffer<KernelType>(groups * group_output_channels *
                                         kernel_height * kernel_width *
                                         group_input_channels);
    bias = xnnpack::Buffer<BiasType>(groups * group_output_channels);
    operator_output =
        xnnpack::Buffer<T>(batch_size * output_height * output_width * groups *
...
template <class T>
class QuantizedDeconvolutionTestBase
    : public DeconvolutionTestBase<T, T, int32_t> {
 protected:
  QuantizedDeconvolutionTestBase() {
    ...
    accumulators = xnnpack::Buffer<int32_t>(
        ...
        this->groups * this->group_output_channels);
  }
        ...
        for (size_t ox = 0; ox < this->output_width; ox++) {
          for (size_t g = 0; g < this->groups; g++) {
            for (size_t oc = 0; oc < this->group_output_channels; oc++) {
              accumulators
                  [(((i * this->output_height + oy) * this->output_width + ox) *
                        this->groups +
                    g) *
...
TEST_F(DeconvolutionTestQS8, define) {
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_channelwise_quantized_tensor_value(
          subgraph, xnn_datatype_qcint32, kernel_scale.data(), bias_dims.size(),
          /*channel_dim=*/0, bias_dims.data(), bias.data(),
  ...
  ASSERT_EQ(
      ...
      xnn_define_deconvolution_2d(
          subgraph, padding_top, padding_right, padding_bottom, padding_left,
          adjustment_height, adjustment_width, kernel_height, kernel_width,
          upsampling_height, upsampling_width, dilation_height, dilation_width,
          groups, group_input_channels, group_output_channels, output_min,
          output_max, input_id, kernel_id, bias_id, output_id,
          /*flags=*/0));
  ...
  ASSERT_EQ(node->params.deconvolution_2d.group_input_channels,
            group_input_channels);
  ASSERT_EQ(node->params.deconvolution_2d.group_output_channels,
            group_output_channels);
...
TEST_F(DeconvolutionTestQD8F32QC8W, define) {
  xnnpack::Buffer<float> requantization_scales(group_output_channels * groups,
                                               1.0f);
...
TEST_F(DeconvolutionTestQS8, matches_operator_api) {
                      ...
                      accumulators[(((i * output_height + oy) * output_width +
                          ...
                          (int32_t(
                               input[((i * input_height + iy) * input_width +
                                      ix) *
                                         g * group_input_channels +
                                     ic]) -
                           int32_t(input_zero_point)) *
                          int32_t(kernel[(((g * group_output_channels + oc) *
                                               kernel_height +
                                           ky) *
                                              kernel_width +
                                          kx) *
  ...
  const xnn_status status = xnn_create_deconvolution2d_nhwc_qs8_qc8w(
      ...
      input_zero_point, input_scale, kernel_scale.data(), kernel.data(),
      bias.data(), output_zero_point, output_scale, quantized_output_min,
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_deconvolution2d_nhwc_qs8_qc8w(
                op, batch_size, input_height, input_width, adjustment_height,
                adjustment_width,
                /*output_height_out=*/nullptr, /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_deconvolution2d_nhwc_qs8_qc8w(
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value(
                subgraph, xnn_datatype_qcint32, kernel_scale.data(),
                bias_dims.size(), /*channel_dim=*/0, bias_dims.data(),
...
TEST_F(DeconvolutionTestQU8, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_deconvolution2d_nhwc_qu8(
                ...
                /*output_height_out=*/nullptr, /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_deconvolution2d_nhwc_qu8(
                                    op, input.data(), operator_output.data()));
...
TEST_F(DeconvolutionTestF16, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_deconvolution2d_nhwc_f16(
                ...
                /*output_height_out=*/nullptr, /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_deconvolution2d_nhwc_f16(
                                    op, input.data(), operator_output.data()));
...
TEST_F(DeconvolutionTestQD8F32QC8W,
  ...
  xnnpack::Buffer<float> convert_input(batch_size * input_height * input_width *
                                           groups * group_input_channels +
  ...
  const size_t quantized_batch_size =
      input_height * input_width * group_input_channels * groups;
  ...
  ASSERT_EQ(
      ...
      xnn_reshape_convert_nc_f32_qd8(
          convert_op, batch_size, quantized_batch_size, quantized_batch_size,
          quantized_batch_size, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success,
  ...
  status = xnn_create_deconvolution2d_nhwc_qd8_f32_qc8w(
      ...
      /*flags=*/0, nullptr, nullptr, &deconvolution_op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_setup_deconvolution2d_nhwc_qd8_f32_qc8w(
                deconvolution_op, operator_dq_data.data(),
                operator_output.data(), quantization_params.data()));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_dynamically_quantized_tensor_value(
                subgraph, xnn_datatype_qdint8, input_dims.size(),
                /*num_nonbatch_dims=*/3, input_dims.data(),
  ...
  ASSERT_EQ(
      ...
      xnn_define_tensor_value(subgraph, xnn_datatype_fp32, bias_dims.size(),
                              bias_dims.data(), bias.data(),
                              /*external_id=*/2, /*flags=*/0, &bias_id));
  ASSERT_NE(bias_id, XNN_INVALID_NODE_ID);
  ...
  ASSERT_EQ(
      ...
      xnn_define_deconvolution_2d(
          ...
          upsampling_height, upsampling_width, dilation_height, dilation_width,
          groups, group_input_channels, group_output_channels, output_min,
          output_max, dq_quantized_id, kernel_id, bias_id, output_id,
          /*flags=*/0));
...
TEST_F(DeconvolutionTestF32, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_deconvolution2d_nhwc_f32(
                ...
                /*output_height_out=*/nullptr, /*output_width_out=*/nullptr,
                /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_deconvolution2d_nhwc_f32(
                                    op, input.data(), operator_output.data()));
