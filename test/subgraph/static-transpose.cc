...
TEST(Transpose, ReshapeOverflowInputElements) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  ...
  const size_t perm[3] = {2, 0, 1};
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_static_transpose(subgraph, 3, perm, input_id, output_id, 0));
...
TEST(Transpose, ReshapeOverflowOutputSize) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
