...
TEST(MEAN_ND_QU8, reduce_first_axis) {
  MeanOperatorTester()
    .input_shape({kDim1, kDim2})
    .reduction_axes({0})
    .TestQU8();
...
TEST(MEAN_ND_QU8, reduce_last_axis) {
  MeanOperatorTester()
    .input_shape({kDim1, kDim2, kDim3})
    .reduction_axes({0,2})
    .TestQU8();
...
TEST(MEAN_ND_QU8, reduce_last_axis2) {
  MeanOperatorTester()
    .input_shape({kDim1, kDim2, kDim3})
    .reduction_axes({0,2})
    .TestQU8();
...
TEST(MEAN_ND_QU8, reduce_6d_multithreaded) {
  std::vector<size_t> reduction_axes;
  for (uint32_t bm1 = 1; bm1 < (uint32_t(1) << 6); bm1++) {
    const bool reduce_dim1 = (bm1 & UINT32_C(1)) != 0;
    const bool reduce_dim2 = (bm1 & UINT32_C(2)) != 0;
