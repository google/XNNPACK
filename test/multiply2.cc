...
using Multiply2TestF16 = BinaryTest<xnn_float16>;
...
TEST_F(Multiply2TestF16, matches_operator_api)
{
  ...
  // Call operator API.
  const xnn_status status = xnn_create_multiply_nd_f16(0, &op);
...
TEST_F(Multiply2TestF32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_multiply_nd_f32(0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);
