...
template <class T>
class StaticResizeBilinear2DTestBase : public ::testing::Test {
 protected:
  StaticResizeBilinear2DTestBase() {
    ...
    input =
        xnnpack::Buffer<T>(XNN_EXTRA_BYTES / sizeof(T) +
...
using StaticResizeBilinear2DTestF16 =
    StaticResizeBilinear2DTestBase<xnn_float16>;
...
TEST_F(StaticResizeBilinear2DTestQS8, define) {
  ...
  ASSERT_EQ(xnn_status_success, xnn_define_static_resize_bilinear_2d(
                                    subgraph, output_height, output_width,
...
TEST_F(StaticResizeBilinear2DTestQS8, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_resize_bilinear2d_nhwc(
      xnn_datatype_qint8, output_height, output_width, /*flags=*/0, &op);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_resize_bilinear2d_nhwc(
                op, batch_size, input_height, input_width, channels, channels,
                channels, &workspace_size, &workspace_alignment,
                /*threadpool=*/nullptr));
  ASSERT_EQ(workspace_size, 0);
  ASSERT_EQ(workspace_alignment, 1);
  ASSERT_EQ(xnn_status_success, xnn_setup_resize_bilinear2d_nhwc(
                                    op, /*workspace=*/nullptr, input.data(),
...
TEST_F(StaticResizeBilinear2DTestQU8, matches_operator_api) {
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_resize_bilinear2d_nhwc(
      xnn_datatype_quint8, output_height, output_width, /*flags=*/0, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
...
TEST_F(StaticResizeBilinear2DTestF16, matches_operator_api) {
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_resize_bilinear2d_nhwc(
      xnn_datatype_fp16, output_height, output_width, /*flags=*/0, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
...
TEST_F(StaticResizeBilinear2DTestF32, matches_operator_api) {
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_resize_bilinear2d_nhwc(
      xnn_datatype_fp32, output_height, output_width, /*flags=*/0, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
