...
template <class T, class BiasType = T>
class DepthwiseConvolutionTestBase : public ::testing::Test {
 protected:
  DepthwiseConvolutionTestBase() {
    ...
    operator_output = xnnpack::Buffer<T>(batch_size * output_height *
                                         output_width * output_channels);
    subgraph_output = xnnpack::Buffer<T>(batch_size * output_height *
                                         output_width * output_channels);
  }
...
template <class T>
class QuantizedDepthwiseConvolutionTestBase
    : public DepthwiseConvolutionTestBase<T, int32_t> {
 protected:
  QuantizedDepthwiseConvolutionTestBase() {
    ...
    accumulators = xnnpack::Buffer<int32_t>(
        this->batch_size * this->output_height * this->output_width *
        this->input_channels * this->depth_multiplier);
  ...
  typedef typename std::conditional<std::is_same<T, uint8_t>::value, uint8_t,
                                    int8_t>::type ZeroPointType;
  ZeroPointType input_zero_point;
  ZeroPointType kernel_zero_point;
  ZeroPointType output_zero_point = 0;
};

using DepthwiseConvolutionTestQC8 =
    QuantizedDepthwiseConvolutionTestBase<int8_t>;
using DepthwiseConvolutionTestQS8 =
    QuantizedDepthwiseConvolutionTestBase<int8_t>;
using DepthwiseConvolutionTestQU8 =
    QuantizedDepthwiseConvolutionTestBase<uint8_t>;
using DepthwiseConvolutionTestF16 =
    DepthwiseConvolutionTestBase<xnn_float16, float>;
...
TEST_F(DepthwiseConvolutionTestQC8, define) {
  ...
  xnnpack::Buffer<float> requantization_scales(
      input_channels * depth_multiplier, 1.0f);
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value(
                ...
                filter_dims.size(), 3, filter_dims.data(), filter.data(),
                /*external_id=*/1,
                /*flags=*/0, &filter_id));
  ...
  ASSERT_EQ(
      ...
      xnn_define_depthwise_convolution_2d(
          ...
          dilation_width, depth_multiplier, input_channels, output_min,
          output_max, input_id, filter_id, bias_id, output_id,
          /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_depthwise_convolution_2d);
  ASSERT_EQ(node->params.depthwise_convolution_2d.input_padding_top,
            input_padding_top);
  ASSERT_EQ(node->params.depthwise_convolution_2d.input_padding_right,
            input_padding_right);
  ASSERT_EQ(node->params.depthwise_convolution_2d.input_padding_bottom,
            input_padding_bottom);
  ASSERT_EQ(node->params.depthwise_convolution_2d.input_padding_left,
            input_padding_left);
  ...
  ASSERT_EQ(node->params.depthwise_convolution_2d.subsampling_height,
            subsampling_height);
  ASSERT_EQ(node->params.depthwise_convolution_2d.subsampling_width,
            subsampling_width);
  ASSERT_EQ(node->params.depthwise_convolution_2d.dilation_height,
            dilation_height);
  ASSERT_EQ(node->params.depthwise_convolution_2d.dilation_width,
            dilation_width);
  ASSERT_EQ(node->params.depthwise_convolution_2d.depth_multiplier,
            depth_multiplier);
  ASSERT_EQ(node->params.depthwise_convolution_2d.input_channels,
            input_channels);
...
TEST_F(DepthwiseConvolutionTestQS8, define) {
  ...
  uint32_t filter_id = XNN_INVALID_NODE_ID;
  ASSERT_EQ(xnn_status_success,
            xnn_define_quantized_tensor_value(subgraph, xnn_datatype_qint8, 0,
                                              kernel_scale, filter_dims.size(),
                                              filter_dims.data(), filter.data(),
                                              /*external_id=*/1,
                                              /*flags=*/0, &filter_id));
...
TEST_F(DepthwiseConvolutionTestQU8, define) {
  ...
  uint32_t filter_id = XNN_INVALID_NODE_ID;
  ASSERT_EQ(xnn_status_success,
            xnn_define_quantized_tensor_value(subgraph, xnn_datatype_quint8, 0,
                                              kernel_scale, filter_dims.size(),
                                              filter_dims.data(), filter.data(),
                                              /*external_id=*/1,
                                              /*flags=*/0, &filter_id));
...
TEST_F(DepthwiseConvolutionTestF16, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_tensor_value(subgraph, xnn_datatype_fp32,
                                    filter_dims.size(), filter_dims.data(),
...
TEST_F(DepthwiseConvolutionTestQC8, matches_operator_api) {
  ...
  xnnpack::Buffer<float> requantization_scales(input_channels *
                                               depth_multiplier);
  ...
  compute_depthwise_convolution_qs8_reference_results(
      batch_size, output_height, output_width, input_height, input_width,
      ...
      depth_multiplier, input_zero_point, input, filter, accumulators,
      /*has_bias=*/true, bias);
    ...
    for (size_t px = 0; px < batch_size * output_height * output_width; px++) {
      accumulated_min =
          std::min(accumulated_min,
                   accumulators[px * input_channels * depth_multiplier + c]);
      accumulated_max =
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qs8_qc8w(
      input_padding_top, input_padding_right, input_padding_bottom,
      input_padding_left, kernel_height, kernel_width, subsampling_height,
      subsampling_width, dilation_height, dilation_width,
      /*groups=*/input_channels, /*group_input_channels=*/1,
      /*group_output_channels=*/depth_multiplier, input_channels,
      input_channels * depth_multiplier, input_zero_point, input_scale,
      requantization_scales.data(), filter.data(), bias.data(),
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_qs8_qc8w(
                op, workspace.data(), input.data(), operator_output.data()));
...
TEST_F(DepthwiseConvolutionTestQS8, matches_operator_api) {
  ...
  compute_convolution_qs8_reference_results(
      ...
      /*group_output_channels=*/depth_multiplier, input_zero_point, input,
      filter, accumulators,
      /*has_bias=*/true, bias);
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qs8(
      ...
      /*group_output_channels=*/depth_multiplier, input_channels,
      input_channels * depth_multiplier, input_zero_point, input_scale,
      kernel_scale, filter.data(), bias.data(), output_zero_point, output_scale,
  ...
  xnnpack::Buffer<char, XNN_ALLOCATION_ALIGNMENT> workspace(workspace_size);
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_qs8(op, workspace.data(), input.data(),
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_quantized_tensor_value(
                subgraph, xnn_datatype_qint8, kernel_zero_point, kernel_scale,
                filter_dims.size(), filter_dims.data(), filter.data(),
                /*external_id=*/1,
                /*flags=*/0, &filter_id));
...
TEST_F(DepthwiseConvolutionTestQU8, matches_operator_api) {
  ...
  compute_convolution_qu8_reference_results(
      ...
      /*groups=*/input_channels,
      /*group_input_channels=*/1,
      /*group_output_channels=*/depth_multiplier, input_zero_point,
      kernel_zero_point, input, filter, accumulators,
      /*has_bias=*/true, bias);
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_qu8(
      ...
      /*group_output_channels=*/depth_multiplier, input_channels,
      input_channels * depth_multiplier, input_zero_point, input_scale,
      kernel_zero_point, kernel_scale, filter.data(), bias.data(),
      output_zero_point, output_scale, quantized_output_min,
      quantized_output_max,
      /*flags=*/XNN_FLAG_DEPTHWISE_CONVOLUTION, nullptr, nullptr, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
  ...
  xnnpack::Buffer<char, XNN_ALLOCATION_ALIGNMENT> workspace(workspace_size);
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_qu8(op, workspace.data(), input.data(),
                                             operator_output.data()));
...
TEST_F(DepthwiseConvolutionTestF16, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_convolution2d_nhwc_f16(
      ...
      /*groups=*/input_channels, /*group_input_channels=*/1,
      /*group_output_channels=*/depth_multiplier, input_channels,
      input_channels * depth_multiplier, filter.data(), bias.data(), output_min,
      output_max,
      /*flags=*/XNN_FLAG_DEPTHWISE_CONVOLUTION | XNN_FLAG_FP32_STATIC_WEIGHTS,
      nullptr, nullptr, &op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(
      op, xnn_delete_operator);
  ...
  xnnpack::Buffer<char, XNN_ALLOCATION_ALIGNMENT> workspace(workspace_size);
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_f16(op, workspace.data(), input.data(),
                                             operator_output.data()));
  ...
  ASSERT_EQ(
      xnn_status_success,
      xnn_define_tensor_value(subgraph, xnn_datatype_fp32, filter_dims.size(),
                              filter_dims.data(), filter.data(),
...
TEST_F(DepthwiseConvolutionTestF32, matches_operator_api) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_setup_convolution2d_nhwc_f32(op, workspace.data(), input.data(),
                                             operator_output.data()));
...
TEST_F(DepthwiseConvolutionTestF32, reshape_output) {
  ...
  ASSERT_EQ(output_shape->dim[1],
            runtime->opdata[0].operator_objects[0]->output_height);
  ASSERT_EQ(output_shape->dim[2],
            runtime->opdata[0].operator_objects[0]->output_width);
