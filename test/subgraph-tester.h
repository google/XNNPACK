...
class SubgraphTester {
 public:
  ...
  SubgraphTester& AddInputTensorF32(const std::vector<size_t>& dims, uint32_t external_id) {
    ...
    std::generate(data, data + num_elements, [&]() { return f32dist(rng_); });
    auto it = external_tensors_.insert({external_id, std::move(input)});
  ...
  SubgraphTester& AddEvenSplit2(size_t split_dim, uint32_t input_id, uint32_t output1_id, uint32_t output2_id) {
    ...
    const xnn_status status =
      xnn_define_even_split(subgraph_.get(), xnn_even_split2, split_dim, input_id, 2, output_ids, 0 /* flags */);
    EXPECT_EQ(status, xnn_status_success);
    return *this;
  }
  ...
  SubgraphTester& AddEvenSplit3(size_t split_dim, uint32_t input_id, uint32_t output1_id, uint32_t output2_id, uint32_t output3_id) {
    ...
    const xnn_status status =
      xnn_define_even_split(subgraph_.get(), xnn_even_split3, split_dim, input_id, 3, output_ids, 0 /* flags */);
    EXPECT_EQ(status, xnn_status_success);
    return *this;
  }
  ...
  SubgraphTester& AddEvenSplit3(uint32_t input_id, uint32_t output_id0, uint32_t output_id1, uint32_t output_id2) {
    ...
    const xnn_status status =
      xnn_define_even_split(subgraph_.get(), xnn_even_split3, 0, input_id, 3, output_id, 0 /*flags */);
    EXPECT_EQ(status, xnn_status_success);
 ...
 protected:
  ...
  std::unordered_map<uint32_t, xnnpack::Buffer<char>> external_tensors_;
  uint32_t output_id_;
