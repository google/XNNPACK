...
#ifndef XNNPACK_TEST_OPERATORS_SLICE_OPERATOR_TESTER_H_
#define XNNPACK_TEST_OPERATORS_SLICE_OPERATOR_TESTER_H_
...
class SliceOperatorTester {
 public:
  ...
  size_t num_offsets() const { return offsets_.size(); }
  ...
  size_t size(size_t i) const { return i < sizes_.size() ? sizes_[i] : 1; }

  size_t num_sizes() const { return sizes_.size(); }

  size_t output_dim(size_t i) const { return size(i); }
  ...
  void TestX8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ComputeReference(input_dims, output_dims, input_offsets, input,
                       output_ref);
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_slice_nd_x8(0, &slice_op));
      ASSERT_NE(nullptr, slice_op);

      // Smart pointer to automatically delete slice_op.
      std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)>
          auto_slice_op(slice_op, xnn_delete_operator);

      ASSERT_EQ(
          xnn_status_success,
          xnn_reshape_slice_nd_x8(slice_op, num_dims(), input_shape().data(),
                                  offsets().data(), sizes().data(),
                                  /*threadpool=*/nullptr));

      ASSERT_EQ(xnn_status_success,
                xnn_setup_slice_nd_x8(slice_op, input.data(), output.data()));
  ...
  void TestX16() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_slice_nd_x16(0, &slice_op));
      ASSERT_NE(nullptr, slice_op);
      ...
      ASSERT_EQ(
          xnn_status_success,
          xnn_reshape_slice_nd_x16(slice_op, num_dims(), input_shape().data(),
                                   offsets().data(), sizes().data(),
                                   /*threadpool=*/nullptr));

      ASSERT_EQ(xnn_status_success,
                xnn_setup_slice_nd_x16(slice_op, input.data(), output.data()));
  ...
  void TestX32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_slice_nd_x32(0, &slice_op));
      ASSERT_NE(nullptr, slice_op);
      ...
      ASSERT_EQ(
          xnn_status_success,
          xnn_reshape_slice_nd_x32(slice_op, num_dims(), input_shape().data(),
                                   offsets().data(), sizes().data(),
                                   /*threadpool=*/nullptr));

      ASSERT_EQ(xnn_status_success,
                xnn_setup_slice_nd_x32(slice_op, input.data(), output.data()));
  ...
  void TestRunX32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
      ASSERT_EQ(xnn_status_success,
                xnn_run_slice_nd_x32(num_dims(), input_shape().data(),
                                     offsets().data(), sizes().data(),
 ...
 private:
  ...
  void ComputeReference(
      ...
      const xnnpack::Buffer<T>& input, xnnpack::Buffer<T>& output) const {
            ...
            for (size_t m = 0; m < output_dims[4]; m++) {
              for (size_t n = 0; n < output_dims[5]; n++) {
                const size_t output_index =
                    ((((i * output_dims[1] + j) * output_dims[2] + k) *
                          output_dims[3] +
                      l) *
                         output_dims[4] +
                     m) *
  ...
  size_t iterations_{
      1};  // Use less iteration because we test a lot of dimensions.
};

#endif  // XNNPACK_TEST_OPERATORS_SLICE_OPERATOR_TESTER_H_
