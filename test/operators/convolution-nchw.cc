...
TEST(CONVOLUTION_NCHW_F16,
     kernel_1x1_varying_input_channels_with_fp32_weights) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
...
TEST(CONVOLUTION_NCHW_F16,
     kernel_1x1_varying_output_channels_with_fp32_weights) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
...
// Weights cache is not supported for SPMM microkernel, add a test here, but
// skip the assertions.
...
TEST(CONVOLUTION_NCHW_F16,
     batched_1x1_varying_input_channels_with_fp32_weights) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  for (size_t input_channels = 1; input_channels <= 16; input_channels *= 4) {
    ConvolutionOperatorTester()
        .weights_type(ConvolutionOperatorTester::WeightsType::FP32)
...
TEST(CONVOLUTION_NCHW_F16,
     batched_1x1_varying_output_channels_with_fp32_weights) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
  for (size_t output_channels = 1; output_channels < 19; output_channels *= 2) {
    ConvolutionOperatorTester()
        .weights_type(ConvolutionOperatorTester::WeightsType::FP32)
