 ...
 public:
  ...
  void TestBF16toQD8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const float max_val = std::uniform_real_distribution<float>(
          min_val *
              (1.0f + std::numeric_limits<uint8_t>::max() * 7.8125e-3f),
          10.0f)(rng);
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        ...
        // bf16 has 7-bit mantissa (vs f16's 10), so scale/zero_point are
        // less precise.  Use a wider tolerance than the f16 test (0.8f).
        const float max_acceptable_error =
            2.0f * (rmax - rmin) / std::numeric_limits<uint8_t>::max();
  ...
  void TestBF16toQDU8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      xnn_status status = xnn_create_convert_nc_bf16_qdu8(0, &convert_op);
      if (status == xnn_status_unsupported_hardware) {
        GTEST_SKIP();
      }
      ASSERT_EQ(xnn_status_success, status);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_reshape_convert_nc_bf16_qdu8(
                    convert_op, batch_size(), channels(), input_stride(),
                    output_stride(), /*threadpool=*/nullptr));
      ASSERT_EQ(xnn_status_success, xnn_setup_convert_nc_bf16_qdu8(
                                        convert_op, input.data(), output.data(),
                                        /*row_sum=*/nullptr,
                                        quantization_params.data()));
      ASSERT_EQ(xnn_status_success,
  ...
  void TestF32toQDU8QC2W() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(
          xnn_status_success,
          xnn_create_convert_nc_f32_qdu8(
              /*flags=*/XNN_NODE_FLAG_REQUIRES_ROW_SUM, &convert_op));
      ASSERT_NE(nullptr, convert_op);
...
TEST(CONVERT_NC_BF16_QDU8, unit_batch) {
  for (size_t channels = 1; channels < 100; channels++) {
    ConvertOperatorTester()
        .batch_size(1)
        .channels(channels)
...
TEST(CONVERT_NC_BF16_QDU8, small_batch) {
  for (size_t channels = 1; channels < 100; channels++) {
    ConvertOperatorTester()
        .batch_size(3)
        .channels(channels)
...
TEST(CONVERT_NC_BF16_QDU8, small_batch_with_input_stride) {
  for (size_t channels = 10; channels < 11; channels += 15) {
    ConvertOperatorTester()
        .batch_size(3)
        .channels(channels)
...
TEST(CONVERT_NC_BF16_QDU8, small_batch_with_output_stride) {
  for (size_t channels = 1; channels < 100; channels += 15) {
    ConvertOperatorTester()
        .batch_size(3)
        .channels(channels)
...
TEST(CONVERT_NC_BF16_QDU8, small_batch_with_input_and_output_stride) {
  for (size_t channels = 1; channels < 100; channels += 15) {
    ConvertOperatorTester()
        .batch_size(3)
        .channels(channels)
