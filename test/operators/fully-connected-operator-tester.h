...
class FullyConnectedOperatorTester {
 public:
  ...
  void TestQP8F32QC2W() const {
    ...
    // The M=1 kernel uses an M=1 packed LHS layout, while multi-row kernels
    // use the MR reported by the GEMM configuration.
  ...
  void TestQS8QC2W() const {
    ...
    for (size_t iteration = 0; iteration < kIterations; ++iteration) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_reshape_fully_connected_nc_qs8_qc2w(
                                        fully_connected_op, batch_size(),
                                        /*threadpool=*/nullptr));
