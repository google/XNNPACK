...
#ifndef XNNPACK_TEST_OPERATORS_DEPTH_TO_SPACE_OPERATOR_TESTER_H_
#define XNNPACK_TEST_OPERATORS_DEPTH_TO_SPACE_OPERATOR_TESTER_H_
...
class DepthToSpaceOperatorTester {
 public:
  DepthToSpaceOperatorTester& input_size(size_t input_height,
                                         size_t input_width) {
  ...
  size_t output_height() const { return input_height() * block_size(); }

  size_t output_width() const { return input_width() * block_size(); }
  ...
  void TestNHWCxX8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_depth_to_space_nhwc_x8(
      ...
      std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)>
          auto_depth_to_space_op(depth_to_space_op, xnn_delete_operator);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_setup_depth_to_space_nhwc_x8(depth_to_space_op,
              ...
              for (size_t bx = 0; bx < block_size(); bx++) {
                for (size_t oc = 0; oc < output_channels(); oc++) {
                  const size_t input_index =
                      ((i * input_height() + iy) * input_width() + ix) *
                          input_channels_stride() +
                      (by * block_size() + bx) * output_channels() + oc;
                  const size_t output_index =
                      ((i * output_height() + iy * block_size() + by) *
                           output_width() +
                       ix * block_size() + bx) *
                          output_channels_stride() +
                      oc;
  ...
  void TestNHWCxX16() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_depth_to_space_nhwc_x16(
                                        block_size(), 0, &depth_to_space_op));
      ASSERT_NE(nullptr, depth_to_space_op);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_setup_depth_to_space_nhwc_x16(depth_to_space_op,
                                                  input.data(), output.data()));
  ...
  void TestNHWCxX32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_depth_to_space_nhwc_x32(
                                        block_size(), 0, &depth_to_space_op));
      ASSERT_NE(nullptr, depth_to_space_op);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_setup_depth_to_space_nhwc_x32(depth_to_space_op,
                                                  input.data(), output.data()));
  ...
  void TestNCHW2NHWCxX16() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_depth_to_space_nchw2nhwc_x16(
                                        block_size(), 0, &depth_to_space_op));
      ASSERT_NE(nullptr, depth_to_space_op);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_setup_depth_to_space_nchw2nhwc_x16(
                    depth_to_space_op, input.data(), output.data()));
          ...
          for (size_t by = 0; by < block_size(); by++) {
            for (size_t ix = 0; ix < input_width(); ix++) {
              for (size_t bx = 0; bx < block_size(); bx++) {
                for (size_t oc = 0; oc < output_channels(); oc++) {
                  const size_t input_index =
                      i * input_channels_stride() * input_height() *
                          input_width() +
                      (((by * block_size() + bx) * output_channels() + oc) *
                           input_height() +
                       iy) *
  ...
  void TestNCHW2NHWCxX32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_depth_to_space_nchw2nhwc_x32(
                                        block_size(), 0, &depth_to_space_op));
      ASSERT_NE(nullptr, depth_to_space_op);
...
#endif  // XNNPACK_TEST_OPERATORS_DEPTH_TO_SPACE_OPERATOR_TESTER_H_
