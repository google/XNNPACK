...
TEST(AVERAGE_POOLING_NHWC_F16, small_pool) {
  ...
  const std::pair<size_t, size_t> pooling_size =
      SmallPoolSize(avgpool_config->primary_tile);
...
TEST(AVERAGE_POOLING_NHWC_F16, small_pool_with_tf_same_padding) {
  ...
  for (size_t channels = 1; channels <= 100; channels += 15) {
    for (size_t stride = 1; stride <= 2; stride++) {
      for (size_t input_height = pooling_size.first + 3;
           input_height <= pooling_size.first + 4; input_height++) {
        AveragePoolingOperatorTester()
      ...
      for (size_t input_width = pooling_size.second + 2;
           input_width <= pooling_size.second + 3; input_width++) {
        AveragePoolingOperatorTester()
            .input_height(pooling_size.first + 3)
      ...
      for (size_t input_height = pooling_size.second + 3;
           input_height <= pooling_size.second + 4; input_height++) {
        AveragePoolingOperatorTester()
            .input_height(input_height)
            .input_width(pooling_size.first + 2)
            .padding_tf_same(true)
      ...
      for (size_t input_width = pooling_size.first + 2;
           input_width <= pooling_size.first + 3; input_width++) {
        AveragePoolingOperatorTester()
            .input_height(pooling_size.second + 3)
            .input_width(input_width)
            .padding_tf_same(true)
...
TEST(AVERAGE_POOLING_NHWC_F16, large_pool) {
  ...
  const std::pair<size_t, size_t> pooling_size =
      LargePoolSize(avgpool_config->primary_tile * 2);
  for (size_t channels = 1; channels <= 100; channels += 15) {
    AveragePoolingOperatorTester()
        .input_height(pooling_size.first + 3)
        .input_width(pooling_size.second + 2)
...
TEST(AVERAGE_POOLING_NHWC_F16, large_pool_with_tf_same_padding_multithreaded) {
  ...
  const std::pair<size_t, size_t> pooling_size =
      LargePoolSize(avgpool_config->primary_tile + 1);
  AveragePoolingOperatorTester()
...
TEST(AVERAGE_POOLING_NHWC_F32, zero_bytes_overflow) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  xnn_operator_t avgpool_op = nullptr;
  ASSERT_EQ(xnn_status_success,
            xnn_create_average_pooling2d_nhwc_f32(
...
TEST(AVERAGE_POOLING_NHWC_F32, batch_stride_overflow) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  xnn_operator_t avgpool_op = nullptr;
  ASSERT_EQ(xnn_status_success,
            xnn_create_average_pooling2d_nhwc_f32(
  ...
  ASSERT_EQ(xnn_status_out_of_memory,
            xnn_reshape_average_pooling2d_nhwc_f32(
                avgpool_op, /*batch_size=*/overflow_batch,
