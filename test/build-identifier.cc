...
TEST(BuildIdentifierTest, CheckSucceedsForIdentity) {
  EXPECT_TRUE(xnn_experimental_check_build_identifier(
...
TEST(BuildIdentifierTest, CheckFailsWithDifferentDataSize) {
  EXPECT_FALSE(xnn_experimental_check_build_identifier(
      nullptr, xnn_experimental_get_build_identifier_size() - 1));
  EXPECT_FALSE(xnn_experimental_check_build_identifier(
      nullptr, xnn_experimental_get_build_identifier_size() + 1));
}
...
TEST(BuildIdentifierTest, CheckFailsWithDifferentData) {
  ...
  for (size_t i = 0; i < xnn_experimental_get_build_identifier_size(); ++i) {
    ...
    ++wrong_data[i];
    EXPECT_FALSE(xnn_experimental_check_build_identifier(
        wrong_data.data(), xnn_experimental_get_build_identifier_size()))
        << "byte " << i;
  }
}
