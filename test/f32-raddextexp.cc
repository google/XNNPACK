 ...
 public:
  ...
  void Test(xnn_f32_raddextexp_ukernel_fn raddextexp) const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      // Verify results.
      ASSERT_NEAR(sum_ref, exp2(double(sum[1])) * double(sum[0]),
                  std::abs(sum_ref) * 1.0e-6)
          << "elements = " << elements() << ", y:value = " << sum[0]
          << ", y:exponent = " << sum[1];
    }
  }
...
#define XNN_TEST_RADDEXTEXP_ELEMENT_DIV(ukernel, arch_flags, element_tile, \
  ...
  TEST(ukernel, element_gt) {                                              \
    TEST_REQUIRES_ARCH_FLAGS(arch_flags);                                  \
    for (size_t element_size = element_tile * 2;                           \
         element_size < element_tile * 10; element_size += element_tile) { \
