...
struct FullyConnectedTestParam {
  FullyConnectedTestParam(bool use_bias_, uint8_t kernel_zero_point_)
...
class FullyConnectedTestQP8F32QC8W
    : public FullyConnectedTestBase<int8_t, uint8_t, float, float, true> {};
...
TEST_P(FullyConnectedTestQP8F32QC8W, define) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value_v2(
                subgraph, xnn_datatype_qcint8, kernel_zero_point,
                kernel_scale.data(), kernel_dims.size(),
                /*channel_dim=*/0, kernel_dims.data(), kernel.data(),
                /*external_id=*/1, /*flags=*/0, &kernel_id));
...
TEST_P(FullyConnectedTestQP8F32QC8W, matches_operator_api) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));

  if (xnn_init_qp8_f32_qc8w_gemm_config() == nullptr) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_channelwise_quantized_tensor_value_v2(
                subgraph, xnn_datatype_qcint8, /*kernel_zero_point=*/0,
                kernel_scale.data(), kernel_dims.size(),
                /*channel_dim=*/0, kernel_dims.data(), kernel.data(),
                /*external_id=*/1, /*flags=*/0, &kernel_id));
...
TEST_P(FullyConnectedTestQP8F32QC8W, matches_operator_api_with_reshape) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  status = xnn_create_fully_connected_nc_qp8_f32_qc8w(
      input_channels, output_channels, input_channels, output_channels,
      kernel_scale.data(), kernel.data(), nullptr, output_min, output_max,
      /*flags=*/0, nullptr, nullptr, &fc_op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_fc_op(
      fc_op, xnn_delete_operator);
...
TEST_P(FullyConnectedTestQP8F32QC8W, matches_operator_api_transposed_weights) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  xnnpack::Buffer<float> subgraph_output(batch_size * output_channels);
  xnnpack::Buffer<float> operator_output(batch_size * output_channels);
  kernel = xnnpack::Buffer<uint8_t>(input_channels * output_channels);
...
INSTANTIATE_TEST_SUITE_P(FullyConnectedTestQP8F32QC8W,
                         FullyConnectedTestQP8F32QC8W,
                         testing::ValuesIn<FullyConnectedTestParam>(
                             {FullyConnectedTestParam(true, 0)}));
...
class FullyConnectedTestF32QC4W
    : public FullyConnectedTestBase<float, uint8_t, float> {};
...
TEST_F(FullyConnectedTestQC8, matches_operator_api) {
  ...
  fprintf(stderr, "%s: batch_size=%u, output_channels=%zu.\n", __FUNCTION__,
          batch_size, output_channels);
  for (size_t i = 0; i < batch_size * output_channels; i++) {
    fprintf(stderr, "%s: [%zu, %zu]: %i, %ie\n", __FUNCTION__,
            i / output_channels, i % output_channels, subgraph_output[i],
            operator_output[i]);
  }
...
TEST_F(FullyConnectedTestQP8F32QB4W, define) {
  ...
  std::generate(kernel_scale.begin(), kernel_scale.end(),
                [&]() { return math_cvt_bf16_fp32(scale_dist(rng)); });
  uint32_t kernel_id = XNN_INVALID_VALUE_ID;
  ASSERT_EQ(xnn_status_success,
            xnn_define_blockwise_quantized_tensor_value(
                subgraph, xnn_datatype_qbint4, kernel_zero_point,
...
TEST_F(FullyConnectedTestQP8F32QB4W, matches_operator_api) {
  ...
  status = xnn_create_fully_connected_nc_qp8_f32_qb4w(
      input_channels, output_channels, input_channels, output_channels,
      block_size, kernel_zero_point, kernel_scale.data(), kernel.data(),
      bias.data(), output_min, output_max,
      /*flags=*/0, nullptr, nullptr, &fc_op);
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_fc_op(
      fc_op, xnn_delete_operator);
