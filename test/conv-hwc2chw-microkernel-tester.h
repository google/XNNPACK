...
class ConvHWC2CHWMicrokernelTester {
 public:
  ConvHWC2CHWMicrokernelTester& output_channels_tile(
  ...
  ConvHWC2CHWMicrokernelTester& input_size(uint32_t input_height,
  ...
  ConvHWC2CHWMicrokernelTester& subsampling_height(
  ...
  void Test(xnn_f32_conv_hwc2chw_ukernel_fn conv,
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
        ...
        for (size_t oy = 0; oy < output_height(); oy++) {
          for (size_t ox = 0; ox < output_width(); ox++) {
            for (size_t oc = 0; oc < output_channels(); oc++) {
              ...
              output_ref[((i * output_channels() + oc) * output_height() + oy) *
                             output_width() +
                         ox] = acc;
            }
          }
      ...
      conv(input_height(), input_width(), output_y_start(), output_y_end(),
           ...
           padding_top(), output_channels(), output_width() * sizeof(float),
           output_height() * output_width() * sizeof(float), &params);
        ...
        for (size_t y = output_y_start(); y < output_y_end(); y++) {
          for (size_t x = 0; x < output_width(); x++) {
            for (size_t c = 0; c < output_channels(); c++) {
              EXPECT_GE(
                  output[((i * output_channels() + c) * output_height() + y) *
                             output_width() +
                         x],
                  output_min)
                  << "(x, y) = (" << x << ", " << y << "), channel = " << c;
              EXPECT_LE(
                  ...
                  << "(x, y) = (" << x << ", " << y << "), channel = " << c;
              ASSERT_NEAR(
                  output_ref[((i * output_channels() + c) * output_height() +
                              y) *
                                 output_width() +
                             x],
                  output[((i * output_channels() + c) * output_height() + y) *
  ...
  void Test(xnn_f16_conv_hwc2chw_ukernel_fn conv,
    ...
    xnnpack::Buffer<xnn_float16> output(batch_size() * output_channels() *
                                        output_height() * output_width());
    xnnpack::Buffer<float> output_ref(batch_size() * output_channels() *
                                      output_height() * output_width());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      conv(input_height(), input_width(), output_y_start(), output_y_end(),
           ...
           output_height() * output_width() * sizeof(xnn_float16), &params);
            ...
            for (size_t c = 0; c < output_channels(); c++) {
              ...
              ASSERT_NEAR(
                  ...
                  std::max(
                      1.0e-4f,
                      1.0e-2f *
                          std::abs(output_ref[((i * output_channels() + c) *
                                                   output_height() +
                                               y) *
                                                  output_width() +
                                              x])))
