 ...
 public:
  ...
  void TestF32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const float fmin = relu_activation() ? 0.0f : float(qmin());
      const float fmax = relu_activation() ? std::numeric_limits<float>::infinity() : float(qmax());
      ASSERT_EQ(xnn_status_success,
      ...
      for (size_t i = 0; i < batch_size(); i++) {
        for (size_t c = 0; c < channels(); c++) {
          ASSERT_LE(output[i * output_stride() + c], fmax)
            << "at position " << i << ", batch size = " << batch_size() << ", channels = " << channels();
          ASSERT_GE(output[i * output_stride() + c], fmin)
            << "at position " << i << ", batch size = " << batch_size() << ", channels = " << channels();
