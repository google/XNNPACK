...
namespace {
inline size_t compute_output_dimension(size_t padded_input_dimension,
...
class ArgmaxPoolingTestF32 : public ::testing::Test {
 protected:
  ArgmaxPoolingTestF32() {
    ...
    output_height = compute_output_dimension(
        input_height + input_padding_top + input_padding_bottom,
        pooling_height);
    output_width = compute_output_dimension(
        input_width + input_padding_left + input_padding_right, pooling_width);
    input_dims = {batch_size, input_height, input_width, channels};
    output_dims = {batch_size, output_height, output_width, channels};
    input = xnnpack::Buffer<float>(XNN_EXTRA_BYTES / sizeof(float) +
                                   batch_size * input_height * input_width *
                                       channels);
    operator_output = xnnpack::Buffer<float>(batch_size * output_height *
                                             output_width * channels);
    operator_output_index = xnnpack::Buffer<uint32_t>(
        batch_size * output_height * output_width * channels);
    subgraph_output = xnnpack::Buffer<float>(batch_size * output_height *
                                             output_width * channels);
    subgraph_output_index = xnnpack::Buffer<uint32_t>(
        batch_size * output_height * output_width * channels);
  }
...
TEST_F(ArgmaxPoolingTestF32, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_argmax_pooling_2d(
                ...
                pooling_width, input_id, output_value_id, output_index_id,
                /*flags=*/0));
...
TEST_F(ArgmaxPoolingTestF32, matches_operator_api) {
  ...
  const xnn_status status = xnn_create_argmax_pooling2d_nhwc_f32(
      ...
      input_padding_left, pooling_height, pooling_width,
      /*flags=*/0, &op);
  ...
  std::array<xnn_external_value, 3> external = {
      ...
      xnn_external_value{output_value_id, subgraph_output.data()},
...
TEST_F(ArgmaxPoolingTestF32, reshape_output) {
  ...
  ASSERT_EQ(
      ...
      xnn_define_argmax_pooling_2d(
          ...
          /*input_padding_bottom=*/1, /*input_padding_left=*/4, pooling_height,
          pooling_width, input_id, output_value_id, output_index_id,
          /*flags=*/0));
  ...
  dims[3] = 17;
  ASSERT_EQ(xnn_status_success,
            xnn_reshape_external_value(runtime, 0, dims.size(), dims.data()));
