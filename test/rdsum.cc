...
class Tester2 {
 public:
  ...
  void Test(UKernelFn<Input, Output, Params> ukernel,
    ...
    for (size_t channels = this->channels().begin;
        channels < this->channels().end; channels += this->channels().step) {
      const size_t input_stride1 =
          input_stride1_ == -1 ? channels : input_stride1_;
      xnnpack::Buffer<Input> zero(channels, 0, xnnpack::XnnExtraBytes);
      for (size_t rows = this->rows().begin; rows < this->rows().end;
          rows += this->rows().step) {
        const size_t input_stride2 = k2 == 1 ? 0 : input_stride1 * rows;
        const size_t input_stride3 = k3 == 1 ? 0 : input_stride2 * k2;
        xnnpack::Buffer<Input> input(
            rows * input_stride1 * k2 * k3 + channels, xnnpack::XnnExtraBytes);
        xnnpack::Buffer<Output> output(channels);
        ...
        xnnpack::Buffer<Output> expected(channels, static_cast<Output>(0));
        for (size_t r3 = 0; r3 < k3; ++r3) {
          for (size_t r2 = 0; r2 < k2; ++r2) {
            for (size_t r1 = 0; r1 < rows; ++r1) {
              const Input* input_row = input.data() + r1 * input_stride1 +
                  r2 * input_stride2 + r3 * input_stride3;
              for (size_t c = 0; c < channels; ++c) {
                expected[c] += input_row[c];
              }
            }
        ...
        ukernel(channels, rows, k2, k3, input.data(),
                input_stride1 * sizeof(Input),
                input_stride2 * sizeof(Input),
                input_stride3 * sizeof(Input),
                zero.data(), output.data(), &params);

        const float tolerance =
            channels * rows * k2 * k3 * max_abs_value * scale * 2.0f *
            xnnpack::NumericLimits<Output>::epsilon();
        for (size_t c = 0; c < channels; ++c) {
          ASSERT_NEAR(expected[c], output[c], tolerance);
        }
