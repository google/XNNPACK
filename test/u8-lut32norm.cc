...
TEST(U8_LUT32NORM__SCALAR, n_eq_1) {
  LUTNormMicrokernelTester().n(1).Test(xnn_u8_lut32norm_ukernel__scalar);
}

TEST(U8_LUT32NORM__SCALAR, small_n) {
  for (size_t n = 2; n <= 16; n++) {
    LUTNormMicrokernelTester().n(n).Test(xnn_u8_lut32norm_ukernel__scalar);
  }
}
...
TEST(U8_LUT32NORM__SCALAR, n_eq_1_inplace) {
  LUTNormMicrokernelTester().n(1).inplace(true).Test(
      xnn_u8_lut32norm_ukernel__scalar);
}
