...
TEST_F(FingerprintCacheTest, SetAndGetFingerprint) {
  const struct xnn_fingerprint expected{/*id=*/xnn_fingerprint_id_test,
                                        /*value=*/314};
...
TEST_F(FingerprintCacheTest, SetGetFingerprintMultipleTimesDoesntDeadlock) {
  constexpr struct xnn_fingerprint finger1{/*id=*/xnn_fingerprint_id_test,
                                           /*value=*/314};
  constexpr struct xnn_fingerprint finger2{/*id=*/xnn_fingerprint_id_test,
                                           /*value=*/654};
...
TEST_F(FingerprintCacheTest, InitializeAndFinalize) {
  struct fingerprint_context context =
      create_fingerprint_context(xnn_fingerprint_id_test);
  EXPECT_THAT(context.status, Eq(xnn_status_uninitialized));
  EXPECT_THAT(context.fingerprint_id, Eq(xnn_fingerprint_id_test));
  finalize_fingerprint_context(&context);
  EXPECT_THAT(xnn_get_fingerprint(xnn_fingerprint_id_test), NotNull());
}
...
TEST_F(FingerprintCacheTest, ReserveAndWrite) {
  ...
  const xnn_fingerprint* fingerprint =
      xnn_get_fingerprint(xnn_fingerprint_id_test);
  ASSERT_THAT(fingerprint, NotNull());
  EXPECT_THAT(fingerprint->id, Eq(xnn_fingerprint_id_test));
