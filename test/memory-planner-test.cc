...
TEST(MemoryPlanner, PersistentValuesReusedByInternalValue) {
  uint32_t input_id = 0;
  ...
  tester
      .AddInputTensorF32({1, 3, 3, 3}, input_id)
      .AddDynamicTensorF32({1, 3, 3, 3}, clamp_out_id, XNN_VALUE_FLAG_PERSISTENT) // 108 bytes.
      .AddDynamicTensorF32({1, 3, 3, 3}, leaky_relu_out_id, /*flags=*/0) // 108 bytes.
      .AddOutputTensorF32({1, 3, 3, 3}, output_id)
  ...
  // leaky_relu_out_id can reuse persistent value clamp_out_id.
  ASSERT_EQ(runtime->workspace->size,
            round_up_po2(1 * 3 * 3 * 3 * sizeof(float), XNN_EXTRA_BYTES) * 1 + MEMORY_ARENA_EXTRA_BYTES);
