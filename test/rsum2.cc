...
class Tester {
 public:
  ...
  void Test(UKernelFn<Input, Output, Params> ukernel,
    ...
    Output expected = 0.0f;
    for (size_t i = 0; i < batch_size(); ++i) {
      const Output x = input[i];
      expected += x * x;
    }
    ...
    const float tolerance = batch_size() * max_abs_value * max_abs_value *
                            scale * 2.0f *
...
INSTANTIATE_TEST_SUITE_P(
    rsum2, Test, testing::ValuesIn(kernels),
...
TEST_P(Test, batch_div) {
  ...
  TEST_REQUIRES_ARCH_FLAGS(param.arch_flags);
  const size_t batch_tile = param.batch_tile * get_batch_scale(param.elem_size);
  for (size_t batch_size = batch_tile;
       batch_size < batch_tile * 5 && !HasFatalFailure();
...
TEST_P(Test, batch_lt) {
  ...
  TEST_REQUIRES_ARCH_FLAGS(param.arch_flags);
  const size_t batch_tile = param.batch_tile * get_batch_scale(param.elem_size);
  for (size_t batch_size = 1; batch_size < batch_tile && !HasFatalFailure();
       batch_size++) {
...
TEST_P(Test, batch_gt) {
  ...
  TEST_REQUIRES_ARCH_FLAGS(param.arch_flags);
  const size_t batch_tile = param.batch_tile * get_batch_scale(param.elem_size);
  for (size_t batch_size = batch_tile + 1;
       batch_size < batch_tile * 2 && !HasFatalFailure(); batch_size++) {
    Tester().batch_size(batch_size).Test(param.kernel);
  }
}
...
TEST_P(Test, scale) {
  const KernelInfo& param = GetParam();
  TEST_REQUIRES_ARCH_FLAGS(param.arch_flags);
  for (float scale = 0.3f; scale < 5.0f && !HasFatalFailure(); scale *= 3.0f) {
