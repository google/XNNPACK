...
class F32SimdWASMSIMDTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(F32SimdWASMSIMDTest, Mul) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Fmadd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Fmsub) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
  const xnn_simd_f32_t c =
      xnn_loadu_f32(inputs_.data() + 2 * xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Fnmadd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
  const xnn_simd_f32_t c =
      xnn_loadu_f32(inputs_.data() + 2 * xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Sub) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Div) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Max) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Min) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Abs) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, Neg) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, ReduceAdd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, ReduceMin) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, ReduceMax) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, And) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, Xor) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, AndNot) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdWASMSIMDTest, ShiftLeft) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, ShiftRight) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdWASMSIMDTest, ShiftRightSigned) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdWASMSIMDTest, CmpEq) {
  for (size_t k = 0; k < xnn_simd_size_f32; k++) {
    if (rng_() & 1) {
...
TEST_F(F32SimdWASMSIMDTest, Reduce_Add) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdWASMSIMDTest, StoreTail) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
