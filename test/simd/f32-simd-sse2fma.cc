...
class F32SimdSSE2FMATest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(F32SimdSSE2FMATest, Fmadd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdSSE2FMATest, Fmsub) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
  const xnn_simd_f32_t c =
      xnn_loadu_f32(inputs_.data() + 2 * xnn_simd_size_f32);
...
TEST_F(F32SimdSSE2FMATest, Fnmadd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
  const xnn_simd_f32_t c =
      xnn_loadu_f32(inputs_.data() + 2 * xnn_simd_size_f32);
...
TEST_F(F32SimdSSE2FMATest, Round) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, ReduceAdd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, ReduceMin) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, ReduceMax) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, AndNot) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  const xnn_simd_f32_t b = xnn_loadu_f32(inputs_.data() + xnn_simd_size_f32);
...
TEST_F(F32SimdSSE2FMATest, ShiftLeft) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, ShiftRight) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdSSE2FMATest, ShiftRightSigned) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdSSE2FMATest, CmpEq) {
  for (size_t k = 0; k < xnn_simd_size_f32; k++) {
    if (rng_() & 1) {
...
TEST_F(F32SimdSSE2FMATest, Reduce_Add) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2FMATest, StoreTail) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
