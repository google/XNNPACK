...
#include "src/xnnpack/simd/s16-wasmsimd.h"
#include "test/replicable_random_device.h"

namespace xnnpack {

class S16SimdWASMSIMDTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(S16SimdWASMSIMDTest, Min) {
...
TEST_F(S16SimdWASMSIMDTest, Max) {
  const xnn_simd_s16_t a = xnn_loadu_s16(inputs_.data());
  const xnn_simd_s16_t b = xnn_loadu_s16(inputs_.data() + xnn_simd_size_s16);
...
TEST_F(S16SimdWASMSIMDTest, SignComplement) {
  const xnn_simd_s16_t a = xnn_loadu_s16(inputs_.data());
...
TEST_F(S16SimdWASMSIMDTest, LoadTail) {
...
TEST_F(S16SimdWASMSIMDTest, StoreTail) {
  const xnn_simd_s16_t a = xnn_loadu_s16(inputs_.data());
  for (size_t num_elements = 1; num_elements < xnn_simd_size_s16;
      num_elements++) {
