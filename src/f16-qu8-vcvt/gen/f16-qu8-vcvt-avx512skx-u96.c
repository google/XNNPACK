...
void xnn_f16_qu8_vcvt_ukernel__avx512skx_u96(
    size_t batch,
    ...
    const struct xnn_f16_qu8_cvt_params* restrict params)
{
  ...
  for (; batch >= 96 * sizeof(xnn_float16); batch -= 96 * sizeof(xnn_float16)) {
    __m512 vx0123 = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) input));
    __m512 vx4567 = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) (input + 16)));
    __m512 vx89AB = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) (input + 32)));
    __m512 vxCDEF = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) (input + 48)));
    __m512 vxGHIJ = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) (input + 64)));
    __m512 vxKLMN = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i*) (input + 80)));
    input += 96;
  ...
  if XNN_UNLIKELY(batch != 0) {
    ...
    assert(batch <= 15 * sizeof(xnn_float16));

    // Prepare mask for valid elements (depends on batch).
    batch >>= XNN_LOG2_SIZEOF_FLOAT16;
    const __mmask16 vmask = _cvtu32_mask16((uint32_t) ((UINT32_C(1) << batch) - UINT32_C(1)));

    __m512 vx0123 = _mm512_cvtph_ps(_mm256_maskz_loadu_epi16(vmask, input));
    vx0123 = _mm512_mul_ps(vx0123, vscale);
    vx0123 = _mm512_min_ps(vx0123, voverflow_max);
