    ...
    const struct xnn_f16_default_params* restrict params) XNN_OOB_READS
{
  ...
  for (; batch >= 16 * sizeof(uint16_t); batch -= 16 * sizeof(uint16_t)) {
    ...
    __m256 vy0 = _mm256_cvtph_ps(_mm256_cvtps_ph(_mm256_min_ps(va0, vb), _MM_FROUND_TO_NEAREST_INT));
    __m256 vy1 = _mm256_cvtph_ps(_mm256_cvtps_ph(_mm256_min_ps(va1, vb), _MM_FROUND_TO_NEAREST_INT));
