...
//   Template: src/f16-vbinary/vop.c.in
...
void xnn_f16_vmul_ukernel__wasmrelaxedsimdfp16_u8(
    size_t batch,
    ...
    const struct xnn_f16_default_params* restrict params)
{
  ...
  for (; batch >= xnn_simd_bytes_f16; batch -= xnn_simd_bytes_f16) {
    ...
    const xnn_simd_f16_t vb = xnn_loadu_f16(input_b);
    input_b += xnn_simd_size_f16;
    const xnn_simd_f16_t vy = xnn_mul_f16(va, vb);
  ...
  if XNN_UNLIKELY(batch != 0) {
    ...
    const xnn_simd_f16_t vb = xnn_load_tail_f16(input_b, num_elements);
    const xnn_simd_f16_t vy = xnn_mul_f16(va, vb);
