...
//   Template: src/f16-vbinary/f16-f32acc-vop.c.in
...
void xnn_f16_f32acc_vadd_ukernel__wasmrelaxedsimd_u8(
    size_t batch,
    ...
    const struct xnn_f16_default_params* restrict params)
{
  ...
  for (; batch >= 8 * sizeof(xnn_float16);
    ...
    const xnn_simd_f32_t vb0 = load_f16_f32(input_b + 0);
    const xnn_simd_f32_t va1 = load_f16_f32(input_a + 4);
    const xnn_simd_f32_t vb1 = load_f16_f32(input_b + 4);
    input_a += 8;
    input_b += 8;
    const xnn_simd_f32_t vy0 = xnn_add_f32(va0, vb0);
    store_f32_f16(output + 0, vy0);
    const xnn_simd_f32_t vy1 = xnn_add_f32(va1, vb1);
  ...
  for (; batch >= 4 * sizeof(xnn_float16);
    ...
    input_b += 4;
    store_f32_f16(output, xnn_add_f32(va, vb));
    output += 4;
  }
  ...
  if XNN_UNLIKELY(batch != 0) {
    ...
    store_tail_f32_f16(output, xnn_add_f32(va, vb), num_elements);
  }
}
