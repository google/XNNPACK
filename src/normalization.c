...
void xnn_normalize_slice(
    const size_t num_dims, const size_t offsets[XNN_MIN_ELEMENTS(1)],
  ...
  *num_normalized_dims = num_dims;
  ...
  // First normalization pass will remove all slices of size 1, by merging it to
  ...
  for (size_t i = 0; i < num_dims; i++) {
    ...
    // If the innermost dimension is size 1, we can't merge it anywhere, so skip
    // it.
    if (size == 1 && i != 0) {
      normalized_offsets[XNN_MAX_TENSOR_DIMS - 1 - i + 1 + num_size_one] +=
          offset * normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1 - i + 1 +
                                          num_size_one];
      normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1 - i + 1 + num_size_one] *=
          input_dim;
      normalized_output_shape[XNN_MAX_TENSOR_DIMS - 1 - i + 1 + num_size_one] *=
          size;
      num_size_one++;
    } else {
      normalized_offsets[XNN_MAX_TENSOR_DIMS - 1 - i + num_size_one] = offset;
      normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1 - i + num_size_one] =
          input_dim;
      normalized_output_shape[XNN_MAX_TENSOR_DIMS - 1 - i + num_size_one] =
          size;
    }
  }
  ...
  for (size_t i = 0; i < new_num_dims; i++) {
    ...
    if (merge_previous_dim) {
      ...
      if (!merge_current_dim) {
        // If merge_current_dim, we can merge current dimension with the next
        // dim, so don't advance num_sliced_dims.
        num_sliced_dims += 1;
      }
    }
...
// Returns true if input stride and output stride are NULL or the expected
// input/output stride matches the actual input/output stride.
static bool can_dimension_be_removed(const size_t* input_stride,
                                     const size_t* output_stride,
                                     const size_t* shape,
                                     const size_t* inverted_perm,
                                     size_t input_dim, const size_t num_dims) {
  const size_t output_dim = inverted_perm[input_dim];
  if (input_dim == 0 && output_dim == 0) {
    return true;
  }
  if (input_stride != NULL && input_dim > 0) {
    if (input_stride[input_dim - 1] !=
        input_stride[input_dim] * shape[input_dim]) {
      return false;
    }
  }
  if (output_stride != NULL && output_dim > 0) {
    if (output_stride[output_dim - 1] !=
        output_stride[output_dim] * shape[input_dim]) {
      return false;
    }
  }
  return true;
...
static void fold_into_previous_dim(size_t* shape, size_t* perm,
                                   size_t* inverted_perm, size_t* input_stride,
                                   size_t* output_stride, size_t num_dims,
                                   size_t input_dim) {
...
void xnn_normalize_transpose_permutation(
    const size_t num_dims, const size_t element_size, const size_t* perm,
    const size_t* shape, const size_t* input_stride,
    const size_t* output_stride, size_t* normalized_num_dims,
    size_t* normalized_element_size_out, size_t* normalized_perm,
  ...
  while (input_dim < output_dims) {
    ...
    const bool previous_dim_in_output_is_previous_dim_in_input =
        input_dim > 0 && normalized_inverted_perm[input_dim] ==
  ...
  if (normalized_perm[output_dims - 1] == output_dims - 1) {
    ...
    if (output_dims > 1 &&
        can_dimension_be_removed(normalized_input_stride_ptr,
                                 normalized_output_stride_ptr, normalized_shape,
                                 normalized_inverted_perm, last_dim,
                                 output_dims)) {
      output_dims -= 1;
    } else {
  ...
  if (output_stride == NULL) {
    ...
    for (size_t i = output_dims - 1; i > 0; --i) {
      normalized_output_stride[i - 1] =
          normalized_output_stride[i] * normalized_shape[normalized_perm[i]];
    }
  } else {
...
void xnn_normalize_reduction(size_t* num_reduction_axes_ptr,
  ...
  // Running product of input dimensions for a sequence of adjacent axes, e.g.
  ...
  // Invariant num_consumed_input_dims <= num_produced_input_dims holds at each
  // iteration.
  ...
  // &reduction_axes[num_produced_reduction_axes] holds at each iteration.
  ...
  if (num_consumed_input_dims != num_input_dims) {
    // Merge input dimensions in the [num_consumed_input_dims:num_input_dims]
    // range.
    size_t normalized_dim = input_dims[num_consumed_input_dims++];
