...
{
  if (normalized_offsets != NULL) {
    memset(normalized_offsets, 0, XNN_MAX_TENSOR_DIMS * sizeof(size_t));
  }
  if (normalized_input_shape != NULL) {
    for (size_t i = 0; i < XNN_MAX_TENSOR_DIMS; i++) {
      normalized_input_shape[i] = 1;
    }
  }
  if (normalized_output_shape != NULL) {
    for (size_t i = 0; i < XNN_MAX_TENSOR_DIMS; i++) {
      normalized_output_shape[i] = 1;
    }
  }
  if (num_normalized_dims != NULL) {
    *num_normalized_dims = 0;
  }
  if (num_dims == 0 || num_dims > XNN_MAX_TENSOR_DIMS || offsets == NULL ||
      sizes == NULL || input_shape == NULL || normalized_offsets == NULL ||
      normalized_input_shape == NULL || normalized_output_shape == NULL ||
      num_normalized_dims == NULL) {
    return;
  }
  *num_normalized_dims = num_dims;

  // First normalization pass will remove all slices of size 1, by merging it
  // to an adjacent inner dimension.
  size_t num_size_one = 0;
  for (size_t i = 0; i < num_dims; i++) {
    const size_t offset = offsets[num_dims - 1 - i];
    const size_t input_dim = input_shape[num_dims - 1 - i];
    const size_t size =
      sizes[num_dims - 1 - i] > 0 ? sizes[num_dims - 1 - i] : input_dim;

    // If the innermost dimension is size 1, we can't merge it anywhere, so
    // skip it.
    if (size == 1 && i != 0) {
      const size_t idx = XNN_MAX_TENSOR_DIMS - 1 - i + 1 + num_size_one;
      size_t scaled_offset;
      if (!xnn_safe_mul(offset, normalized_input_shape[idx], &scaled_offset) ||
          !xnn_safe_add(
            normalized_offsets[idx], scaled_offset, &normalized_offsets[idx]) ||
          !xnn_safe_mul(
            normalized_input_shape[idx], input_dim,
            &normalized_input_shape[idx]) ||
          !xnn_safe_mul(
            normalized_output_shape[idx], size,
            &normalized_output_shape[idx])) {
        *num_normalized_dims = 0;
        return;
      }
      num_size_one++;
    } else {
      const size_t idx = XNN_MAX_TENSOR_DIMS - 1 - i + num_size_one;
  ...
  for (size_t i = 0; i < new_num_dims; i++) {
    ...
    if (merge_previous_dim) {
      const size_t idx = XNN_MAX_TENSOR_DIMS - 1 - num_sliced_dims;
      size_t scaled_offset;
      if (!xnn_safe_mul(offset, normalized_input_shape[idx], &scaled_offset) ||
          !xnn_safe_add(
            normalized_offsets[idx], scaled_offset, &normalized_offsets[idx]) ||
      ...
      normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1 - num_sliced_dims] =
        input_dim;
      normalized_output_shape[XNN_MAX_TENSOR_DIMS - 1 - num_sliced_dims] =
        size;
      if (!merge_current_dim) {
        num_sliced_dims += 1;
      }
  ...
  // new_num_dims <= num_dims due to merge of size == 1, so we are left with
  // some extra values at the front of the normalized values, set them to
  // default values.
...
static bool can_dimension_be_removed(
  ...
  if (input_stride != NULL && input_dim > 0) {
    ...
    if (!xnn_safe_mul(
          input_stride[input_dim], shape[input_dim], &expected_stride) ||
        input_stride[input_dim - 1] != expected_stride) {
      return false;
    }
  }
  if (output_stride != NULL && output_dim > 0) {
    size_t expected_stride;
    if (!xnn_safe_mul(
          output_stride[output_dim], shape[input_dim], &expected_stride) ||
        output_stride[output_dim - 1] != expected_stride) {
      return false;
    }
  }
  return true;
    ...
    size_t input_dim)
{
  ...
  if (input_dim > 0) {
    if (!xnn_safe_mul(
          shape[input_dim - 1], shape[input_dim], &shape[input_dim - 1])) {
      shape[input_dim - 1] = SIZE_MAX;
    }
  }
...
{
  if (normalized_num_dims != NULL) {
    *normalized_num_dims = 0;
  }
  if (normalized_element_size_out != NULL) {
  ...
  if (normalized_element_size_out != NULL) {
    *normalized_element_size_out = 0;
  }
  if (num_dims > XNN_MAX_TENSOR_DIMS || perm == NULL || shape == NULL ||
      normalized_num_dims == NULL || normalized_element_size_out == NULL ||
      normalized_perm == NULL || normalized_shape == NULL ||
      normalized_input_stride == NULL || normalized_output_stride == NULL) {
    return;
  }
  for (size_t i = 0; i < num_dims; ++i) {
    if (perm[i] >= num_dims) {
  ...
  size_t input_dim = 0;
  // Remove dimensions of size 1 and fold dimensions which are adjacent in both
  // input and output tensors.
  while (input_dim < output_dims) {
    const bool has_size_1 = normalized_shape[input_dim] == 1;
    const bool previous_dim_in_output_is_previous_dim_in_input =
      input_dim > 0 &&
      normalized_inverted_perm[input_dim] ==
        normalized_inverted_perm[input_dim - 1] + 1;
    const bool strides_allow_fold_left = can_dimension_be_removed(
      normalized_input_stride_ptr, normalized_output_stride_ptr,
      normalized_shape, normalized_inverted_perm, input_dim, output_dims);
    if (strides_allow_fold_left &&
        (has_size_1 || previous_dim_in_output_is_previous_dim_in_input)) {
      fold_into_previous_dim(
        normalized_shape, normalized_perm, normalized_inverted_perm,
        normalized_input_stride_ptr, normalized_output_stride_ptr,
  ...
  if (normalized_perm[output_dims - 1] == output_dims - 1) {
    ...
    if (!xnn_safe_mul(
          element_size, normalized_shape[last_dim],
          &normalized_element_size)) {
      *normalized_num_dims = 0;
      *normalized_element_size_out = 0;
      return;
    }
    if (output_dims > 1 && can_dimension_be_removed(
                             normalized_input_stride_ptr,
      ...
      if (normalized_input_stride != NULL) {
        if (!xnn_safe_mul(
              ...
              &normalized_input_stride[last_dim])) {
          *normalized_num_dims = 0;
          return;
        }
      }
      if (normalized_output_stride != NULL) {
        if (!xnn_safe_mul(
              normalized_output_stride[normalized_perm[last_dim]],
              normalized_shape[last_dim],
              &normalized_output_stride[normalized_perm[last_dim]])) {
          *normalized_num_dims = 0;
          return;
        }
  ...
  // If input_strides is not provided, calculate it using normalized_shape and
  // normalized_element_size.
  if (input_stride == NULL) {
    normalized_input_stride[output_dims - 1] = normalized_element_size;
    for (size_t i = output_dims - 1; i > 0; --i) {
      if (!xnn_safe_mul(
            normalized_input_stride[i], normalized_shape[i],
            &normalized_input_stride[i - 1])) {
        *normalized_num_dims = 0;
        return;
      }
    }
    ...
    for (size_t i = 0; i < output_dims; ++i) {
      if (!xnn_safe_mul(
            ...
            &normalized_input_stride[i])) {
        *normalized_num_dims = 0;
        return;
      }
    }
  }
  // If output_strides is not provided, calculate it using normalized_shape and
  // normalized_element_size.
  if (output_stride == NULL) {
    normalized_output_stride[output_dims - 1] = normalized_element_size;
    for (size_t i = output_dims - 1; i > 0; --i) {
      if (!xnn_safe_mul(
            normalized_output_stride[i],
            normalized_shape[normalized_perm[i]],
            &normalized_output_stride[i - 1])) {
        *normalized_num_dims = 0;
        return;
      }
    }
...
void xnn_normalize_reduction(
    ...
    size_t* input_dims)
{
  if (num_input_dims_ptr == NULL || num_reduction_axes_ptr == NULL) {
    return;
  }
  if (*num_input_dims_ptr == 0) {
    *num_reduction_axes_ptr = 0;
    return;
  }
  if (input_dims == NULL ||
      (*num_reduction_axes_ptr > 0 && reduction_axes == NULL)) {
    return;
  }
  ...
  for (; num_reduction_axes != 0; num_reduction_axes -= 1) {
    ...
    if (axis == axes_sequence_start + axes_sequence_length) {
      ...
      if (!xnn_safe_mul(
        ...
        num_reduction_elements = SIZE_MAX;
      }
