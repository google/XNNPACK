...
void xnn_f32_vsqrt_ukernel__fma3_rsqrt_u24(
  ...
  const __m256 kThree = _mm256_load_ps(params->fma.three);
  const __m256 kNegHalf = _mm256_load_ps(params->fma.neg_half);
  ...
  if XNN_UNLIKELY(batch != 0) {
    ...
    const __m256i vmask = _mm256_loadu_si256(
        (const __m256i*)((uintptr_t)&params->fma.mask_table[7] - batch));
