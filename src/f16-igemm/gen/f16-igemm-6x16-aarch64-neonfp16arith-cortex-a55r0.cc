...
// Converted from: src/f16-igemm/f16-igemm-6x16-minmax-asm-aarch64-neonfp16arith-cortex-a55r0.S
void Generator::generate(size_t max_mr, size_t nc_mod_nr, size_t kc, size_t ks, const jit_gemm_params* jit_gemm_params)
{
  ...
  if (max_mr > 1) {
    ld1({v0.s()}, 2, mem[x15], 4); // A1
  }
  if (max_mr > 3) {
    ld1({v1.s()}, 2, mem[x21], 4); // A3
  }
  if (max_mr > 5) {
    ld1({v2.s()}, 2, mem[x23], 4); // A5
  }
  ldr(q18, mem[x5], 16);
  ldr(d19, mem[x5], 8);
  ...
  if (max_mr > 3) {
    st1({v26.v16b(), v27.v16b()}, mem[x10], x0);
  }
  if (max_mr > 2) {
  ...
  if (max_mr > 1) {
    ld1({v0.h()}, 4, mem[x15], 2); // A1
  }
  if (max_mr > 3) {
    ld1({v1.h()}, 4, mem[x21], 2); // A3
  }
  if (max_mr > 5) {
    ld1({v2.h()}, 4, mem[x23], 2); // A5
  }
  fmla(v20.v8h(), v16.v8h(), v0.h()[0]);
  if (max_mr > 1) {
    fmla(v22.v8h(), v16.v8h(), v0.h()[4]);
...
xnn_status_t xnn_generate_f16_igemm_ukernel_6x16__aarch64_neonfp16arith_cortex_a55r0(xnn_code_buffer* code, size_t max_mr, size_t nc_mod_nr, size_t kc, size_t ks, const void* params) {
