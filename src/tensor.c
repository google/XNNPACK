...
static void set_shape(struct xnn_value* value, size_t num_dims,
                      const size_t* dims) {
...
static enum xnn_status check_zero_point(enum xnn_datatype datatype,
                                        int32_t zero_point) {
  switch (datatype) {
    ...
    case xnn_datatype_qbint4:
      if (zero_point < 0 || zero_point > 15) {
        xnn_log_error(
            "failed to create Quantized Dense Tensor value: invalid zero point "
            "%" PRId32 " outside the [0, 15] range",
            zero_point);
    ...
    case xnn_datatype_qint8:
      if ((int32_t)(int8_t)zero_point != zero_point) {
        xnn_log_error(
            "failed to create Quantized Dense Tensor value: invalid zero point "
            "%" PRId32 " outside the [-128, 127] range",
            zero_point);
        return xnn_status_invalid_parameter;
      }
      break;
    case xnn_datatype_quint8:
      if ((int32_t)(uint8_t)zero_point != zero_point) {
        xnn_log_error(
            "failed to create Quantized Dense Tensor value: invalid zero point "
            "%" PRId32 " outside the [0, 255] range",
            zero_point);
        return xnn_status_invalid_parameter;
      }
      break;
    ...
    case xnn_datatype_qint32:
      if (zero_point != 0) {
        xnn_log_error(
            "failed to create Quantized Dense Tensor value: invalid non-zero "
            "zero point %" PRId32,
            zero_point);
        return xnn_status_invalid_parameter;
      }
    ...
    default:
      xnn_log_error(
          "failed to create Quantized Dense Tensor value: unsupported datatype "
          "%s (%d)",
...
enum xnn_status xnn_define_tensor_value(xnn_subgraph_t subgraph,
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to create Dense Tensor value: XNNPACK is not initialized");
  ...
  if (num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to create Dense Tensor value: num of dimensions exceeds "
        "XNNPACK limit (%d)",
        XNN_MAX_TENSOR_DIMS);
    return xnn_status_unsupported_parameter;
  }
  ...
  switch (datatype) {
    ...
    default:
      xnn_log_error(
          "failed to create Dense Tensor value: unsupported datatype %s (%d)",
          xnn_datatype_to_string(datatype), datatype);
      return xnn_status_unsupported_parameter;
  }
...
enum xnn_status xnn_define_quantized_tensor_value(
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to create Quantized Dense Tensor value: XNNPACK is not "
        "initialized");
    return xnn_status_uninitialized;
  }
  ...
  enum xnn_status status = xnn_validate_quantized_tensor(datatype, zero_point,
                                                         scale, num_dims, dims);
...
enum xnn_status xnn_define_dynamically_quantized_tensor_value(
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to create Dynamically Quantized Dense Tensor value: XNNPACK is "
        "not initialized");
    return xnn_status_uninitialized;
  }
  ...
  if (num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to create Dynamically Quantized Dense Tensor value: num of "
        "dimensions exceeds XNNPACK limit (%d)",
        XNN_MAX_TENSOR_DIMS);
    return xnn_status_unsupported_parameter;
  }
  ...
  switch (datatype) {
    ...
    default:
      xnn_log_error(
          "failed to create Dynamically Quantized Dense Tensor value: "
          "unsupported datatype %s (%d)",
          xnn_datatype_to_string(datatype), datatype);
      return xnn_status_unsupported_parameter;
  }

  if ((flags &
       (XNN_VALUE_FLAG_EXTERNAL_INPUT | XNN_VALUE_FLAG_EXTERNAL_OUTPUT)) != 0) {
    xnn_log_error(
        "failed to create Dynamically Quantized Dense Tensor value: "
  ...
  value->quantization.dynamic_params_size =
      xnn_tensor_get_dynamic_quant_param_size(value);
  value->flags = flags;
  value->data = NULL;
  set_allocation_type(value);
...
enum xnn_status xnn_validate_quantized_tensor(enum xnn_datatype datatype,
  ...
  if (num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to create Quantized Dense Tensor value: num of dimensions "
        "exceeds XNNPACK limit (%d)",
        XNN_MAX_TENSOR_DIMS);
    return xnn_status_unsupported_parameter;
  }
  ...
  if (scale <= 0.0f || !isnormal(scale)) {
    xnn_log_error(
        "failed to create Quantized Dense Tensor value with %.7g scale: scale "
        "must be finite, normalized, and positive",
        scale);
    return xnn_status_invalid_parameter;
  }
  return xnn_status_success;
...
enum xnn_status xnn_validate_channelwise_quantized_tensor(
  ...
  if (num_dims == 0) {
    xnn_log_error(
        "failed to create Channelwise Quantized Dense Tensor value: no channel "
        "dimension exists");
    return xnn_status_invalid_parameter;
  }
  ...
  if (num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to create Channelwise Quantized Dense Tensor value: num of "
        "dimensions exceeds XNNPACK limit (%d)",
        XNN_MAX_TENSOR_DIMS);
    return xnn_status_unsupported_parameter;
  }
  ...
  if (channel_dim >= num_dims) {
    xnn_log_error(
        ...
        "channel dimension index %zu is out of range for %zu-dimensional "
        "tensor",
...
enum xnn_status xnn_define_channelwise_quantized_tensor_value_v2(
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to create Channelwise Quantized Dense Tensor value: XNNPACK is "
        "not initialized");
    return xnn_status_uninitialized;
  }
...
enum xnn_status xnn_define_blockwise_quantized_tensor_value(
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to create Blockwise Quantized Dense Tensor value: XNNPACK is "
        "not initialized");
    return xnn_status_uninitialized;
  }
  ...
  if (num_dims == 0) {
    xnn_log_error(
        "failed to create Blockwise Quantized Dense Tensor value: no channel "
        "dimension exists");
    return xnn_status_invalid_parameter;
  }
