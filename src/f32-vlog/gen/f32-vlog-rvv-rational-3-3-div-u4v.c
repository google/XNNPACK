...
static XNN_INLINE vfloat32m4_t xnn_signed_getexp_f32(vfloat32m4_t a, size_t vl) {
  ...
  a =  __riscv_vreinterpret_f32m4(__riscv_vor_mu(__riscv_vmfeq(a, 0.0f, vl), __riscv_vreinterpret_i32m4(a), __riscv_vreinterpret_i32m4(a), sign_mask.s, vl));

  // Extract the exponent and shift the exponent to the most significant bits of
  // the mantissa.
  const vint32m4_t exp =
    __riscv_vsra(__riscv_vand(__riscv_vreinterpret_i32m4(a), sign_and_exp_mask, vl), 8, vl);
  ...
  return __riscv_vfsub(__riscv_vreinterpret_f32m4(__riscv_vor(exp, bias_256.s, vl)), bias_383, vl);
}


void xnn_f32_vlog_ukernel__rvv_rational_3_3_div_u4v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    const vfloat32m4_t vexp = xnn_signed_getexp_f32(vx, vl);

    // Normalize `x` to an exponent of zero.
    vx = __riscv_vreinterpret_f32m4(__riscv_vor(__riscv_vand(__riscv_vreinterpret_i32m4(vx), vmantissa_bits_mask, vl), vone.s, vl));
    ...
    vfloat32m4_t vp = __riscv_vfadd(__riscv_vfmul(vx, valpha_3, vl), vone.f, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx, vp, vl), vone.f, vl);
    vp = __riscv_vfmul(vx, vp, vl);

    // Evaluate the denominator polynomial q.
    vfloat32m4_t vq = __riscv_vfadd(__riscv_vfmul(vx, vbeta_3, vl), vbeta_2, vl);
