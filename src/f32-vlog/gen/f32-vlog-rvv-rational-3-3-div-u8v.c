...
static XNN_INLINE vfloat32m8_t xnn_signed_getexp_f32(vfloat32m8_t a, size_t vl) {
  ...
  a =  __riscv_vreinterpret_f32m8(__riscv_vor_mu(__riscv_vmfeq(a, 0.0f, vl), __riscv_vreinterpret_i32m8(a), __riscv_vreinterpret_i32m8(a), sign_mask.s, vl));

  // Extract the exponent and shift the exponent to the most significant bits of
  // the mantissa.
  const vint32m8_t exp =
    __riscv_vsra(__riscv_vand(__riscv_vreinterpret_i32m8(a), sign_and_exp_mask, vl), 8, vl);
  ...
  return __riscv_vfsub(__riscv_vreinterpret_f32m8(__riscv_vor(exp, bias_256.s, vl)), bias_383, vl);
}


void xnn_f32_vlog_ukernel__rvv_rational_3_3_div_u8v(
    size_t batch,
    const float* input,
    float* output,
