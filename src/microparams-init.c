...
size_t xnn_init_qs8_qc8w_conv_minmax_fp32_armsimd32_params(
  ...
  params->fp32_armsimd32.magic_bias_less_zero_point =
      INT32_C(0x4B400000) - (int32_t)output_zero_point;
  params->fp32_armsimd32.output_min =
      (uint32_t)(uint8_t)output_min * UINT32_C(0x01010101);
  params->fp32_armsimd32.output_max =
      (uint32_t)(uint8_t)output_max * UINT32_C(0x01010101);
...
size_t xnn_init_qs8_qc8w_conv_minmax_fp32_neon_params(
  ...
  params->fp32_neon.magic_bias_less_output_zero_point =
      INT32_C(0x4B400000) - (int32_t)output_zero_point;
...
size_t xnn_init_qs8_conv_minmax_fp32_scalar_params(
    union xnn_qs8_conv_minmax_params params[XNN_MIN_ELEMENTS(1)], float scale,
    int8_t output_zero_point, int8_t output_min, int8_t output_max) {
...
size_t xnn_init_qu8_conv_minmax_fp32_armsimd32_params(
  ...
  params->fp32_armsimd32.scale = scale;
  params->fp32_armsimd32.magic_bias = 12582912.0f;
  params->fp32_armsimd32.minus_kernel_zero_point =
      (uint32_t)(uint16_t)minus_kernel_zero_point * UINT32_C(0x00010001);
  params->fp32_armsimd32.magic_bias_less_zero_point =
      INT32_C(0x4B400000) - (int32_t)output_zero_point;
  params->fp32_armsimd32.output_min =
      (uint32_t)output_min * UINT32_C(0x01010101);
  params->fp32_armsimd32.output_max =
      (uint32_t)output_max * UINT32_C(0x01010101);
  return sizeof(params->fp32_armsimd32);
}
#endif  // XNN_ARCH_ARM
...
void xnn_init_qs8_qc8w_scale_fp32_params(size_t channels, size_t channels_tile,
  ...
  for (size_t block_start = 0; block_start < num_blocks; block_start++) {
    ...
    for (; tile_start < tiled_channels; tile_start += channels_tile) {
      ...
      for (size_t tile_offset = 0; tile_offset < tile_size; tile_offset++) {
        ...
        // 1/16 because the weight are << 4 in the innermost loop to save a
        // shift
        unaligned_indexed_store_f32(packed_w, tile_offset,
                                    scale[scale_index] / 16.0f);
      }
      packed_w = (void*)((uintptr_t)packed_w + stride);
    }
  ...
  for (size_t block_start = 0; block_start < num_blocks; block_start++) {
    ...
    for (; tile_start < tiled_channels; tile_start += channels_tile) {
      ...
      for (size_t tile_offset = 0; tile_offset < tile_size; tile_offset++) {
        ...
        // 1/16 because the weight are << 4 in the innermost loop to save a
        // shift
        float scale_16 = math_cvt_bf16_fp32(
            xnn_bfloat16_to_float(scale[scale_index]) / 16.0f);
...
size_t xnn_init_f32_scale_scalar_params(
    struct xnn_f32_scale_params params[XNN_MIN_ELEMENTS(1)], float scale) {
  params->scalar.scale = scale;
  return sizeof(params[0]);
}
...
size_t xnn_init_f16_scaleminmax_scalar_params(
    ...
    xnn_float16 scale, xnn_float16 min, xnn_float16 max) {
  params->scalar.scale = scale;
...
size_t xnn_init_bf16_minmax_scalar_params(
    ...
    xnn_bfloat16 output_min, xnn_bfloat16 output_max) {
...
size_t xnn_init_f16_minmax_scalar_params(
    struct xnn_f16_minmax_params params[XNN_MIN_ELEMENTS(1)], xnn_float16 min,
    xnn_float16 max) {
  params->scalar.min = min;
  params->scalar.max = max;
  return sizeof(params->scalar);
...
size_t xnn_init_f16_qc4w_minmax_scalar_params(
    ...
    xnn_float16 output_min, xnn_float16 output_max, uint8_t kernel_zero_point) {
...
size_t xnn_init_f32_minmax_scalar_params(
    struct xnn_f32_minmax_params params[XNN_MIN_ELEMENTS(1)], float output_min,
    float output_max) {
  params->scalar.min = output_min;
  params->scalar.max = output_max;
  return sizeof(params->scalar);
...
size_t xnn_init_f32_qc4w_minmax_scalar_params(
    ...
    float output_min, float output_max, uint8_t kernel_zero_point) {
  assert(kernel_zero_point <= 15);
  params->scalar.min = output_min;
  params->scalar.max = output_max;
...
size_t xnn_init_f16_elu_scalar_params(
    union xnn_unary_uparams* params, const union xnn_unary_params* op_params,
...
size_t xnn_init_qu8_clamp_scalar_params(
  ...
  params->u8_minmax.scalar.min =
      xnn_qu8_quantize(op_params->clamp.min, output_quantization->scale,
                       output_quantization->zero_point);
  params->u8_minmax.scalar.max =
      xnn_qu8_quantize(op_params->clamp.max, output_quantization->scale,
                       output_quantization->zero_point);
...
size_t xnn_init_qs8_clamp_scalar_params(
  ...
  assert(input_quantization->scale == output_quantization->scale);
  assert(input_quantization->zero_point == output_quantization->zero_point);
  params->s8_minmax.scalar.min =
      xnn_qs8_quantize(op_params->clamp.min, output_quantization->scale,
                       output_quantization->zero_point);
  params->s8_minmax.scalar.max =
      xnn_qs8_quantize(op_params->clamp.max, output_quantization->scale,
                       output_quantization->zero_point);
...
size_t xnn_init_s8_minmax_scalar_params(
    struct xnn_s8_minmax_params params[XNN_MIN_ELEMENTS(1)], int8_t output_min,
    int8_t output_max) {
...
size_t xnn_init_u8_minmax_scalar_params(
    struct xnn_u8_minmax_params params[XNN_MIN_ELEMENTS(1)], uint8_t output_min,
    uint8_t output_max) {
  assert(output_min < output_max);
...
size_t xnn_init_qu8_add_minmax_scalar_params(
  ...
  const float a_output_scale =
      a_quantization->scale / output_quantization->scale;
  const float b_output_scale =
      b_quantization->scale / output_quantization->scale;
  ...
  // Compute requantization parameters.
  const float max_abs_output_scale =
      math_max_f32(abs_a_output_scale, abs_b_output_scale);
  ...
  const uint32_t shift =
      (uint32_t)(20 /* multiplier bits */ - max_scale_exponent);
  assert(shift <= 30);
  assert(shift >= 12);

  // Multipliers are in [0, 2**21) range, largest multiplier is in [2**20,
  // 2**21) range.
  const int32_t abs_a_multiplier = (int32_t)lrintf(
      uint32_as_float(float_as_uint32(abs_a_output_scale) + (shift << 23)));
  const int32_t abs_b_multiplier = (int32_t)lrintf(
      uint32_as_float(float_as_uint32(abs_b_output_scale) + (shift << 23)));
  assert(math_max_s32(abs_a_multiplier, abs_b_multiplier) >=
         INT32_C(0x00100000));
  assert(abs_a_multiplier <= INT32_C(0x00200000));
  assert(abs_b_multiplier <= INT32_C(0x00200000));

  const int32_t a_multiplier =
      signbit(a_output_scale) ? -abs_a_multiplier : abs_a_multiplier;
  const int32_t b_multiplier =
      signbit(b_output_scale) ? -abs_b_multiplier : abs_b_multiplier;

  const int32_t rounding = INT32_C(1) << (shift - 1);
  uparams->scalar.bias =
      rounding - a_multiplier * (int32_t)(uint32_t)a_quantization->zero_point -
  ...
  uparams->scalar.output_zero_point =
      (int32_t)(uint32_t)output_quantization->zero_point;
  return sizeof(uparams->scalar);
}
...
size_t xnn_init_qs8_add_minmax_scalar_params(
  ...
  uparams->scalar.bias = rounding -
                         a_multiplier * (int32_t)a_quantization->zero_point -
...
size_t xnn_init_qu8_mul_minmax_rndnu_neon_params(
  ...
  uparams->rndnu_neon.output_zero_point =
      (int16_t)output_quantization->zero_point;
...
size_t xnn_init_f16_qs8_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  params->f16_qs8_cvt.scalar.scale =
      xnn_float16_from_float(1.0f / output_quantization->scale);
  params->f16_qs8_cvt.scalar.output_zero_point =
      output_quantization->zero_point;
...
size_t xnn_init_f16_qu8_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  return xnn_init_f16_qs8_cvt_scalar_params(
...
size_t xnn_init_f32_qs8_cvt_scalar_params(
  ...
  params->f32_qs8_cvt.scalar.output_zero_point =
      (int16_t)output_quantization->zero_point;
...
size_t xnn_update_f32_reduce_scalar_params(
    struct xnn_reduce_params params[XNN_MIN_ELEMENTS(1)], float scale) {
...
size_t xnn_init_f32_qu8_cvt_scalar_params(
  ...
  params->f32_qu8_cvt.scalar.output_zero_point =
      (int16_t)output_quantization->zero_point;
...
size_t xnn_init_qs8_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  const float input_output_scale =
      input_quantization->scale / output_quantization->scale;
  ...
  params->qs8_cvt.scalar.input_zero_point =
      (int16_t)input_quantization->zero_point;
  params->qs8_cvt.scalar.multiplier = (int32_t)multiplier;
  params->qs8_cvt.scalar.output_zero_point =
      (int16_t)output_quantization->zero_point;
...
size_t xnn_init_qs8_f32_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  params->qs8_f32_cvt.scalar.zero_point =
      (int32_t)input_quantization->zero_point;
...
size_t xnn_init_qs8_f16_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  params->qs8_f16_cvt.scalar.zero_point =
      (int16_t)input_quantization->zero_point;
  params->qs8_f16_cvt.scalar.scale =
      xnn_float16_from_float(input_quantization->scale);
...
size_t xnn_init_qu8_cvt_scalar_params(
  ...
  assert(multiplier >= 1L);
  assert(multiplier <= 32768L);
  params->qu8_cvt.scalar.input_zero_point =
      (uint16_t)input_quantization->zero_point;
  params->qu8_cvt.scalar.multiplier = (int32_t)multiplier;
  params->qu8_cvt.scalar.output_zero_point =
      (int16_t)output_quantization->zero_point;
...
size_t xnn_init_qu8_f32_cvt_scalar_params(
    ...
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization) {
  params->qu8_f32_cvt.scalar.zero_point =
      (int32_t)input_quantization->zero_point;
