...
{
  params->qs8_hswish.scalar.input_zero_point = (uint32_t) input_quantization->zero_point;
  params->qs8_hswish.scalar.output_zero_point= (int32_t) output_quantization->zero_point;
  const float divisor1 = 0x1.555556p-10f;
  const uint32_t input_scale_div = float_as_uint32(input_quantization->scale * divisor1);
  params->qs8_hswish.scalar.input_scale_div_exp = (int32_t) (input_scale_div >> 23) - 126;
  params->qs8_hswish.scalar.input_scale_div_mantissa = (int32_t) ((input_scale_div << 9) >> 18 | UINT16_C(0x4000));
  const float scale_ratio = input_quantization->scale / output_quantization->scale;
  assert(scale_ratio >= 0x1.0p-8f);
  assert(scale_ratio < 0x1.0p+7f);
  params->qs8_hswish.scalar.scale_ratio = (int32_t) lrintf(scale_ratio * 256.0f);
  return sizeof(params->qs8_hswish.scalar);
}
...
size_t xnn_init_qs8_hswish_sse2_params(
  ...
  const struct xnn_quantization_params* output_quantization)
{
  const int16_t input_scale_div = (int16_t) -lrintf(256.0f * input_quantization->scale / 6.0f);
  const float scale_ratio = input_quantization->scale / output_quantization->scale;
  assert(scale_ratio >= 0x1.0p-8f);
  assert(scale_ratio < 0x1.0p+7f);
  const int16_t scale_ratio_param = (int16_t) -lrintf(scale_ratio * 256.0f);
  params->qs8_hswish.sse2.input_zero_point = input_quantization->zero_point;
  params->qs8_hswish.sse2.output_zero_point = output_quantization->zero_point;
  params->qs8_hswish.sse2.input_scale_div = input_scale_div;
  params->qs8_hswish.sse2.scale_ratio = scale_ratio_param;
  return sizeof(params->qs8_hswish.sse2);
}
#endif  // XNN_ARCH_X86 || XNN_ARCH_X86_64
...
size_t xnn_init_qu8_hswish_scalar_params(
  ...
  const struct xnn_quantization_params* output_quantization)
{
  params->qu8_hswish.scalar.input_zero_point = (uint32_t) input_quantization->zero_point;
  params->qu8_hswish.scalar.output_zero_point= (int32_t) output_quantization->zero_point;
  const float divisor1 = 0x1.555556p-10f;
  const uint32_t input_scale_div = float_as_uint32(input_quantization->scale * divisor1);
  params->qu8_hswish.scalar.input_scale_div_exp = (int32_t) (input_scale_div >> 23) - 126;
  params->qu8_hswish.scalar.input_scale_div_mantissa = (int32_t) ((input_scale_div << 9) >> 18 | UINT16_C(0x4000));
  const float scale_ratio = input_quantization->scale / output_quantization->scale;
  assert(scale_ratio >= 0x1.0p-8f);
  assert(scale_ratio < 0x1.0p+7f);
  params->qu8_hswish.scalar.scale_ratio = (int32_t) lrintf(scale_ratio * 256.0f);
  return sizeof(params->qu8_hswish.scalar);
}
  ...
  const struct xnn_quantization_params* output_quantization)
{
  ...
  assert(scale_ratio < 0x1.0p+7f);
  const int16_t scale_ratio_param = (int16_t) -lrintf(scale_ratio * 256.0f);
  params->qu8_hswish.sse2.input_zero_point = input_quantization->zero_point;
  params->qu8_hswish.sse2.output_zero_point = output_quantization->zero_point;
  params->qu8_hswish.sse2.input_scale_div = input_scale_div;
  params->qu8_hswish.sse2.scale_ratio = scale_ratio_param;
  return sizeof(params->qu8_hswish.sse2);
}
#endif  // XNN_ARCH_X86 || XNN_ARCH_X86_64
