...
// Context for Packing Weights (packw) for GEMM microkernels in
// Groups-InputChannels-OutputChannels layout. Kernel has shape GxKxN, bias
// has shape GxN.
...
XNN_PRIVATE void xnn_compute_dwconv_multipass_with_thread(
    const struct dwconv_context context[restrict XNN_MIN_ELEMENTS(1)],
    size_t thread_index, size_t batch_index, size_t output_y);
#endif
...
XNN_PRIVATE void xnn_compute_pixelwise_average_pooling_unipass(
    const struct pixelwise_average_pooling_context
        context[restrict XNN_MIN_ELEMENTS(1)],
    size_t batch_index, size_t output_y);
