...
static inline void xnn_init_qu8_requantization_fp32_params(
    union xnn_qu8_requantization_params params[XNN_MIN_ELEMENTS(1)], float scale, uint8_t zero_point, uint8_t min,
    uint8_t max) {
...
static inline uint8_t xnn_qu8_requantize_fp32(int32_t n,
...
inline static uint8_t xnn_qu8_requantize_rndna(int32_t n,
                                               union xnn_qu8_requantization_params params[XNN_MIN_ELEMENTS(1)]) {
...
inline static uint8_t xnn_qu8_requantize_rndnu(int32_t n,
                                               union xnn_qu8_requantization_params params[XNN_MIN_ELEMENTS(1)]) {
...
typedef void (*xnn_init_qs8_requantization_params_fn)(union xnn_qs8_requantization_params params[XNN_MIN_ELEMENTS(1)],
                                                      float scale, int8_t output_zero_point, int8_t output_min,
                                                      int8_t output_max);

typedef void (*xnn_init_qu8_requantization_params_fn)(union xnn_qu8_requantization_params params[XNN_MIN_ELEMENTS(1)],
                                                      float scale, uint8_t output_zero_point, uint8_t output_min,
                                                      uint8_t output_max);

typedef int8_t (*xnn_qs8_requantize_fn)(int32_t n, union xnn_qs8_requantization_params params[XNN_MIN_ELEMENTS(1)]);

typedef uint8_t (*xnn_qu8_requantize_fn)(int32_t n, union xnn_qu8_requantization_params params[XNN_MIN_ELEMENTS(1)]);

static inline void xnn_init_qs8_requantization_fp32_params(
    union xnn_qs8_requantization_params params[XNN_MIN_ELEMENTS(1)], float scale, int8_t zero_point, int8_t min,
    int8_t max) {
  // Validate requantization parameters.
  assert(scale < 0x1.0p+8f);
  assert(scale >= 0x1.0p-32f);
...
static inline int8_t xnn_qs8_requantize_fp32(int32_t n,
...
inline static int8_t xnn_qs8_requantize_rndna(int32_t n,
                                              union xnn_qs8_requantization_params params[XNN_MIN_ELEMENTS(1)]) {
  const uint32_t abs_n = (n >= 0) ? (uint32_t)n : -(uint32_t)n;
  const uint64_t product = (uint64_t)abs_n * (uint64_t)params->rndna.multiplier;
  const uint32_t abs_scaled_n = (uint32_t)((product + params->rndna.rounding) >> params->rndna.shift);
...
inline static int8_t xnn_qs8_requantize_rndnu(int32_t n,
                                              union xnn_qs8_requantization_params params[XNN_MIN_ELEMENTS(1)]) {
  const int64_t product = (int64_t)n * (int64_t)params->rndnu.multiplier;
  n = (int32_t)asr_s64(product + params->rndnu.rounding, params->rndnu.shift);
...
static inline uint8_t xnn_qu8_quantize_avgpool(int32_t n, union xnn_qu8_avgpool_params params) {
...
static inline int8_t xnn_qs8_quantize_avgpool(int32_t n, union xnn_qs8_avgpool_params params) {
  const int64_t product = (int64_t)n * (int64_t)params.scalar.multiplier;
  const int64_t adjusted_product = product - (int64_t)(n < 0);
...
static inline uint8_t xnn_qu8_quantize_add(uint8_t a, uint8_t b, union xnn_qu8_addsub_minmax_params params) {
  // Multiply by factors and accumulate products.
  int32_t acc = params.scalar.bias + (int32_t)(uint32_t)a * params.scalar.a_multiplier +
...
static inline int8_t xnn_qs8_quantize_add(int8_t a, int8_t b, union xnn_qs8_addsub_minmax_params params) {
  // Multiply by factors and accumulate products.
