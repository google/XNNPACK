...
typedef int8_t (*xnn_qs8_requantize_fn)(int32_t input, float scale,
                                        int8_t output_zero_point,
                                        int8_t output_min, int8_t output_max);

typedef uint8_t (*xnn_qu8_requantize_fn)(int32_t input, float scale,
                                         uint8_t output_zero_point,
                                         uint8_t output_min,
                                         uint8_t output_max);

static inline int8_t xnn_qs8_requantize_fp32(int32_t input, float scale,
                                             int8_t zero_point, int8_t min,
                                             int8_t max) {
...
static inline uint8_t xnn_qu8_requantize_fp32(int32_t input, float scale,
                                              uint8_t zero_point, uint8_t min,
                                              uint8_t max) {
  assert(scale >= 1.0f / 4294967296.0f /* 0x1.0p-32f */);
  assert(scale < 256.0f);
...
static inline int8_t xnn_qs8_requantize_rndnu(int32_t input, float scale,
                                              int8_t zero_point, int8_t min,
                                              int8_t max) {
  assert(scale < 256.0f);
  assert(scale >= 1.0f / 4294967296.0f /* 0x1.0p-32f */);
...
static inline uint8_t xnn_qu8_requantize_rndnu(int32_t input, float scale,
                                               uint8_t zero_point, uint8_t min,
                                               uint8_t max) {
  assert(scale < 256.0f);
  assert(scale >= 1.0f / 4294967296.0f /* 0x1.0p-32f */);
...
static inline uint8_t xnn_qu8_quantize_add(
    uint8_t a, uint8_t b, struct xnn_qu8_add_minmax_params params) {
  // Multiply by factors and accumulate products.
  int32_t acc = params.scalar.bias +
...
static inline int8_t xnn_qs8_quantize_add(
    int8_t a, int8_t b, struct xnn_qs8_add_minmax_params params) {
  // Multiply by factors and accumulate products.
  int32_t acc = params.scalar.bias + (int32_t)a * params.scalar.a_multiplier +
                (int32_t)b * params.scalar.b_multiplier;
...
inline static int8_t xnn_qs8_quantize(float val, float scale,
                                      int32_t zero_point) {
  return (int8_t)lrintf(
      fminf(fmaxf(val / scale + (float)zero_point, -128.0f), 127.0f));
}

inline static uint8_t xnn_qu8_quantize(float val, float scale,
                                       int32_t zero_point) {
