...
// GCC pre-11, Clang pre-8, Android NDK Clang pre-8.0.7, Apple Clang pre-11, and
// ICC pre-16
...
static XNN_INTRINSIC void _mm_storeu_si32(void* address, __m128i v) {
  unaligned_store_u32(address, (uint32_t)_mm_cvtsi128_si32(v));
}

static XNN_INTRINSIC void _mm_storeu_si16(void* address, __m128i v) {
  unaligned_store_u16(address, (uint16_t)_mm_extract_epi16(v, 0));
}
#endif  // GCC pre-11, Clang pre-8, Android NDK Clang pre-8.0.7, Apple Clang
        // pre-11, and ICC pre-16
#endif  // SSE2
...
// GCC pre-7, Clang pre-8, Android NDK Clang pre-8.0.7, Apple Clang pre-11, ICC
// pre-18, and MSVC pre-2019
#if (defined(__GNUC__) && !defined(__clang__) && !defined(__INTEL_COMPILER) && \
     (__GNUC__ < 7)) ||                                                        \
    (defined(__clang__) && !defined(__apple_build_version__) &&                \
     (__clang_major__ < 8)) ||                                                 \
...
static XNN_INTRINSIC __mmask64 _cvtu64_mask64(unsigned long long mask) {
  return (__mmask64)mask;
}

static XNN_INTRINSIC __mmask64 _kshiftli_mask64(__mmask64 a,
                                                unsigned int count) {
  return (__mmask64)((unsigned long long)a << count);
}

static XNN_INTRINSIC __mmask64 _kshiftri_mask64(__mmask64 a,
                                                unsigned int count) {
  return (__mmask64)((unsigned long long)a >> count);
}

#endif  // GCC pre-7, Clang pre-8, Android NDK Clang pre-8.0.7, Apple Clang
        // pre-11, and ICC pre-18
...
static XNN_INTRINSIC float _mm512_reduce_add_ps(__m512 v) {
#if __AVX512DQ__
  const __m256 sum2 =
      _mm256_add_ps(_mm512_castps512_ps256(v), _mm512_extractf32x8_ps(v, 1));
#else   // __AVX512DQ__
  const __m256 sum2 = _mm256_add_ps(
      _mm512_castps512_ps256(v),
      _mm256_castpd_ps(_mm512_extractf64x4_pd(_mm512_castps_pd(v), 1)));
#endif  // __AVX512DQ__
  const __m128 sum4 =
      _mm_add_ps(_mm256_castps256_ps128(sum2), _mm256_extractf128_ps(sum2, 1));
...
static XNN_INTRINSIC float _mm512_reduce_max_ps(__m512 v) {
#if __AVX512DQ__
  const __m256 sum2 =
      _mm256_max_ps(_mm512_castps512_ps256(v), _mm512_extractf32x8_ps(v, 1));
#else   // __AVX512DQ__
  const __m256 sum2 = _mm256_max_ps(
      _mm512_castps512_ps256(v),
      _mm256_castpd_ps(_mm512_extractf64x4_pd(_mm512_castps_pd(v), 1)));
#endif  // __AVX512DQ__
  const __m128 sum4 =
      _mm_max_ps(_mm256_castps256_ps128(sum2), _mm256_extractf128_ps(sum2, 1));
...
#if defined(__GNUC__) && !defined(__clang__) && !defined(__INTEL_COMPILER) && \
    (__GNUC__ < 9)
static XNN_INTRINSIC __m512i _mm512_set_epi8(
    char e63, char e62, char e61, char e60, char e59, char e58, char e57,
    char e56, char e55, char e54, char e53, char e52, char e51, char e50,
    char e49, char e48, char e47, char e46, char e45, char e44, char e43,
    char e42, char e41, char e40, char e39, char e38, char e37, char e36,
    char e35, char e34, char e33, char e32, char e31, char e30, char e29,
    char e28, char e27, char e26, char e25, char e24, char e23, char e22,
    char e21, char e20, char e19, char e18, char e17, char e16, char e15,
    char e14, char e13, char e12, char e11, char e10, char e09, char e08,
    char e07, char e06, char e05, char e04, char e03, char e02, char e01,
    char e00) {
...
static XNN_INTRINSIC __m512i _mm512_loadu_epi16(void const* mem_addr) {
  return _mm512_set1_epi16((int)unaligned_load_s16(mem_addr));
}

static XNN_INTRINSIC __m512i _mm512_loadu_epi32(void const* mem_addr) {
  return _mm512_set1_epi32((int)unaligned_load_s32(mem_addr));
}

static XNN_INTRINSIC void _mm512_storeu_epi16(void* mem_addr, __m512i a) {
  _mm512_storeu_si512(mem_addr, a);
}

static XNN_INTRINSIC void _mm512_storeu_epi32(void* mem_addr, __m512i a) {
  _mm512_storeu_si512(mem_addr, a);
}
#endif  // GCC pre-10
...
static XNN_INTRINSIC __m512i _mm512_dpbusd_epi32_madd(__m512i i32,
                                                      const __m512i u8,
                                                      const __m512i u4) {
...
static XNN_INTRINSIC __m256i _mm256_dpbusd_offset_epi32_madd(__m256i i32,
  ...
  const __m256i biased_i4 =
      offset == 0 ? i4 : _mm256_sub_epi8(i4, _mm256_set1_epi8(offset));
...
// AVXVNNI replacement that uses vpmaddubsw.
// u2 is uint2 in lower 2 bits (unbiased, zero point 0).
static XNN_INTRINSIC __m256i _mm256_dpbusd_epi32_madd_kzp0(__m256i i32,
                                                           const __m256i u8,
                                                           const __m256i u2) {
  return _mm256_dpbusd_offset_epi32_madd(i32, u8, u2, 0);
}
...
// u4 is uint4 in lower 4 bits.
static XNN_INTRINSIC __m128i _mm_dpbusd_epi32_madd(__m128i i32,
                                                   const __m128i u8,
                                                   const __m128i u4) {
...
// SSE VNNI replacement for 2 bit unbiased (zero point 0).
// u2 is uint2 in lower 2 bits.
static XNN_INTRINSIC __m128i _mm_dpbusd_epi32_madd_kzp0(__m128i i32,
                                                        const __m128i u8,
                                                        const __m128i u2) {
  const __m128i vminus_one = _mm_set1_epi16(-1);
  const __m128i i10 = _mm_maddubs_epi16(u8, u2);  // u8 * u2 = i10
  const __m128i v =
      _mm_madd_epi16(i10, vminus_one);  // convert 16 bits to 32 bits
  return _mm_sub_epi32(i32, v);
}
...
static XNN_INTRINSIC int32x4_t vcvtnq_s32_f32(float32x4_t v) {
...
#if XNN_ARCH_ARM && \
    (defined(__ARM_FEATURE_FP16_SCALAR_ARITHMETIC) && defined(__clang__))
#include <arm_fp16.h>

static XNN_INTRINSIC float16_t vaddh_f16(float16_t a, float16_t b) {
  return a + b;
}

static XNN_INTRINSIC float16_t vdivh_f16(float16_t a, float16_t b) {
  return a / b;
}

static XNN_INTRINSIC float16_t vmaxnmh_f16(float16_t a, float16_t b) {
  return XNN_UNPREDICTABLE(b < a) ? a : b;
}

static XNN_INTRINSIC float16_t vminnmh_f16(float16_t a, float16_t b) {
  return XNN_UNPREDICTABLE(b < a) ? b : a;
}

static XNN_INTRINSIC float16_t vmulh_f16(float16_t a, float16_t b) {
  return a * b;
}

static XNN_INTRINSIC float16_t vsubh_f16(float16_t a, float16_t b) {
  return a - b;
}

static XNN_INTRINSIC float16_t vsqrth_f16(float16_t v) {
...
// GNU-style assembly is not supported, or "x" constraint is not supported
// (Clang pre-10)
...
// AArch64 GCC pre-8, 8.1-8.4, 9.1-9.3
    ...
    (__GNUC__ < 8 || __GNUC__ == 8 && __GNUC_MINOR__ < 5 || \
     __GNUC__ == 9 && __GNUC_MINOR__ < 4)
static XNN_INTRINSIC uint8x16x4_t vld1q_u8_x4(const uint8_t* address) {
  uint8x16x4_t result;
...
#pragma clang loop unroll(enable)
  for (int newtRaph = 0; newtRaph < 3; newtRaph++) {
    ...
    vtmp =
        Q6_Vqf32_vmpy_Vqf32Vqf32(vX0, vtmp);  // X0[i] * (1.0 - X0[i] * den[i])
    vX0 = Q6_Vqf32_vadd_Vqf32Vqf32(
        vX0, vtmp);  // X0[i] = X0[i] + X0[i] * (1.0 - X0[i] * den[i])
  }

  // multiply result by same normalization factor applied to denominator
  // earlier.
