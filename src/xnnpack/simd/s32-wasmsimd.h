...
static XNN_INLINE xnn_simd_s32_t xnn_popcnt_s32(xnn_simd_s32_t a) {
  xnn_simd_s32_t bytes = wasm_i8x16_popcnt(a);
  xnn_simd_s32_t half_sum = wasm_i16x8_add(wasm_i16x8_extend_low_i8x16(bytes), wasm_i16x8_extend_high_i8x16(bytes));
  xnn_simd_s32_t result = wasm_i32x4_add(wasm_i32x4_extend_low_i16x8(half_sum), wasm_i32x4_extend_high_i16x8(half_sum));
  return result;
}
