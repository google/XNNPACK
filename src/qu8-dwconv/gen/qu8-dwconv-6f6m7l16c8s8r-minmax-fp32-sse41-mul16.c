...
void xnn_qu8_dwconv_minmax_fp32_ukernel_6f6m7l16c8s8r__sse41_mul16(
    size_t channels,
    size_t output_width,
    const uint8_t** input,
    const void* weights,
    ...
    // First pass to process 6 inputs.
    {
      ...
      if XNN_UNLIKELY(c != 0) {
        do {
          ...
          w = (const void*) ((uintptr_t) w + 8 * sizeof(int32_t) + 48 * sizeof(uint8_t));
    ...
    for (size_t ks = kernel_size - 6; ks > 7; ks -= 6) {
      ...
      if XNN_UNLIKELY(c != 0) {
        do {
          ...
          w = (const void*) ((uintptr_t) w + 48 * sizeof(uint8_t));
