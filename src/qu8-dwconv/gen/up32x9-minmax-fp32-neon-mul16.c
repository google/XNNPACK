...
void xnn_qu8_dwconv_minmax_fp32_ukernel_up32x9__neon_mul16(
    size_t channels,
    size_t output_width,
    const uint8_t** input,
    const void* weights,
    ...
    const union xnn_qu8_conv_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN XNN_DISABLE_MSAN
{
  ...
  do {
    ...
    for (; c >= 32; c -= 32) {
      ...
      vaccOPQR = vmaxq_s32(vaccOPQR, vmagic_min);
      vaccSTUV = vmaxq_s32(vaccSTUV, vmagic_min);
      ...
      vaccOPQR = vsubq_s32(vaccOPQR, vmagic_bias_less_output_zero_point);
      vaccSTUV = vsubq_s32(vaccSTUV, vmagic_bias_less_output_zero_point);
