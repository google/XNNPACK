    ...
    const void* params)
{
  ...
  for (; n >= 32 * sizeof(float); n -= 32 * sizeof(float)) {
    ...
    const v128_t vdenorm6 = wasm_f32x4_sub(wasm_v128_or(wasm_u32x4_shr(vnonsign6, 16), vmagic_mask), vmagic_bias);
    const v128_t vdenorm7 = wasm_f32x4_sub(wasm_v128_or(wasm_u32x4_shr(vnonsign7, 16), vmagic_mask), vmagic_bias);
