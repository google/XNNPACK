    ...
    const void* params)
{
  ...
  for (; n >= 24 * sizeof(float); n -= 24 * sizeof(float)) {
    ...
    const v128_t vdenorm4 = wasm_f32x4_sub(wasm_v128_or(wasm_u32x4_shr(vnonsign4, 16), vmagic_mask), vmagic_bias);
    const v128_t vdenorm5 = wasm_f32x4_sub(wasm_v128_or(wasm_u32x4_shr(vnonsign5, 16), vmagic_mask), vmagic_bias);
