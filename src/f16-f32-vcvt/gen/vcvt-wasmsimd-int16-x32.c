    ...
    const union xnn_f16_f32_cvt_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; n >= 32 * sizeof(uint16_t); n -= 32 * sizeof(uint16_t)) {
    ...
    const v128_t vf4 = wasm_v128_or(wasm_v16x8_shuffle(vzero, vsign2, 0,  8, 1,  9, 2, 10, 3, 11),
      wasm_v128_bitselect(vnorm4, vdenorm4, vxmask4));
    const v128_t vf5 = wasm_v128_or(wasm_v16x8_shuffle(vzero, vsign2, 4, 12, 5, 13, 6, 14, 7, 15),
      wasm_v128_bitselect(vnorm5, vdenorm5, vxmask5));
    const v128_t vf6 = wasm_v128_or(wasm_v16x8_shuffle(vzero, vsign3, 0,  8, 1,  9, 2, 10, 3, 11),
      wasm_v128_bitselect(vnorm6, vdenorm6, vxmask6));
    const v128_t vf7 = wasm_v128_or(wasm_v16x8_shuffle(vzero, vsign3, 4, 12, 5, 13, 6, 14, 7, 15),
      wasm_v128_bitselect(vnorm7, vdenorm7, vxmask7));
