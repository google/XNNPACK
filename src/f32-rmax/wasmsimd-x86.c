    ...
    float* y)
{
  ...
  v128_t vmax0123 = wasm_f32x4_pmax(wasm_f32x4_pmax(vmax0, vmax1), wasm_f32x4_pmax(vmax2, vmax3));
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    const v128_t vx = wasm_v128_load(x);
    vmax0123 = wasm_f32x4_pmax(vx, vmax0123);
    x += 4;
  }
  vmax0123 = wasm_f32x4_pmax(vmax0123, wasm_v32x4_shuffle(vmax0123, vmax0123, 2, 3, 0, 1));
