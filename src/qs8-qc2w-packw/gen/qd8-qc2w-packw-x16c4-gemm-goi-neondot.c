...
void xnn_qd8_qc2w_packw_gemm_goi_ukernel_x16c4__neondot(
  size_t g,
  size_t nc,
  size_t kc,
  size_t nr,
  ...
  const struct xnn_qd8_qc2w_packing_params* params)
{
  ...
  do {
    ...
    for (; n >= 16; n -= 16) {
      ...
      float* packed_zp = (float*) (packed_b + 16);
      if (kernel_zero_points != NULL) {
        for (size_t i = 0; i < 16; ++i) {
          packed_zp[i] = kernel_zero_points[i];
        }
        kernel_zero_points += 16;
      } else {
        for (size_t i = 0; i < 16; ++i) {
          packed_zp[i] = 0.0f;
        }
      }
      out = (uint8_t*) (packed_zp + 16);
      ...
      for (; k >= 4; k -= 4) {
        ...
        w7 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w8[(i >> 2) + 0 * 1];
          const uint8_t b1 = w8[(i >> 2) + 1 * 1];
          const uint8_t b2 = w8[(i >> 2) + 2 * 1];
          const uint8_t b3 = w8[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w8 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w9[(i >> 2) + 0 * 1];
          const uint8_t b1 = w9[(i >> 2) + 1 * 1];
          const uint8_t b2 = w9[(i >> 2) + 2 * 1];
          const uint8_t b3 = w9[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w9 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w10[(i >> 2) + 0 * 1];
          const uint8_t b1 = w10[(i >> 2) + 1 * 1];
          const uint8_t b2 = w10[(i >> 2) + 2 * 1];
          const uint8_t b3 = w10[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w10 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w11[(i >> 2) + 0 * 1];
          const uint8_t b1 = w11[(i >> 2) + 1 * 1];
          const uint8_t b2 = w11[(i >> 2) + 2 * 1];
          const uint8_t b3 = w11[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w11 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w12[(i >> 2) + 0 * 1];
          const uint8_t b1 = w12[(i >> 2) + 1 * 1];
          const uint8_t b2 = w12[(i >> 2) + 2 * 1];
          const uint8_t b3 = w12[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w12 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w13[(i >> 2) + 0 * 1];
          const uint8_t b1 = w13[(i >> 2) + 1 * 1];
          const uint8_t b2 = w13[(i >> 2) + 2 * 1];
          const uint8_t b3 = w13[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w13 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w14[(i >> 2) + 0 * 1];
          const uint8_t b1 = w14[(i >> 2) + 1 * 1];
          const uint8_t b2 = w14[(i >> 2) + 2 * 1];
          const uint8_t b3 = w14[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w14 += 4;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = w15[(i >> 2) + 0 * 1];
          const uint8_t b1 = w15[(i >> 2) + 1 * 1];
          const uint8_t b2 = w15[(i >> 2) + 2 * 1];
          const uint8_t b3 = w15[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
      ...
      if (k != 0) {
        ...
        w7 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w8[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w8[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w8[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w8[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w8 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w9[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w9[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w9[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w9[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w9 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w10[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w10[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w10[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w10[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w10 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w11[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w11[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w11[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w11[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w11 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w12[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w12[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w12[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w12[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w12 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w13[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w13[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w13[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w13[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w13 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w14[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w14[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w14[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w14[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
        ...
        w14 += k;
        for (size_t i = 0; i < 4; ++i) {
          const uint8_t b0 = temp_w15[(i >> 2) + 0 * 1];
          const uint8_t b1 = temp_w15[(i >> 2) + 1 * 1];
          const uint8_t b2 = temp_w15[(i >> 2) + 2 * 1];
          const uint8_t b3 = temp_w15[(i >> 2) + 3 * 1];
          const uint8_t shift = (i & 3) * 2;
          const uint8_t val0 = (b0 >> shift) & 3;
          const uint8_t val1 = (b1 >> shift) & 3;
          const uint8_t val2 = (b2 >> shift) & 3;
