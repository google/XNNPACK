...
void xnn_qs8_f32_gemm_minmax_ukernel_4x4__scalar(
    size_t mr,
    size_t nc,
    size_t kc,
    ...
    const union xnn_qs8_conv_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  vzp[2] = mr > 2 ? unaligned_indexed_load_s32(input_zp, 2) : vzp[0];
  vscale[2] = mr > 2 ? unaligned_indexed_load_f32(input_scale, 2) : vscale[0];
  vzp[3] = mr > 3 ? unaligned_indexed_load_s32(input_zp, 3) : vzp[0];
  vscale[3] = mr > 3 ? unaligned_indexed_load_f32(input_scale, 3) : vscale[0];
  ...
  do {
    ...
    float vout2x0 = vfpacc2x0 + vbias0;
    float vout3x0 = vfpacc3x0 + vbias0;
    float vout0x1 = vfpacc0x1 + vbias1;
    float vout1x1 = vfpacc1x1 + vbias1;
    float vout2x1 = vfpacc2x1 + vbias1;
    float vout3x1 = vfpacc3x1 + vbias1;
    float vout0x2 = vfpacc0x2 + vbias2;
    float vout1x2 = vfpacc1x2 + vbias2;
    float vout2x2 = vfpacc2x2 + vbias2;
    float vout3x2 = vfpacc3x2 + vbias2;
    float vout0x3 = vfpacc0x3 + vbias3;
    float vout1x3 = vfpacc1x3 + vbias3;
    float vout2x3 = vfpacc2x3 + vbias3;
    float vout3x3 = vfpacc3x3 + vbias3;
