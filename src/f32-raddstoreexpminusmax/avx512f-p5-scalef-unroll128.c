...
void xnn_f32_raddstoreexpminusmax_ukernel__avx512f_p5_scalef_unroll128(
    size_t n,
    const float* input,
    float* output,
    float* sum,
    float max)
{
  ...
  for (; n >= 128 * sizeof(float); n -= 128 * sizeof(float)) {
    ...
    //   f = 2**n * (1 + t * (c1 + t * (c2 + t * (c3 + t * (c4 + t * c5)))))
    //     = 2**n * p
  ...
  __m512 vacc = _mm512_add_ps(_mm512_add_ps(vacc0, vacc1), _mm512_add_ps(vacc2, vacc3));
