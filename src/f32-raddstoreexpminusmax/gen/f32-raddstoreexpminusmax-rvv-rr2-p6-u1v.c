...
static inline vfloat32m1_t eval_poly_horner(vfloat32m1_t x,
                                                  float c6, float c5,
                                                  float c4, float c3, float c2,
                                                  float c1, float c0, size_t vl) {
  vfloat32m1_t z;
  vfloat32m1_t y = __riscv_vfmv_v_f_f32m1(c5, vl);
  y = __riscv_vfmacc_vf_f32m1(y, c6, x, vl);

  z = __riscv_vfmv_v_f_f32m1(c4, vl);
  y = __riscv_vfmadd_vv_f32m1(y, x, z, vl);

  z = __riscv_vfmv_v_f_f32m1(c3, vl);
  y = __riscv_vfmadd_vv_f32m1(y, x, z, vl);

  z = __riscv_vfmv_v_f_f32m1(c2, vl);
  y = __riscv_vfmadd_vv_f32m1(y, x, z, vl);

  z = __riscv_vfmv_v_f_f32m1(c1, vl);
  y = __riscv_vfmadd_vv_f32m1(y, x, z, vl);

  z = __riscv_vfmv_v_f_f32m1(c0, vl);
  y = __riscv_vfmadd_vv_f32m1(y, x, z, vl);
  return y;
}
...
static inline vfloat32m1_t softexp_f32m1(
    vfloat32m1_t x, size_t vl,
  ...
  x = __riscv_vfmax_vf_f32m1(x, xmin, vl);
  ...
  vfloat32m1_t v = __riscv_vfmul_vf_f32m1(x, r_ln2f, vl);

  vint16mf2_t q = __riscv_vfncvt_x_f_w_i16mf2(v, vl);
  vfloat32m1_t z = __riscv_vfwcvt_f_x_v_f32m1(q, vl);

  // Use Cody-Waite range reduction method (note two constants to represent log(2)) to improve accuracy.
  vfloat32m1_t s = __riscv_vfnmsac_vf_f32m1(x, l2uf, z, vl);
  s = __riscv_vfnmsac_vf_f32m1(s, l2lf, z, vl);

  // 1. Approximate e^s by degree-6 polynomial approximation
  vfloat32m1_t u = eval_poly_horner(s, c6, c5, c4, c3, c2, 1.0f, 1.0f, vl);

  // 2. Reconstruction: compute u = u*2^q
  const int16_t p = (24 - 1);
  const int16_t bias = (128 - 1);
  vint32m1_t qw = __riscv_vwadd_vx_i32m1(q, bias, vl);
  vint32m1_t qq = __riscv_vsll_vx_i32m1(qw, p, vl);
  vfloat32m1_t qf = __riscv_vreinterpret_v_i32m1_f32m1(qq);
  u = __riscv_vfmul_vv_f32m1(u, qf, vl);
  return u;
}

void xnn_f32_raddstoreexpminusmax_ukernel__rvv_rr2_p6_u1v(
    size_t batch,
    const float* input,
    const float* max,
    float* output,
    ...
    const union xnn_f32_expminus_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    vx = __riscv_vfsub_vf_f32m1(vx, *max, vl);
    input += vl;
    vfloat32m1_t vexp = softexp_f32m1(vx, vl, params);
    __riscv_vse32_v_f32m1(output, vexp, vl);
    output += vl;
    vsum = __riscv_vfadd_vv_f32m1_tu(vsum, vsum, vexp, vl);
  } while(avl > 0);

  vfloat32m1_t v0 = __riscv_vfmv_s_f_f32m1(0.0f, 1);
  *sum = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredusum_vs_f32m1_f32m1(vsum, v0, n));
}
