    ...
    const struct xnn_qd8_quantization_params* restrict quantization_params) XNN_DISABLE_UBSAN
{
  ...
  do {
    ...
    const float vkernel_zero_point0 = ((const float*) w)[4];
    const float vkernel_zero_point1 = ((const float*) w)[5];
    const float vkernel_zero_point2 = ((const float*) w)[6];
    const float vkernel_zero_point3 = ((const float*) w)[7];
