    ...
    const union xnn_f32_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 8 * sizeof(float); n -= 8 * sizeof(float)) {
    ...
    vy0123 = wasm_f32x4_pmax(vy0123, vy_min);
    vy4567 = wasm_f32x4_pmax(vy4567, vy_min);

    vy0123 = wasm_f32x4_pmin(vy0123, vy_max);
    vy4567 = wasm_f32x4_pmin(vy4567, vy_max);
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    vy = wasm_f32x4_pmax(vy, vy_min);
    vy = wasm_f32x4_pmin(vy, vy_max);
