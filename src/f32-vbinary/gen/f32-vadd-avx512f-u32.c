...
void xnn_f32_vadd_ukernel__avx512f_u32(
    size_t batch,
    const float* input_a,
    const float* input_b,
    float* output,
    const struct xnn_f32_default_params* restrict params)
{
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    __m512 vacc0 = _mm512_add_ps(va0, _mm512_loadu_ps(input_b + 0));
    __m512 vacc1 = _mm512_add_ps(va1, _mm512_loadu_ps(input_b + 16));
    input_b += 32;
