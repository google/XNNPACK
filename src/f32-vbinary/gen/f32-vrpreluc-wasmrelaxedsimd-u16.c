...
void xnn_f32_vrpreluc_ukernel__wasmrelaxedsimd_u16(
    size_t batch,
    const float* input_a,
    const float* input_b,
    float* output,
    const struct xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 16 * sizeof(float); batch -= 16 * sizeof(float)) {
    ...
    vy0 = wasm_i32x4_relaxed_laneselect(vy0, vb, vmask);
    vy1 = wasm_i32x4_relaxed_laneselect(vy1, vb, vmask);
    vy2 = wasm_i32x4_relaxed_laneselect(vy2, vb, vmask);
    vy3 = wasm_i32x4_relaxed_laneselect(vy3, vb, vmask);
  ...
  for (; batch >= 4 * sizeof(float); batch -= 4 * sizeof(float)) {
    ...
    vy = wasm_i32x4_relaxed_laneselect(vy, vb, vmask);
