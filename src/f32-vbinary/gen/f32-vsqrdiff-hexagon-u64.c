...
void xnn_f32_vsqrdiff_ukernel__hexagon_u64(
    size_t batch,
    const float* input_a,
    const float* input_b,
    float* output,
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 64 * sizeof(float); batch -= 64 * sizeof(float)) {
    ...
    HVX_Vector vacc0 = Q6_Vqf32_vsub_VsfVsf(va0, vb0);
    HVX_Vector vacc1 = Q6_Vqf32_vsub_VsfVsf(va1, vb1);

    vacc0 = Q6_Vqf32_vmpy_VsfVsf(vacc0, vacc0);
    vacc1 = Q6_Vqf32_vmpy_VsfVsf(vacc1, vacc1);
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    vacc = Q6_Vqf32_vmpy_VsfVsf(vacc, vacc);
