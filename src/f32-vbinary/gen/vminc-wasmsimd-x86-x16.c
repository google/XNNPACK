    ...
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 16 * sizeof(float); n -= 16 * sizeof(float)) {
    ...
    v128_t vy0123 = wasm_f32x4_pmin(va0123, vb);
    v128_t vy4567 = wasm_f32x4_pmin(va4567, vb);
    v128_t vy89AB = wasm_f32x4_pmin(va89AB, vb);
    v128_t vyCDEF = wasm_f32x4_pmin(vaCDEF, vb);
