...
void xnn_f32_vprelu_ukernel__wasmrelaxedsimd_u8(
    size_t batch,
    const float* input_a,
    const float* input_b,
    float* output,
    const struct xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 8 * sizeof(float); batch -= 8 * sizeof(float)) {
    ...
    vacc0 = wasm_i32x4_relaxed_laneselect(vacc0, va0, vmask0);
    vacc1 = wasm_i32x4_relaxed_laneselect(vacc1, va1, vmask1);
