    ...
    const union xnn_f32_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 16 * sizeof(float); batch -= 16 * sizeof(float)) {
    ...
    __m256 vacc0 = _mm256_add_ps(va0, _mm256_loadu_ps(input_b));
    __m256 vacc1 = _mm256_add_ps(va1, _mm256_loadu_ps(input_b + 8));
    input_b += 16;
  ...
  for (; batch >= 8 * sizeof(float); batch -= 8 * sizeof(float)) {
    ...
    __m256 vacc = _mm256_add_ps(va, _mm256_loadu_ps(input_b));
    input_b += 8;
  ...
  if XNN_UNLIKELY(batch != 0) {
    ...
    __m256 vacc = _mm256_add_ps(va, vb);
    vacc = _mm256_max_ps(voutput_min, vacc);
    vacc = _mm256_min_ps(voutput_max, vacc);
