...
void xnn_f32_vprelu_ukernel__wasm_u8(
    size_t batch,
    const float* input_a,
    const float* input_b,
    float* output,
    const struct xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 8 * sizeof(float); batch -= 8 * sizeof(float)) {
    ...
    vacc4 = XNN_UNPREDICTABLE(va4 < 0.0f) ? va4 * vb4 : va4;
    vacc5 = XNN_UNPREDICTABLE(va5 < 0.0f) ? va5 * vb5 : va5;
    vacc6 = XNN_UNPREDICTABLE(va6 < 0.0f) ? va6 * vb6 : va6;
    vacc7 = XNN_UNPREDICTABLE(va7 < 0.0f) ? va7 * vb7 : va7;
