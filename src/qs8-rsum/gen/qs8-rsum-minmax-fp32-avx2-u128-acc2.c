...
void xnn_qs8_rsum_minmax_fp32_ukernel__avx2_u128_acc2(
    size_t batch,
    const int8_t* input,
    int8_t* output,
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  // There are 2 registers and each register has 16 lanes so batch size is 8192
  ...
  while (batch >= 128) {
    ...
    __m256i left1 = _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vacc16_1));
    __m256i right1 = _mm256_cvtepi16_epi32(_mm256_extractf128_si256(vacc16_1, 1));
    vacc = _mm256_add_epi32(vacc, _mm256_add_epi32(left1, right1));
