    ...
    const union xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  // There are 4 registers and each register has 16 lanes so batch size is 16384
  ...
  while (batch >= 128) {
    ...
    vacc = _mm256_add_epi32(vacc, _mm256_add_epi32(left2, right2));
    ...
    vacc = _mm256_add_epi32(vacc, _mm256_add_epi32(left3, right3));
