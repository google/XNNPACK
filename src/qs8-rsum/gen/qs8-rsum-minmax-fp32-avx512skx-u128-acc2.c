...
void xnn_qs8_rsum_minmax_fp32_ukernel__avx512skx_u128_acc2(
    size_t batch,
    const int8_t* input,
    int8_t* output,
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  // There are 32 lanes in the accumulator register and 2 registers.
  int num_batches = (batch + 16383) >> 14;
  const __m512i vone = _mm512_set1_epi8(1);
  for (; num_batches > 0; --num_batches) {
    __m512i vacc16_0 = _mm512_setzero_si512();
    __m512i vacc16_1 = _mm512_setzero_si512();
    for (int current_batch = min(batch, 16384); current_batch >= 128; current_batch -= 128) {
    ...
    batch = (batch >= 16384 ? (batch - 16384) : batch & 127);
  }
