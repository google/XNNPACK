    ...
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; num_batches > 0; --num_batches) {
    ...
    for (size_t current_batch = 256; current_batch > 0; current_batch -= 64) {
      ...
      const __m256i vt1 = _mm256_cvtepi8_epi16(_mm_loadu_si128((const __m128i*) input)); input += 16;
      const __m256i vt2 = _mm256_cvtepi8_epi16(_mm_loadu_si128((const __m128i*) input)); input += 16;
      const __m256i vt3 = _mm256_cvtepi8_epi16(_mm_loadu_si128((const __m128i*) input)); input += 16;
