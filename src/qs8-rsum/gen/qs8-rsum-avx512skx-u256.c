...
void xnn_qs8_rsum_ukernel__avx512skx_u256(
    size_t batch,
    const int8_t* input,
    int32_t* output,
    const union xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 8192; batch -= 8192) {
    ...
    for (size_t current_batch = 8192; current_batch > 0; current_batch -= 256) {
      ...
      vacc16_0 = _mm512_add_epi16(vacc16_0, vt2);
      vacc16_0 = _mm512_add_epi16(vacc16_0, vt3);
    }
    vacc0 = _mm512_add_epi32(vacc0, _mm512_madd_epi16(vone_16, vacc16_0));
  }

  if (XNN_LIKELY(batch >= 256)) {
