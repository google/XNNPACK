...
#include "xnnpack/simd/u32-wasmsimd.h"

#include "xnnpack/common.h"
#include "xnnpack/microparams.h"


void xnn_u32_f32_vcvt_ukernel__wasmsimd_u4(
    size_t batch,
...
void xnn_u32_f32_vcvt_ukernel__wasmsimd_u8(
    size_t batch,
    const uint32_t* input,
    float* output,
    const struct xnn_u32_f32_cvt_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 8 * sizeof(int32_t); batch -= 8 * sizeof(uint32_t)) {
...
void xnn_u32_f32_vcvt_ukernel__wasmsimd_u12(
    size_t batch,
    const uint32_t* input,
    float* output,
    const struct xnn_u32_f32_cvt_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 12 * sizeof(int32_t); batch -= 12 * sizeof(uint32_t)) {
    const xnn_simd_u32_t vx0 = xnn_loadu_u32(input);
    const xnn_simd_u32_t vx1 = xnn_loadu_u32(input + 1 * xnn_simd_size_u32);
...
void xnn_u32_f32_vcvt_ukernel__wasmsimd_u16(
    size_t batch,
    const uint32_t* input,
    float* output,
    const struct xnn_u32_f32_cvt_params params[restrict XNN_MIN_ELEMENTS(1)])
