...
//   Template: src/f16-f32acc-rdsum2/avx.c.in
...
void xnn_f16_f32acc_rdsum2_ukernel_7p7x__f16c_u16(
    size_t channels,
    size_t k1,
    size_t k2,
    size_t k3,
    ...
    for (size_t j = 0; j < k2; ++j) {
      ...
      for (; channels >= 16; channels -= 16) {
        ...
        for (int r = k1; r > 0; r -= 7) {
          ...
          vin0 = _mm256_mul_ps(vin0, vin0);
          vin1 = _mm256_mul_ps(vin1, vin1);
        ...
        input_row = (const xnn_float16*) ((uintptr_t) input_row + 16 * sizeof(xnn_float16));
      }
      if (channels != 0) {
        ...
        for (int r = k1; r > 0; r -= 7) {
          ...
          for (int i = 0; i < num_full_chunks; ++i) {
            __m256 vin0 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i0[i*8]));
            __m256 vin1 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i1[i*8]));
            __m256 vin2 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i2[i*8]));
            __m256 vin3 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i3[i*8]));
            __m256 vin4 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i4[i*8]));
            __m256 vin5 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i5[i*8]));
            __m256 vin6 = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*) &i6[i*8]));
            vin0 = _mm256_mul_ps(vin0, vin0);
            vin1 = _mm256_mul_ps(vin1, vin1);
            vin2 = _mm256_mul_ps(vin2, vin2);
            vin3 = _mm256_mul_ps(vin3, vin3);
            vin4 = _mm256_mul_ps(vin4, vin4);
            vin5 = _mm256_mul_ps(vin5, vin5);
            vin6 = _mm256_mul_ps(vin6, vin6);
            vacc[i] = _mm256_add_ps(vin0, vacc[i]);
            vacc[i] = _mm256_add_ps(vin1, vacc[i]);
            vacc[i] = _mm256_add_ps(vin2, vacc[i]);
            vacc[i] = _mm256_add_ps(vin3, vacc[i]);
            vacc[i] = _mm256_add_ps(vin4, vacc[i]);
            vacc[i] = _mm256_add_ps(vin5, vacc[i]);
            vacc[i] = _mm256_add_ps(vin6, vacc[i]);
          }

          if (remainder) {
            __m256 vin0 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i0[num_full_chunks*8]));
            __m256 vin1 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i1[num_full_chunks*8]));
            __m256 vin2 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i2[num_full_chunks*8]));
            __m256 vin3 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i3[num_full_chunks*8]));
            __m256 vin4 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i4[num_full_chunks*8]));
            __m256 vin5 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i5[num_full_chunks*8]));
            __m256 vin6 = _mm256_cvtph_ps(_mm_loadu_si128((const  __m128i*) &i6[num_full_chunks*8]));
            vin0 = _mm256_mul_ps(vin0, vin0);
            vin1 = _mm256_mul_ps(vin1, vin1);
            vin2 = _mm256_mul_ps(vin2, vin2);
            vin3 = _mm256_mul_ps(vin3, vin3);
            ...
            vin5 = _mm256_mul_ps(vin5, vin5);
            vin6 = _mm256_mul_ps(vin6, vin6);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin0);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin1);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin2);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin3);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin4);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin5);
            vacc[num_full_chunks] = _mm256_add_ps(vacc[num_full_chunks],  vin6);
          }
          i0 = (const uint16_t*) ((uintptr_t) i0 + input_increment);
          i1 = (const uint16_t*) ((uintptr_t) i1 + input_increment);
          i2 = (const uint16_t*) ((uintptr_t) i2 + input_increment);
...
void xnn_f16_f32acc_rdsum2_ukernel_7p7x__f16c_u32(
    size_t channels,
    size_t k1,
    size_t k2,
    size_t k3,
  ...
  for (size_t k = 0; k < k3; ++k) {
    for (size_t j = 0; j < k2; ++j) {
      ...
      for (; channels >= 32; channels -= 32) {
        ...
        input_row = (const xnn_float16*) ((uintptr_t) input_row + 32 * sizeof(xnn_float16));
      }
      if (channels != 0) {
        input_increment = 7 * input_stride1;
        const uint16_t* i0 = (const uint16_t*) input_row;
...
void xnn_f16_f32acc_rdsum2_ukernel_7p7x__f16c_u64(
    size_t channels,
    size_t k1,
    size_t k2,
    size_t k3,
  ...
  for (size_t k = 0; k < k3; ++k) {
    for (size_t j = 0; j < k2; ++j) {
      ...
      for (; channels >= 64; channels -= 64) {
        ...
        input_row = (const xnn_float16*) ((uintptr_t) input_row + 64 * sizeof(xnn_float16));
      }
      if (channels != 0) {
        input_increment = 7 * input_stride1;
        const uint16_t* i0 = (const uint16_t*) input_row;
...
void xnn_f16_f32acc_rdsum2_ukernel_7p7x__f16c_u128(
    size_t channels,
    size_t k1,
    size_t k2,
    size_t k3,
    ...
    for (size_t j = 0; j < k2; ++j) {
      ...
      for (; channels >= 128; channels -= 128) {
        ...
        for (int r = k1; r > 0; r -= 7) {
          ...
          vin6 = _mm256_mul_ps(vin6, vin6);
          vin7 = _mm256_mul_ps(vin7, vin7);
          vin8 = _mm256_mul_ps(vin8, vin8);
          vin9 = _mm256_mul_ps(vin9, vin9);
          vin10 = _mm256_mul_ps(vin10, vin10);
          vin11 = _mm256_mul_ps(vin11, vin11);
          vin12 = _mm256_mul_ps(vin12, vin12);
          vin13 = _mm256_mul_ps(vin13, vin13);
          vin14 = _mm256_mul_ps(vin14, vin14);
          vin15 = _mm256_mul_ps(vin15, vin15);
          vacc0 = _mm256_add_ps(vin0, vacc0);
          vacc1 = _mm256_add_ps(vin1, vacc1);
          vacc2 = _mm256_add_ps(vin2, vacc2);
          vacc3 = _mm256_add_ps(vin3, vacc3);
        ...
        input_row = (const xnn_float16*) ((uintptr_t) input_row + 128 * sizeof(xnn_float16));
      }
      if (channels != 0) {
        input_increment = 7 * input_stride1;
        const uint16_t* i0 = (const uint16_t*) input_row;
