    ...
    const union xnn_qs8_f16_cvt_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 64 * sizeof(int8_t); batch -= 64 * sizeof(int8_t)) {
    ...
    vxWXYZabcd = _mm256_add_epi32(vxWXYZabcd, vminus_zero_point);
    vxefghijkl = _mm256_add_epi32(vxefghijkl, vminus_zero_point);
    vxmnopqrst = _mm256_add_epi32(vxmnopqrst, vminus_zero_point);
    vxuvwxyz01 = _mm256_add_epi32(vxuvwxyz01, vminus_zero_point);
