...
void xnn_f32_bf16_vcvt_ukernel__neon_u32(
    size_t batch,
    const float* input,
    xnn_bfloat16* output,
    const void* params) XNN_OOB_READS
{
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    const float32x4_t vf6 = vld1q_f32(input); input += 4;
    const float32x4_t vf7 = vld1q_f32(input); input += 4;
    ...
    uint32x4_t vi6 = vreinterpretq_u32_f32(vf6);
    uint32x4_t vi7 = vreinterpretq_u32_f32(vf7);
    ...
    const uint32x4_t vlsb6 = vandq_u32(vshrq_n_u32(vi6, 16), vone);
    const uint32x4_t vlsb7 = vandq_u32(vshrq_n_u32(vi7, 16), vone);
    ...
    const uint32x4_t vrounded6 = vaddq_u32(vaddq_u32(vi6, vbias), vlsb6);
    const uint32x4_t vrounded7 = vaddq_u32(vaddq_u32(vi7, vbias), vlsb7);
    ...
    const uint32x4_t vabsi6 = vandq_u32(vi6, vabs_mask);
    const uint32x4_t vabsi7 = vandq_u32(vi7, vabs_mask);
    ...
    const uint32x4_t vnanmask6 = vcgtq_u32(vabsi6, vexp_mask);
    const uint32x4_t vnanmask7 = vcgtq_u32(vabsi7, vexp_mask);
    ...
    vi6 = vbslq_u32(vnanmask6, vorrq_u32(vi6, vquiet), vrounded6);
    vi7 = vbslq_u32(vnanmask7, vorrq_u32(vi7, vquiet), vrounded7);
    ...
    const uint16x8_t vbf3 = vcombine_u16(vshrn_n_u32(vi6, 16), vshrn_n_u32(vi7, 16));

    vst1q_u16(o, vbf0); o += 8;
    vst1q_u16(o, vbf1); o += 8;
    vst1q_u16(o, vbf2); o += 8;
    vst1q_u16(o, vbf3); o += 8;
  }
