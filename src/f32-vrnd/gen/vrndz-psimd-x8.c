...
void xnn_f32_vrndz_ukernel__psimd_x8(
    size_t n,
    const float* x,
    float* y,
    const union xnn_f32_rnd_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 8 * sizeof(float); n -= 8 * sizeof(float)) {
    ...
    const psimd_f32 vadjustment4567 = psimd_andmask_f32(vrndabsx4567 > vabsx4567, vone);

    const psimd_f32 vflrabsx0123 = psimd_sub_f32(vrndabsx0123, vadjustment0123);
    const psimd_f32 vflrabsx4567 = psimd_sub_f32(vrndabsx4567, vadjustment4567);

    const psimd_f32 vy0123 = psimd_blend_f32(vrndmask0123, vx0123, vflrabsx0123);
    const psimd_f32 vy4567 = psimd_blend_f32(vrndmask4567, vx4567, vflrabsx4567);
