...
void xnn_f32_vrndu_ukernel__psimd_x8(
    size_t n,
    const float* x,
    float* y,
    const union xnn_f32_rnd_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 8 * sizeof(float); n -= 8 * sizeof(float)) {
    ...
    const psimd_s32 vadjmask4567 = (vrndx4567 >= vx4567) | vsign_mask;

    const psimd_f32 vadjrndx0123 = psimd_add_f32(vrndx0123, vone);
    const psimd_f32 vadjrndx4567 = psimd_add_f32(vrndx4567, vone);

    const psimd_f32 vy0123 = psimd_blend_f32(vadjmask0123, vrndx0123, vadjrndx0123);
    const psimd_f32 vy4567 = psimd_blend_f32(vadjmask4567, vrndx4567, vadjrndx4567);
