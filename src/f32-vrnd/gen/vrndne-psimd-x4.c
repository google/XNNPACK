...
//   Template: src/f32-vrnd/vrndne-psimd.c.in
...
void xnn_f32_vrndne_ukernel__psimd_x4(
    size_t n,
    const float* x,
    float* y,
    const union xnn_f32_rnd_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    const psimd_f32 vy0123 = psimd_blend_f32(vrndmask0123, vx0123, vrndabsx0123);
