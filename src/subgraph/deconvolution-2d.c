...
{
  // `bias_datatype` of `xnn_datatype_invalid` can indicate no bias.
  switch (filter_datatype) {
    case xnn_datatype_fp32:
      return
          input_datatype == xnn_datatype_fp32 && output_datatype == xnn_datatype_fp32 &&
          (bias_datatype == xnn_datatype_fp32 || bias_datatype == xnn_datatype_invalid);
#ifndef XNN_NO_QS8_OPERATORS
    case xnn_datatype_qint8:
      return
          input_datatype == xnn_datatype_qint8 && output_datatype == xnn_datatype_qint8 &&
          (bias_datatype == xnn_datatype_qint32 || bias_datatype == xnn_datatype_invalid);
#endif  // !defined(XNN_NO_QS8_OPERATORS)
#ifndef XNN_NO_QU8_OPERATORS
    case xnn_datatype_quint8:
      return
          input_datatype == xnn_datatype_quint8 && output_datatype == xnn_datatype_quint8 &&
          (bias_datatype == xnn_datatype_qint32 || bias_datatype == xnn_datatype_invalid);
#endif  // !defined(XNN_NO_QU8_OPERATORS)
    default:
      XNN_UNREACHABLE;
  ...
  uint32_t flags)
{
  ...
  enum xnn_datatype bias_datatype = bias_value == NULL ? xnn_datatype_invalid : bias_value->datatype;
  if (!check_datatypes(input_value->datatype, filter_value->datatype, bias_datatype, output_value->datatype)) {
    if (bias_value != NULL) {
      xnn_log_error(
      ...
      xnn_log_error(
        "failed to define %s operator with input ID #%" PRIu32 ", filter ID #%" PRIu32 ", no bias, and output ID #%" PRIu32
