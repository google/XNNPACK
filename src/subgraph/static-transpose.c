...
static enum xnn_status create_transpose_operator(
  ...
  switch (xnn_datatype_size_bits(input_value->datatype)) {
    case 32:
      status = xnn_create_transpose_nd_x32(node->flags,
                                           &opdata->operator_objects[0]);
      break;
    case 16:
      status = xnn_create_transpose_nd_x16(node->flags,
                                           &opdata->operator_objects[0]);
      break;
    case 8:
      status =
          xnn_create_transpose_nd_x8(node->flags, &opdata->operator_objects[0]);
      break;
    default:
      XNN_UNREACHABLE;
  }
  ...
  if (status == xnn_status_success) {
    ...
    memcpy(opdata->shape2.dim, node->params.transpose.perm,
           opdata->shape2.num_dims * sizeof(size_t));
  }
...
static enum xnn_status reshape_transpose_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_transpose_nd_x16: {
      status = xnn_reshape_transpose_nd_x16(opdata->operator_objects[0],
                                            num_dims, input->shape.dim,
                                            opdata->shape2.dim, threadpool);
      break;
    }
    case xnn_operator_type_transpose_nd_x32: {
      status = xnn_reshape_transpose_nd_x32(opdata->operator_objects[0],
                                            num_dims, input->shape.dim,
                                            opdata->shape2.dim, threadpool);
      break;
    }
    case xnn_operator_type_transpose_nd_x8: {
      status = xnn_reshape_transpose_nd_x8(opdata->operator_objects[0],
                                           num_dims, input->shape.dim,
                                           opdata->shape2.dim, threadpool);
      break;
    }
...
static enum xnn_status setup_transpose_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_transpose_nd_x16: {
      status = xnn_setup_transpose_nd_x16(opdata->operator_objects[0],
                                          input_data, output_data);
      break;
    }
    case xnn_operator_type_transpose_nd_x32: {
      status = xnn_setup_transpose_nd_x32(opdata->operator_objects[0],
                                          input_data, output_data);
      break;
    }
    case xnn_operator_type_transpose_nd_x8: {
      status = xnn_setup_transpose_nd_x8(opdata->operator_objects[0],
                                         input_data, output_data);
      break;
    }
    default:
...
enum xnn_status xnn_define_static_transpose(xnn_subgraph_t subgraph,
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_static_transpose)) != xnn_status_success) {
    return status;
  }

  if (num_dims == 0) {
    xnn_log_error(
        "failed to define %s operator with %zu num_dims: num_dims must be "
        "non-zero",
  ...
  for (size_t i = 0; i < num_dims; ++i) {
    if (perm[i] >= num_dims) {
      xnn_log_error(
          "failed to define %s operator with %zu perm and %zu num_dims: 0 <= "
          "perm < num_dims",
          xnn_node_type_to_string(xnn_node_type_static_transpose), perm[i],
          num_dims);
      return xnn_status_invalid_parameter;
    }
  }
  ...
  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_static_transpose, input_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_static_transpose,
                                               input_id, input_value);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_static_transpose,
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_static_transpose,
  ...
  status = xnn_subgraph_check_datatype_matches(xnn_node_type_static_transpose,
  ...
  status = xnn_subgraph_check_quantization_parameter_matches(
      xnn_node_type_static_transpose, input_id, input_value, output_id,
      output_value);
  if (status != xnn_status_success) {
    return status;
  }
