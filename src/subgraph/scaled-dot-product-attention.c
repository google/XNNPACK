...
static enum xnn_status resize_scaled_dot_product_attention_output_tensor(
    ...
    size_t num_values, size_t old_workspace_size) {
  ...
  const size_t query_batch_size =
      xnn_shape_multiply_batch_dims(&query->shape, 3);
  ...
  const size_t output_batch_size =
      xnn_shape_multiply_batch_dims(&output->shape, 3);
  const size_t output_num_dims = output->shape.num_dims;

  if (query_num_dims != output_num_dims) {
    xnn_log_error(
        "failed to resize %s operator's output: number of dimensions mismatch, "
        "query dims: %zu, output dims: %zu",
  ...
  if (new_output_size > output->size ||
      new_workspace_size > old_workspace_size) {
...
static enum xnn_status reshape_scaled_dot_product_attention_operator(
  ...
  const size_t query_num_dims = query->shape.num_dims;
  if (query_num_dims < 3) {
    xnn_log_error("failed to reshape %s operator with query ID #%" PRIu32
  ...
  if (key_num_dims < 2) {
    xnn_log_error("failed to reshape %s operator with key ID #%" PRIu32
  ...
  if (key_num_dims != query_num_dims && key_num_dims != query_num_dims - 1) {
    xnn_log_error("failed to reshape %s operator with key ID #%" PRIu32
                  ": key must have either the same number of dimensions as or"
                  " 1 less dimension than query",
                  xnn_node_type_to_string(opdata->type), key_id);
    return xnn_status_invalid_parameter;
  }
  ...
  status = xnn_subgraph_check_batch_dims_match(opdata->type, query_id, query,
                                               key_id, key, num_batch_dims);
  ...
  if (!is_multi_query) {
    ...
    if (key_heads != query_heads) {
      xnn_log_error("failed to reshape %s operator with key ID #%" PRIu32
                    ": key heads (%zu) must be equals to query heads (%zu)",
                    xnn_node_type_to_string(opdata->type), key_id, key_heads,
                    query_heads);
      return xnn_status_invalid_parameter;
    }
  }
  ...
  if (key_channels != query_channels) {
    xnn_log_error("failed to reshape %s operator with key ID #%" PRIu32
                  ": key channels (%zu) must be equals to query channels"
                  " (%zu)",
                  xnn_node_type_to_string(opdata->type), key_id, key_channels,
                  query_channels);
    return xnn_status_invalid_parameter;
  }
  ...
  if (!is_multi_query) {
    ...
    if (value_heads != query_heads) {
      xnn_log_error("failed to reshape %s operator with value ID #%" PRIu32
                    ": value heads (%zu) must be equals to query (%zu)",
                    xnn_node_type_to_string(opdata->type), value_id,
    ...
    if (key_heads != value_heads) {
      xnn_log_error("failed to reshape %s operator with key ID #%" PRIu32
                    " and value ID #%" PRIu32
                    ": key heads (%zu) must be "
                    "equal to value heads (%zu)",
                    xnn_node_type_to_string(opdata->type), key_id, value_id,
  ...
  if (scale->shape.dim[0] != query_channels) {
    xnn_log_error("failed to reshape %s operator with scale ID #%" PRIu32
                  ": scale channels (%zu) must be equal to query channels "
                  "(%zu)",
                  xnn_node_type_to_string(opdata->type), scale_id,
                  scale->shape.dim[0], query_channels);
    return xnn_status_invalid_parameter;
  }

  if (mask->shape.dim[0] != query_tokens) {
    xnn_log_error("failed to reshape %s operator with mask ID #%" PRIu32
                  ": mask query tokens (%zu) must be equal to query tokens "
                  "(%zu)",
                  xnn_node_type_to_string(opdata->type), mask_id,
                  mask->shape.dim[0], query_tokens);
    return xnn_status_invalid_parameter;
  }

  if (mask->shape.dim[1] != key_tokens) {
    xnn_log_error("failed to reshape %s operator with mask ID #%" PRIu32
                  ": mask key/value tokens (%zu) must be equal to key/value "
                  "tokens (%zu)",
                  xnn_node_type_to_string(opdata->type), mask_id,
                  mask->shape.dim[1], key_tokens);
    return xnn_status_invalid_parameter;
  }
  ...
  status = xnn_subgraph_check_batch_dims_match(
      opdata->type, query_id, query, output_id, output, num_batch_dims);
  if (status != xnn_status_success) {
    return status;
  }

  if (output_heads != query_heads) {
    xnn_log_error("failed to reshape %s operator with output ID #%" PRIu32
                  ": output heads (%zu) must be equals to query heads"
                  " (%zu)",
                  xnn_node_type_to_string(opdata->type), output_id,
                  output_heads, query_heads);
    return xnn_status_invalid_parameter;
  }

  if (output_tokens != query_tokens) {
    xnn_log_error("failed to reshape %s operator with output ID #%" PRIu32
                  ": output tokens (%zu) must be equals to query tokens"
                  " (%zu)",
                  xnn_node_type_to_string(opdata->type), output_id,
  ...
  if (output_channels != value_channels) {
    xnn_log_error("failed to reshape %s operator with output ID #%" PRIu32
                  ": output channels (%zu) must be equals to value "
                  "channels (%zu)",
                  xnn_node_type_to_string(opdata->type), output_id,
  ...
  const size_t key_heads =
      is_multi_query ? 1 : key->shape.dim[key_num_dims - 3];
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_scaled_dot_product_attention_nhtc_f32:
      status = xnn_reshape_scaled_dot_product_attention_nhtc_f32(
          opdata->operator_objects[0], batch_size, query_heads, query_tokens,
          key_heads, key_tokens, query_channels, value_channels,
  ...
  return resize_scaled_dot_product_attention_output_tensor(
      opdata, values, num_values, old_workspace_size);
}
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_scaled_dot_product_attention_nhtc_f32:
      return xnn_setup_scaled_dot_product_attention_nhtc_f32(
          opdata->operator_objects[0], opdata->workspace, query_data, key_data,
          attention_value_data, scale_data, mask_data, output_data);
    case xnn_operator_type_scaled_dot_product_attention_nhtc_f16:
...
static enum xnn_status check_inputs(xnn_subgraph_t subgraph,
                                    uint32_t input_id) {
  const enum xnn_node_type node_type =
      xnn_node_type_scaled_dot_product_attention;
  enum xnn_status status = xnn_subgraph_check_input_node_id(
  ...
  status =
      xnn_subgraph_check_input_type_dense(node_type, input_id, input_value);
  if (status != xnn_status_success) {
    return status;
  }
...
enum xnn_status xnn_define_scaled_dot_product_attention(
  ...
  if (cap_type == xnn_attention_logits_cap_type_tanh) {
    ...
    if (!isfinite(cap_value) || cap_value <= 0.0f) {
      xnn_log_error(
          "failed to define %s operator with logits cap tanh: cap (%f) must be "
          "finite and positive",
  ...
  const size_t query_num_dims = query->shape.num_dims;
  if (query_num_dims < 3) {
    xnn_log_error("failed to define %s operator with query ID #%" PRIu32
                  ": query must have at least 3 dimensions",
  ...
  const size_t key_num_dims = key->shape.num_dims;
  if (key_num_dims < 2) {
    xnn_log_error("failed to define %s operator with key ID #%" PRIu32
                  ": key must have at least 2 dimensions",
  ...
  if (key_num_dims != query_num_dims && key_num_dims != query_num_dims - 1) {
    xnn_log_error("failed to define %s operator with key ID #%" PRIu32
                  ": key (%zu) must have either the same dimensions as or 1 "
                  "less than query (%zu)",
                  xnn_node_type_to_string(node_type), key_id, key_num_dims,
                  query_num_dims);
    return xnn_status_invalid_parameter;
  }
  ...
  status = xnn_subgraph_check_batch_dims_match(node_type, query_id, query,
                                               key_id, key, num_batch_dims);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (!is_multi_query) {
    ...
    if (key->shape.dim[key_heads_dim] != heads) {
      xnn_log_error("failed to define %s operator with key ID #%" PRIu32
                    ": key heads (%zu) must be equal to query heads (%zu)",
                    xnn_node_type_to_string(node_type), key_id,
                    key->shape.dim[key_heads_dim], heads);
      return xnn_status_invalid_parameter;
    }
  }
  ...
  if (key->shape.dim[key_channels_dim] != channels) {
    xnn_log_error("failed to define %s operator with key ID #%" PRIu32
                  ": key channels (%zu) must be equal to query channels "
                  "(%zu)",
                  xnn_node_type_to_string(node_type), key_id,
                  key->shape.dim[key_channels_dim], channels);
    return xnn_status_invalid_parameter;
  }
  ...
  if (value_num_dims != key_num_dims) {
    xnn_log_error("failed to define %s operator with value ID #%" PRIu32
                  ": value (%zu) must have the same dimensions as key (%zu)",
                  xnn_node_type_to_string(node_type), value_id, value_num_dims,
                  key_num_dims);
    return xnn_status_invalid_parameter;
  }
  ...
  if (!is_multi_query) {
    ...
    if (value->shape.dim[value_heads_dim] != heads) {
      xnn_log_error("failed to define %s operator with value ID #%" PRIu32
                    ": value heads (%zu) must be equal to query heads (%zu)",
                    xnn_node_type_to_string(node_type), value_id,
                    value->shape.dim[value_heads_dim], heads);
      return xnn_status_invalid_parameter;
    }
  }
  ...
  if (key_tokens != value_tokens) {
    xnn_log_error("failed to define %s operator with key ID #%" PRIu32
                  ...
                  ": key tokens (%zu) must be equal "
                  "to value tokens (%zu)",
                  xnn_node_type_to_string(node_type), key_id, value_id,
                  key_tokens, value_tokens);
    return xnn_status_invalid_parameter;
  }
  ...
  if (scale->shape.num_dims != 1) {
    xnn_log_error("failed to define %s operator with scale ID #%" PRIu32
                  ": scale must have only 1 dimension, found %zu",
                  xnn_node_type_to_string(node_type), scale_id,
                  scale->shape.num_dims);
    return xnn_status_invalid_parameter;
  }
  ...
  if (mask->shape.num_dims != 2) {
    xnn_log_error("failed to define %s operator with mask ID #%" PRIu32
                  ": mask must have only 2 dimension, found %zu",
                  xnn_node_type_to_string(node_type), mask_id,
                  mask->shape.num_dims);
    return xnn_status_invalid_parameter;
  }
  ...
  if (mask->shape.dim[0] != query_tokens) {
    xnn_log_error("failed to define %s operator with mask ID #%" PRIu32
                  ": mask query tokens (%zu) must match query (%zu)",
                  xnn_node_type_to_string(node_type), mask_id,
                  mask->shape.dim[0], query_tokens);
    return xnn_status_invalid_parameter;
  }
  ...
  if (mask->shape.dim[1] != key_tokens) {
    xnn_log_error("failed to define %s operator with mask ID #%" PRIu32
                  ": mask key/value tokens (%zu) must match key/value (%zu)",
                  xnn_node_type_to_string(node_type), mask_id,
                  mask->shape.dim[1], key_tokens);
    return xnn_status_invalid_parameter;
  }

  status = xnn_subgraph_check_output_node_id(node_type, output_id,
                                             subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (output_num_dims != query_num_dims) {
    xnn_log_error("failed to define %s operator with output ID #%" PRIu32
                  ": output (%zu) must have the same dimensions as query "
                  "(%zu)",
                  xnn_node_type_to_string(node_type), output_id,
                  output_num_dims, query_num_dims);
    return xnn_status_invalid_parameter;
  }

  // Output batch size must match query.
  status = xnn_subgraph_check_batch_dims_match(
      node_type, query_id, query, output_id, output, num_batch_dims);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (output_heads != heads) {
    xnn_log_error("failed to define %s operator with output ID #%" PRIu32
                  ": output heads (%zu) must be equal to query heads (%zu)",
                  xnn_node_type_to_string(node_type), output_id, output_heads,
                  heads);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_tokens != query_tokens) {
    xnn_log_error("failed to define %s operator with output ID #%" PRIu32
                  ": output tokens (%zu) must match query (%zu)",
                  xnn_node_type_to_string(node_type), output_id, output_tokens,
                  query_tokens);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_channels != value_channels) {
    xnn_log_error("failed to define %s operator with output ID #%" PRIu32
                  ": output channels (%zu) must be equal to value channels "
                  "(%zu)",
                  xnn_node_type_to_string(node_type), output_id,
                  output_channels, value_channels);
    return xnn_status_invalid_parameter;
  ...
  if (cap_type == xnn_attention_logits_cap_type_tanh) {
    memcpy(&node->params.scaled_dot_product_attention.cap_tanh_params,
           cap_params, sizeof(struct xnn_attention_logits_cap_tanh_params));
  }
  node->num_inputs = 5;
