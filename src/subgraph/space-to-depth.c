  ...
  const struct xnn_caches* caches)
{
  ...
  assert(values[input_id].shape.num_dims == 4);
  assert(values[output_id].shape.num_dims == 4);
...
{
  enum xnn_status status;
  status = xnn_subgraph_check_xnnpack_initialized(xnn_node_type_space_to_depth);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_input_node_id(xnn_node_type_space_to_depth, input_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
