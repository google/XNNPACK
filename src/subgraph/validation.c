...
enum xnn_status xnn_subgraph_check_xnnpack_initialized(
    enum xnn_node_type node_type)
{
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
        "failed to define %s operator: XNNPACK is not initialized",
...
enum xnn_status xnn_subgraph_check_input_node_id(
    enum xnn_node_type node_type, uint32_t input_id, size_t num_values)
{
    ...
    size_t nth)
{
  if (input_value == NULL) {
    xnn_log_error(
        "failed to define %s operator with %zu input ID #%" PRIu32
        ": input value is NULL",
        xnn_node_type_to_string(node_type), nth, input_id);
    return xnn_status_invalid_parameter;
  }
...
enum xnn_status xnn_subgraph_check_output_node_id(
    enum xnn_node_type node_type, uint32_t output_id, size_t num_values)
{
    ...
    const struct xnn_value* output_value)
{
  if (input_value == NULL || output_value == NULL) {
    xnn_log_error(
        "failed to define %s operator: Value pointer is NULL",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  if (input_value->datatype == xnn_datatype_invalid ||
      output_value->datatype == xnn_datatype_invalid) {
    xnn_log_error(
        "failed to define %s operator: Value has invalid datatype",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
...
enum xnn_status xnn_subgraph_check_datatype_matches_two_inputs(
    ...
    const struct xnn_value* output_value)
{
  if (input1_value == NULL || input2_value == NULL || output_value == NULL) {
    xnn_log_error(
        "failed to define %s operator: Value pointer is NULL",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  if (input1_value->datatype == xnn_datatype_invalid ||
      input2_value->datatype == xnn_datatype_invalid ||
      output_value->datatype == xnn_datatype_invalid) {
    xnn_log_error(
        "failed to define %s operator: Value has invalid datatype",
        xnn_node_type_to_string(node_type));
  ...
  const enum xnn_datatype input1_datatype =
      xnn_subgraph_effective_datatype(input1_value);
  const enum xnn_datatype input2_datatype =
      xnn_subgraph_effective_datatype(input2_value);
  const enum xnn_datatype output_datatype =
      xnn_subgraph_effective_datatype(output_value);
  if (input1_datatype != input2_datatype ||
      input1_datatype != output_datatype) {
    xnn_log_error(
        "failed to define %s operator with input IDs #%" PRIu32
...
enum xnn_status xnn_subgraph_check_output_min_max(
    enum xnn_node_type node_type, float output_min, float output_max)
{
  if (isnan(output_min)) {
    xnn_log_error(
        "failed to define %s operator with NaN output lower bound: lower "
        "bound must be non-NaN",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  ...
  if (isnan(output_max)) {
    xnn_log_error(
        "failed to define %s operator with NaN output upper bound: upper "
        "bound must be non-NaN",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_min > output_max) {
    xnn_log_error(
        "failed to define %s operator with [%.7g, %.7g] output range: "
    ...
    const struct xnn_value* output_value)
{
  ...
  if (output_value->datatype == xnn_datatype_qint8 ||
    ...
    if (isnan(input_value->quantization.scale) ||
        isnan(output_value->quantization.scale) ||
        input_value->quantization.scale <= 0.0f ||
        output_value->quantization.scale <= 0.0f) {
      xnn_log_error(
          "failed to define %s operator: quantization scale must be "
          "positive non-NaN",
          xnn_node_type_to_string(node_type));
      return xnn_status_invalid_parameter;
    }
    if (input_value->quantization.zero_point !=
...
{
  if (tensor1_value == NULL || tensor2_value == NULL) {
    xnn_log_error(
        "failed to define %s operator: Value pointer is NULL",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  ...
  if (num_batch_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to define %s operator: number of batch dimensions (%zu) "
        "must be at most %d",
        xnn_node_type_to_string(node_type), num_batch_dims,
        XNN_MAX_TENSOR_DIMS);
    return xnn_status_invalid_parameter;
  }

  if (tensor1_value->shape.num_dims > XNN_MAX_TENSOR_DIMS ||
      tensor2_value->shape.num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
        "failed to define %s operator: tensor dimensions exceed maximum (%d)",
        xnn_node_type_to_string(node_type), XNN_MAX_TENSOR_DIMS);
    return xnn_status_invalid_parameter;
  }

  if (tensor1_value->shape.num_dims < num_batch_dims) {
    xnn_log_error(
        "failed to define %s operator with value ID #%" PRIu32
  ...
  for (size_t i = 0; i < num_batch_dims; i++) {
    if (tensor1_value->shape.dim[i] != tensor2_value->shape.dim[i]) {
      xnn_log_error(
          "failed to define %s operator with value IDs #%" PRIu32
          " and #%" PRIu32
