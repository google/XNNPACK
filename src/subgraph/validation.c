...
enum xnn_status xnn_subgraph_check_input_node_id(enum xnn_node_type node_type,
                                                 uint32_t input_id,
                                                 size_t num_values) {
...
enum xnn_status xnn_subgraph_check_nth_input_node_id(
    ...
    size_t nth) {
  if (input_id >= num_values) {
    xnn_log_error("failed to define %s operator with the input %zu ID #%" PRIu32
                  ": invalid Value ID",
...
enum xnn_status xnn_subgraph_check_input_type_dense(
    enum xnn_node_type node_type, uint32_t input_id,
...
enum xnn_status xnn_subgraph_check_nth_input_type_dense(
    enum xnn_node_type node_type, uint32_t input_id,
    const struct xnn_value* input_value, size_t nth) {
  if (input_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error("failed to define %s operator with %zu input ID #%" PRIu32
                  ": unsupported Value type %d (expected dense tensor)",
                  xnn_node_type_to_string(node_type), nth, input_id,
                  input_value->type);
    return xnn_status_invalid_parameter;
...
enum xnn_status xnn_subgraph_check_output_node_id(enum xnn_node_type node_type,
                                                  uint32_t output_id,
                                                  size_t num_values) {
...
enum xnn_status xnn_subgraph_check_output_type_dense(
    enum xnn_node_type node_type, uint32_t output_id,
...
enum xnn_status xnn_subgraph_check_datatype_matches(
    enum xnn_node_type node_type, uint32_t input_id,
    const struct xnn_value* input_value, uint32_t output_id,
    const struct xnn_value* output_value) {
...
enum xnn_status xnn_subgraph_check_datatype_matches_two_inputs(
    enum xnn_node_type node_type, uint32_t input1_id,
    const struct xnn_value* input1_value, uint32_t input2_id,
    const struct xnn_value* input2_value, uint32_t output_id,
    const struct xnn_value* output_value) {
...
enum xnn_status xnn_subgraph_check_output_min_max(enum xnn_node_type node_type,
                                                  float output_min,
                                                  float output_max) {
  if (isnan(output_min)) {
    xnn_log_error(
        "failed to define %s operator with NaN output lower bound: lower bound "
        "must be non-NaN",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  ...
  if (isnan(output_max)) {
    xnn_log_error(
        "failed to define %s operator with NaN output upper bound: upper bound "
        "must be non-NaN",
        xnn_node_type_to_string(node_type));
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_min > output_max) {
    xnn_log_error(
        "failed to define %s operator with [%.7g, %.7g] output range: lower "
...
enum xnn_status xnn_subgraph_check_batch_dims_match(
    enum xnn_node_type node_type, uint32_t tensor1_id,
    const struct xnn_value* tensor1_value, uint32_t tensor2_id,
    const struct xnn_value* tensor2_value, size_t num_batch_dims) {
  if (tensor1_value->shape.num_dims < num_batch_dims) {
    xnn_log_error("failed to define %s operator with value ID #%" PRIu32
                  ": number of dimensions of value (%zu) must "
                  "be at least %zu",
