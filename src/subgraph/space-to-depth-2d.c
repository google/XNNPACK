  ...
  switch (xnn_datatype_size_bytes(input_value->datatype)) {
    case 1:
      status = xnn_create_space_to_depth_nhwc_x8(
          node->params.space_to_depth_2d.block_size, node->flags,
...
static enum xnn_status setup_space_to_depth_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_space_to_depth_nhwc_x16:
      return xnn_setup_space_to_depth_nhwc_x16(opdata->operator_objects[0],
                                               input_data, output_data);
    case xnn_operator_type_space_to_depth_nhwc_x32:
      return xnn_setup_space_to_depth_nhwc_x32(opdata->operator_objects[0],
                                               input_data, output_data);
    case xnn_operator_type_space_to_depth_nhwc_x8:
      return xnn_setup_space_to_depth_nhwc_x8(opdata->operator_objects[0],
                                              input_data, output_data);
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status xnn_define_space_to_depth_2d(xnn_subgraph_t subgraph,
  ...
  enum xnn_status status;
  status =
      xnn_subgraph_check_xnnpack_initialized(xnn_node_type_space_to_depth_2d);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_input_node_id(xnn_node_type_space_to_depth_2d,
                                            input_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_space_to_depth_2d,
  ...
  if (!xnn_datatype_is_byte_addressable(input_value->datatype)) {
    xnn_log_error("failed to define %s operator with input ID #%" PRIu32
                  ...
                  xnn_node_type_to_string(xnn_node_type_space_to_depth_2d),
                  input_id, xnn_datatype_to_string(input_value->datatype),
                  input_value->datatype);
    return xnn_status_invalid_parameter;
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_space_to_depth_2d,
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_space_to_depth_2d,
  ...
  status = xnn_subgraph_check_datatype_matches(xnn_node_type_space_to_depth_2d,
  ...
  status = xnn_subgraph_check_quantization_parameter_matches(
      xnn_node_type_space_to_depth_2d, input_id, input_value, output_id,
      output_value);
  if (status != xnn_status_success) {
    return status;
  }
