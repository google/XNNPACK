...
static enum xnn_status create_even_split_operator_helper(
    const uint32_t output_id, const struct xnn_node* node,
    struct xnn_operator_data* opdata, const enum xnn_datatype datatype,
    size_t index) {
  if (output_id == XNN_INVALID_VALUE_ID) {
    // Node's output value has been optimized away, don't even create operator
    // object.
  ...
  switch (xnn_datatype_size_bits(datatype)) {
    case 8:
      return xnn_create_copy_nc_x8(node->flags,
                                   &opdata->operator_objects[index]);
    case 16:
      return xnn_create_copy_nc_x16(node->flags,
                                    &opdata->operator_objects[index]);
    case 32:
      return xnn_create_copy_nc_x32(node->flags,
                                    &opdata->operator_objects[index]);
    default:
      XNN_UNREACHABLE;
  }
...
static enum xnn_status create_even_split_n_operator(
    ...
    struct xnn_code_cache* code_cache, size_t num_splits,
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    status = create_even_split_operator_helper(opdata->outputs[i], node, opdata,
                                               datatype, operator_index);
    ++operator_index;
...
static enum xnn_status create_even_split2_operator(
  ...
  return create_even_split_n_operator(node, values, num_values, opdata,
                                      code_cache, /*num_splits=*/2,
                                      weights_cache);
}
...
static enum xnn_status create_even_split4_operator(
    ...
    struct xnn_code_cache* code_cache, xnn_weights_cache_t weights_cache) {
  return create_even_split_n_operator(node, values, num_values, opdata,
                                      code_cache, /*num_splits=*/4,
                                      weights_cache);
}

static enum xnn_status reshape_even_split_operator_helper(
    const struct xnn_value* values, const uint32_t num_values,
    struct xnn_operator_data* opdata, size_t operator_index,
    size_t output_index, size_t num_splits, int32_t axis,
  ...
  switch (opdata->operator_objects[operator_index]->type) {
    case xnn_operator_type_copy_nc_x16:
      return xnn_reshape_copy_nc_x16(opdata->operator_objects[operator_index],
                                     opdata->batch_size, channels, input_stride,
                                     output_stride, threadpool);
    case xnn_operator_type_copy_nc_x32:
      return xnn_reshape_copy_nc_x32(opdata->operator_objects[operator_index],
                                     opdata->batch_size, channels, input_stride,
                                     output_stride, threadpool);
    case xnn_operator_type_copy_nc_x8:
      return xnn_reshape_copy_nc_x8(opdata->operator_objects[operator_index],
                                    opdata->batch_size, channels, input_stride,
                                    output_stride, threadpool);
    default:
      XNN_UNREACHABLE;
...
static enum xnn_status reshape_even_split_n_operator(
    ...
    size_t num_values, size_t num_splits, pthreadpool_t threadpool) {
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    status = reshape_even_split_operator_helper(values, num_values, opdata,
                                                operator_index, i, num_splits,
                                                axis, threadpool);
    ++operator_index;
    if (status != xnn_status_success) {
      return status;
...
static enum xnn_status reshape_even_split2_operator(
  ...
  return reshape_even_split_n_operator(opdata, values, num_values,
                                       /*num_splits=*/2, threadpool);
}
...
static enum xnn_status reshape_even_split4_operator(
    ...
    size_t num_values, pthreadpool_t threadpool) {
  return reshape_even_split_n_operator(opdata, values, num_values,
                                       /*num_splits=*/4, threadpool);
}

static enum xnn_status setup_even_split_operator_helper(
    const struct xnn_value* values, const uint32_t num_values,
    const struct xnn_operator_data* opdata, size_t output_index,
    size_t operator_index, const void* input_data, pthreadpool_t threadpool) {
  const uint32_t output_id = opdata->outputs[output_index];
  assert(output_id != XNN_INVALID_VALUE_ID);
  assert(output_id < num_values);
  if (values[output_id].allocation_type == xnn_allocation_type_invalid) {
  ...
  switch (opdata->operator_objects[operator_index]->type) {
    case xnn_operator_type_copy_nc_x16:
      return xnn_setup_copy_nc_x16(
          ...
          (const uint16_t*)input_data + output_index * channels, output_data);
    case xnn_operator_type_copy_nc_x32:
      return xnn_setup_copy_nc_x32(
          opdata->operator_objects[operator_index],
          (const uint32_t*)input_data + output_index * channels, output_data);
    case xnn_operator_type_copy_nc_x8:
      return xnn_setup_copy_nc_x8(
          opdata->operator_objects[operator_index],
          (const uint8_t*)input_data + output_index * channels, output_data);
    default:
      XNN_UNREACHABLE;
  }
}
...
static enum xnn_status setup_even_split_n_operator(
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    const uint32_t output_id = opdata->outputs[i];
    if (values[output_id].type == xnn_value_type_invalid) continue;
    status = setup_even_split_operator_helper(
        values, num_values, opdata, i, operator_index, input_data, threadpool);
    ++operator_index;
    if (status != xnn_status_success) {
      return status;
    }
...
static enum xnn_status setup_even_split2_operator(
    const struct xnn_operator_data* opdata, const struct xnn_value* values,
    size_t num_values, pthreadpool_t threadpool) {
  return setup_even_split_n_operator(opdata, values, num_values,
                                     /*num_splits=*/2, threadpool);
  ;
}
...
enum xnn_status check_output_value(xnn_subgraph_t subgraph, int32_t split_dim,
  ...
  status =
      xnn_subgraph_check_output_type_dense(node_type, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_datatype_matches(node_type, input_id, input_value,
...
static enum xnn_status check_datatype_copyable(xnn_subgraph_t subgraph,
                                               uint32_t input_id,
                                               uint32_t output_id,
                                               const char* nth,
                                               enum xnn_node_type node_type) {
  ...
  enum xnn_status status = xnn_subgraph_check_datatype_matches(
      node_type, input_id, input_value, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
  return xnn_subgraph_check_quantization_parameter_matches(
      node_type, input_id, input_value, output_id, output_value);
}

enum xnn_status xnn_define_even_split_n(enum xnn_node_type node_type,
  ...
  if ((status = xnn_subgraph_check_input_node_id(
           node_type, input_id, subgraph->num_values)) != xnn_status_success) {
    return status;
  }
  ...
  status = check_output_value(subgraph, split_dim, input_id, output_ids[0],
                              "first", node_type);
  if (status != xnn_status_success) {
    return status;
  }
  status = check_output_value(subgraph, split_dim, input_id, output_ids[1],
                              "second", node_type);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (num_outputs > 2) {
    status = check_output_value(subgraph, split_dim, input_id, output_ids[2],
                                "third", node_type);
    if (status != xnn_status_success) {
      return status;
    }
  ...
  if (num_outputs > 3) {
    status = check_output_value(subgraph, split_dim, input_id, output_ids[3],
                                "fourth", node_type);
    if (status != xnn_status_success) {
      return status;
    }
  ...
  check_datatype_copyable(subgraph, input_id, output_ids[0], "first",
                          node_type);
  check_datatype_copyable(subgraph, input_id, output_ids[1], "second",
                          node_type);
  if (num_outputs > 2) {
    check_datatype_copyable(subgraph, input_id, output_ids[2], "third",
                            node_type);
  }
  if (num_outputs > 3) {
    check_datatype_copyable(subgraph, input_id, output_ids[3], "fourth",
                            node_type);
  }
...
enum xnn_status xnn_define_even_split2(xnn_subgraph_t subgraph,
  ...
  return xnn_define_even_split_n(xnn_node_type_even_split2, subgraph, split_dim,
                                 input_id, XNN_COUNT_OF(output_ids), output_ids,
                                 flags);
}
...
enum xnn_status xnn_define_even_split3(xnn_subgraph_t subgraph,
  ...
  return xnn_define_even_split_n(xnn_node_type_even_split3, subgraph, split_dim,
                                 input_id, XNN_COUNT_OF(output_ids), output_ids,
                                 flags);
}
...
enum xnn_status xnn_define_even_split4(xnn_subgraph_t subgraph,
                                       ...
                                       uint32_t flags) {
  const uint32_t output_ids[4] = {output1_id, output2_id, output3_id,
                                  output4_id};
  return xnn_define_even_split_n(xnn_node_type_even_split4, subgraph, split_dim,
                                 input_id, XNN_COUNT_OF(output_ids), output_ids,
                                 flags);
}
