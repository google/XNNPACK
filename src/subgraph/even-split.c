...
enum xnn_status xnn_define_even_split(xnn_subgraph_t subgraph,
  ...
  for (int i = 0; i < num_outputs; ++i) {
    status = check_output_value(subgraph, split_dim, input_id, output_ids[i],
                                "Nth", node_type);
    if (status != xnn_status_success) {
      return status;
    }
  ...
  if (num_outputs > XNN_MAX_OUTPUTS) {
    xnn_log_error(
        "failed to define %s operator with %zu inputs: number of inputs (%zu) "
        "exceeds the supported maximum (%zu)",
        xnn_node_type_to_string(node_type), num_outputs, num_outputs,
        (size_t)XNN_MAX_OUTPUTS);
    return xnn_status_invalid_parameter;
  }

  for (int i = 0; i < num_outputs; ++i) {
    check_datatype_copyable(subgraph, input_id, output_ids[i], "Nth",
                            node_type);
  }
