...
static enum xnn_status create_slice_operator(
  ...
  switch (xnn_datatype_size_bits(input_value->datatype)) {
    case 8:
      status =
          xnn_create_slice_nd_x8(/*flags=*/0, &opdata->operator_objects[0]);
      break;
    case 16:
      status =
          xnn_create_slice_nd_x16(/*flags=*/0, &opdata->operator_objects[0]);
      break;
    case 32:
      status =
          xnn_create_slice_nd_x32(/*flags=*/0, &opdata->operator_objects[0]);
      break;
    default:
      XNN_UNREACHABLE;
  }
  ...
  if (status == xnn_status_success) {
    ...
    memcpy(opdata->begins, node->params.slice.begins,
           num_dims * sizeof(int64_t));
...
static enum xnn_status reshape_slice_operator(struct xnn_operator_data* opdata,
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_slice_nd_x8:
      status = xnn_reshape_slice_nd_x8(opdata->operator_objects[0], num_dims,
                                       input_value->shape.dim, offsets, sizes,
                                       threadpool);
      break;
    case xnn_operator_type_slice_nd_x16:
      status = xnn_reshape_slice_nd_x16(opdata->operator_objects[0], num_dims,
                                        input_value->shape.dim, offsets, sizes,
                                        threadpool);
      break;
    case xnn_operator_type_slice_nd_x32:
      status = xnn_reshape_slice_nd_x32(opdata->operator_objects[0], num_dims,
                                        input_value->shape.dim, offsets, sizes,
                                        threadpool);
      break;
    default:
...
static enum xnn_status setup_slice_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_slice_nd_x8:
      return xnn_setup_slice_nd_x8(opdata->operator_objects[0], input_data,
                                   output_data);
      break;
    case xnn_operator_type_slice_nd_x16:
      return xnn_setup_slice_nd_x16(opdata->operator_objects[0], input_data,
                                    output_data);
      break;
    case xnn_operator_type_slice_nd_x32:
      return xnn_setup_slice_nd_x32(opdata->operator_objects[0], input_data,
                                    output_data);
      break;
    default:
      XNN_UNREACHABLE;
...
enum xnn_status xnn_define_static_slice_v3(
    xnn_subgraph_t subgraph, size_t num_dims, const int64_t* begins,
    const int64_t* ends, const int64_t* strides, uint32_t input_id,
    uint32_t output_id, uint32_t flags) {
  enum xnn_status status =
      xnn_subgraph_check_xnnpack_initialized(xnn_node_type_static_slice);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_input_node_id(xnn_node_type_static_slice,
  ...
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_static_slice,
  ...
  if (strides != NULL) {
    for (size_t i = 0; i < num_dims; i++) {
      if (strides[i] != 1) {
        xnn_log_error("failed to define %s operator with input ID #%" PRIu32
                      ": Illegal stride value %" PRIi64 " in dimension #%zu",
                      xnn_node_type_to_string(xnn_node_type_static_slice),
                      input_id, strides[i], i);
        return xnn_status_invalid_parameter;
      }
    }
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_static_slice,
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_static_slice,
  ...
  status =
      xnn_subgraph_check_datatype_matches(xnn_node_type_static_slice, input_id,
                                          input_value, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_quantization_parameter_matches(
      xnn_node_type_static_slice, input_id, input_value, output_id,
      output_value);
  if (status != xnn_status_success) {
    return status;
  }
...
enum xnn_status xnn_define_static_slice_v2(xnn_subgraph_t subgraph,
  ...
  return xnn_define_static_slice_v3(subgraph, num_dims, offsets, ends,
                                    /*strides*/ NULL, input_id, output_id,
                                    flags);
}
