...
static enum xnn_status create_unpooling_operator(
  ...
  const enum xnn_status status = xnn_create_unpooling2d_nhwc_x32(
      ...
      node->params.pooling_2d.pooling_width, channel_dim /* channels */,
...
static enum xnn_status setup_unpooling_operator(
  ...
  return xnn_setup_unpooling2d_nhwc_x32(opdata->operator_objects[0],
                                        input_value_data, input_index_data,
                                        output_data);
}
...
enum xnn_status xnn_define_unpooling_2d(
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_unpooling_2d)) != xnn_status_success) {
    return status;
  }
  ...
  if (pooling_size == 1) {
    xnn_log_error(
        "failed to define %s operator with 1 pooling element: 1x1 pooling is "
        "meaningless",
        xnn_node_type_to_string(xnn_node_type_unpooling_2d));
    return xnn_status_invalid_parameter;
  }
  ...
  if (input_value_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error("failed to define %s operator with input value ID #%" PRIu32
                  ": unsupported Value type %d (expected dense tensor)",
                  xnn_node_type_to_string(xnn_node_type_unpooling_2d),
                  input_value_id, input_value_value->type);
    return xnn_status_invalid_parameter;
  }
  ...
  switch (input_value_value->datatype) {
    ...
    default:
      xnn_log_error("failed to define %s operator with input value ID #%" PRIu32
                    ...
                    xnn_node_type_to_string(xnn_node_type_unpooling_2d),
                    input_value_id,
                    xnn_datatype_to_string(input_value_value->datatype),
                    input_value_value->datatype);
      return xnn_status_invalid_parameter;
  }

  if (input_index_id >= subgraph->num_values) {
    xnn_log_error("failed to define %s operator with input index ID #%" PRIu32
                  ": invalid Value ID",
                  xnn_node_type_to_string(xnn_node_type_unpooling_2d),
                  input_index_id);
    return xnn_status_invalid_parameter;
  ...
  if (input_index_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error("failed to define %s operator with input index ID #%" PRIu32
                  ": unsupported Value type %d (expected dense tensor)",
                  xnn_node_type_to_string(xnn_node_type_unpooling_2d),
                  input_index_id, input_index_value->type);
    return xnn_status_invalid_parameter;
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_unpooling_2d,
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_unpooling_2d,
