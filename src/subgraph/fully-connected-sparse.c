...
static enum xnn_status reshape_fully_connected_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_convolution_nchw_f16:
      status = xnn_reshape_convolution2d_nchw_f16(opdata->operator_objects[0],
                                                  batch_size, 1, 1, NULL, NULL,
                                                  threadpool);
      break;
    case xnn_operator_type_convolution_nchw_f32:
      status = xnn_reshape_convolution2d_nchw_f32(opdata->operator_objects[0],
                                                  batch_size, 1, 1, NULL, NULL,
                                                  threadpool);
      break;
    default:
...
enum xnn_status xnn_define_fully_connected_sparse(
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_fully_connected_sparse)) != xnn_status_success) {
    return status;
  }

  status = xnn_subgraph_check_output_min_max(
      xnn_node_type_fully_connected_sparse, output_min, output_max);
  if (status != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_fully_connected_sparse, input_id,
  ...
  status = xnn_subgraph_check_input_type_dense(
      xnn_node_type_fully_connected_sparse, input_id, input_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_node_id(
      xnn_node_type_fully_connected_sparse, output_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_type_dense(
      xnn_node_type_fully_connected_sparse, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
