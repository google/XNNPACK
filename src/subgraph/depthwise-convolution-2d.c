    ...
    switch (filter_datatype) {
      case xnn_datatype_fp32:
        status = xnn_create_convolution2d_nchw_f32(
            ...
            node->flags | XNN_FLAG_DEPTHWISE_CONVOLUTION, code_cache,
    ...
    switch (filter_datatype) {
        ...
        switch (output_datatype) {
          case xnn_datatype_fp32:
            status = xnn_create_convolution2d_nhwc_f32_f16(
                ...
                filter_data, bias_data, node->activation.output_min,
                node->activation.output_max,
                node->flags | XNN_FLAG_DEPTHWISE_CONVOLUTION, NULL, NULL,
                &opdata->operator_objects[0]);
            break;
          case xnn_datatype_fp16:
        ...
        switch (output_datatype) {
          ...
          case xnn_datatype_fp16:
            status = xnn_create_convolution2d_nhwc_f16(
                ...
                NULL, NULL, &opdata->operator_objects[0]);
            break;
          default:
            XNN_UNREACHABLE;
        }
      ...
      case xnn_datatype_qcint8: {
        ...
        status = xnn_create_convolution2d_nhwc_qs8_qc8w(
            ...
            output_max, node->flags | XNN_FLAG_DEPTHWISE_CONVOLUTION, NULL,
            NULL, &opdata->operator_objects[0]);
        break;
      }
  ...
  switch (opdata->operator_objects[0]->type) {
    ...
    case xnn_operator_type_convolution_nhwc_f32:
      status = xnn_reshape_convolution2d_nhwc_f32(
          opdata->operator_objects[0], batch_size, input_height, input_width,
          &opdata->workspace_size, &opdata->workspace_alignment, &output_height,
...
static enum xnn_status setup_depthwise_convolution_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_convolution_nchw_f16:
      return xnn_setup_convolution2d_nchw_f16(opdata->operator_objects[0],
    ...
    case xnn_operator_type_convolution_nhwc_qc8:
      ...
      break;
    case xnn_operator_type_convolution_nhwc_qs8:
      return xnn_setup_convolution2d_nhwc_qs8(opdata->operator_objects[0],
                                              opdata->workspace, input_data,
                                              output_data);
      break;
    case xnn_operator_type_convolution_nhwc_qu8:
      return xnn_setup_convolution2d_nhwc_qu8(opdata->operator_objects[0],
                                              opdata->workspace, input_data,
                                              output_data);
      break;
    default:
...
enum xnn_status xnn_define_depthwise_convolution_2d(
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_depthwise_convolution_2d)) != xnn_status_success) {
    return status;
  }

  if (kernel_width == 0 || kernel_height == 0) {
    xnn_log_error(
        "failed to define %s operator with %" PRIu32 "x%" PRIu32
  ...
  if (depth_multiplier == 0) {
    xnn_log_error(
        "failed to define %s operator with %" PRIu32
        " depth multiplier: depth multiplier must be non-zero",
        xnn_node_type_to_string(xnn_node_type_depthwise_convolution_2d),
        depth_multiplier);
    return xnn_status_invalid_parameter;
  }
  ...
  if (input_channels == 0) {
    xnn_log_error(
        "failed to define %s operator with %zu input channels: number of "
  ...
  status = xnn_subgraph_check_output_min_max(
      xnn_node_type_depthwise_convolution_2d, output_min, output_max);
  if (status != xnn_status_success) {
    return status;
  }

  const uint32_t supported_flags =
      XNN_FLAG_TENSORFLOW_SAME_PADDING | XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER;
  const uint32_t invalid_flags = flags & ~supported_flags;
  if (invalid_flags != 0) {
    xnn_log_error(
        "failed to define %s operator with 0x%08" PRIx32
        " flags: invalid flags 0x%08" PRIx32,
        xnn_node_type_to_string(xnn_node_type_depthwise_convolution_2d), flags,
        invalid_flags);
    return xnn_status_invalid_parameter;
  }
  ...
  // Convert TensorFlow SAME padding to explicit padding specification whenever
  // possible
  if ((flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) != 0 &&
      (subsampling_height | subsampling_width) == 1) {
  ...
  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_depthwise_convolution_2d, input_id,
           subgraph->num_values)) != xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(
      xnn_node_type_depthwise_convolution_2d, input_id, input_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (filter_value->datatype) {
    ...
    case xnn_datatype_qint8:
      if (filter_value->quantization.zero_point != 0) {
        xnn_log_error(
            "failed to define %s operator with filter ID #%" PRIu32
            ": unsupported quantization zero point %" PRId32 " for datatype %s",
            xnn_node_type_to_string(xnn_node_type_depthwise_convolution_2d),
  ...
  status = xnn_subgraph_check_output_node_id(
      xnn_node_type_depthwise_convolution_2d, output_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_type_dense(
      xnn_node_type_depthwise_convolution_2d, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (filter_value->datatype == xnn_datatype_qcint8) {
    if (filter_value->quantization.channel_dimension !=
        filter_value->shape.num_dims - 1) {
