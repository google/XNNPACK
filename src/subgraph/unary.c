...
static enum xnn_status create_convert_operator(
  ...
  switch (input_datatype) {
    case xnn_datatype_fp32:
      switch (output_datatype) {
        case xnn_datatype_qdint8:
          status = xnn_create_convert_nc_f32_qd8(node->flags,
                                                 &opdata->operator_objects[0]);
          break;
        case xnn_datatype_qduint8:
          status = xnn_create_convert_nc_f32_qdu8(node->flags,
                                                  &opdata->operator_objects[0]);
          break;
    ...
    case xnn_datatype_fp16:
      switch (output_datatype) {
        case xnn_datatype_qdint8:
          status = xnn_create_convert_nc_f16_qd8(node->flags,
                                                 &opdata->operator_objects[0]);
          break;
        case xnn_datatype_qduint8:
          status = xnn_create_convert_nc_f16_qdu8(node->flags,
                                                  &opdata->operator_objects[0]);
          break;
        default:
          break;
  ...
  if (status == xnn_status_uninitialized) {
    status = xnn_create_unary_elementwise_nc(
        xnn_unary_convert, input_datatype, output_datatype, NULL, NULL, NULL,
...
static enum xnn_status reshape_convert_operator(
  ...
  const size_t batch_size =
      xnn_shape_multiply_non_channel_dims(&input_value->shape);
  const size_t num_input_dims = input_value->shape.num_dims;
  const size_t channel_dim =
      num_input_dims == 0 ? 1 : input_value->shape.dim[num_input_dims - 1];
  ...
  size_t dq_batch_size =
      xnn_shape_multiply_batch_dims(&input_value->shape, num_nonbatch_dims);
  size_t dq_channel_stride = xnn_shape_multiply_trailing_dims(
...
static enum xnn_status setup_convert_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    ...
    case xnn_operator_type_convert_nc_f32_qd8: {
      void* quantization_params = output_value->quantization.dynamic_params;
      assert(quantization_params != NULL);
      return xnn_setup_convert_nc_f32_qd8(opdata->operator_objects[0],
                                          input_data, output_data,
                                          quantization_params);
    }
    case xnn_operator_type_convert_nc_f16_qdu8: {
    ...
    case xnn_operator_type_convert_nc_f32_qdu8: {
      void* quantization_params = output_value->quantization.dynamic_params;
      assert(quantization_params != NULL);
      return xnn_setup_convert_nc_f32_qdu8(opdata->operator_objects[0],
                                           input_data, output_data,
                                           quantization_params);
    }
    case xnn_operator_type_convert_nc_f32_qp8:
      return xnn_setup_convert_nc_f32_qp8(opdata->operator_objects[0],
                                          input_data, output_data);
    default:
      return xnn_setup_unary_elementwise_nc(opdata->operator_objects[0],
                                            input_data, output_data);
  }
}
...
static enum xnn_status create_unary_operator(
  ...
  return xnn_create_unary_elementwise_nc(
      node->unary_operator, value_in->datatype, value_out->datatype,
      &node->params.unary, &in_quantization, &out_quantization, node->flags,
      &opdata->operator_objects[0]);
}

static enum xnn_status reshape_unary_operator(struct xnn_operator_data* opdata,
  ...
  enum xnn_status status = xnn_reshape_unary_elementwise_nc(
      op, batch_size, channel_dim, channel_dim, channel_dim, threadpool);
  if (status != xnn_status_success) {
    return status;
  }
  return resize_unary_elementwise_output_tensor(opdata, values, num_values,
...
enum xnn_status xnn_define_unary(xnn_subgraph_t subgraph,
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_unary_elementwise)) != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_unary_elementwise, input_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_output_node_id(
           xnn_node_type_unary_elementwise, output_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_unary_elementwise,
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_unary_elementwise,
