...
#define XNN_PRINT_FLAG(flags, flag_name)      \
  if (flags & flag_name) {                    \
    fprintf(out, "%s" #flag_name, separator); \
    separator = "|";                          \
  }
...
void xnn_subgraph_log_impl(const char* filename, size_t line_number,
  ...
  for (int node_id = 0; node_id < subgraph->num_nodes; node_id++) {
    ...
    if (node->flags) {
      fprintf(out, ", flags=");
      char* separator = "";
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_INLINE_LHS_PACKING);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_DEPTHWISE_CONVOLUTION);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TRANSPOSE_WEIGHTS);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TENSORFLOW_SAME_PADDING);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_INPUT_NHWC);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TRANSPOSE_A);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TENSORFLOW_RESHAPE_2D);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TENSORFLOW_LEGACY_MODE);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_FP32_STATIC_WEIGHTS);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_FP32_STATIC_BIASES);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_ALIGN_CORNERS);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_YIELD_WORKERS);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_KEEP_DIMS);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_NO_BROADCAST);
      XNN_PRINT_FLAG(node->flags, XNN_FLAG_SLOW_CONSISTENT_ARITHMETIC);
    }
    fprintf(out, ".\n");
  }
  ...
  for (int value_id = 0; value_id < subgraph->num_values; value_id++) {
    ...
    if (value->flags &
      ...
      fprintf(out, ", flags=");
      char* separator = "";
      XNN_PRINT_FLAG(value->flags, XNN_FLAG_SQUASH_GROUPS);
      XNN_PRINT_FLAG(value->flags, XNN_VALUE_FLAG_ONE_CONSUMER);
      XNN_PRINT_FLAG(value->flags, XNN_VALUE_FLAG_FP16_COMPATIBLE);
      XNN_PRINT_FLAG(value->flags, XNN_VALUE_FLAG_LAYOUT_NCHW);
    }
    fprintf(out, ".\n");
  }
}
