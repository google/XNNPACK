  ...
  for (int node_id = 0; node_id < subgraph->num_nodes; node_id++) {
    ...
    if (node->flags) {
      ...
      XNN_PRINT_FLAGS(
          ...
          XNN_FLAG_TRANSPOSE_A, XNN_FLAG_TENSORFLOW_RESHAPE_2D,
          XNN_FLAG_TENSORFLOW_LEGACY_MODE, XNN_FLAG_FP32_STATIC_WEIGHTS,
          XNN_FLAG_FP32_STATIC_BIASES, XNN_FLAG_ALIGN_CORNERS,
          XNN_FLAG_YIELD_WORKERS, XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER,
          XNN_FLAG_KEEP_DIMS, XNN_FLAG_NO_BROADCAST,
