...
static enum xnn_status create_softmax_operator(
  ...
  switch (input_value->datatype) {
    case xnn_datatype_fp32:
      status =
          xnn_create_softmax_nc_f32(node->flags, &opdata->operator_objects[0]);
      break;
    case xnn_datatype_fp16:
      status =
          xnn_create_softmax_nc_f16(node->flags, &opdata->operator_objects[0]);
      break;
    default:
      XNN_UNREACHABLE;
  }
...
static enum xnn_status reshape_softmax_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_softmax_nc_f32:
      status = xnn_reshape_softmax_nc_f32(opdata->operator_objects[0],
    ...
    case xnn_operator_type_softmax_nc_f16:
      status = xnn_reshape_softmax_nc_f16(opdata->operator_objects[0],
                                          /*channels=*/channel_dim,
                                          /*input_stride=*/channel_dim,
                                          /*output_stride=*/channel_dim,
                                          batch_size, threadpool);
...
static enum xnn_status setup_softmax_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_softmax_nc_f32:
      return xnn_setup_softmax_nc_f32(opdata->operator_objects[0], input_data,
                                      output_data);
    case xnn_operator_type_softmax_nc_f16:
      return xnn_setup_softmax_nc_f16(opdata->operator_objects[0], input_data,
                                      output_data);
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status xnn_define_softmax(xnn_subgraph_t subgraph, uint32_t input_id,
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_softmax)) != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_softmax, input_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_softmax, input_id,
                                               input_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_node_id(xnn_node_type_softmax, output_id,
                                             subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_softmax,
