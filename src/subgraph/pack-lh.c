...
static enum xnn_status create_pack_lh_operator(
  ...
  switch (input_value->datatype) {
    case xnn_datatype_qint8:
      status = xnn_create_pack_lh_x8(node->flags, &opdata->operator_objects[0]);
      break;
    case xnn_datatype_fp16:
      status =
          xnn_create_pack_lh_x16(node->flags, &opdata->operator_objects[0]);
      break;
    case xnn_datatype_fp32:
      status =
          xnn_create_pack_lh_x32(node->flags, &opdata->operator_objects[0]);
      break;
    default:
      XNN_UNREACHABLE;
  }
...
static enum xnn_status reshape_pack_lh_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_pack_lh_x8:
      status = xnn_reshape_pack_lh_x8(opdata->operator_objects[0], num_groups,
                                      batch_size, channels, &output_size_bytes,
                                      threadpool);
      break;
    case xnn_operator_type_pack_lh_x16:
      status = xnn_reshape_pack_lh_x16(opdata->operator_objects[0], num_groups,
                                       batch_size, channels, &output_size_bytes,
                                       threadpool);
      break;
    case xnn_operator_type_pack_lh_x32:
      status = xnn_reshape_pack_lh_x32(opdata->operator_objects[0], num_groups,
                                       batch_size, channels, &output_size_bytes,
                                       threadpool);
      break;
    default:
  ...
  if (output_size_bytes > output_value->size ||
...
static enum xnn_status setup_pack_lh_operator(
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_pack_lh_x32:
      return xnn_setup_pack_lh_x32(opdata->operator_objects[0], input_data,
                                   output_data);
    case xnn_operator_type_pack_lh_x16:
      return xnn_setup_pack_lh_x16(opdata->operator_objects[0], input_data,
                                   output_data);
    case xnn_operator_type_pack_lh_x8:
      return xnn_setup_pack_lh_x8(opdata->operator_objects[0], input_data,
                                  output_data);
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status xnn_define_pack_lh(xnn_subgraph_t subgraph, uint32_t input_id,
  ...
  if ((status = xnn_subgraph_check_xnnpack_initialized(
           xnn_node_type_pack_lh)) != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_input_node_id(
           xnn_node_type_pack_lh, input_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_pack_lh, input_id,
                                               input_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_node_id(xnn_node_type_pack_lh, output_id,
                                             subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_pack_lh,
