...
enum xnn_status resize_unary_elementwise_output_tensor(
  ...
  memcpy(&output->shape.dim[0], &input->shape.dim[0],
         input->shape.num_dims * sizeof(size_t));
  ...
  if (new_size > output->size || opdata->workspace_size > old_workspace_size) {
    ...
    if (output->datatype == xnn_datatype_qdint8 ||
      ...
      // reallocation will use this to adjust memory needed for dynamic quant
      // params
      output->quantization.dynamic_params_size =
          xnn_tensor_get_dynamic_quant_param_size(output);
    }
...
enum xnn_status resize_binary_elementwise_output_tensor(
  ...
  if (dims0 == 0) {
    ...
    memcpy(&output->shape.dim[0], &input1->shape.dim[0],
           sizeof(size_t) * input1->shape.num_dims);
  } else if (dims1 == 0) {
    output->shape.num_dims = input0->shape.num_dims;
    memcpy(&output->shape.dim[0], &input0->shape.dim[0],
           sizeof(size_t) * input0->shape.num_dims);
  } else {
    for (size_t i = 0; i < out_dims; ++i) {
      ...
      if (!(d0 == d1 || d0 == 1 || d1 == 1)) {
        xnn_log_error("Input dimensions %zu and %zu are not broadcastable.", d0,
                      d1);
