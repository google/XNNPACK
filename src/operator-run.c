...
void xnn_compute_transposec_2d(const struct transpose_context* context,
...
void xnn_compute_transposec_3d(const struct transpose_context* context,
...
void xnn_compute_transposec_4d(const struct transpose_context* context,
...
void xnn_compute_transposec_5d(const struct transpose_context* context,
...
void xnn_compute_transposec_6d(const struct transpose_context* context,
                               size_t i, size_t j, size_t k, size_t l, size_t m,
...
void xnn_compute_transposev_2d(const struct transpose_context* context,
                               size_t i, size_t j, size_t tile_i,
                               size_t tile_j) {
...
void xnn_compute_transposev_3d(const struct transpose_context* context,
                               size_t i, size_t j, size_t k, size_t tile_j,
                               size_t tile_k) {
...
void xnn_compute_transposev_4d(const struct transpose_context* context,
                               size_t i, size_t j, size_t k, size_t l,
                               size_t tile_k, size_t tile_l) {
...
void xnn_compute_transposev_5d(const struct transpose_context* context,
                               size_t i, size_t j, size_t k, size_t l, size_t m,
                               size_t tile_l, size_t tile_m) {
...
void xnn_compute_transposev_6d(const struct transpose_context* context,
                               size_t i, size_t j, size_t k, size_t l, size_t m,
                               size_t n, size_t tile_m, size_t tile_n) {
...
void xnn_compute_batched_packw_gemm_gio(
  ...
  while (n_block_size > 0) {
    const size_t n_step = min(n_block_size, context->nr);
    ...
    if (context->pack_weights_and_biases) {
      context->pack_weights_and_biases(
          /*flags=*/XNN_FLAG_TRANSPOSE_WEIGHTS, context->gemm_config,
          context->kc, n_step,
...
void xnn_compute_batched_packw_gemm_goi(
  ...
  while (n_block_size > 0) {
    ...
    if (context->pack_weights_and_biases) {
      context->pack_weights_and_biases(
          /*flags=*/0, context->gemm_config, context->kc, n_step,
...
size_t xnn_x32_pack_lh_offset__neonsme2(size_t m, size_t k, size_t mr,
                                        size_t kr, size_t sr);
...
void xnn_compute_grouped_batch_igemm(
  ...
  context->ukernel.function[XNN_UARCH_DEFAULT](
      mr_block_size, nr_block_size, context->kc, context->ks_scaled,
...
void xnn_compute_dwconv_indirection(const struct dwconv_indirection_init_context
                                        context[restrict XNN_MIN_ELEMENTS(1)],
...
void xnn_compute_dwconv_unipass(
  ...
  context->ukernel(output_c_tile, context->output_width, indirect_input,
                   ...
                   output_increment, input_offset, context->zero,
                   &context->params);
}
...
void xnn_compute_argmax_pooling(
  ...
  context->ukernel(context->output_width, context->pooling_size,
                   context->channels, indirect_input, input_offset, output,
                   index, context->input_increment, context->output_increment);
}
...
void xnn_compute_average_pooling(
  ...
  context->ukernel(
      ...
      context->input_increment, context->output_increment, &context->params);
}
...
void xnn_compute_resize_bilinear_chw(const struct resize_bilinear_chw_context
                                         context[restrict XNN_MIN_ELEMENTS(1)],
                                     size_t batch_index, size_t channel_start,
...
void xnn_compute_scaled_dot_product_attention(
  ...
  const size_t query_tile_offset = batch_index * context->query_batch_stride +
                                   head_index * context->query_head_stride +
                                   tokens_start * query_key_scaled_channels;
  const size_t key_value_tokens_scaled = context->key_value_tokens_scaled;
  const size_t key_value_tokens_start_scaled =
      tokens_start * key_value_tokens_scaled;
  const size_t cn_stride = context->cn_stride;
  const void* scaled_query =
      (void*)((uintptr_t)context->scaled_query + query_tile_offset);
...

  {
    ...
    // Q_scaled = Q * Scale (along channels). Q and Q_scaled have dimensions
    // [tokens_block_size, query_key_channels].
  ...
  const size_t logits_batch_offset =
      batch_index * context->logits_batch_stride +
      head_index * context->logits_head_stride;
  void* const logits =
      (void*)((uintptr_t)context->logits_buffer + logits_batch_offset +
              key_value_tokens_start_scaled);
  ...
  {
    const size_t tokens_block_size_scaled =
        tokens_block_size * key_value_tokens_scaled;
...

  {
    ...
    const size_t output_tile_offset =
        batch_index * context->output_batch_stride +
        head_index * context->output_head_stride +
...
void xnn_compute_scaled_dot_product_attention_with_thread(
  ...
  const size_t cn_stride = context->cn_stride;
  const void* scaled_query =
      (void*)((uintptr_t)context->scaled_query +
              thread_index * context->scaled_query_thread_stride);
  const void* minmax_params = &context->minmax_params;
  ...
  void* const logits = (void*)((uintptr_t)context->logits_buffer +
                               thread_index * context->logits_thread_stride);
...
void xnn_compute_elementwise_binary_2d(
    ...
    size_t i, size_t j) {
  const void* a =
      (const void*)((uintptr_t)context->a + i * context->a_stride[3] +
                    j * context->a_stride[4]);
  const void* b =
      (const void*)((uintptr_t)context->b + i * context->b_stride[3] +
                    j * context->b_stride[4]);
  void* y = (void*)((uintptr_t)context->y + i * context->y_stride[3] +
                    j * context->y_stride[4]);
  context->ukernel(context->elements, a, b, y, &context->params);
}
...
void xnn_compute_elementwise_binary_3d(
  ...
  const void* a =
      (const void*)((uintptr_t)context->a + i * context->a_stride[2] +
                    j * context->a_stride[3] + k * context->a_stride[4]);
  const void* b =
      (const void*)((uintptr_t)context->b + i * context->b_stride[2] +
                    j * context->b_stride[3] + k * context->b_stride[4]);
  void* y = (void*)((uintptr_t)context->y + i * context->y_stride[2] +
                    j * context->y_stride[3] + k * context->y_stride[4]);
  context->ukernel(context->elements, a, b, y, &context->params);
}
...
void xnn_compute_elementwise_binary_4d(
    ...
    size_t i, size_t j, size_t k, size_t l) {
  const void* a =
      (const void*)((uintptr_t)context->a + i * context->a_stride[1] +
                    j * context->a_stride[2] + k * context->a_stride[3] +
                    l * context->a_stride[4]);
  const void* b =
      (const void*)((uintptr_t)context->b + i * context->b_stride[1] +
                    j * context->b_stride[2] + k * context->b_stride[3] +
                    l * context->b_stride[4]);
  void* y = (void*)((uintptr_t)context->y + i * context->y_stride[1] +
                    j * context->y_stride[2] + k * context->y_stride[3] +
                    l * context->y_stride[4]);
  context->ukernel(context->elements, a, b, y, &context->params);
}
...
void xnn_compute_univector_strided(const struct univector_strided_context
                                       context[restrict XNN_MIN_ELEMENTS(1)],
...
void xnn_compute_univector_contiguous(const struct univector_contiguous_context
                                          context[restrict XNN_MIN_ELEMENTS(1)],
                                      size_t offset, size_t size) {
  ...
  for (size_t i = 0; i < input_shape1; ++i) {
    ...
    for (size_t j = 0; j < input_shape3; ++j) {
      ...
      for (size_t k = 0; k < output2_block_size; ++k) {
        ...
        context->ukernel.rsum(context->channels, input_row, output,
                              &context->params);
...
void xnn_compute_discontiguous_reduce(
  ...
  for (size_t i = 0; i < input_shape0; ++i) {
    ...
    for (size_t j = 0; j < input_shape2; ++j) {
      ...
      context->ukernel.rdsum(context->channels, output2_block_size, input_row,
                             input_stride[4], context->zero, output,
                             &context->params);
      // input_stride[4] is the number of bytes of input which have been
      // processed by the microkernel call.
      input_row = (const void*)((uintptr_t)input_row + input_stride[4]);
    ...
    switch (op->compute[i].type) {
      ...
      case xnn_parallelization_type_1d_tile_1d:
        ...
        pthreadpool_parallelize_1d_tile_1d(
            threadpool, op->compute[i].task_1d_tile_1d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
      ...
      case xnn_parallelization_type_2d:
        ...
        pthreadpool_parallelize_2d(
            threadpool, op->compute[i].task_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1], flags);
        break;
      case xnn_parallelization_type_2d_with_thread:
        assert(op->compute[i].range[0] != 0);
        assert(op->compute[i].range[1] != 0);
        pthreadpool_parallelize_2d_with_thread(
            threadpool, op->compute[i].task_2d_with_thread,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1], flags);
        break;
      case xnn_parallelization_type_2d_tile_1d:
        ...
        pthreadpool_parallelize_2d_tile_1d(
            threadpool, op->compute[i].task_2d_tile_1d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
      ...
      case xnn_parallelization_type_2d_tile_2d:
        ...
        pthreadpool_parallelize_2d_tile_2d(
            threadpool, op->compute[i].task_2d_tile_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
      ...
      case xnn_parallelization_type_3d:
        ...
        pthreadpool_parallelize_3d(
            threadpool, op->compute[i].task_3d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], flags);
        break;
      case xnn_parallelization_type_3d_tile_1d:
        ...
        pthreadpool_parallelize_3d_tile_1d(
            threadpool, op->compute[i].task_3d_tile_1d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
      ...
      case xnn_parallelization_type_3d_tile_1d_with_thread:
        ...
        pthreadpool_parallelize_3d_tile_1d_with_thread(
            threadpool, op->compute[i].task_3d_tile_1d_with_thread,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].tile[0], flags);
        break;
      case xnn_parallelization_type_3d_tile_2d:
        ...
        pthreadpool_parallelize_3d_tile_2d(
            threadpool, op->compute[i].task_3d_tile_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
      ...
      case xnn_parallelization_type_4d:
        ...
        pthreadpool_parallelize_4d(
            threadpool, op->compute[i].task_4d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].range[3], flags);
        break;
      case xnn_parallelization_type_4d_tile_2d:
        ...
        pthreadpool_parallelize_4d_tile_2d(
            threadpool, op->compute[i].task_4d_tile_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
      ...
      case xnn_parallelization_type_5d:
        ...
        pthreadpool_parallelize_5d(
            threadpool, op->compute[i].task_5d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].range[3],
            op->compute[i].range[4], flags);
        break;
      case xnn_parallelization_type_5d_tile_2d:
        ...
        pthreadpool_parallelize_5d_tile_2d(
            threadpool, op->compute[i].task_5d_tile_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].range[3],
            op->compute[i].range[4], op->compute[i].tile[0],
            op->compute[i].tile[1], flags);
        break;
      case xnn_parallelization_type_6d_tile_2d:
        ...
        pthreadpool_parallelize_6d_tile_2d(
            threadpool, op->compute[i].task_6d_tile_2d,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].range[3],
      ...
      case xnn_parallelization_type_2d_tile_1d_with_uarch:
        ...
        pthreadpool_parallelize_2d_tile_1d_with_uarch(
            threadpool, op->compute[i].task_2d_tile_1d_with_id,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
      ...
      case xnn_parallelization_type_2d_tile_2d_with_uarch:
        ...
        pthreadpool_parallelize_2d_tile_2d_with_uarch(
            threadpool, op->compute[i].task_2d_tile_2d_with_id,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            0 /* default uarch index */, XNN_MAX_UARCH_TYPES - 1,
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].tile[0], op->compute[i].tile[1], flags);
      ...
      case xnn_parallelization_type_3d_tile_1d_with_uarch:
        ...
        pthreadpool_parallelize_3d_tile_1d_with_uarch(
            threadpool, op->compute[i].task_3d_tile_1d_with_id,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            0 /* default uarch index */, XNN_MAX_UARCH_TYPES - 1,
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].tile[0], flags);
      ...
      case xnn_parallelization_type_3d_tile_1d_with_uarch_with_thread:
        ...
        pthreadpool_parallelize_3d_tile_1d_with_uarch_with_thread(
            threadpool, op->compute[i].task_3d_tile_1d_with_id_with_thread,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            0 /* default uarch index */, XNN_MAX_UARCH_TYPES - 1,
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].tile[0], flags);
      ...
      case xnn_parallelization_type_3d_tile_2d_with_uarch:
        ...
        pthreadpool_parallelize_3d_tile_2d_with_uarch(
            threadpool, op->compute[i].task_3d_tile_2d_with_id,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            0 /* default uarch index */, XNN_MAX_UARCH_TYPES - 1,
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].tile[0],
      ...
      case xnn_parallelization_type_4d_tile_2d_with_uarch:
        ...
        pthreadpool_parallelize_4d_tile_2d_with_uarch(
            threadpool, op->compute[i].task_4d_tile_2d_with_id,
            (void*)((uintptr_t)&op->context + op->compute[i].context_offset),
            0 /* default uarch index */, XNN_MAX_UARCH_TYPES - 1,
            op->compute[i].range[0], op->compute[i].range[1],
            op->compute[i].range[2], op->compute[i].range[3],
