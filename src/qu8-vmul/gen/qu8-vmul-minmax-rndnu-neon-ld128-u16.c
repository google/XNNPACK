...
void xnn_qu8_vmul_minmax_rndnu_ukernel__neon_ld128_u16(
    size_t batch,
    ...
    const union xnn_qu8_mul_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  #if XNN_ARCH_ARM64
    const uint8x16_t va_zero_point = vdupq_n_u8(params->rndnu_neon.a_zero_point);
    const uint8x16_t vb_zero_point = vdupq_n_u8(params->rndnu_neon.b_zero_point);
  #else
