...
static XNN_INLINE vfloat16m4_t xnn_round_f16(vfloat16m4_t a, size_t vl) {
  // preserve NaN
  vbool4_t nan_bv = __riscv_vmfeq(a, a, vl);
  // magnitude < (1 << FLT16_MANT_DIG)
  vfloat16m4_t mag = __riscv_vfabs(a, vl);
  vbool4_t mag_bv = __riscv_vmflt(mag, (1 << __FLT16_MANT_DIG__), vl);
  vbool4_t mask_bv = __riscv_vmnand(nan_bv, mag_bv, vl);

  vint16m4_t a_rnd = __riscv_vfcvt_x(a, __RISCV_FRM_RNE, vl);
  vfloat16m4_t result = __riscv_vfcvt_f(a_rnd, vl);
  result = __riscv_vmerge(result, a, mask_bv, vl);
  return result;
}


void xnn_f16_vcos_ukernel__rvvfp16arith_rational_3_2_div_u4v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  const xnn_float16 vpi_half_hi = 1.5703125f;     // M_PI / 2 (first 11 bits of mantissa)
  const xnn_float16 vpi_half_lo = 4.837513e-4f;   // M_PI / 2 (remaining bits)
  ...
  do {
    ...
    vfloat16m4_t vx_div_2pi = __riscv_vfmul(vx, v2pi_inv, vl);
    ...
    vfloat16m4_t vp = __riscv_vfadd(__riscv_vfmul(vx2, valpha_3, vl), vone, vl);
    vp = __riscv_vfmul(vx, vp, vl);

    // Evaluate the denominator polynomial q.
    vfloat16m4_t vq = __riscv_vfadd(__riscv_vfmul(vx2, vbeta_2, vl), vone, vl);

    // Divide the numerator by the denominator.
    const vfloat16m4_t vy = __riscv_vfdiv(vp, vq, vl);
