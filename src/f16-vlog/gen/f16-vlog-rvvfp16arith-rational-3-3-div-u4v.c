...
static XNN_INLINE vfloat16m4_t xnn_signed_getexp_f16(vfloat16m4_t a, size_t vl) {
  ...
  a =  __riscv_vreinterpret_f16m4(__riscv_vor_mu(__riscv_vmfeq(a, 0.0f, vl), __riscv_vreinterpret_i16m4(a), __riscv_vreinterpret_i16m4(a), sign_mask.s, vl));

  // Extract the exponent and shift the exponent to the most significant bits of
  // the mantissa.
  const vint16m4_t exp =
    __riscv_vsra(__riscv_vand(__riscv_vreinterpret_i16m4(a), sign_and_exp_mask, vl), 5, vl);
  ...
  return __riscv_vfsub(__riscv_vreinterpret_f16m4(__riscv_vor(exp, bias_32.s, vl)), bias_47, vl);
}

void xnn_f16_vlog_ukernel__rvvfp16arith_rational_3_3_div_u4v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m4_t vx = __riscv_vle16_v_f16m4(input, vl); input += vl;

    // Scale `x` with `sqrt(2)` so that the exponent is rounded up.
    vx = __riscv_vfmul(vx, vsqrt2, vl);

    // Extract the exponent.
    const vfloat16m4_t vexp = xnn_signed_getexp_f16(vx, vl);

    // Normalize `x` to an exponent of zero.
    vx = __riscv_vreinterpret_f16m4(__riscv_vor(__riscv_vand(__riscv_vreinterpret_i16m4(vx), vmantissa_bits_mask, vl), vone.s, vl));
