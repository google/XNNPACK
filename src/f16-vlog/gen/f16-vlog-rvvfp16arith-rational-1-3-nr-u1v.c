...
static XNN_INLINE vfloat16m1_t xnn_signed_getexp_f16(vfloat16m1_t a, size_t vl) {
  ...
  vuint16m1_t va = __riscv_vreinterpret_v_f16m1_u16m1(a);
  vbool16_t is_zero = __riscv_vmfeq_vf_f16m1_b16(a, 0.0f, vl);
  va = __riscv_vor_vx_u16m1_mu(is_zero, va, va, sign_mask, vl);

  // Extract the exponent and shift the exponent to the most significant bits of
  // the mantissa.
  const vuint16m1_t exp =
      __riscv_vsrl_vx_u16m1(__riscv_vand_vx_u16m1(va, sign_and_exp_mask, vl), 5, vl);
  ...
  return __riscv_vfsub_vf_f16m1(__riscv_vreinterpret_v_u16m1_f16m1(__riscv_vor_vx_u16m1(exp, bias_32_bits, vl)), bias_47, vl);
}


void xnn_f16_vlog_ukernel__rvvfp16arith_rational_1_3_nr_u1v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vx = __riscv_vfmul_vf_f16m1(vx, vsqrt2, vl);

    // Extract the exponent.
    const vfloat16m1_t vexp = xnn_signed_getexp_f16(vx, vl);

    // Normalize `x` to an exponent of zero.
    vx = __riscv_vreinterpret_v_u16m1_f16m1(__riscv_vor_vx_u16m1(__riscv_vand_vx_u16m1(__riscv_vreinterpret_v_f16m1_u16m1(vx), vmantissa_bits_mask, vl), vone_bits, vl));
    ...
    vx = __riscv_vfsub_vf_f16m1(__riscv_vfmul_vf_f16m1(vx, vsqrt1_2, vl), vone, vl);

    // Evaluate the denominator polynomial q.
    vfloat16m1_t vq = __riscv_vfadd_vf_f16m1(__riscv_vfmul_vf_f16m1(vx, vbeta_3, vl), vbeta_2, vl);
    vq = __riscv_vfadd_vv_f16m1(__riscv_vfmul_vv_f16m1(vx, vq, vl), __riscv_vfmv_v_f_f16m1(vbeta_1, vl), vl);
    vq = __riscv_vfadd_vv_f16m1(__riscv_vfmul_vv_f16m1(vx, vq, vl), __riscv_vfmv_v_f_f16m1(vone, vl), vl);
    ...
    vfloat16m1_t vy = __riscv_vfdiv_vv_f16m1(vx, vq, vl);

    // Put it all together, i.e. `log(x) = `log(2)*exp + y`.
    vy = __riscv_vfmacc_vf_f16m1(vy, vln2, vexp, vl);

    __riscv_vse16_v_f16m1(output, vy, vl); output += vl;
