...
static XNN_INLINE vfloat16m2_t xnn_signed_getexp_f16(vfloat16m2_t a, size_t vl) {
  ...
  a =  __riscv_vreinterpret_f16m2(__riscv_vor_mu(__riscv_vmfeq(a, 0.0f, vl), __riscv_vreinterpret_i16m2(a), __riscv_vreinterpret_i16m2(a), sign_mask.s, vl));

  // Extract the exponent and shift the exponent to the most significant bits of
  // the mantissa.
  const vint16m2_t exp =
    __riscv_vsra(__riscv_vand(__riscv_vreinterpret_i16m2(a), sign_and_exp_mask, vl), 5, vl);
  ...
  return __riscv_vfsub(__riscv_vreinterpret_f16m2(__riscv_vor(exp, bias_32.s, vl)), bias_47, vl);
}

void xnn_f16_vlog_ukernel__rvvfp16arith_rational_3_3_div_u2v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    const vfloat16m2_t vexp = xnn_signed_getexp_f16(vx, vl);

    // Normalize `x` to an exponent of zero.
    vx = __riscv_vreinterpret_f16m2(__riscv_vor(__riscv_vand(__riscv_vreinterpret_i16m2(vx), vmantissa_bits_mask, vl), vone.s, vl));
    ...
    vfloat16m2_t vp = __riscv_vfadd(__riscv_vfmul(vx, valpha_3, vl), vone.f, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx, vp, vl), vone.f, vl);
    vp = __riscv_vfmul(vx, vp, vl);

    // Evaluate the denominator polynomial q.
    vfloat16m2_t vq = __riscv_vfadd(__riscv_vfmul(vx, vbeta_3, vl), vbeta_2, vl);
