...
static XNN_INLINE vfloat16m4_t xnn_setexp_f16(vfloat16m4_t vx, size_t vl) {
  ...
  return __riscv_vreinterpret_f16m4(__riscv_vsll(__riscv_vreinterpret_u16m4(__riscv_vfadd(vx, vmagic, vl)), 10, vl));
}

// Quick-and-dirty round to nearest, only works for xnn_float16s in the range
// `[2^-9, 2^9)`.
static XNN_INLINE vfloat16m4_t xnn_qd_round_f16(vfloat16m4_t vx, size_t vl) {
...
void xnn_f16_vexp_ukernel__rvvfp16arith_poly_3_u4v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m4_t vz_prime = __riscv_vfmul(vx, vlog2e, vl);
    vz_prime = __riscv_vfmin(__riscv_vfmax(vz_prime, vm15, vl), v16, vl);

    // Decompose x * log2e into `z` (integer part) and `r` (remainder).
    const vfloat16m4_t vz = xnn_qd_round_f16(vz_prime, vl);
    const vfloat16m4_t vr = __riscv_vfsub(vz_prime, vz, vl);

    // Compute 2^z.
    vfloat16m4_t v2z = xnn_setexp_f16(vz, vl);

    // Evaluate the interpolation polynomial for `2^r`.
    vfloat16m4_t v2r = __riscv_vfadd(__riscv_vfmul(vr, valpha_3, vl), valpha_2, vl);
    v2r = __riscv_vfadd(__riscv_vfmul(vr, v2r, vl), valpha_1, vl);
    v2r = __riscv_vfadd(__riscv_vfmul(vr, v2r, vl), vone, vl);

    // Compute 2^z * 2^r.
    const vfloat16m4_t vy = __riscv_vfmul(v2z, v2r, vl);
