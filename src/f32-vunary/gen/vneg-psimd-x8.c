...
void xnn_f32_vneg_ukernel__psimd_x8(
    size_t n,
    const float* x,
    float* y,
    const union xnn_f32_neg_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 8 * sizeof(float); n -= 8 * sizeof(float)) {
    ...
    const psimd_f32 vy0123 = psimd_neg_f32(vx0123);
    const psimd_f32 vy4567 = psimd_neg_f32(vx4567);
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    const psimd_f32 vy = psimd_neg_f32(vx);
