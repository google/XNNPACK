...
void xnn_f32_vsqr_ukernel__psimd_x8(
    size_t n,
    const float* x,
    float* y,
    const void* params) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 8 * sizeof(float); n -= 8 * sizeof(float)) {
    ...
    const psimd_f32 vy0123 = psimd_mul_f32(vx0123, vx0123);
    const psimd_f32 vy4567 = psimd_mul_f32(vx4567, vx4567);
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    const psimd_f32 vy = psimd_mul_f32(vx, vx);
  ...
  if XNN_UNLIKELY(n != 0) {
    const psimd_f32 vx = psimd_load_f32(x);
    psimd_f32 vy = psimd_mul_f32(vx, vx);
    if (n & (2 * sizeof(float))) {
