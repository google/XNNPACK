...
void xnn_qu8_vrpreluc_ukernel__rvv_u4v(
    size_t batch,
    ...
    const union xnn_qs8_vprelu_scalar_params* restrict params)
{
  ...
  if (slope >= 0) {
    ...
    vfloat32m4_t vfpacc = __riscv_vfmul_vf_f32m4(
        __riscv_vfcvt_f_x_v_f32m4(
            __riscv_vmv_v_x_i32m4(slope, vlmax), vlmax),
        params->scalar.rprelu_positive_multiplier, vlmax);
    vfpacc =
        __riscv_vfmax_vf_f32m4(vfpacc, output_min_less_zero_point, vlmax);
    vfpacc =
        __riscv_vfmin_vf_f32m4(vfpacc, output_max_less_zero_point, vlmax);

    const vint32m4_t vacc = __riscv_vadd_vx_i32m4(
        __riscv_vfcvt_x_f_v_i32m4(vfpacc, vlmax), output_zero_point,
        vlmax);
    const vuint32m4_t vout32 =
        __riscv_vreinterpret_v_i32m4_u32m4(vacc);
    const vuint8m1_t vout8 =
        __riscv_vncvt_x(__riscv_vncvt_x(vout32, vlmax), vlmax);

    for (size_t vl; batch > 0; batch -= vl, output += vl) {
      vl = __riscv_vsetvl_e8m1(batch);
      __riscv_vse8_v_u8m1(output, vout8, vl);
    }
    return;
  }
  ...
  for (size_t vl; batch > 0; batch -= vl, input_a += vl, output += vl) {
    ...
    const vuint8m1_t va8 = __riscv_vle8_v_u8m1(input_a, vl);
    vint32m4_t va32 = __riscv_vreinterpret_v_u32m4_i32m4(
        __riscv_vzext_vf4_u32m4(va8, vl));
    va32 = __riscv_vsub_vx_i32m4(va32, input_zero_point, vl);

    vint32m4_t vacc = __riscv_vmul_vx_i32m4(va32, slope, vl);

    vfloat32m4_t vfpacc = __riscv_vfmul_vf_f32m4(
        __riscv_vfcvt_f_x_v_f32m4(vacc, vl), negative_multiplier, vl);
    ...
    const vuint32m4_t vout32 = __riscv_vreinterpret_v_i32m4_u32m4(
