...
void xnn_qu8_vrpreluc_ukernel__rvv_u1v(
    size_t batch,
    ...
    const union xnn_qs8_vprelu_scalar_params* restrict params)
{
  ...
  if (slope >= 0) {
    ...
    vfloat32m1_t vfpacc = __riscv_vfmul_vf_f32m1(
        __riscv_vfcvt_f_x_v_f32m1(
            __riscv_vmv_v_x_i32m1(slope, vlmax), vlmax),
        params->scalar.rprelu_positive_multiplier, vlmax);
    vfpacc =
        __riscv_vfmax_vf_f32m1(vfpacc, output_min_less_zero_point, vlmax);
    vfpacc =
        __riscv_vfmin_vf_f32m1(vfpacc, output_max_less_zero_point, vlmax);

    const vint32m1_t vacc = __riscv_vadd_vx_i32m1(
        __riscv_vfcvt_x_f_v_i32m1(vfpacc, vlmax), output_zero_point,
        vlmax);
    const vuint32m1_t vout32 =
        __riscv_vreinterpret_v_i32m1_u32m1(vacc);
    const vuint8mf4_t vout8 =
        __riscv_vncvt_x(__riscv_vncvt_x(vout32, vlmax), vlmax);

    for (size_t vl; batch > 0; batch -= vl, output += vl) {
      vl = __riscv_vsetvl_e8mf4(batch);
  ...
  for (size_t vl; batch > 0; batch -= vl, input_a += vl, output += vl) {
    ...
    vint32m1_t vacc = __riscv_vmul_vx_i32m1(va32, slope, vl);

    vfloat32m1_t vfpacc = __riscv_vfmul_vf_f32m1(
        __riscv_vfcvt_f_x_v_f32m1(vacc, vl), negative_multiplier, vl);
