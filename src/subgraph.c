...
enum xnn_status xnn_insert_clamp_node(xnn_subgraph_t subgraph, float output_min, float output_max, struct xnn_node *node) {
  ...
  return xnn_define_unary(subgraph, xnn_unary_clamp, &params, new_id, output_id, /*flags=*/0);
}
    ...
    if (value->fp16_compatible) {
      ...
      if (xnn_value_is_static(value)) {
        ...
        xnn_run_unary_elementwise_nc(xnn_unary_convert, xnn_datatype_fp32, xnn_datatype_fp16, 
                                     ...
                                     /*output_quantization=*/NULL, 0, num_elements, 1, 1, 1,
                                     NULL, value->data, value->fp16_temp_data);
        ...
        switch (producer->type) {
          ...
          case xnn_node_type_max_pooling_2d:
            ...
            producer->params.unary.clamp.min =
              math_max_f32(producer->params.unary.clamp.min, consumer->params.unary.clamp.min);
            producer->params.unary.clamp.max =
              math_min_f32(producer->params.unary.clamp.max, consumer->params.unary.clamp.max);
...
enum xnn_unary_operator xnn_node_type_to_unary_operator(enum xnn_node_type node_type) {
  switch (node_type) {
    case xnn_node_type_abs:
