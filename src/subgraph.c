  ...
  for (uint32_t node_id = 0; node_id < subgraph->num_nodes; node_id++) {
    ...
    switch (node->type) {
      ...
      case xnn_node_type_convolution_2d: {
        ...
        xnn_log_info(
            "Inspecting node #%u (%s) with inputs=%s, kernel=%s, and "
            "outputs=%s (pqs8_qc8w_igemm_config=%p).",
            node_id, xnn_node_type_to_string(node->type),
            xnn_datatype_to_string(input_datatype),
            xnn_datatype_to_string(kernel_datatype),
            xnn_datatype_to_string(output_datatype),
            xnn_init_pqs8_qc8w_igemm_config());
        ...
        if (input_datatype == xnn_datatype_qint8 &&
            ...
            xnn_init_pqs8_qc8w_igemm_config() != NULL) {
          // Note that there is currently no option to not use inlining for this
          // iGEMM kernel.
          xnn_log_info("Setting assumed_datatype=%s for node #%u (%s).",
