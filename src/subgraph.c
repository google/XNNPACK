...
enum xnn_status xnn_subgraph_pack_static_values_to_fp16(xnn_subgraph_t subgraph) {
  for (uint32_t n = 0; n < subgraph->num_values; n++) {
    struct xnn_value* value = &subgraph->values[n];
    if (xnn_value_is_static(value->allocation_type) &&
        (value->flags & XNN_VALUE_FLAG_PACK_TO_FP16)) {
      if (value->datatype != xnn_datatype_fp32) {
        xnn_log_error("failed to pack value #%" PRIu32 " to FP16: unsupported datatype %s (expected FP32)",
                      n, xnn_datatype_to_string(value->datatype));
      ...
      if (fp16_data == NULL) {
        xnn_log_error("failed to allocate %zu bytes for packed fp16 tensor data", fp16_size);
        return xnn_status_out_of_memory;
      }
