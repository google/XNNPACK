...
void xnn_f32_ibilinear_chw_ukernel__rvv_2x1v(
    ...
    size_t input_increment)
{
  ...
  for (size_t cb = 0; cb < channels; cb += 2) {
    ...
    for (size_t p = output_pixels, vl; p > 0;
      ...
      const vuint64m2x2_t vptrs =
          __riscv_vlseg2e64_v_u64m2x2((const uint64_t*)i, vl);
      const vuint64m2_t vti_addr =
          __riscv_vget_v_u64m2x2_u64m2(vptrs, 0);
      const vuint64m2_t vbi_addr =
          __riscv_vget_v_u64m2x2_u64m2(vptrs, 1);

      const vuint64m2_t wpair =
          __riscv_vle64_v_u64m2((const uint64_t*)w, vl);
      const vfloat32m1_t vwh = __riscv_vreinterpret_v_u32m1_f32m1(
          __riscv_vnsrl(wpair, 0, vl));
      const vfloat32m1_t vwv = __riscv_vreinterpret_v_u32m1_f32m1(
      ...
      for (size_t c = block; c != 0; c--) {
        const vfloat32m1x2_t vti = __riscv_vluxseg2ei64_v_f32m1x2(
            (const float*)channel_offset, vti_addr, vl);
        const vfloat32m1x2_t vbi = __riscv_vluxseg2ei64_v_f32m1x2(
            (const float*)channel_offset, vbi_addr, vl);

        const vfloat32m1_t vtl =
            __riscv_vget_v_f32m1x2_f32m1(vti, 0);
        const vfloat32m1_t vtr =
            __riscv_vget_v_f32m1x2_f32m1(vti, 1);
        const vfloat32m1_t vbl =
            __riscv_vget_v_f32m1x2_f32m1(vbi, 0);
        const vfloat32m1_t vbr =
            __riscv_vget_v_f32m1x2_f32m1(vbi, 1);

        const vfloat32m1_t vt = __riscv_vfmacc_vv_f32m1(
            vtl, __riscv_vfsub_vv_f32m1(vtr, vtl, vl), vwh, vl);
        const vfloat32m1_t vb = __riscv_vfmacc_vv_f32m1(
            vbl, __riscv_vfsub_vv_f32m1(vbr, vbl, vl), vwh, vl);
        const vfloat32m1_t vo = __riscv_vfmacc_vv_f32m1(
            vt, __riscv_vfsub_vv_f32m1(vb, vt, vl), vwv, vl);

        __riscv_vse32_v_f32m1(o, vo, vl);
