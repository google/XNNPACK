...
size_t xnn_pqs8_qc2w_gemm_minmax_fp32_ukernel_1x64c4__neonsme2_get_mr(void) {
#if XNN_ENABLE_KLEIDIAI
...
size_t xnn_pqs8_qc2w_gemm_minmax_fp32_ukernel_1x64c4__neonsme2_get_nr(void) {
#if XNN_ENABLE_KLEIDIAI
  // The DOT kernel's preferred N scheduling step spans four physical RHS
  // panels.  XNNPACK needs the shared physical panel width used by both the
  // DOT and MOPA kernels for packed-weight offsets.
  const struct kai_matmul_uker_api api =
...
void xnn_pqs8_qc2w_gemm_minmax_fp32_ukernel_1x64c4__neonsme2(
    size_t m, size_t n, size_t k, const void* lhs, const void* rhs_packed,
