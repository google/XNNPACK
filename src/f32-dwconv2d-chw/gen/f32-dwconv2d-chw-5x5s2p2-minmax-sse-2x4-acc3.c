...
void xnn_f32_dwconv2d_chw_ukernel_5x5s2p2__sse_2x4_acc3(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    for (; w > 8 * sizeof(float); w -= 8 * sizeof(float)) {
      ...
      __m128 vo1p2 = _mm_mul_ps(vi4x8ACE, vk22);
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi6x8ACE, vk42));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi2x9BDF, vk03));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi5x9BDF, vk33));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi2x68AC, vk00));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi1x68AC, vk10));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi3x68AC, vk10));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi6x68AC, vk40));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi3x79BD, vk11));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi2x79BD, vk21));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi4x79BD, vk21));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi2xACEG, vk04));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi4xACEG, vk24));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi3xACEG, vk34));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi5xACEG, vk34));
