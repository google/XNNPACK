...
void xnn_f32_dwconv2d_chw_ukernel_3x3s2p1__neonfma_1x4_acc4(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    for (; w >= 8 * sizeof(float); w -= 8 * sizeof(float)) {
      ...
      vo0p2 = vfmaq_lane_f32(vo0p2, vi1x7BDF, vget_low_f32(vw4567), 0);

      vo0p3 = vfmaq_lane_f32(vo0p3, vi2x7BDF, vget_high_f32(vw4567), 1);
