...
void xnn_f32_dwconv2d_chw_ukernel_5x5s2p2__wasmsimd_arm_3x4_acc2(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    v128_t vi7x8ACE = wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 0, 2, 4 + 0, 4 + 2);
    v128_t vi7x9BDF = wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 1, 3, 4 + 1, 4 + 3);
    v128_t vi8x8ACE = wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 0, 2, 4 + 0, 4 + 2);
    v128_t vi8x9BDF = wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 1, 3, 4 + 1, 4 + 3);
    ...
    for (; w > 8 * sizeof(float); w -= 8 * sizeof(float)) {
      ...
      const v128_t vi7xGIKM = wasm_v32x4_shuffle(vi7xGHIJ, vi7xKLMN, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi7xHJLN = wasm_v32x4_shuffle(vi7xGHIJ, vi7xKLMN, 1, 3, 4 + 1, 4 + 3);
      const v128_t vi8xGIKM = wasm_v32x4_shuffle(vi8xGHIJ, vi8xKLMN, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi8xHJLN = wasm_v32x4_shuffle(vi8xGHIJ, vi8xKLMN, 1, 3, 4 + 1, 4 + 3);
