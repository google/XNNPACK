...
void xnn_f32_dwconv2d_chw_ukernel_3x3s2p1__wasmsimd_arm_4x4(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    for (; w >= 8 * sizeof(float); w -= 8 * sizeof(float)) {
      ...
      const v128_t vi7x8ACE = wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi7x9BDF = wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 1, 3, 4 + 1, 4 + 3);
      const v128_t vi8x8ACE = wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi8x9BDF = wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 1, 3, 4 + 1, 4 + 3);
    ...
    if XNN_LIKELY(w != 0) {
      ...
      const v128_t vi7x8ACE = wasm_v128_and(vmask_even, wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 0, 2, 4 + 0, 4 + 2));
      const v128_t vi7x9BDF = wasm_v128_and(vmask_odd,  wasm_v32x4_shuffle(vi7x89AB, vi7xCDEF, 1, 3, 4 + 1, 4 + 3));
      const v128_t vi8x8ACE = wasm_v128_and(vmask_even, wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 0, 2, 4 + 0, 4 + 2));
      const v128_t vi8x9BDF = wasm_v128_and(vmask_odd,  wasm_v32x4_shuffle(vi8x89AB, vi8xCDEF, 1, 3, 4 + 1, 4 + 3));
