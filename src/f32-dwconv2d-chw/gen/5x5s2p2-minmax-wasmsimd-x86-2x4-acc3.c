...
//   Template: src/f32-dwconv2d-chw/5x5s2p2-wasmsimd.c.in
...
void xnn_f32_dwconv2d_chw_ukernel_5x5s2p2__wasmsimd_x86_2x4_acc3(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    v128_t vi5x8ACE = wasm_v32x4_shuffle(vi5x89AB, vi5xCDEF, 0, 2, 4 + 0, 4 + 2);
    v128_t vi5x9BDF = wasm_v32x4_shuffle(vi5x89AB, vi5xCDEF, 1, 3, 4 + 1, 4 + 3);
    v128_t vi6x8ACE = wasm_v32x4_shuffle(vi6x89AB, vi6xCDEF, 0, 2, 4 + 0, 4 + 2);
    v128_t vi6x9BDF = wasm_v32x4_shuffle(vi6x89AB, vi6xCDEF, 1, 3, 4 + 1, 4 + 3);
    ...
    for (; w > 8 * sizeof(float); w -= 8 * sizeof(float)) {
      ...
      const v128_t vi5xGIKM = wasm_v32x4_shuffle(vi5xGHIJ, vi5xKLMN, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi5xHJLN = wasm_v32x4_shuffle(vi5xGHIJ, vi5xKLMN, 1, 3, 4 + 1, 4 + 3);
      const v128_t vi6xGIKM = wasm_v32x4_shuffle(vi6xGHIJ, vi6xKLMN, 0, 2, 4 + 0, 4 + 2);
      const v128_t vi6xHJLN = wasm_v32x4_shuffle(vi6xGHIJ, vi6xKLMN, 1, 3, 4 + 1, 4 + 3);
