    ...
    const struct xnn_f32_scale_params* restrict params)
{
  ...
  size_t vlmax = __riscv_vsetvl_e32m1(batch);
  vfloat32m1_t vsum = __riscv_vle32_v_f32m1(input, vlmax); input += vlmax; batch -= vlmax;
 
  for (; batch > 0;) {
    size_t vl = __riscv_vsetvl_e32m1(batch); batch -= vl;

    vfloat32m1_t vinput = __riscv_vle32_v_f32m1(input, vl); input += vl;
    vsum = __riscv_vfadd_vv_f32m1(vsum, vinput, vl);
