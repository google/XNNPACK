    ...
    const struct xnn_f32_scale_params* restrict params)
{
  ...
  size_t vlmax = __riscv_vsetvl_e32m2(batch);
  vfloat32m2_t vsum = __riscv_vle32_v_f32m2(input, vlmax); input += vlmax; batch -= vlmax;
 
  for (; batch > 0;) {
    size_t vl = __riscv_vsetvl_e32m2(batch); batch -= vl;

    vfloat32m2_t vinput = __riscv_vle32_v_f32m2(input, vl); input += vl;
    vsum = __riscv_vfadd_vv_f32m2(vsum, vinput, vl);
