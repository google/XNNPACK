...
static const struct xnn_ibilinear_chw_config* get_ibilinear_nchw_config(
...
enum xnn_status xnn_create_resize_bilinear2d_nchw(
  ...
  if (output_width == 0 || output_height == 0) {
    xnn_log_error(
        "failed to reshape %s operator with %zux%zu output: output dimensions "
        "must be non-zero",
        xnn_operator_type_to_string(xnn_operator_type_resize_bilinear_nchw),
...
enum xnn_status xnn_reshape_resize_bilinear2d_nchw(
  ...
  const size_t log2_data_element_size =
      resize_op->ibilinear_chw_config->log2_data_element_size;
  const size_t log2_weight_element_size =
      resize_op->ibilinear_chw_config->log2_weight_element_size;
  const size_t output_height = resize_op->output_height;
  const size_t output_width = resize_op->output_width;
  if (output_height * output_width !=
      resize_op->last_output_height * resize_op->last_output_width) {
    const size_t indirection_buffer_size =
        sizeof(void*) * (output_height * output_width * 4);
    const size_t packed_weights_size = (output_height * output_width * 2)
                                       << log2_weight_element_size;

    const void** indirection_buffer = (const void**)xnn_reallocate_memory(
        resize_op->indirection_buffer, indirection_buffer_size);
    ...
    xnn_log_debug(
        "allocated %zu bytes for indirection buffer in %s operator",
        indirection_buffer_size,
        xnn_operator_type_to_string(xnn_operator_type_resize_bilinear_nchw));

    // Note: packed weights must be SIMD-aligned, so we can't use
    // xnn_reallocate_memory
    xnn_release_simd_memory(resize_op->packed_weights.pointer);
    resize_op->packed_weights.pointer =
        xnn_allocate_simd_memory(packed_weights_size);
  ...
  const size_t input_pixel_stride_in_bytes =
      1 << log2_data_element_size;  // Since the layout in CHW the pixels
  if (input_height != resize_op->last_input_height ||
    ...
    resize_op->ibilinear_chw_config->indirection_init(
        input_pixel_stride_in_bytes, input_height, input_width, output_height,
        output_width, dummy_input, resize_op->indirection_buffer,
        resize_op->packed_weights.pointer, !!(flags & XNN_FLAG_ALIGN_CORNERS),
  ...
  const struct xnn_ibilinear_chw_config* ibilinear_chw =
      resize_op->ibilinear_chw_config;
  // Resize bilinear packed weights can change when the operator is resized, we
  // will not use weights cache.
  ...
  resize_op->context.resize_bilinear_chw = (struct resize_bilinear_chw_context){
      ...
      .output_batch_stride =
          (output_pixel_stride * output_height * output_width)
          << log2_data_element_size,
  ...
  if (num_threads > 1) {
    ...
    const size_t max_channel_tile = divide_round_up(
        output_channel_tile, num_threads * target_tiles_per_thread);
    if (max_channel_tile < output_channel_tile) {
      ...
      output_channel_tile =
          min(output_channel_tile,
              divide_round_up(output_channel_tile,
                              max_channel_tile * output_channel_subtile) *
                  output_channel_subtile);
    }
  }

  resize_op->compute[0].type = xnn_parallelization_type_2d_tile_1d;
  resize_op->compute[0].task_2d_tile_1d =
      (pthreadpool_task_2d_tile_1d_t)xnn_compute_resize_bilinear_chw;
