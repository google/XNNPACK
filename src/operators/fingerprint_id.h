...
// ```
#define XNN_FINGERPRINT_ID_VALUE(op, in, out, weights, ...)            \
  XNN_EXPAND(XNN_GLUE(                                                 \
      XNN_OVERLOAD(XNN_CONCAT_FLAGS_,                                  \
                   XNN_COUNT_ARGS(op, in, out, weights, __VA_ARGS__)), \
      (xnn_fingerprint_id_helper_, op << XNN_FINGERPRINT_ID_OP_OFFSET, \
...
#define XNN_EXPAND_TYPES(prefix, ...)                                    \
  XNN_CONCAT(XNN_EXPAND_TYPES_IMPL, XNN_COUNT_ARGS(__VA_ARGS__))(prefix, \
                                                                 __VA_ARGS__)
#define XNN_EXPAND_TYPES_IMPL_1(prefix, a) XNN_CONCAT(prefix, a, a, a)
#define XNN_EXPAND_TYPES_IMPL_2(prefix, a, b) XNN_CONCAT(prefix, a, a, b)
#define XNN_EXPAND_TYPES_IMPL_3(prefix, a, b, c) XNN_CONCAT(prefix, a, b, c)
...
enum xnn_fingerprint_id_helper {
  ...
  xnn_fingerprint_id_helper_qc8 = 14,
  // Flag values
  //
  ...
  xnn_fingerprint_id_helper_flag_mask = (1 << 8) - 1,
};
