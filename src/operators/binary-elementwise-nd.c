...
enum xnn_status xnn_create_minimum_nd_s32(
    ...
    xnn_operator_t* minimum_op_out)
{
  const struct xnn_binary_elementwise_config* s32_minimum_config = xnn_init_s32_vmin_config();
  if (s32_minimum_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_minimum_nd_s32));
    return xnn_status_unsupported_hardware;
  }
    ...
    pthreadpool_t threadpool)
{
  ...
  return reshape_binary_elementwise_nd(
    min_op, xnn_operator_type_minimum_nd_s32,
    num_input1_dims, input1_shape,
    num_input2_dims, input2_shape,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_INT32_T,
    &min_op->params.s32_default, sizeof(min_op->params.s32_default),
    &min_op->params.s32_default, sizeof(min_op->params.s32_default),
    threadpool);
}
