...
enum xnn_status xnn_create_binary_elementwise_nd(
  ...
  op->num_extra_params = 1;
  op->extra_params = xnn_allocate_zero_memory(sizeof(union xnn_binary_uparams));
  if (op->extra_params == NULL) {
    xnn_log_error("failed to allocate %zu bytes for %s operator descriptor",
                  sizeof(union xnn_binary_uparams),
                  xnn_binary_operator_to_string(type));
    return xnn_status_out_of_memory;
  }
...
enum xnn_status xnn_run_binary_elementwise_nd(
  ...
  op.num_compute_invocations = 1;
  union xnn_params params2;
  memset(&params2, 0, sizeof(union xnn_params));
  op.extra_params = &params2;
