...
static enum xnn_status generate_fingerprint_parameters(struct deconv2d_context* context) {
  ...
  { // Generate kernel data.
    const size_t filter_element_size = 1 << context->gemm_config->log2_filter_element_size;
    size_t kernel_size = context->groups * context->group_output_channels * context->kernel_height * context->kernel_width * context->group_input_channels;
    context->kernel_for_cleanup = xnn_allocate_simd_memory(kernel_size * filter_element_size);
    context->kernel = context->kernel_for_cleanup;
    fill_fingerprint_buffer(context->kernel_for_cleanup, kernel_size * filter_element_size);
    context->bias_for_cleanup = xnn_allocate_simd_memory(context->groups * context->group_output_channels);
    context->bias = context->bias_for_cleanup;
    fill_fingerprint_buffer(context->bias_for_cleanup, context->groups * context->group_output_channels);
  }
  return xnn_status_success;
}
