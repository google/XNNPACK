...
static enum xnn_status create_deconvolution2d_nhwc(
  ...
  if (max(stride_height, stride_width) > 1 &&
    ...
    deconvolution_op->subconvolution_buffer =
  ...
  switch (ukernel_type) {
    case xnn_microkernel_type_igemm:
      pack_conv_goki_w(groups, group_output_channels, kernel_size,
                       ...
                       /*scale=*/NULL, weights_ptr, nr * extra_weights_bytes,
                       packing_params);
      break;
    case xnn_microkernel_type_subconv2d:
      pack_deconv_goki_w(
          ...
          deconvolution_op->subconvolution_buffer, packing_params);
        ...
        for (size_t sh = 0; sh < stride_height; sh++) {
          for (size_t sw = 0; sw < stride_width; sw++) {
            ...
            init_kernel_scale_params(group_output_channels, gemm_config->nr,
                                     ...
                                     gemm_config->nr * weights_stride, 0,
...
enum xnn_status xnn_create_deconvolution2d_nhwc_qs8_qc8w(
  ...
  enum xnn_status status = create_deconvolution2d_nhwc_qs8_qc8w(
      ...
      requantization_scale, kernel, bias, output_zero_point, output_scale,
...
enum xnn_status xnn_create_deconvolution2d_nhwc_qs8(
  ...
  enum xnn_status status = create_deconvolution2d_nhwc_qs8_qc8w(
      ...
      dilation_width, groups, group_input_channels, group_output_channels,
      input_pixel_stride, output_pixel_stride, input_zero_point, input_scale,
      duplicated_requantization_scale, kernel, bias, output_zero_point,
      output_scale, output_min, output_max, flags, code_cache, weights_cache,
      deconvolution_op_out);
...
enum xnn_status xnn_create_deconvolution2d_nhwc_qu8(
  ...
  return create_deconvolution2d_nhwc(
      ...
      input_zero_point /* input padding byte */,
      kernel_zero_point /* packed weights padding byte */,
...
static enum xnn_status reshape_conv_path(
  ...
  if (input_height != deconvolution_op->last_input_height ||
    ...
    const void** indirection_buffer = (const void**)xnn_reallocate_memory(
        deconvolution_op->indirection_buffer, indirection_buffer_size);
    ...
    deconvolution_op->input = (void*)((uintptr_t)deconvolution_op->zero_buffer +
  ...
  if (groups == 1) {
    ...
    if (xnn_is_hmp_igemm_ukernel(igemm_ukernel)) {
      if (batch_size > 1) {
        ...
        if (dynamic_quantization) {
          deconvolution_op->compute[igemm_compute_index]
              .task_3d_tile_2d_with_id = (pthreadpool_task_3d_tile_2d_with_id_t)
      ...
      if (batch_size > 1) {
        ...
        if (dynamic_quantization) {
          deconvolution_op->compute[igemm_compute_index].task_3d_tile_2d =
        ...
        if (dynamic_quantization) {
          deconvolution_op->compute[igemm_compute_index].task_2d_tile_2d =
...
static enum xnn_status reshape_subconv2d_path(
  ...
  if (any_size_change) {
    ...
    const size_t modulo_padding_top =
        deconvolution_op->padding_top % stride_height;
    const size_t modulo_padding_left =
        deconvolution_op->padding_left % stride_width;
    for (size_t offset_y = 0; offset_y < stride_height; offset_y++) {
      for (size_t offset_x = 0; offset_x < stride_width; offset_x++) {
...
static enum xnn_status reshape_deconvolution2d_nhwc(
  ...
  deconvolution_op->output_height = xnn_compute_deconvolution_output_dimension(
      input_height,
      deconvolution_op->padding_top + deconvolution_op->padding_bottom,
      adjustment_height, deconvolution_op->kernel_height,
      deconvolution_op->dilation_height, deconvolution_op->stride_height);
  deconvolution_op->output_width = xnn_compute_deconvolution_output_dimension(
      input_width,
      deconvolution_op->padding_left + deconvolution_op->padding_right,
      adjustment_width, deconvolution_op->kernel_width,
      deconvolution_op->dilation_width, deconvolution_op->stride_width);
...
enum xnn_status reshape_deconvolution2d_nhwc_qx8_f32_qc8w(
  ...
  deconvolution_op->zero_buffers = xnn_reallocate_memory(
      deconvolution_op->zero_buffers, batch_size * sizeof(void*));
  deconvolution_op->zero_buffers[0] = deconvolution_op->zero_buffer;
  for (size_t i = 1; i < batch_size; ++i) {
    deconvolution_op->zero_buffers[i] =
        xnn_allocate_simd_memory(deconvolution_op->zero_size);
  }
...
static enum xnn_status setup_conv_path(xnn_operator_t deconvolution_op,
                                       const void* input, void* output) {
  assert(deconvolution_op->ukernel.type == xnn_microkernel_type_igemm);

  deconvolution_op->context.igemm.igemm.a_offset =
      (size_t)((uintptr_t)input - (uintptr_t)deconvolution_op->last_input);
  ...
  deconvolution_op->context.igemm.igemm.zero_buffers =
      deconvolution_op->zero_buffers;
  deconvolution_op->context.igemm.igemm.quantization_params =
      ...
      for (size_t offset_x = 0; offset_x < stride_width; offset_x++) {
        subconvolution_params->output =
            (void*)((uintptr_t)subconvolution_params->output +
                    ((uintptr_t)output -
                     (uintptr_t)deconvolution_op->last_output));
        ++subconvolution_params;
      }
    }
    deconvolution_op->last_output = output;
...
static enum xnn_status setup_deconvolution2d_nhwc(
    ...
    const void* quantization_params, void* output) {
  if (deconvolution_op->type != expected_operator_type) {
    xnn_log_error(
...
enum xnn_status xnn_setup_deconvolution2d_nhwc_qs8(
  ...
  return setup_deconvolution2d_nhwc(
      ...
      /*quantization_params=*/NULL, output);
}
...
enum xnn_status xnn_setup_deconvolution2d_nhwc_qd8_f32_qc8w(
  ...
  return setup_deconvolution2d_nhwc(
      ...
      input, quantization_params, output);
}
