...
static enum xnn_status create_dynamic_fully_connected_nc(
  ...
  for (size_t i = 0; i < mr; i++) {
    dynamic_fully_connected_op->ukernel.gemm.gemm_cases[i] =
        gemm_ukernels->gemm[i];
  }
  if (flags & XNN_FLAG_TRANSPOSE_WEIGHTS) {
    dynamic_fully_connected_op->ukernel.gemm.packw_gemm_gio =
        gemm_config->pack_gemm_gio;
  } else {
    dynamic_fully_connected_op->ukernel.gemm.packw_gemm_goi =
  ...
  if (gemm_nr2_config != NULL) {
    ...
    for (size_t i = 0; i < gemm_nr2_config->mr; i++) {
      dynamic_fully_connected_op->ukernel.gemm_nr2.gemm_cases[i] =
          gemm_nr2_ukernels->gemm[i];
    }
    if (flags & XNN_FLAG_TRANSPOSE_WEIGHTS) {
      dynamic_fully_connected_op->ukernel.gemm_nr2.packw_gemm_gio =
          gemm_nr2_config->pack_gemm_gio;
    } else {
      dynamic_fully_connected_op->ukernel.gemm_nr2.packw_gemm_goi =
          gemm_nr2_config->pack_gemm_goi;
    }
  }
...
static enum xnn_status reshape_dynamic_fully_connected_nc(
  ...
  if (ukernel->nr > output_channels) {
    ...
    if (gemm_nr2_mr != 0 &&
        dynamic_fully_connected_op->ukernel.gemm_nr2.gemm_cases[gemm_nr2_mr - 1]
                .function[XNN_UARCH_DEFAULT] != NULL) {
      use_gemm_nr2 = true;
  ...
  *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
  ...
  const bool packed_lhs = (dynamic_fully_connected_op->type ==
                           xnn_operator_type_dynamic_fully_connected_nc_pf16) ||
...
static enum xnn_status setup_dynamic_fully_connected_nc(
  ...
  if (dynamic_fully_connected_op->flags & XNN_FLAG_TRANSPOSE_WEIGHTS) {
    ...
    dynamic_fully_connected_op->context.gemm.packw_gemm_gio.packed_weights =
        workspace;
  } else {
    ...
    dynamic_fully_connected_op->context.gemm.packw_gemm_goi.packed_weights =
        workspace;
  }
