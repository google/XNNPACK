...
static enum xnn_status create_dynamic_fully_connected_nc(
  ...
  dynamic_fully_connected_op->num_extra_params = 1;
  dynamic_fully_connected_op->extra_params =
      xnn_allocate_zero_memory(params2_size);
  if (dynamic_fully_connected_op->extra_params == NULL) {
    xnn_log_error("failed to allocate %zu bytes for %s operator descriptor",
                  params2_size,
                  xnn_operator_type_to_string(operator_type));
