  ...
  *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
  ...
  size_t nc =
    ...
    if (xnn_is_hmp_gemm_ukernel(gemm_ukernel)) {
      dynamic_fully_connected_op->compute[1].type = xnn_parallelization_type_2d_tile_2d_dynamic_with_uarch;
      dynamic_fully_connected_op->compute[1].task_2d_tile_2d_dynamic_with_id = (pthreadpool_task_2d_tile_2d_dynamic_with_id_t) xnn_compute_hmp_gemm;
    } else {
