...
static inline size_t compute_output_dimension(size_t padded_input_dimension,
...
enum xnn_status xnn_reshape_argmax_pooling2d_nhwc_f32(
  ...
  if (argmax_pooling_op->flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) {
    argmax_pooling_op->output_height =
        divide_round_up(input_height, pooling_height);
    argmax_pooling_op->output_width =
        divide_round_up(input_width, pooling_width);

    const uint32_t padding_height =
        argmax_pooling_op->output_height * pooling_height - input_height;
    const uint32_t padding_width =
        argmax_pooling_op->output_width * pooling_width - input_width;
    argmax_pooling_op->padding_top = padding_height / 2;
    argmax_pooling_op->padding_left = padding_width / 2;
    argmax_pooling_op->padding_bottom =
        padding_height - argmax_pooling_op->padding_top;
    argmax_pooling_op->padding_right =
        padding_width - argmax_pooling_op->padding_left;
  } else {
    argmax_pooling_op->output_height =
        compute_output_dimension(argmax_pooling_op->padding_top + input_height +
                                     argmax_pooling_op->padding_bottom,
                                 argmax_pooling_op->kernel_height);
    argmax_pooling_op->output_width =
        compute_output_dimension(argmax_pooling_op->padding_left + input_width +
                                     argmax_pooling_op->padding_right,
  ...
  const struct xnn_argmaxpool_config* argmaxpool_config =
      argmax_pooling_op->argmaxpool_config;
  ...
  const size_t indirection_buffer_size =
      sizeof(void*) * (output_height * step_height);

  // Allocate indirection buffer as size is known here. We initialize the buffer
  // in setup, when input pointer is known.
  const void** indirection_buffer = (const void**)xnn_reallocate_memory(
      argmax_pooling_op->indirection_buffer, indirection_buffer_size);
  ...
  argmax_pooling_op->context.argmax_pooling = (struct argmax_pooling_context){
      ...
      .input_batch_stride =
          input_height * input_width * input_pixel_stride * sizeof(float),
  ...
  argmax_pooling_op->context.argmax_pooling.ukernel =
      argmaxpool_config->ukernel;
  argmax_pooling_op->compute[0].task_2d =
      (pthreadpool_task_2d_t)xnn_compute_argmax_pooling;
...
enum xnn_status xnn_setup_argmax_pooling2d_nhwc_f32(
  ...
  const size_t pooling_size = pooling_height * pooling_width;
  const size_t output_width = argmax_pooling_op->output_width;
  // TODO(zhin): Consider storing step_width and step_height in operator, this
  // is already calculated in reshape.
  const size_t step_width = pooling_width;
  const size_t step_height =
      pooling_size + (output_width - 1) * step_width * pooling_height;
  ...
  argmax_pooling_op->context.argmax_pooling.indirect_input =
      argmax_pooling_op->indirection_buffer,
