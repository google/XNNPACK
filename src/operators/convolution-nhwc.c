...
static enum xnn_status create_vmulcaddc_path(
    ...
    int packed_weights_padding_byte, const void* vmulcaddc_params,
...
static enum xnn_status create_dwconv_path(
  ...
  if (flags & XNN_FLAG_DEPTHWISE_CONVOLUTION) {
    pack_dwconv_hwg_w(
        ...
        dwconv_ukernel->channel_tile, dwconv_ukernel->channel_subtile,
        dwconv_ukernel->channel_round, kernel, bias, /*scale=*/NULL,
        weights_ptr, dwconv_ukernel->channel_tile * extra_weights_bytes,
        dwconv_ukernel->channel_subtile * extra_weights_bytes, packing_params);
  } else {
    pack_dwconv_ghw_w(
        primary_tile, kernel_height, kernel_width, groups,
        dwconv_ukernel->channel_tile, dwconv_ukernel->channel_subtile,
  ...
  uint32_t cache_seed = primary_tile ^ kernel_height ^ kernel_width ^ groups ^
                        dwconv_ukernel->channel_tile ^
                        dwconv_ukernel->channel_subtile ^
                        dwconv_ukernel->channel_round ^ extra_weights_bytes;
  if (flags & XNN_FLAG_DEPTHWISE_CONVOLUTION) {
    ...
    if (!weights_already_cached) {
      ...
      for (uint32_t group = 0; group < groups; group++) {
        init_kernel_scale_params(
            ...
            0, kernel_scale_params, group_weights);
    ...
    if (!weights_already_cached) {
      ...
      for (uint32_t group = 0; group < groups; group++) {
        init_scale_params(group_output_channels, gemm_config->nr,
                          gemm_config->nr, gemm_config->nr * weights_stride,
...
static enum xnn_status create_convolution2d_nhwc(
    ...
    xnn_pack_conv_goki_w_fn pack_conv_goki_w, const void* packing_params,
    int input_padding_byte, int packed_weights_padding_byte,
  ...
  switch (ukernel_type) {
    ...
    case xnn_microkernel_type_dwconv: {
      status = create_dwconv_path(
          ...
          packed_weights_padding_byte, extra_weights_bytes, init_scale_params,
          scale_params, dwconv_params, dwconv_params_size, dwconv_ukernel,
    ...
    case xnn_microkernel_type_igemm: {
      status = create_igemm(
          ...
          pack_conv_goki_w, packing_params, packed_weights_padding_byte,
          extra_weights_bytes, init_scale_params, scale_params,
          init_kernel_scale_params, kernel_scale_params, gemm_params,
          gemm_params_size, gemm_config, linear_activation, relu_activation,
          operator_type, convolution_op, &zero_size);
      if (status != xnn_status_success) {
        goto error;
      }
      break;
...
enum xnn_status create_convolution2d_nhwc_f32(
    ...
    float output_min, float output_max, uint32_t flags,
    const struct xnn_gemm_config* gemm_config, xnn_code_cache_t code_cache,
    xnn_weights_cache_t weights_cache, xnn_operator_t* convolution_op_out) {
  if (isnan(output_min)) {
    xnn_log_error(
        "failed to create %s operator with NaN output lower bound: lower bound "
...
enum xnn_status xnn_create_convolution2d_nhwc_f32(
  ...
  return create_convolution2d_nhwc_f32(
      ...
      gemm_config, code_cache, weights_cache, convolution_op_out);
}
...
static enum xnn_status reshape_igemm(
  ...
  if (convolution_op->flags & XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER) {
    ...
    *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
    ...
    convolution_op->context.igemm.conv2d_igemm_indirection_init =
    ...
    convolution_op->compute[0].context_offset =
        offsetof(struct xnn_operator,
                 context.igemm.conv2d_igemm_indirection_init) -
    ...
    *workspace_alignment = 1;
    ...
    if (input_size_changed(convolution_op)) {
      const void** indirection_buffer = (const void**)xnn_reallocate_memory(
          (void*)convolution_op->indirection_buffer, indirection_buffer_size);
      ...
      convolution_op->input = (void*)((uintptr_t)convolution_op->zero_buffer +
      ...
      xnn_indirection_init_conv2d(
          /*output_tile_size=*/mr,
          /*output_start=*/0,
          /*output_end=*/tiled_output_size, convolution_op->indirection_buffer,
          convolution_op->input, convolution_op->zero_buffer,
  ...
  memcpy(&convolution_op->context.igemm.igemm.params, &convolution_op->params,
         sizeof(convolution_op->context.igemm.igemm.params));

  size_t nc = group_output_channels;
  if (num_threads > 1) {
    const size_t num_other_tiles =
        groups * batch_size * divide_round_up(output_size, mr);
  ...
  if (groups == 1) {
    ...
    if (xnn_is_hmp_igemm_ukernel(igemm_ukernel)) {
      if (batch_size > 1) {
        ...
        if (dynamic_quantization) {
          convolution_op->compute[igemm_compute_index].task_3d_tile_2d_with_id =
        ...
        if (dynamic_quantization) {
          convolution_op->compute[igemm_compute_index].task_2d_tile_2d_with_id =
              (pthreadpool_task_2d_tile_2d_with_id_t)xnn_compute_hmp_dqigemm;
        } else {
          convolution_op->compute[igemm_compute_index].task_2d_tile_2d_with_id =
              (pthreadpool_task_2d_tile_2d_with_id_t)xnn_compute_hmp_igemm;
        }
      }
    } else {
      if (batch_size > 1) {
        ...
        if (dynamic_quantization) {
          convolution_op->compute[igemm_compute_index].task_3d_tile_2d =
              (pthreadpool_task_3d_tile_2d_t)xnn_compute_batch_dqigemm;
        } else {
          convolution_op->compute[igemm_compute_index].task_3d_tile_2d =
              (pthreadpool_task_3d_tile_2d_t)xnn_compute_batch_igemm;
        }
      } else {
        convolution_op->compute[igemm_compute_index].type =
        ...
        if (dynamic_quantization) {
          convolution_op->compute[igemm_compute_index].task_2d_tile_2d =
              (pthreadpool_task_2d_tile_2d_t)xnn_compute_dqigemm;
        } else {
          convolution_op->compute[igemm_compute_index].task_2d_tile_2d =
              (pthreadpool_task_2d_tile_2d_t)xnn_compute_igemm;
        }
      }
    }
#else
    ...
    if (xnn_is_hmp_igemm_ukernel(igemm_ukernel)) {
      ...
      if (batch_size > 1) {
        ...
        if (dynamic_quantization) {
          convolution_op->compute[igemm_compute_index].task_3d_tile_2d =
              (pthreadpool_task_3d_tile_2d_t)xnn_compute_grouped_dqigemm;
        } else {
          convolution_op->compute[igemm_compute_index].task_3d_tile_2d =
              (pthreadpool_task_3d_tile_2d_t)xnn_compute_grouped_igemm;
        }
      }
    }
#else
    ...
    if (batch_size > 1) {
      ...
      convolution_op->compute[igemm_compute_index].range[3] =
          group_output_channels;
    } else {
...
static enum xnn_status reshape_dwconv(
  ...
  const size_t step_width =
      convolution_op->dilation_width == 1
          ? min(convolution_op->stride_width, kernel_width)
          : kernel_width;
  ...
  if (is_transient_indirection_buffer) {
    ...
    convolution_op->context.dwconv.dwconv_indirection_init =
    ...
    convolution_op->compute[0].type = xnn_parallelization_type_1d_tile_1d;
    convolution_op->compute[0].context_offset =
        offsetof(struct xnn_operator, context.dwconv.dwconv_indirection_init) -
        offsetof(struct xnn_operator, context);
    convolution_op->compute[0].task_1d_tile_1d =
    ...
    if (input_size_changed(convolution_op)) {
      const void** indirection_buffer = (const void**)xnn_reallocate_memory(
          convolution_op->indirection_buffer, indirection_buffer_size);
      if (indirection_buffer == NULL) {
        xnn_log_error(
            "failed to allocate %zu bytes for %s operator indirection buffer",
            indirection_buffer_size,
      ...
      xnn_indirection_init_dwconv2d(
          ...
          convolution_op->indirection_buffer, convolution_op->input,
          convolution_op->input_pixel_stride << log2_input_element_size,
          convolution_op->zero_buffer, convolution_op->input_height,
          convolution_op->input_width, convolution_op->output_height,
          convolution_op->output_width, convolution_op->kernel_height,
          convolution_op->kernel_width, convolution_op->stride_height,
          convolution_op->stride_width, convolution_op->dilation_height,
          convolution_op->dilation_width, convolution_op->padding_top,
          convolution_op->padding_left, step_height, step_width, primary_tile);
    }
  }
  ...
  memcpy(&convolution_op->context.dwconv.dwconv.params, &convolution_op->params,
         sizeof(convolution_op->context.dwconv.dwconv.params));
  ...
  *workspace_alignment =
      total_workspace_size == 0 ? 1 : XNN_ALLOCATION_ALIGNMENT;
...
static enum xnn_status reshape_vmulcaddc(xnn_operator_t convolution_op,
                                         ...
                                         size_t num_threads) {
  const size_t batch_output_size = convolution_op->batch_size *
                                   convolution_op->output_height *
                                   convolution_op->output_width;
...
static enum xnn_status reshape_convolution2d_nhwc(
    ...
    size_t* workspace_alignment, size_t* output_height_out,
  ...
  if (convolution_op->flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) {
    ...
    const uint32_t effective_kernel_height =
        (convolution_op->kernel_height - 1) * convolution_op->dilation_height +
        1;
    const uint32_t effective_kernel_width =
        (convolution_op->kernel_width - 1) * convolution_op->dilation_width + 1;
    const size_t total_padding_height =
        (convolution_op->output_height - 1) * convolution_op->stride_height +
        effective_kernel_height - input_height;
    const size_t total_padding_width =
        (convolution_op->output_width - 1) * convolution_op->stride_width +
    ...
    convolution_op->padding_bottom =
        total_padding_height - convolution_op->padding_top;
    convolution_op->padding_right =
        total_padding_width - convolution_op->padding_left;
  } else {
    convolution_op->output_height = xnn_compute_convolution_output_dimension(
        convolution_op->padding_top + input_height +
            convolution_op->padding_bottom,
        convolution_op->kernel_height, convolution_op->dilation_height,
        convolution_op->stride_height);
    convolution_op->output_width = xnn_compute_convolution_output_dimension(
        convolution_op->padding_left + input_width +
            convolution_op->padding_right,
        convolution_op->kernel_width, convolution_op->dilation_width,
...
enum xnn_status reshape_convolution2d_nhwc_qx8_f16_qc8w(
  ...
  if (input_size_changed(convolution_op)) {
    ...
    convolution_op->zero_buffers = xnn_reallocate_memory(
        convolution_op->zero_buffers, batch_size * sizeof(void*));
    convolution_op->zero_buffers[0] = convolution_op->zero_buffer;
    for (size_t i = 1; i < batch_size; ++i) {
      convolution_op->zero_buffers[i] =
          xnn_allocate_simd_memory(convolution_op->zero_size);
    }
  }
  return reshape_convolution2d_nhwc(
      ...
      /*dynamic_quantization=*/true, workspace_size, workspace_alignment,
...
enum xnn_status xnn_reshape_convolution2d_nhwc_qu8(
  ...
  return reshape_convolution2d_nhwc(
      ...
      /*dynamic_quantization=*/false, workspace_size, workspace_alignment,
      output_height_out, output_width_out, threadpool);
}
...
static enum xnn_status setup_igemm(xnn_operator_t convolution_op,
  ...
  if (convolution_op->flags & XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER) {
    ...
    convolution_op->context.igemm.conv2d_igemm_indirection_init.input =
        convolution_op->input;
  } else {
    convolution_op->context.igemm.igemm.a_offset =
        (size_t)((uintptr_t)convolution_op->input -
                 (uintptr_t)convolution_op->last_input);
  }
  convolution_op->context.igemm.igemm.zero_size = convolution_op->zero_size;
  convolution_op->context.igemm.igemm.zero_buffers =
      convolution_op->zero_buffers;
  convolution_op->context.igemm.igemm.c = convolution_op->output;
  convolution_op->context.igemm.igemm.quantization_params =
...
static enum xnn_status setup_dwconv(xnn_operator_t convolution_op,
  ...
  if (convolution_op->flags & XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER) {
    ...
    convolution_op->context.dwconv.dwconv.indirect_input =
        (const void**)workspace;
    convolution_op->context.dwconv.dwconv_indirection_init.input =
        convolution_op->input;
    convolution_op->context.dwconv.dwconv_indirection_init.indirection_buffer =
        (const void**)workspace;
  } else {
    convolution_op->context.dwconv.dwconv.input_offset =
        (size_t)((uintptr_t)convolution_op->input -
                 (uintptr_t)convolution_op->last_input);
  }
