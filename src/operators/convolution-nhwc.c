...
#define OPERATOR_TYPE_CASE(...)            \
  XNN_CONCAT_IMPL(XNN_OPERATOR_TYPE_NAME_, \
                  XNN_COUNT_ARGS(__VA_ARGS__))(__VA_ARGS__)
...
static struct fingerprint_buffers generate_fingerprint_data(
  ...
  struct fingerprint_buffers data = {.data = buffer};
  data.kernel = buffer;
  buffer += kernel_size * kernel_element_size;
  buffer += XNN_ALLOCATION_ALIGNMENT - ((((uintptr_t)buffer - 1) % XNN_ALLOCATION_ALIGNMENT) + 1);
  data.bias = buffer;
  buffer += bias_size * context->gemm_config->bias_element_size;
  buffer += XNN_ALLOCATION_ALIGNMENT - ((((uintptr_t)buffer - 1) % XNN_ALLOCATION_ALIGNMENT) + 1);
  data.kernel_scale = (float*)buffer;
  buffer += kernel_scale_size * sizeof(float);
  buffer += XNN_ALLOCATION_ALIGNMENT - ((((uintptr_t)buffer - 1) % XNN_ALLOCATION_ALIGNMENT) + 1);
