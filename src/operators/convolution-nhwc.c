...
struct convolution2d_nhwc_context {
  ...
  size_t vmuladdc_params_size;
  void* vmuladdc_params_ptr;
  const struct xnn_dwconv_config* dwconv_ukernel;
  const struct xnn_vmulcaddc_config* vmulcaddc_config;
...
static enum xnn_status init_vmuladdc_params_f16(
  ...
  if XNN_LIKELY (vmulcaddc_config->init.f16 != NULL) {
    vmulcaddc_config->init.f16(&context->vmuladdc_params.f16,
                               context->fp16_output_min,
                               context->fp16_output_max);
    context->vmuladdc_params_size = sizeof(context->vmuladdc_params.f16);
    context->vmuladdc_params_ptr = &context->vmuladdc_params.f16;
  }
  return xnn_status_success;
}
...
static enum xnn_status init_vmuladdc_params_f32(
  ...
  if XNN_LIKELY (vmulcaddc_config->init.f32 != NULL) {
    vmulcaddc_config->init.f32(&context->vmuladdc_params.f32,
                               context->output_min, context->output_max);
    context->vmuladdc_params_size = sizeof(context->vmuladdc_params.f32);
    context->vmuladdc_params_ptr = &context->vmuladdc_params.f32;
  }
  return xnn_status_success;
}
...
enum xnn_status create_convolution2d_nhwc_helper(
  ...
  status = create_convolution2d_nhwc(
      context->input_padding_top, context->input_padding_right,
      context->input_padding_bottom, context->input_padding_left,
      context->kernel_height, context->kernel_width,
      context->subsampling_height, context->subsampling_width,
      context->dilation_height, context->dilation_width, context->groups,
      context->group_input_channels, context->group_output_channels,
      context->input_channel_stride, context->output_channel_stride,
      context->kernel, context->bias, context->flags,
      /*log2_input_element_size=*/context->gemm_config->log2_input_element_size,
      /*log2_filter_element_size=*/
      context->gemm_config->log2_filter_element_size,
      /*bias_element_size=*/context->gemm_config->bias_element_size,
      /*pack_vmulcaddc_w=*/context->pack_vmulcaddc_w,
      /*pack_dwconv_hwg_w=*/context->pack_dwconv_hwg_w,
      /*pack_dwconv_ghw_w=*/context->pack_dwconv_ghw_w,
      /*pack_conv_kgo_w=*/context->pack_conv_kgo_w,
      /*pack_conv_goki_w=*/context->pack_conv_goki_w,
      /*packing_params=*/context->packing_params_ptr,
      /*input_padding_byte=*/context->input_padding_bytes,
      /*extra_weights_bytes=*/variant->extra_weights_bytes,
      /*init_scale_params=*/variant->init_scale_params,
      /*scale_params=*/context->scale_params,
      /*init_kernel_scale_params=*/variant->init_kernel_scale_params,
      /*kernel_scale_params=*/context->kernel_scale_params,
      /*gemm_params=*/&context->gemm_params,
      /*gemm_params_size=*/context->gemm_params_size,
      /*dwconv_params=*/context->dwconv_params_ptr,
      /*dwconv_params_size=*/context->dwconv_params_size,
      /*vmulcaddc_params=*/context->vmuladdc_params_ptr,
      /*vmulcaddc_params_size=*/context->vmuladdc_params_size,
      /*gemm_config=*/context->gemm_config,
      /*dwconv_ukernel=*/context->dwconv_ukernel,
      /*vmulcaddc_config=*/context->vmulcaddc_config,
      /*linear_activation=*/false,
      /*relu_activation=*/false,
      /*operator_type=*/context->operator_type,
      /*dynamic_quantization=*/variant->dynamic_quantization,
      /*weights_cache=*/context->weights_cache, convolution_op_out);
