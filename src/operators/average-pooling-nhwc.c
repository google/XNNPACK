...
enum xnn_status create_average_pooling2d_nhwc(
    ...
    enum xnn_operator_type operator_type, xnn_operator_t average_pooling_op) {
...
enum xnn_status xnn_create_average_pooling2d_nhwc_f16(
  ...
  status = create_average_pooling2d_nhwc(
      input_padding_top, input_padding_right, input_padding_bottom,
      input_padding_left, pooling_height, pooling_width, stride_height,
      stride_width, output_min, output_max, flags,
      xnn_operator_type_average_pooling_nhwc_f16, average_pooling_op);
  ...
  const uint32_t pooling_size = pooling_height * pooling_width;
  avgpool_config->init.f16(
      &average_pooling_op->params.f16_scaleminmax,
      xnn_float16_from_float(1.0f / (float)(int32_t)pooling_size),
  ...
  if (any_padding || tf_same_padding) {
    // pavgpool does not include padding (zero) elements when calculating the
    // average.
...
enum xnn_status xnn_create_average_pooling2d_nhwc_f32(
  ...
  status = create_average_pooling2d_nhwc(
      ...
      input_padding_left, pooling_height, pooling_width, stride_height,
      stride_width, output_min, output_max, flags,
      xnn_operator_type_average_pooling_nhwc_f32, average_pooling_op);
  if (status != xnn_status_success) {
    goto error;
  }
  const struct xnn_avgpool_config* avgpool_config =
      xnn_init_f32_avgpool_config();
  status = xnn_status_unsupported_hardware;
  if (avgpool_config == NULL) {
    xnn_log_error(
        "failed to create %s operator: unsupported hardware configuration",
  ...
  avgpool_config->init.f32(&average_pooling_op->params.f32_scaleminmax,
                           1.0f / (float)(int32_t)pooling_size, output_min,
                           output_max);
  const bool tf_same_padding = (flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) != 0;
  const bool any_padding = (input_padding_left | input_padding_top |
                            input_padding_right | input_padding_bottom);
  if (any_padding || tf_same_padding) {
    // pavgpool does not include padding (zero) elements when calculating the
    // average.
    average_pooling_op->ukernel.type =
...
static enum xnn_status reshape_average_pooling2d(
    ...
    size_t output_pixel_stride, uint32_t log2_data_element_size,
    uint32_t log2_weight_element_size, uint32_t log2_accumulator_element_size,
    ...
    enum xnn_operator_type operator_type, bool is_pixelwise) {
  if (channels == 0) {
    xnn_log_error(
  ...
  if (input_pixel_stride < channels) {
    xnn_log_error(
        ...
        xnn_operator_type_to_string(operator_type), input_pixel_stride,
        channels);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_pixel_stride < channels) {
    xnn_log_error(
        ...
        "stride must be at least as large as the number of channels (%zu)",
        xnn_operator_type_to_string(operator_type), output_pixel_stride,
        channels);
    return xnn_status_invalid_parameter;
  }

  const size_t zero_bytes =
      (channels << log2_data_element_size) + XNN_EXTRA_BYTES;
  ...
  const bool input_size_changed =
      (input_height != last_input_height || input_width != last_input_width ||
       channels != last_input_channels);
  void* zero_buffer = average_pooling_op->zero_buffer;
  if (input_size_changed) {
    xnn_release_simd_memory(zero_buffer);
    zero_buffer = (void*)xnn_allocate_zero_simd_memory(zero_bytes);
    if (zero_buffer == NULL) {
  ...
  const bool tf_same_padding =
      (average_pooling_op->flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) != 0;
  if (tf_same_padding) {
    average_pooling_op->output_height =
        compute_output_dimension_with_tf_same_padding(
            input_height, average_pooling_op->stride_height);
    average_pooling_op->output_width =
        compute_output_dimension_with_tf_same_padding(
    ...
    average_pooling_op->padding_bottom =
        total_padding_height - average_pooling_op->padding_top;
    average_pooling_op->padding_right =
        total_padding_width - average_pooling_op->padding_left;
  } else {
    average_pooling_op->output_height =
        xnn_compute_convolution_output_dimension(
            average_pooling_op->padding_top + input_height +
                average_pooling_op->padding_bottom,
            average_pooling_op->kernel_height, 1,
            average_pooling_op->stride_height);
    average_pooling_op->output_width = xnn_compute_convolution_output_dimension(
        average_pooling_op->padding_left + input_width +
            average_pooling_op->padding_right,
        average_pooling_op->kernel_width, 1, average_pooling_op->stride_width);
  }
  ...
  const size_t step_width =
      min(average_pooling_op->stride_width, pooling_width);
  const size_t step_height =
      pooling_size + (output_width - 1) * step_width * pooling_height;

  const size_t indirect_top_height = divide_round_up(
      average_pooling_op->padding_top, average_pooling_op->stride_height);
  const size_t indirect_bot_height = divide_round_up(
      average_pooling_op->padding_bottom, average_pooling_op->stride_height);
  ...
  if (input_size_changed) {
    ...
    const size_t indirection_buffer_size =
        sizeof(void*) *
        ((pooling_size - 1) + indirection_buffer_output_height * step_height);

    const void** indirection_buffer = (const void**)xnn_reallocate_memory(
        average_pooling_op->indirection_buffer, indirection_buffer_size);
    ...
    average_pooling_op->input =
        (void*)((uintptr_t)average_pooling_op->zero_buffer +
                XNN_ALLOCATION_ALIGNMENT);
    average_pooling_op->last_input = average_pooling_op->input;
    xnn_indirection_init_dwconv2d_compressed(
        /*output_y_start=*/0,
        /*output_y_end=*/average_pooling_op->output_height,
        average_pooling_op->indirection_buffer, average_pooling_op->input,
        average_pooling_op->input_pixel_stride << log2_data_element_size,
        average_pooling_op->zero_buffer, average_pooling_op->input_height,
        average_pooling_op->input_width, average_pooling_op->output_height,
        average_pooling_op->output_width, average_pooling_op->kernel_height,
        average_pooling_op->kernel_width, average_pooling_op->stride_height,
        average_pooling_op->stride_width, average_pooling_op->dilation_height,
        average_pooling_op->dilation_width, average_pooling_op->padding_top,
        average_pooling_op->padding_left, step_height, step_width,
  ...
  const size_t output_width_stride = average_pooling_op->output_pixel_stride
  ...
  if (is_pixelwise) {
    ...
    average_pooling_op->ukernel.subtype =
        xnn_microkernel_type_pixelwise_average_pooling;

    if (input_size_changed) {
      const size_t pixelwise_buffer_size = (output_height * output_width)
                                           << log2_weight_element_size;
      void* pixelwise_buffer = xnn_reallocate_memory(
          average_pooling_op->pixelwise_buffer, pixelwise_buffer_size);
  ...
  average_pooling_op->context
      .average_pooling = (struct average_pooling_context){
      ...
      .indirect_bot_start =
          average_pooling_op->output_height - indirect_bot_height,
      .input_batch_stride =
          input_height * input_width * average_pooling_op->input_pixel_stride
          << log2_data_element_size,
      .input_y_stride = average_pooling_op->stride_height * input_width *
                            average_pooling_op->input_pixel_stride
                        << log2_data_element_size,
  ...
  memcpy(&average_pooling_op->context.average_pooling.params, params,
         params_size);
  ...
  average_pooling_op->compute[0].task_2d =
      (pthreadpool_task_2d_t)xnn_compute_average_pooling;
...
enum xnn_status xnn_reshape_average_pooling2d_nhwc_f16(
  ...
  size_t pooling_params_size =
      sizeof(average_pooling_op->params.f16_scaleminmax);
  const bool is_pixelwise = average_pooling_op->ukernel.type ==
                            xnn_microkernel_type_pixelwise_average_pooling;

  return reshape_average_pooling2d(
      average_pooling_op, batch_size, input_height, input_width, channels,
      ...
      average_pooling_op->avgpool_config, pooling_params, pooling_params_size,
      output_height_out, output_width_out, threadpool,
      xnn_operator_type_average_pooling_nhwc_f16, is_pixelwise);
}
...
enum xnn_status xnn_reshape_average_pooling2d_nhwc_f32(
  ...
  return reshape_average_pooling2d(
      ...
      average_pooling_op->avgpool_config, pooling_params, pooling_params_size,
      output_height_out, output_width_out, threadpool,
      xnn_operator_type_average_pooling_nhwc_f32, is_pixelwise);
}
...
enum xnn_status xnn_setup_average_pooling2d_nhwc_f16(
  ...
  return setup_average_pooling2d(average_pooling_op, input, output);
}
