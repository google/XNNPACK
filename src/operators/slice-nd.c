...
static enum xnn_status create_slice_nd(uint32_t flags,
...
static enum xnn_status reshape_slice_nd(
    xnn_operator_t slice_op, enum xnn_operator_type expected_operator_type,
    size_t num_dims, const size_t* input_shape, const size_t* offsets,
    const size_t* sizes, uint32_t log2_element_size, pthreadpool_t threadpool) {
  ...
  const struct xnn_unary_elementwise_config* copy_config =
      slice_op->unary_elementwise_config;
  ...
  xnn_normalize_slice(num_dims, offsets, sizes, input_shape, normalized_offsets,
                      normalized_input_shape, normalized_output_shape,
                      &num_normalized_dims);
  ...
  // TODO(b/246969669): move strides calculation into normalization to simplify
  // code here.
  ...
  for (size_t i = 1; i < XNN_MAX_TENSOR_DIMS; i++) {
    slice_op->context.slice.input_stride[i - 1] = input_stride
                                                  << log2_element_size;
    slice_op->context.slice.output_stride[i - 1] = output_stride
                                                   << log2_element_size;
  ...
  slice_op->context.slice.contiguous_size =
      normalized_output_shape[XNN_MAX_TENSOR_DIMS - 1] << log2_element_size;

  switch (num_normalized_dims) {
    case 1:
    case 2:
      slice_op->compute[0].type = xnn_parallelization_type_1d;
      slice_op->compute[0].task_1d =
          (pthreadpool_task_1d_t)xnn_compute_slice_1d;
      slice_op->compute[0].range[0] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 2];
      break;
    case 3:
      slice_op->compute[0].type = xnn_parallelization_type_2d;
      slice_op->compute[0].task_2d =
          (pthreadpool_task_2d_t)xnn_compute_slice_2d;
      slice_op->compute[0].range[0] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 3];
      slice_op->compute[0].range[1] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 2];
      break;
    case 4:
      slice_op->compute[0].type = xnn_parallelization_type_3d;
      slice_op->compute[0].task_3d =
          (pthreadpool_task_3d_t)xnn_compute_slice_3d;
      slice_op->compute[0].range[0] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 4];
      slice_op->compute[0].range[1] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 3];
      slice_op->compute[0].range[2] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 2];
      break;
    case 5:
      slice_op->compute[0].type = xnn_parallelization_type_4d;
      slice_op->compute[0].task_4d =
          (pthreadpool_task_4d_t)xnn_compute_slice_4d;
      slice_op->compute[0].range[0] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 5];
      slice_op->compute[0].range[1] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 4];
      slice_op->compute[0].range[2] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 3];
      slice_op->compute[0].range[3] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 2];
      break;
    case 6:
      // TODO(b/246969669): write normalized_output_shape in reverse order to
      // simplify code here.
      slice_op->compute[0].type = xnn_parallelization_type_5d;
      slice_op->compute[0].task_5d =
          (pthreadpool_task_5d_t)xnn_compute_slice_5d;
      slice_op->compute[0].range[0] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 6];
      slice_op->compute[0].range[1] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 5];
      slice_op->compute[0].range[2] =
          normalized_output_shape[XNN_MAX_TENSOR_DIMS - 4];
...
enum xnn_status xnn_reshape_slice_nd_x8(
  ...
  return reshape_slice_nd(slice_op, xnn_operator_type_slice_nd_x8, num_dims,
                          input_shape, offsets, sizes,
                          0 /* log2(element size) */, threadpool);
}

enum xnn_status xnn_reshape_slice_nd_x16(
    xnn_operator_t slice_op, size_t num_dims, const size_t* input_shape,
    const size_t* offsets, const size_t* sizes, pthreadpool_t threadpool) {
  return reshape_slice_nd(slice_op, xnn_operator_type_slice_nd_x16, num_dims,
                          input_shape, offsets, sizes,
                          1 /* log2(element size) */, threadpool);
}

enum xnn_status xnn_reshape_slice_nd_x32(
    xnn_operator_t slice_op, size_t num_dims, const size_t* input_shape,
    const size_t* offsets, const size_t* sizes, pthreadpool_t threadpool) {
  return reshape_slice_nd(slice_op, xnn_operator_type_slice_nd_x32, num_dims,
                          input_shape, offsets, sizes,
                          2 /* log2(element size) */, threadpool);
}
...
static enum xnn_status setup_slice_nd(
  ...
  for (size_t i = 1; i < slice_op->context.slice.num_normalized_dims; i++) {
    slice_op->context.slice.input =
        (void*)((uintptr_t)slice_op->context.slice.input +
                slice_op->context.slice.offsets[i] *
                    slice_op->context.slice.input_stride[i - 1]);
  }
...
enum xnn_status xnn_setup_slice_nd_x8(xnn_operator_t slice_op,
                                      const void* input, void* output) {
  return setup_slice_nd(slice_op, xnn_operator_type_slice_nd_x8, input, output);
}

enum xnn_status xnn_setup_slice_nd_x16(xnn_operator_t slice_op,
                                       const void* input, void* output) {
  return setup_slice_nd(slice_op, xnn_operator_type_slice_nd_x16, input,
                        output);
}

enum xnn_status xnn_setup_slice_nd_x32(xnn_operator_t slice_op,
                                       const void* input, void* output) {
  return setup_slice_nd(slice_op, xnn_operator_type_slice_nd_x32, input,
                        output);
}

static enum xnn_status xnn_run_slice_nd(
    enum xnn_operator_type operator_type, size_t num_dims,
    const size_t* input_shape, const size_t* offsets, const size_t* sizes,
    const void* input, void* output, uint32_t log2_element_size, uint32_t flags,
  ...
  enum xnn_status status =
      reshape_slice_nd(&slice_op, operator_type, num_dims, input_shape, offsets,
                       sizes, log2_element_size, threadpool);

  if (status != xnn_status_success) {
    return status;
  }

  status = setup_slice_nd(&slice_op, operator_type, input, output);
...
enum xnn_status xnn_run_slice_nd_x32(size_t num_dims, const size_t* input_shape,
  ...
  return xnn_run_slice_nd(xnn_operator_type_slice_nd_x32, num_dims, input_shape,
                          offsets, sizes, input, output,
                          2 /* log2(element size) */, flags, threadpool);
}
