...
static enum xnn_status init_lut_op(
  ...
  op->lookup_table =
      xnn_allocate_simd_memory(lookup_table_elements * sizeof(uint8_t));
  ...
  if (reference_config->init) {
    reference_config->init(&uparams, params, input_quantization,
                           output_quantization);
  }
  ...
  reference_config->ukernel(lookup_table_elements, lookup_table, lookup_table,
                            &uparams);
    ...
    if (op_type == xnn_unary_convert) {
      if (input_datatype == xnn_datatype_fp32 &&
        ...
        return xnn_init_f32_to_qu8_cvt_config();
                 ...
                 output_datatype == xnn_datatype_qpint8) {
      ...
      } else if (input_datatype == xnn_datatype_quint8 &&
                 output_datatype == xnn_datatype_fp32) {
    ...
    switch (op_type) {
      case xnn_unary_clamp:
        if (input_quantization->scale != output_quantization->scale ||
          ...
          xnn_log_debug(
              "unsupported operator clamp for datatype QINT8: quantization "
              "parameters differ");
          return NULL;
        }
    ...
    switch (op_type) {
      case xnn_unary_clamp:
        if (input_quantization->scale != output_quantization->scale ||
            input_quantization->zero_point != output_quantization->zero_point) {
          xnn_log_debug(
              "unsupported operator clamp for datatype QUINT8: quantization "
              "parameters differ");
          return NULL;
        }
...
static enum xnn_status init_op(
    xnn_operator_t op, enum xnn_unary_operator op_type,
    enum xnn_datatype input_datatype, enum xnn_datatype output_datatype,
    const union xnn_unary_params* params,
    const struct xnn_quantization_params* input_quantization,
    const struct xnn_quantization_params* output_quantization, uint32_t flags) {
  op->type = xnn_operator_type_unary_elementwise;
  op->flags = flags;
  op->unary_elementwise.log2_input_size =
      xnn_datatype_log2_size_bytes(input_datatype);
  op->unary_elementwise.log2_output_size =
      xnn_datatype_log2_size_bytes(output_datatype);
  ...
  if (config && config->ukernel) {
    ...
    if (config->init != NULL) {
      config->init(&op->params.unary, params, input_quantization,
                   output_quantization);
    }
    return xnn_status_success;
  }
  ...
  config =
      xnn_init_unary_reference_config(op_type, input_datatype, output_datatype);
  if (config) {
    if (xnn_datatype_size_bytes(input_datatype) == 1 &&
        xnn_datatype_size_bytes(output_datatype) == 1) {
      // We can use a LUT for this op.
      return init_lut_op(op, config, params, input_quantization,
                         output_quantization);
    }

    xnn_log_debug(
        "unsupported operator %s for datatypes %s -> %s, falling back to "
        "reference kernel",
    ...
    xnn_log_error("unsupported operator %s for datatypes %s -> %s",
                  xnn_unary_operator_to_string(op_type),
                  xnn_datatype_to_string(input_datatype),
                  xnn_datatype_to_string(output_datatype));
...
enum xnn_status xnn_create_unary_elementwise_nc(
  ...
  enum xnn_status status =
      init_op(op, op_type, input_datatype, output_datatype, params,
              input_quantization, output_quantization, flags);
...
static bool is_contiguous(xnn_operator_t op) {
  ...
  return (((input_stride ^ channels) | (output_stride ^ channels)) == 0) ||
         batch_size == 1;
}
...
enum xnn_status xnn_reshape_unary_elementwise_nc(
  ...
  if (op->lookup_table) {
    ...
    if (is_contiguous(op)) {
      ...
      op->compute[0].task_1d_tile_1d =
          (pthreadpool_task_1d_tile_1d_t)xnn_compute_lut_contiguous;
    ...
    if (is_contiguous(op)) {
      ...
      memcpy(&op->context.univector_contiguous.params, &op->params.unary,
             sizeof(op->params.unary));

      const size_t range = (batch_size * channels)
                           << op->unary_elementwise.log2_input_size;
      op->compute[0].type = xnn_parallelization_type_1d_tile_1d;
      op->compute[0].task_1d_tile_1d =
      ...
      memcpy(&op->context.univector_strided.params, &op->params.unary,
             sizeof(op->params.unary));

      op->compute[0].type = xnn_parallelization_type_1d_tile_1d;
      op->compute[0].task_1d_tile_1d =
          (pthreadpool_task_1d_tile_1d_t)xnn_compute_univector_strided;
      op->compute[0].range[0] = batch_size;
...
static enum xnn_status create_unary_elementwise_nc(
  ...
  init_unary_elementwise_nc(flags, params, params_size, operator_type,
                            unary_elementwise_config, unary_elementwise_op);
...
static enum xnn_status reshape_unary_elementwise_nc(
    ...
    uint32_t log2_input_size, uint32_t log2_output_size, const void* params,
    size_t params_size, pthreadpool_t threadpool) {
...
static enum xnn_status setup_unary_elementwise_nc(
  ...
  if ((((input_stride ^ channels) | (output_stride ^ channels)) == 0) ||
      unary_elementwise_op->batch_size == 1) {
...
enum xnn_status create_convert_nc_f16_qx8(
  ...
  const struct xnn_reduce_config* f16_rminmax_config =
      xnn_init_f16_rminmax_config();
  ...
  enum xnn_status status =
      create_unary_elementwise_nc(flags, cvt_config, &params, sizeof(params),
...
enum xnn_status xnn_create_convert_nc_f16_qd8(uint32_t flags,
                                              xnn_operator_t* convert_op_out) {
  return create_convert_nc_f16_qx8(flags, xnn_init_f16_to_qs8_cvt_config(),
...
enum xnn_status xnn_create_convert_nc_f16_qdu8(uint32_t flags,
                                               xnn_operator_t* convert_op_out) {
  return create_convert_nc_f16_qx8(flags, xnn_init_f16_to_qu8_cvt_config(),
                                   xnn_operator_type_convert_nc_f16_qdu8,
                                   convert_op_out);
}

enum xnn_status xnn_create_convert_nc_f32_qd8(uint32_t flags,
                                              xnn_operator_t* convert_op_out) {
  return create_convert_nc_f32_qx8(flags, xnn_init_f32_to_qs8_cvt_config(),
...
enum xnn_status xnn_create_convert_nc_f32_qdu8(uint32_t flags,
                                               xnn_operator_t* convert_op_out) {
  return create_convert_nc_f32_qx8(flags, xnn_init_f32_to_qu8_cvt_config(),
                                   xnn_operator_type_convert_nc_f32_qdu8,
                                   convert_op_out);
}
...
enum xnn_status xnn_create_convert_nc_f32_qp8(
  ...
  enum xnn_status status = create_unary_elementwise_nc(
      flags, xnn_init_f32_to_qp8_cvt_config(), &params, sizeof(params),
...
enum xnn_status xnn_create_copy_nc_x8(uint32_t flags,
  ...
  return create_unary_elementwise_nc(flags, xnn_init_xx_copy_config(),
...
enum xnn_status reshape_convert_nc_f16_qx8(xnn_operator_t convert_op,
  ...
  memcpy(&convert_op->context.f16_qd8_convert.params,
         &convert_op->params.f16_default,
         sizeof(convert_op->params.f16_default));

  convert_op->compute[0].type = xnn_parallelization_type_1d;
  convert_op->compute[0].task_1d =
      (pthreadpool_task_1d_t)xnn_compute_f16_qd8_convert;
  switch (expected_type) {
    ...
    case xnn_operator_type_convert_nc_f16_qdu8:
      convert_op->compute[0].task_1d =
          (pthreadpool_task_1d_t)xnn_compute_f16_qdu8_convert;
      break;
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status reshape_convert_nc_f32_qx8(xnn_operator_t convert_op,
                                           size_t batch_size, size_t channels,
                                           size_t input_stride,
                                           size_t output_stride,
                                           enum xnn_operator_type expected_type,
  ...
  memcpy(&convert_op->context.f32_qd8_convert.params,
         &convert_op->params.f32_default,
         sizeof(convert_op->params.f32_default));

  convert_op->compute[0].type = xnn_parallelization_type_1d;
  switch (expected_type) {
    case xnn_operator_type_convert_nc_f32_qd8:
      convert_op->compute[0].task_1d =
          (pthreadpool_task_1d_t)xnn_compute_f32_qd8_convert;
      break;
    case xnn_operator_type_convert_nc_f32_qdu8:
      convert_op->compute[0].task_1d =
          (pthreadpool_task_1d_t)xnn_compute_f32_qdu8_convert;
      break;
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status xnn_reshape_convert_nc_f16_qd8(
  ...
  return reshape_convert_nc_f16_qx8(
      convert_op, batch_size, channels, input_stride, output_stride,
      xnn_operator_type_convert_nc_f16_qd8, threadpool);
}
...
enum xnn_status xnn_reshape_convert_nc_f16_qdu8(
  ...
  return reshape_convert_nc_f16_qx8(
      convert_op, batch_size, channels, input_stride, output_stride,
      xnn_operator_type_convert_nc_f16_qdu8, threadpool);
}

enum xnn_status xnn_reshape_convert_nc_f32_qd8(
    xnn_operator_t convert_op, size_t batch_size, size_t channels,
    size_t input_stride, size_t output_stride, pthreadpool_t threadpool) {
  return reshape_convert_nc_f32_qx8(
      convert_op, batch_size, channels, input_stride, output_stride,
      xnn_operator_type_convert_nc_f32_qd8, threadpool);
}
...
enum xnn_status xnn_reshape_convert_nc_f32_qdu8(
  ...
  return reshape_convert_nc_f32_qx8(
      convert_op, batch_size, channels, input_stride, output_stride,
      xnn_operator_type_convert_nc_f32_qdu8, threadpool);
}
...
enum xnn_status xnn_reshape_copy_nc_x8(xnn_operator_t copy_op,
  ...
  return reshape_unary_elementwise_nc(
      copy_op, xnn_operator_type_copy_nc_x8, batch_size, channels, input_stride,
      output_stride,
      /*log2_input_size=*/XNN_LOG2_SIZEOF_UINT8_T,
      /*log2_output_size=*/XNN_LOG2_SIZEOF_UINT8_T,
      /*params=*/NULL, /*params_size=*/0, threadpool);
}
...
enum xnn_status xnn_reshape_copy_nc_x16(xnn_operator_t copy_op,
                                        ...
                                        pthreadpool_t threadpool) {
  return reshape_unary_elementwise_nc(
      copy_op, xnn_operator_type_copy_nc_x16, batch_size, channels,
...
enum xnn_status xnn_reshape_copy_nc_x32(xnn_operator_t copy_op,
                                        ...
                                        pthreadpool_t threadpool) {
  return reshape_unary_elementwise_nc(
      copy_op, xnn_operator_type_copy_nc_x32, batch_size, channels,
      input_stride, output_stride,
...
enum xnn_status setup_convert_nc_f16_qx8(
    xnn_operator_t convert_op, const void* input, void* output,
    enum xnn_operator_type expected_operator_type,
  ...
  convert_op->context.f16_qd8_convert.quantization_params =
...
enum xnn_status setup_convert_nc_f32_qx8(
    xnn_operator_t convert_op, const float* input, void* output,
    enum xnn_operator_type expected_operator_type,
    struct xnn_quantization_params* quantization_params) {
  if (convert_op->type != expected_operator_type) {
    xnn_log_error(
...
enum xnn_status xnn_setup_convert_nc_f16_qd8(
    ...
    struct xnn_quantization_params* quantization_params) {
  return setup_convert_nc_f16_qx8(convert_op, input, output,
                                  xnn_operator_type_convert_nc_f16_qd8,
                                  quantization_params);
}
...
enum xnn_status xnn_setup_convert_nc_f32_qd8(
    ...
    struct xnn_quantization_params* quantization_params) {
  return setup_convert_nc_f32_qx8(convert_op, input, output,
                                  xnn_operator_type_convert_nc_f32_qd8,
                                  quantization_params);
}
...
enum xnn_status xnn_setup_copy_nc_x8(xnn_operator_t copy_op, const void* input,
                                     void* output) {
  return setup_unary_elementwise_nc(copy_op, xnn_operator_type_copy_nc_x8,
                                    input, output);
}

enum xnn_status xnn_setup_copy_nc_x16(xnn_operator_t copy_op, const void* input,
                                      void* output) {
  return setup_unary_elementwise_nc(copy_op, xnn_operator_type_copy_nc_x16,
                                    input, output);
}

enum xnn_status xnn_setup_copy_nc_x32(xnn_operator_t copy_op, const void* input,
                                      void* output) {
  return setup_unary_elementwise_nc(copy_op, xnn_operator_type_copy_nc_x32,
                                    input, output);
}

static enum xnn_status run_unary_elementwise_nc(
    enum xnn_operator_type operator_type, size_t channels, size_t input_stride,
    size_t output_stride, size_t batch_size, const void* input, void* output,
    const struct xnn_unary_elementwise_config* unary_elementwise_config,
    const void* params, size_t params_size, uint32_t log2_input_size,
    uint32_t log2_output_size, uint32_t flags, pthreadpool_t threadpool) {
  if (unary_elementwise_config == NULL) {
    xnn_log_error(
        "failed to create %s operator: unsupported hardware configuration",
        xnn_operator_type_to_string(operator_type));
  ...
  if (channels == 0) {
    xnn_log_error(
        "failed to run %s operator with %zu channels: number of channels must "
        "be non-zero",
  ...
  init_unary_elementwise_nc(flags, /*params=*/NULL, /*params_size=*/0,
                            operator_type, unary_elementwise_config,
                            &unary_elementwise_op);

  enum xnn_status status = reshape_unary_elementwise_nc(
      &unary_elementwise_op, operator_type, batch_size, channels, input_stride,
      output_stride, log2_input_size, log2_output_size, params, params_size,
      threadpool);
  if (status != xnn_status_success) {
    return status;
  }

  status = setup_unary_elementwise_nc(&unary_elementwise_op, operator_type,
                                      input, output);
  if (status != xnn_status_success) {
    return status;
  }
...
enum xnn_status xnn_run_copy_nc_x32(size_t channels, size_t input_stride,
  ...
  return run_unary_elementwise_nc(
      xnn_operator_type_copy_nc_x32, channels, input_stride, output_stride,
      batch_size, input, output, xnn_init_xx_copy_config(), NULL, 0,
      /*log2_input_size=*/XNN_LOG2_SIZEOF_UINT32_T,
      /*log2_output_size=*/XNN_LOG2_SIZEOF_UINT32_T, flags, threadpool);
}

// This is an implementation of a deprecated API used by an external project.
// After it is updated, this can be removed.
enum xnn_status xnn_run_convert_nc_f32_f16(size_t channels, size_t input_stride,
                                           size_t output_stride,
                                           size_t batch_size,
