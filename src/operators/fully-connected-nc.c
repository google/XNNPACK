...
static enum xnn_status create_fully_connected_nc(
    ...
    xnn_init_extra_data_params_fn init_extra_data0,
    const void* extra_data0,
    size_t extra_data0_size,
    xnn_init_extra_data_params_fn init_extra_data1,
    const void* extra_data1,
    size_t extra_data1_size,
    const void* params,
    size_t params_size,
  ...
  if (cache_offset == XNN_CACHE_NOT_FOUND) {
    ...
    if (gemm_config->pack_weights_and_biases) {
      gemm_config->pack_weights_and_biases(
          ...
          /*weights=*/kernel,
          /*int_extra_data0_fn=*/init_extra_data0,
          /*extra_data0=*/extra_data0,
          /*extra_data0_size=*/extra_data0_size,
          /*init_extra_data1_fn=*/init_extra_data1,
          /*extra_data1=*/extra_data1,
          /*extra_data1_size=*/extra_data1_size,
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  return create_fully_connected_nc(
    ...
    /*init_extra_data0=*/NULL, 
    /*extra_data0=*/NULL,
    /*extra_data0_size=*/0,
    /*init_extra_data1=*/NULL, 
    /*extra_data1=*/NULL,
    /*extra_data1_size=*/0,
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  return create_fully_connected_nc(
    ...
    /*extra_data0=*/bias,
    /*extra_data_size=*/sizeof(float),
    /*init_extra_data1=*/(xnn_init_extra_data_params_fn)xnn_init_qs8_qc8w_scale_fp32_params,
    ...
    xnn_operator_t* fully_connected_op_out)
{
  if (isnan(output_min)) {
    xnn_log_error(
      "failed to create %s operator with NaN output lower bound: lower bound must be non-NaN",
      xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w));
    return xnn_status_invalid_parameter;
  }
  ...
  if (block_size < XNN_MIN_BLOCKSIZE || block_size % XNN_MIN_BLOCKSIZE != 0) {
    xnn_log_error(
        "failed to create %s operator with block_size: %zu: expecting block_size to be a multiple of %d.",
        xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w), block_size, XNN_MIN_BLOCKSIZE);
    return xnn_status_invalid_parameter;
  }
  ...
  if (input_channels % block_size != 0) {
    xnn_log_error(
      "failed to create %s operator with input_channels: %zu, and block_size: %zu: expecting input_channels %% block_size == 0.",
      xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w), input_channels, block_size);
    return xnn_status_invalid_parameter;
  }
  ...
  if (kernel_zero_point != 8) {
    xnn_log_error(
      "failed to create %s operator with %" PRIu8 " kernel zero point: kernel zero point must be equal to 8 "
      "(unsigned weights) or 0 (signed weights)",
      xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w), kernel_zero_point);
    return xnn_status_invalid_parameter;
  }

  const struct xnn_gemm_config* gemm_config = xnn_init_qd8_bf16_qb4w_gemm_config();
  if (gemm_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                  xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w));
    return xnn_status_unsupported_hardware;
  ...
  return create_fully_connected_nc(
    ...
    /*packing_params=*/&packing_params,
    /*extra_weights_bytes=*/sizeof(float),
    /*init_extra_data0=*/(xnn_init_extra_data_params_fn)xnn_init_qs8_qc8w_scale_u16_params, 
    /*extra_data0=*/bias,
    /*extra_data0_size=*/sizeof(uint16_t),
    /*init_extra_data1=*/(xnn_init_extra_data_params_fn)xnn_init_blockwise_scale_bf16_params, 
    /*extra_data1=*/kernel_scale,
    /*extra_data1_size=*/sizeof(uint16_t),
    &params, sizeof(params),
    gemm_config, &gemm_config->minmax,
    xnn_operator_type_fully_connected_nc_qd8_bf16_qb4w,
    /*weights_cache=*/weights_cache,
    fully_connected_op_out);
}
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  return create_fully_connected_nc_qx8_f32_qc4w(input_channels, output_channels,
                                                ...
                                                code_cache, weights_cache, gemm_config, xnn_operator_type_fully_connected_nc_qd8_f32_qc4w,
                                                fully_connected_op_out);
}
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  return create_fully_connected_nc_qx8_f32_qc4w(input_channels, output_channels,
                                                input_stride, output_stride, kernel_zero_point,
                                                kernel_scale, kernel, bias, output_min, output_max, flags,
                                                code_cache, weights_cache, gemm_config, xnn_operator_type_fully_connected_nc_qdu8_f32_qc4w,
                                                fully_connected_op_out);
}
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  return create_fully_connected_nc(
    ...
    &packing_params,
    /*extra_weights_bytes=*/sizeof(float),
    /*init_extra_data0=*/(xnn_init_extra_data_params_fn)xnn_init_qs8_to_qs8_qc8w_scale_fp32_params,
    /*extra_data0=*/&requantization_scale,
    /*extra_data0_size=*/sizeof(float),
    /*init_extra_data1=*/ NULL,
    /*extra_data1=*/ NULL,
    /*extra_data1_size=*/0,
