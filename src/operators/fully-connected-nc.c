    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  if (cache_offset == XNN_CACHE_NOT_FOUND) {
    ...
    if (weights_ptr == NULL) {
      xnn_log_error(
        "failed to allocate %zu bytes for %s operator packed weights",
        packed_weights_size, xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_f16_qb4w));
      goto error;
    }
    xnn_log_debug("allocated %zu bytes for packed weights in %s operator",
      aligned_total_weights_size, xnn_operator_type_to_string(xnn_operator_type_fully_connected_nc_qd8_f16_qb4w));
  ...
  pthreadpool_t threadpool)
{
  ...
  fully_connected_op->context.gemm = (struct gemm_context){
      ...
      .a_stride = is_qp8_ukernel ? xnn_x8_packq_f32qp8_packed_offset(
                                       mr, input_channels, mr,
                                       fully_connected_op->ukernel.gemm.kr,
                                       fully_connected_op->ukernel.gemm.sr)
