...
static enum xnn_status select_convolution2d_ukernel(
  ...
  if (  // 1x1 convolution (no groups).
      is_1x1 && !any_padding && !nhwc_input && groups == 1) {
    *ukernel_type = xnn_microkernel_type_spmm;
  } else if (  // 3x3 stride-2 with 3 input channels and NHWC input layout.
      is_3x3 && subsampling_height == 2 && subsampling_width == 2 &&
      input_padding_top == 1 && input_padding_left == 1 &&
      input_padding_bottom == 1 && input_padding_right == 1 && nhwc_input &&
      groups == 1) {
