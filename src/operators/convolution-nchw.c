...
static enum xnn_status create_spmm_path(
    ...
    const uint32_t groups, const size_t group_input_channels,
    const size_t group_output_channels, const void* kernel, const void* bias,
  ...
  xnn_analyze_spmm(group_output_channels, group_input_channels, kernel,
                   &spmm_packing_params);
  ...
  if (num_block4_nonzeroes * 5 >= num_nonzero_blocks4 * 18 &&
      spmm4_config != NULL && spmm4_config->ukernel != NULL) {
    // 4-channel blocks have 90%+ non-zeroes

    output_channels_block_size = 4;
    num_output_channel_blocks =
        num_output_channel_blocks / 4 + num_output_channel_blocks % 4;
    spmm_config = spmm4_config;
    // Non-zeroes which don't fit into whole 4-channel blocks, processed
    // one-by-one
  ...
  } else if (num_block2_nonzeroes * 5 >= num_nonzero_blocks2 * 9 &&
             spmm2_config != NULL && spmm2_config->ukernel != NULL) {
    // 2-channel blocks have 90%+ non-zeroes

    output_channels_block_size = 2;
    num_output_channel_blocks =
        num_output_channel_blocks / 2 + num_output_channel_blocks % 2;
  ...
  // 1. An array of int32_t values storing scaled [by sizeof(input element)]
  // difference between input channels
  //    corresponding to successive non-zero blocks.  Used by setup to compute
  //    (array 2).
  // 2. An array of int32_t values storing increment for input pointer after
  // each processed tile. This array is
  //    derived from scaled difference in array 1 using parameters to setup
  //    function.
  // 3. An array of uint32_t values storing the number of non-zero kernel
  // elements per each output channel.
  // 4. An array of float or fp16 values storing all bias elements
  // (group_output_channels) and non-zero kernel elements.
  ...
  const size_t packed_weights_size =
      ...
      ((group_output_channels + num_nonzero_values)
       << log2_filter_element_size) +
      XNN_EXTRA_BYTES;

  convolution_op->packed_weights.pointer =
  ...
  int32_t* input_channel_diffs =
      (int32_t*)convolution_op->packed_weights.pointer;
  int32_t* input_increments =
      (int32_t*)(input_channel_diffs + num_nonzero_blocks);
  uint32_t* output_channel_nonzeros =
      (uint32_t*)(input_increments + num_nonzero_blocks);
  void* nonzero_values =
      (void*)(output_channel_nonzeros + num_output_channel_blocks);

  memset(output_channel_nonzeros, 0,
         num_output_channel_blocks * sizeof(uint32_t));

  size_t first_ic = 0;
  enum xnn_status status =
      xnn_pack_spmm(group_output_channels, output_channels_block_size,
                    group_input_channels, kernel, bias, input_channel_diffs,
                    output_channel_nonzeros, nonzero_values, &first_ic);
...
static enum xnn_status create_conv2d_hwc2chw_path(
    const uint32_t kernel_height, const uint32_t kernel_width,
    const uint32_t groups, const size_t group_input_channels,
    const size_t group_output_channels, const size_t output_height_tile,
    const size_t output_channel_tile, const void* kernel, const void* bias,
    const uint32_t log2_filter_element_size,
  ...
  const size_t packed_group_output_channels =
      round_up(group_output_channels, output_channel_tile);
  const size_t packed_weights_size =
      (groups * packed_group_output_channels *
       (group_input_channels * kernel_height * kernel_width + 1 /* bias */))
      << log2_filter_element_size;
  ...
  if (weights_ptr == NULL) {
    xnn_log_error(
        "failed to reserve or allocate %zu bytes for %s operator "
        "conv2d_hwc2chw packed weights",
  ...
  xnn_pack_dconv_oki_w(group_output_channels, group_input_channels,
                       output_channel_tile, kernel_height, kernel_width, kernel,
                       bias, weights_ptr, NULL);

  if (use_weights_cache(convolution_op)) {
    struct xnn_weights_cache_look_up_key cache_key;
    cache_key.seed =
        group_input_channels ^ group_output_channels ^ output_channel_tile;
...
static enum xnn_status create_dwconv_path(
    const uint32_t kernel_height, const uint32_t kernel_width,
    const uint32_t groups, const void* kernel, const void* bias,
    const uint32_t flags, const uint32_t log2_filter_element_size,
  ...
  const size_t packed_weights_size =
      (groups * (kernel_height * kernel_width + 1 /* bias */))
      << log2_filter_element_size;
  const size_t aligned_total_weights_size =
      round_up_po2(packed_weights_size, XNN_ALLOCATION_ALIGNMENT);
  void* weights_ptr = xnn_get_pointer_to_write_weights(
  ...
  if (flags & XNN_FLAG_DEPTHWISE_CONVOLUTION) {
    pack_chw_dwconv_hwg_w(kernel_height * kernel_width, groups, kernel, bias,
                          weights_ptr, NULL);
  } else {
    cache_seed = ~cache_seed;
    pack_chw_dwconv_ghw_w(kernel_height * kernel_width, groups, kernel, bias,
                          weights_ptr, NULL);
  }
...
enum xnn_status xnn_create_convolution2d_nchw_f16(
  ...
  // + 3x3 stride-2 depthwise convolution with horizontal padding 1 & no
  // vertical padding
  // + 3x3 stride-1 depthwise convolution with horizontal padding 1 & no
  // vertical padding
  // + 5x5 stride-2 depthwise convolution with horizontal padding 2 & no
  // vertical padding
  // + 5x5 stride-1 depthwise convolution with horizontal padding 2 & no
  // vertical padding
  ...
  switch (ukernel_type) {
    case xnn_microkernel_type_spmm: {
      ...
      spmm_config->init.f16(&convolution_op->params.f16_minmax, fp16_output_min,
                            fp16_output_max);

      status = create_spmm_path(kernel_height, kernel_width, groups,
                                group_input_channels, group_output_channels,
                                kernel, bias, log2_filter_element_size,
                                xnn_analyze_spmm, xnn_pack_spmm, spmm_config,
                                NULL, NULL, operator_type, convolution_op);
      if (status != xnn_status_success) {
        goto error;
      }
      break;
    ...
    case xnn_microkernel_type_conv2d_hwc2chw: {
      ...
      xnn_pack_dconv_oki_w_fn xnn_pack_dconv_oki_w =
          (xnn_pack_dconv_oki_w_fn)xnn_pack_f16_dconv_oki_w;
      if (flags & XNN_FLAG_FP32_STATIC_WEIGHTS) {
...
enum xnn_status xnn_create_convolution2d_nchw_f32(
  ...
  switch (ukernel_type) {
    case xnn_microkernel_type_spmm: {
      const xnn_analyze_spmm_w_fn xnn_analyze_spmm =
          (xnn_analyze_spmm_w_fn)xnn_analyze_f32_spmm_w;
      const xnn_pack_spmm_w_fn xnn_pack_spmm =
          (xnn_pack_spmm_w_fn)xnn_pack_f32_spmm_w;
    ...
    case xnn_microkernel_type_conv2d_hwc2chw: {
      ...
      conv_hwc2chw_config->init.f32(&convolution_op->params.f32_minmax,
    ...
    case xnn_microkernel_type_dwconv: {
      ...
      status = create_dwconv_path(
          ...
          dwconv2d_parameters->output_width_tile, dwconv2d_parameters->ukernel,
...
static enum xnn_status reshape_convolution2d_nchw(
    ...
    uint32_t log2_output_element_size, const void* params, void* chw_params,
  ...
  switch (convolution_op->ukernel.type) {
    case xnn_microkernel_type_spmm: {
      ...
      const size_t num_output_channel_blocks =
          convolution_op->num_output_channel_blocks;

      convolution_op->num_nonzero_values = num_nonzero_values;
      convolution_op->num_nonzero_blocks = num_nonzero_blocks;
      convolution_op->num_output_channel_blocks = num_output_channel_blocks;

      const int32_t* input_channel_diffs =
          (const int32_t*)packed_weights(convolution_op);
      int32_t* input_increments =
          ((int32_t*)packed_weights(convolution_op)) + num_nonzero_blocks;
      const uint32_t* output_channel_nonzeros =
          (uint32_t*)(input_increments + num_nonzero_blocks);
      const void* nonzero_values =
          (const void*)(output_channel_nonzeros + num_output_channel_blocks);
      ...
      memcpy(&convolution_op->context.spmm.params, params,
             sizeof(convolution_op->context.spmm.params));
      ...
      if (num_threads > 1) {
        ...
        const size_t max_mc =
            divide_round_up(input_size, num_threads * target_tiles_per_thread);
        if (max_mc < mc) {
      ...
      convolution_op->compute[0].task_2d_tile_1d =
          (pthreadpool_task_2d_tile_1d_t)xnn_compute_spmm;
      convolution_op->compute[0].range[0] = batch_size;
      convolution_op->compute[0].range[1] = input_size
                                            << log2_input_element_size;
    ...
    case xnn_microkernel_type_conv2d_hwc2chw: {
      const size_t zero_size =
          (input_width * convolution_op->group_input_channels
           << log2_input_element_size) +
          XNN_EXTRA_BYTES;
      ...
      memcpy(&convolution_op->context.conv2d.params, params,
             sizeof(convolution_op->context.conv2d.params));

      const size_t output_height_tile =
          convolution_op->ukernel.conv2d.output_height_tile;
      size_t output_height_slice = output_height;
      if (num_threads > 1) {
        const size_t target_tiles_per_thread = 5;
        const size_t max_output_height_slice = divide_round_up(
            output_height, num_threads * target_tiles_per_thread);
        if (max_output_height_slice < output_height_slice) {
          output_height_slice =
              min(output_height_slice,
                  divide_round_up(output_height_slice, max_output_height_slice *
                                                           output_height_tile) *
                      output_height_tile);
        }
      }
      convolution_op->compute[0].type = xnn_parallelization_type_2d_tile_1d;
      convolution_op->compute[0].task_2d_tile_1d =
          (pthreadpool_task_2d_tile_1d_t)xnn_compute_conv2d_hwc2chw;
      convolution_op->compute[0].range[0] = batch_size;
    ...
    case xnn_microkernel_type_dwconv: {
      const size_t zero_size =
          (input_width << log2_input_element_size) + 2 * XNN_EXTRA_BYTES;
      ...
      memcpy(&convolution_op->context.dwconv2d.params, chw_params,
             sizeof(convolution_op->context.dwconv2d.params));

      convolution_op->compute[0].type = xnn_parallelization_type_2d;
      convolution_op->compute[0].task_2d =
          (pthreadpool_task_2d_t)xnn_compute_dwconv2d_chw;
      convolution_op->compute[0].range[0] = batch_size;
...
enum xnn_status xnn_reshape_convolution2d_nchw_f16(
  ...
  return reshape_convolution2d_nchw(
      convolution_op, xnn_operator_type_convolution_nchw_f16, batch_size,
      ...
      &convolution_op->params.f16_minmax, &convolution_op->params.f16_chw,
...
enum xnn_status xnn_reshape_convolution2d_nchw_f32(
    ...
    pthreadpool_t threadpool) {
  return reshape_convolution2d_nchw(
      convolution_op, xnn_operator_type_convolution_nchw_f32, batch_size,
      input_height, input_width,
      ...
      &convolution_op->params.f32_minmax, &convolution_op->params.f32_chw,
      output_height_out, output_width_out, threadpool);
}
...
static enum xnn_status setup_convolution2d_nchw(
  ...
  switch (convolution_op->ukernel.type) {
    case xnn_microkernel_type_spmm: {
      convolution_op->context.spmm.input =
          (const void*)((uintptr_t)input +
                        (convolution_op->first_input_channel *
...
enum xnn_status xnn_setup_convolution2d_nchw_f16(xnn_operator_t convolution_op,
  ...
  return setup_convolution2d_nchw(
      convolution_op, xnn_operator_type_convolution_nchw_f16, input, output);
}
...
enum xnn_status xnn_setup_convolution2d_nchw_f32(xnn_operator_t convolution_op,
                                                 ...
                                                 float* output) {
  return setup_convolution2d_nchw(
      convolution_op, xnn_operator_type_convolution_nchw_f32, input, output);
}
