...
static void init_constant_pad_nd(uint32_t padding_value, uint32_t flags,
...
static enum xnn_status create_constant_pad_nd(
    uint32_t padding_value, uint32_t flags,
    enum xnn_operator_type operator_type, xnn_operator_t* constant_pad_op_out) {
  ...
  init_constant_pad_nd(padding_value, flags, operator_type, fill_config,
                       pad_config, constant_pad_op);
...
enum xnn_status xnn_create_constant_pad_nd_x8(
  ...
  return create_constant_pad_nd(padding_pattern * UINT32_C(0x01010101), flags,
...
enum xnn_status xnn_create_constant_pad_nd_x16(
  ...
  return create_constant_pad_nd(padding_pattern * UINT32_C(0x00010001), flags,
...
enum xnn_status xnn_create_constant_pad_nd_x32(
    const void* padding_value, uint32_t flags,
    xnn_operator_t* constant_pad_op_out) {
  return create_constant_pad_nd(*((const uint32_t*)padding_value), flags,
...
static enum xnn_status reshape_constant_pad_nd(
    xnn_operator_t constant_pad_op,
    enum xnn_operator_type expected_operator_type, size_t num_dims,
    const size_t* input_shape, const size_t* pre_paddings,
    const size_t* post_paddings, uint32_t log2_element_size,
  ...
  for (size_t i = 0; i < num_dims; i++) {
    ...
    if (is_current_dim_padded || is_previous_dim_padded) {
      normalized_pre_paddings[XNN_MAX_TENSOR_DIMS - 1 - num_squeezed_dims] =
          pre_padding;
      normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1 - num_squeezed_dims] =
          input_dim;
      normalized_output_shape[XNN_MAX_TENSOR_DIMS - 1 - num_squeezed_dims] =
          pre_padding + input_dim + post_padding;
  ...
  const struct xnn_xx_fill_config* xx_fill_config =
      constant_pad_op->fill_config;
  ...
  for (size_t i = 0; i < XNN_MAX_TENSOR_DIMS; i++) {
    constant_pad_op->context.pad.pre_paddings[i] =
        normalized_pre_paddings[XNN_MAX_TENSOR_DIMS - 1 - i];
    constant_pad_op->context.pad.input_size[i] =
  ...
  for (size_t i = 1; i < XNN_MAX_TENSOR_DIMS; i++) {
    constant_pad_op->context.pad.input_stride[i - 1] = input_stride
                                                       << log2_element_size;
    constant_pad_op->context.pad.output_stride[i - 1] = output_stride
                                                        << log2_element_size;
  ...
  constant_pad_op->context.pad.output_size[0] =
  ...
  constant_pad_op->context.pad.post_paddings[0] =
      constant_pad_op->context.pad.output_size[0] -
      constant_pad_op->context.pad.pre_paddings[0] -
      constant_pad_op->context.pad.input_size[0];

  constant_pad_op->compute[0].type = xnn_parallelization_type_5d;
  constant_pad_op->compute[0].task_5d =
      (pthreadpool_task_5d_t)xnn_compute_pad_5d;
...
enum xnn_status xnn_reshape_constant_pad_nd_x8(xnn_operator_t constant_pad_op,
  ...
  return reshape_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x8, num_dims,
      input_shape, pre_padding, post_padding,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT8_T, threadpool);
}
...
enum xnn_status xnn_reshape_constant_pad_nd_x16(xnn_operator_t constant_pad_op,
                                                ...
                                                pthreadpool_t threadpool) {
  return reshape_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x16, num_dims,
      input_shape, pre_padding, post_padding,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT16_T, threadpool);
}
...
enum xnn_status xnn_reshape_constant_pad_nd_x32(xnn_operator_t constant_pad_op,
                                                ...
                                                pthreadpool_t threadpool) {
  return reshape_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x32, num_dims,
      input_shape, pre_padding, post_padding,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT32_T, threadpool);
}
  ...
  for (size_t i = 1; i < XNN_MAX_TENSOR_DIMS; i++) {
    constant_pad_op->context.pad.input =
        (const void*)((uintptr_t)constant_pad_op->context.pad.input -
                      (constant_pad_op->context.pad.pre_paddings[i] *
                       constant_pad_op->context.pad.input_stride[i - 1]));
  }
...
enum xnn_status xnn_setup_constant_pad_nd_x8(xnn_operator_t constant_pad_op,
  ...
  return setup_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x8, input, output);
}

enum xnn_status xnn_setup_constant_pad_nd_x16(xnn_operator_t constant_pad_op,
                                              const void* input, void* output) {
  return setup_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x16, input, output);
}

enum xnn_status xnn_setup_constant_pad_nd_x32(xnn_operator_t constant_pad_op,
                                              const void* input, void* output) {
  return setup_constant_pad_nd(
      constant_pad_op, xnn_operator_type_constant_pad_nd_x32, input, output);
}
...
enum xnn_status run_constant_pad_nd(
    ...
    void* output, uint32_t log2_element_size, const uint32_t padding_value,
    enum xnn_operator_type operator_type, pthreadpool_t threadpool) {
  ...
  enum xnn_status status = reshape_constant_pad_nd(
      &constant_pad_op, operator_type, num_dims, input_shape, pre_paddings,
      post_paddings, log2_element_size, threadpool);
  ...
  status =
      setup_constant_pad_nd(&constant_pad_op, operator_type, input, output);
...
enum xnn_status xnn_run_constant_pad_nd_x8(
  ...
  const uint32_t padding_pattern = *((const uint8_t*)padding_value);
  return run_constant_pad_nd(flags, num_dims, input_shape, pre_paddings,
                             post_paddings, input, output,
                             /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT8_T,
                             padding_pattern * UINT32_C(0x01010101),
                             xnn_operator_type_constant_pad_nd_x32, threadpool);
}
...
enum xnn_status xnn_run_constant_pad_nd_x32(
  ...
  return run_constant_pad_nd(
      flags, num_dims, input_shape, pre_paddings, post_paddings, input, output,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT32_T, padding_pattern,
      xnn_operator_type_constant_pad_nd_x32, threadpool);
}
