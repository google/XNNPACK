...
static inline size_t compute_output_dimension_with_tf_same_padding(
    size_t input_dimension, size_t stride_dimension) {
...
static enum xnn_status create_max_pooling2d_nhwc(
    ...
    enum xnn_operator_type operator_type, xnn_operator_t* max_pooling_op_out) {
  ...
  if (pooling_size == 1) {
    xnn_log_error(
        "failed to create %s operator with 1 pooling element: 1x1 pooling is "
        "meaningless",
        xnn_operator_type_to_string(operator_type));
    goto error;
  }
  ...
  if (stride_height > pooling_height) {
    xnn_log_error("failed to create %s operator with %" PRIu32
                  " stride height: must be less than pooling height %" PRIu32,
                  xnn_operator_type_to_string(operator_type), stride_height,
                  pooling_height);
  ...
  if (stride_width > pooling_width) {
    xnn_log_error("failed to create %s operator with %" PRIu32
                  " stride width: must be less than pooling width %" PRIu32,
                  xnn_operator_type_to_string(operator_type), stride_width,
                  pooling_width);
    return xnn_status_invalid_parameter;
  }
...
enum xnn_status xnn_create_max_pooling2d_nhwc_s8(
  ...
  return create_max_pooling2d_nhwc(
      ...
      stride_width, dilation_height, dilation_width, flags, &params,
      sizeof(params), maxpool_config, xnn_operator_type_max_pooling_nhwc_s8,
      max_pooling_op_out);
}
...
enum xnn_status xnn_create_max_pooling2d_nhwc_u8(
  ...
  return create_max_pooling2d_nhwc(
      ...
      input_padding_left, pooling_height, pooling_width, stride_height,
      stride_width, dilation_height, dilation_width, flags, &params,
      sizeof(params), maxpool_config, xnn_operator_type_max_pooling_nhwc_u8,
      max_pooling_op_out);
}
...
enum xnn_status xnn_create_max_pooling2d_nhwc_f32(
  ...
  if (isnan(output_min)) {
    xnn_log_error(
        "failed to create %s with NaN output lower bound: lower bound must be "
        "non-NaN",
  ...
  if (isnan(output_max)) {
    xnn_log_error(
        "failed to create %s with NaN output upper bound: upper bound must be "
        "non-NaN",
        xnn_operator_type_to_string(xnn_operator_type_max_pooling_nhwc_f32));
    return xnn_status_invalid_parameter;
  }
  ...
  return create_max_pooling2d_nhwc(
      ...
      input_padding_left, pooling_height, pooling_width, stride_height,
      stride_width, dilation_height, dilation_width, flags, &params,
      sizeof(params), maxpool_config, xnn_operator_type_max_pooling_nhwc_f32,
      max_pooling_op_out);
}
...
enum xnn_status xnn_create_max_pooling2d_nhwc_f16(
  ...
  return create_max_pooling2d_nhwc(
      ...
      input_padding_left, pooling_height, pooling_width, stride_height,
      stride_width, dilation_height, dilation_width, flags, &params,
      sizeof(params), maxpool_config, xnn_operator_type_max_pooling_nhwc_f16,
      max_pooling_op_out);
}
...
static enum xnn_status reshape_max_pooling2d_nhwc(
    ...
    uint32_t log2_input_element_size, uint32_t log2_output_element_size,
    const struct xnn_maxpool_config maxpool[restrict XNN_MIN_ELEMENTS(1)],
    const void* params, size_t params_size, size_t* output_height_out,
  ...
  if (max_pooling_op->flags & XNN_FLAG_TENSORFLOW_SAME_PADDING) {
    max_pooling_op->output_height =
        compute_output_dimension_with_tf_same_padding(
            input_height, max_pooling_op->stride_height);
    max_pooling_op->output_width =
        compute_output_dimension_with_tf_same_padding(
            input_width, max_pooling_op->stride_width);

    const uint32_t effective_kernel_height =
        (max_pooling_op->kernel_height - 1) * max_pooling_op->dilation_height +
        1;
    const uint32_t effective_kernel_width =
        (max_pooling_op->kernel_width - 1) * max_pooling_op->dilation_width + 1;
    const uint32_t total_padding_height = doz(
        (max_pooling_op->output_height - 1) * max_pooling_op->stride_height +
            effective_kernel_height,
        input_height);
    const uint32_t total_padding_width =
        doz((max_pooling_op->output_width - 1) * max_pooling_op->stride_width +
                effective_kernel_width,
            input_width);
    max_pooling_op->padding_top = total_padding_height / 2;
    max_pooling_op->padding_left = total_padding_width / 2;
    max_pooling_op->padding_bottom =
        total_padding_height - max_pooling_op->padding_top;
    max_pooling_op->padding_right =
        total_padding_width - max_pooling_op->padding_left;
  } else {
    max_pooling_op->output_height = xnn_compute_convolution_output_dimension(
        max_pooling_op->padding_top + input_height +
            max_pooling_op->padding_bottom,
        max_pooling_op->kernel_height, max_pooling_op->dilation_height,
        max_pooling_op->stride_height);
    max_pooling_op->output_width = xnn_compute_convolution_output_dimension(
        max_pooling_op->padding_left + input_width +
            max_pooling_op->padding_right,
        max_pooling_op->kernel_width, max_pooling_op->dilation_width,
  ...
  const size_t step_width =
      max_pooling_op->dilation_width > 1
          ? pooling_width
          : min(max_pooling_op->stride_width, pooling_width);
  ...
  if (input_height != max_pooling_op->last_input_height ||
    ...
    const void** indirection_buffer = (const void**)xnn_reallocate_memory(
        max_pooling_op->indirection_buffer, indirection_buffer_size);
    ...
    // when we have the input pointer.
    ...
    xnn_indirection_init_maxpool2d(
        ...
        max_pooling_op->padding_top, max_pooling_op->padding_left, step_height,
        step_width);
  ...
  const size_t output_width_stride = max_pooling_op->output_pixel_stride
  ...
  max_pooling_op->context.max_pooling = (struct max_pooling_context){
      ...
      .input_batch_stride =
          (input_height * input_width * max_pooling_op->input_pixel_stride)
  ...
  max_pooling_op->compute[0].task_2d =
      (pthreadpool_task_2d_t)xnn_compute_max_pooling;
...
enum xnn_status xnn_reshape_max_pooling2d_nhwc_s8(
  ...
  return reshape_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_s8, batch_size,
      input_height, input_width, channels, input_pixel_stride,
      output_pixel_stride,
      /*log2_input_element_size=*/XNN_LOG2_SIZEOF_INT8_T,
      /*log2_output_element_size=*/XNN_LOG2_SIZEOF_INT8_T,
      max_pooling_op->maxpool_config, &max_pooling_op->params.s8_minmax,
      sizeof(max_pooling_op->params.s8_minmax), output_height_out,
...
enum xnn_status xnn_reshape_max_pooling2d_nhwc_u8(
    ...
    size_t* output_width_out, pthreadpool_t threadpool) {
  return reshape_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_u8, batch_size,
      input_height, input_width, channels, input_pixel_stride,
      output_pixel_stride,
      /*log2_input_element_size=*/XNN_LOG2_SIZEOF_UINT8_T,
      /*log2_output_element_size=*/XNN_LOG2_SIZEOF_UINT8_T,
      max_pooling_op->maxpool_config, &max_pooling_op->params.u8_minmax,
      sizeof(max_pooling_op->params.u8_minmax), output_height_out,
      output_width_out, threadpool);
}
...
enum xnn_status xnn_reshape_max_pooling2d_nhwc_f16(
    ...
    size_t* output_width_out, pthreadpool_t threadpool) {
  return reshape_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_f16, batch_size,
      input_height, input_width, channels, input_pixel_stride,
      output_pixel_stride,
      /*log2_input_element_size=*/XNN_LOG2_SIZEOF_HALF,
      /*log2_output_element_size=*/XNN_LOG2_SIZEOF_HALF,
      max_pooling_op->maxpool_config, &max_pooling_op->params.f16_minmax,
      sizeof(max_pooling_op->params.f16_minmax), output_height_out,
      output_width_out, threadpool);
}
...
enum xnn_status xnn_reshape_max_pooling2d_nhwc_f32(
    ...
    size_t* output_width_out, pthreadpool_t threadpool) {
  return reshape_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_f32, batch_size,
      input_height, input_width, channels, input_pixel_stride,
      output_pixel_stride,
      /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
      /*log2_output_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
      max_pooling_op->maxpool_config, &max_pooling_op->params.f32_minmax,
      sizeof(max_pooling_op->params.f32_minmax), output_height_out,
      output_width_out, threadpool);
}
...
static enum xnn_status setup_max_pooling2d_nhwc(
  ...
  max_pooling_op->context.max_pooling.input_offset =
      (size_t)((uintptr_t)input - (uintptr_t)max_pooling_op->last_input);
...
enum xnn_status xnn_setup_max_pooling2d_nhwc_s8(xnn_operator_t max_pooling_op,
                                                const int8_t* input,
                                                int8_t* output) {
  return setup_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_s8, input, output);
}

enum xnn_status xnn_setup_max_pooling2d_nhwc_u8(xnn_operator_t max_pooling_op,
                                                const uint8_t* input,
                                                uint8_t* output) {
  return setup_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_u8, input, output);
}

enum xnn_status xnn_setup_max_pooling2d_nhwc_f16(xnn_operator_t max_pooling_op,
                                                 const void* input,
                                                 void* output) {
  return setup_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_f16, input, output);
}

enum xnn_status xnn_setup_max_pooling2d_nhwc_f32(xnn_operator_t max_pooling_op,
                                                 const float* input,
                                                 float* output) {
  return setup_max_pooling2d_nhwc(
      max_pooling_op, xnn_operator_type_max_pooling_nhwc_f32, input, output);
}
