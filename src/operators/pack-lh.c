  ...
  pthreadpool_t threadpool)
{
  ...
  pack_lh_op->context.pack_lh = (struct pack_lh_context) {
      ...
      .packed_offset_fn = (xnn_pack_lh_offset_fn)
                           pack_lh_op->pack_lh_config->offset_fn,
      .pack_lh_ukernel = (xnn_pack_lh_ukernel_fn)
  ...
  *output_size_bytes = pack_lh_config->size_fn(batch_size, channels, mr, kr, sr);
  ...
  pack_lh_op->compute[0].task_1d =
      (pthreadpool_task_1d_t) xnn_compute_pack_lh;
  pack_lh_op->compute[0].range[0] = batch_size;
  pack_lh_op->compute[0].tile[0] = batch_size;//mr_packed;
...
enum xnn_status xnn_reshape_pack_lh_x16(
  xnn_operator_t pack_lh_op,
  size_t batch_size,
  size_t channels,
  size_t *output_size_bytes,
  pthreadpool_t threadpool)
{
  ...
  return reshape_pack_lh(pack_lh_op, batch_size, channels, output_size_bytes,
          xnn_operator_type_pack_lh_x16, /*element_size=*/sizeof(xnn_float16),
  ...
  pthreadpool_t threadpool)
{
  ...
  const struct xnn_pack_lh_config *pack_lh_config = xnn_init_x8_pack_lh_config();
  return reshape_pack_lh(pack_lh_op, batch_size, channels, output_size_bytes,
          xnn_operator_type_pack_lh_x8, /*element_size=*/sizeof(xnn_float16),
          gemm_config, pack_lh_config, threadpool);
}
  ...
  pthreadpool_t threadpool)
{
  ...
  const struct xnn_pack_lh_config *pack_lh_config = xnn_init_x32_pack_lh_config();
  return reshape_pack_lh(pack_lh_op, batch_size, channels, output_size_bytes,
          xnn_operator_type_pack_lh_x32, /*element_size=*/sizeof(float),
          gemm_config, pack_lh_config, threadpool);
}
