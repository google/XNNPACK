...
static enum xnn_status create_scaled_dot_product_attention_nhtc(
    ...
    const void* rmax_params, size_t rmax_params_size, const void* tanh_params,
    size_t tanh_params_size, uint32_t flags, xnn_operator_t* attention_op_out) {
  ...
  if (cap_type == xnn_attention_logits_cap_type_tanh) {
    ...
    memcpy(&attention_op->attention.cap_params, cap_tanh_params,
           sizeof(struct xnn_attention_logits_cap_tanh_params));
  }

  attention_op->attention.raddstoreexpminusmax_config =
...
enum xnn_status xnn_create_scaled_dot_product_attention_nhtc_f16(
  ...
  if XNN_LIKELY (gemm_config->init.f16 != NULL) {
    gemm_config->init.f16(&minmax_params, xnn_float16_from_float(-INFINITY),
  ...
  if (cap_type == xnn_attention_logits_cap_type_tanh) {
    ...
    if (cap <= 0.0f || isnan(cap) || cap < 0x1.0p-14f || cap > 65504.0f) {
      xnn_log_error(
          "failed to create %s operator with Cap TanH: cap value (%f) must be "
          "greater than 0, representable "
          "in FP16, and not be NaN",
  ...
  return create_scaled_dot_product_attention_nhtc(
      cap_type, cap_params, operator_type, gemm_config,
      raddstoreexpminusmax_config, rmax_config, vadd_config, vmul_config,
      vtanh_config, &minmax_params, sizeof(minmax_params), &expminus_params,
      sizeof(expminus_params), &rmax_params, sizeof(rmax_params), &tanh_params,
      sizeof(tanh_params), flags, attention_op_out);
...
static enum xnn_status reshape_scaled_dot_product_attention_nhtc(
    xnn_operator_t attention_op, enum xnn_operator_type expected_operator_type,
    size_t batch_size, size_t query_heads, size_t query_tokens,
    size_t key_value_heads, size_t key_value_tokens, size_t query_key_channels,
    size_t value_channels, size_t* workspace_size, size_t* workspace_alignment,
    size_t log2_element_size, size_t element_size,
    xnn_compute_reciprocal_fn compute_reciprocal, void* cap,
    void* cap_reciprocal, size_t cap_size, const void* minmax_params,
    size_t minmax_params_size, const void* expminus_params,
    size_t expminus_params_size, const void* rmax_params,
    size_t rmax_params_size, const void* tanh_params, size_t tanh_params_size,
  ...
  if (batch_size == 0) {
    xnn_log_error(
        "failed to create %s operator with batch size of %zu: batch size must "
        "be non-zero",
  ...
  if (query_heads == 0) {
    xnn_log_error(
        "failed to create %s operator with number of query heads %zu: number "
        "of query heads must be non-zero",
  ...
  if (key_value_heads == 0) {
    xnn_log_error(
        "failed to create %s operator with number of key/value heads %zu: "
        "number of key/value heads must be non-zero",
  ...
  if (key_value_heads != 1 && key_value_heads != query_heads) {
    xnn_log_error(
        "failed to create %s operator with number of key/value heads %zu: "
        "number of key/value heads must be either 1 or "
        "equal to number of query heads",
        xnn_operator_type_to_string(expected_operator_type), key_value_heads);
    return xnn_status_invalid_parameter;
  }
  ...
  if (query_tokens == 0) {
    xnn_log_error(
        "failed to create %s operator with query tokens of %zu: query tokens "
        "must be non-zero",
  ...
  if (key_value_tokens == 0) {
    xnn_log_error(
        "failed to create %s operator with key/value tokens of %zu: key/value "
  ...
  if (query_key_channels == 0) {
    xnn_log_error(
        "failed to create %s operator with %zu channels: query/key channels "
        "must be non-zero",
        xnn_operator_type_to_string(expected_operator_type),
        query_key_channels);
    return xnn_status_invalid_parameter;
  ...
  if (value_channels == 0) {
    xnn_log_error(
        "failed to create %s operator with %zu channels: value channels must "
        "be non-zero",
  ...
  const size_t workspace_multiplier =
      use_threads_workspace_size ? size_using_threads : size_using_batch;
  // Calculate size required for workspace.
  // 1. Workspace for Q scaled, each thread computes a maximum of mr *
  // query_key_channels.
  const size_t scaled_query_size =
      round_up_po2(workspace_multiplier * query_key_channels * element_size +
                       XNN_EXTRA_BYTES,
  ...
  const size_t key_head_stride =
      key_n_stride * (element_size + (key_k_stride << log2_element_size));
  // 2. Workspace for packed key.
  const size_t packed_key_size = round_up_po2(
      batch_size * key_value_heads * key_head_stride, XNN_ALLOCATION_ALIGNMENT);
  ...
  const size_t value_head_stride =
      value_n_stride * (element_size + (value_k_stride << log2_element_size));
  // 3. Workspace for packed key.
  const size_t packed_value_size =
      round_up_po2(batch_size * key_value_heads * value_head_stride,
                   XNN_ALLOCATION_ALIGNMENT);

  // 4. Workspace for logits (Q*K), each thread computes mr * key_value_tokens.
  const size_t logits_size = round_up_po2(
      workspace_multiplier * key_value_tokens * element_size + XNN_EXTRA_BYTES,
      XNN_ALLOCATION_ALIGNMENT);

  const size_t total_workspace_size =
      scaled_query_size + packed_key_size + packed_value_size + logits_size;
  ...
  *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
  ...
  attention_op->compute[0].task_2d_tile_1d =
  ...
  attention_op->compute[1].task_2d_tile_1d =
  ...
  struct xnn_hmp_gemm_ukernel gemm_ukernel =
      attention_op->ukernel.gemm.gemm_cases[mr - 1];

  attention_op->context.gemm.gemm.attention =
      (struct scaled_dot_product_attention_context){
          ...
          .query_batch_stride =
              query_heads * query_tokens * query_key_channels * element_size,
          ...
          .logits_batch_stride =
              query_heads * query_tokens * key_value_tokens * element_size,
          .logits_head_stride = query_tokens * key_value_tokens * element_size,
          .output_batch_stride =
              query_heads * query_tokens * value_channels * element_size,
          ...
          .raddstoreexpminusmax_ukernel =
              attention_op->attention.raddstoreexpminusmax_config->ukernel,
  ...
  if (attention_op->attention.cap_type == xnn_attention_logits_cap_type_tanh) {
    attention_op->context.gemm.gemm.attention.logits_cap.type =
        xnn_attention_logits_cap_type_tanh;
    memcpy(&attention_op->context.gemm.gemm.attention.logits_cap.cap, cap,
           cap_size);
    memcpy(&attention_op->context.gemm.gemm.attention.logits_cap.cap_reciprocal,
           cap_reciprocal, cap_size);
  }

#if XNN_MAX_UARCH_TYPES > 1
  if (xnn_is_hmp_gemm_ukernel(gemm_ukernel)) {
    if (use_threads_workspace_size) {
      attention_op->compute[2].type =
      ...
      attention_op->compute[2].task_3d_tile_1d_with_id_with_thread =
          (pthreadpool_task_3d_tile_1d_with_id_with_thread_t)
              xnn_compute_hmp_scaled_dot_product_attention_with_thread;
    } else {
      attention_op->compute[2].type =
          xnn_parallelization_type_3d_tile_1d_with_uarch;
      attention_op->compute[2].task_3d_tile_1d_with_id =
          (pthreadpool_task_3d_tile_1d_with_id_t)
              xnn_compute_hmp_scaled_dot_product_attention;
    }
  } else {
    if (use_threads_workspace_size) {
      attention_op->compute[2].type =
      ...
      attention_op->compute[2].task_3d_tile_1d_with_thread =
          (pthreadpool_task_3d_tile_1d_with_thread_t)
              xnn_compute_scaled_dot_product_attention_with_thread;
    } else {
      attention_op->compute[2].type = xnn_parallelization_type_3d_tile_1d;
      attention_op->compute[2].task_3d_tile_1d = (pthreadpool_task_3d_tile_1d_t)
          xnn_compute_scaled_dot_product_attention;
    }
  }
#else
  if (use_threads_workspace_size) {
  ...
  attention_op->context.gemm.gemm.attention.packed_v_offset =
      scaled_query_size + packed_key_size;
  attention_op->context.gemm.gemm.attention.logits_offset =
      scaled_query_size + packed_key_size + packed_value_size;

  memcpy(&attention_op->context.gemm.gemm.attention.minmax_params,
         minmax_params, minmax_params_size);
  memcpy(&attention_op->context.gemm.gemm.attention.expminus_params,
         expminus_params, expminus_params_size);
  memcpy(&attention_op->context.gemm.gemm.attention.rmax_params, rmax_params,
         rmax_params_size);
  memcpy(&attention_op->context.gemm.gemm.attention.tanh_params, tanh_params,
         tanh_params_size);
...
enum xnn_status xnn_reshape_scaled_dot_product_attention_nhtc_f16(
    xnn_operator_t attention_op, size_t batch_size, size_t heads,
    size_t query_tokens, size_t key_value_heads, size_t key_value_tokens,
    size_t query_key_channels, size_t value_channels, size_t* workspace_size,
    size_t* workspace_alignment, pthreadpool_t threadpool) {
  xnn_float16 cap =
      xnn_float16_from_float(attention_op->attention.cap_params.cap);
  xnn_float16 cap_reciprocal =
      xnn_float16_from_float(1.0f / attention_op->attention.cap_params.cap);

  return reshape_scaled_dot_product_attention_nhtc(
      attention_op, xnn_operator_type_scaled_dot_product_attention_nhtc_f16,
      batch_size, heads, query_tokens, key_value_heads, key_value_tokens,
      query_key_channels, value_channels, workspace_size, workspace_alignment,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_UINT16_T,
      /*element_size=*/sizeof(uint16_t),
      (xnn_compute_reciprocal_fn)compute_reciprocal_f16, &cap, &cap_reciprocal,
      sizeof(uint16_t), &attention_op->params.f16_minmax,
      sizeof(attention_op->params.f16_minmax),
      &attention_op->params2.f16_default,
      sizeof(attention_op->params2.f16_default),
...
enum xnn_status xnn_reshape_scaled_dot_product_attention_nhtc_f32(
  ...
  return reshape_scaled_dot_product_attention_nhtc(
      ...
      (xnn_compute_reciprocal_fn)compute_reciprocal_f32, &cap, &cap_reciprocal,
      sizeof(float), &attention_op->params.f32_minmax,
      sizeof(attention_op->params.f32_minmax),
      &attention_op->params2.f32_default,
      sizeof(attention_op->params2.f32_default),
...
static enum xnn_status setup_scaled_dot_product_attention_nhtc(
    xnn_operator_t attention_op, enum xnn_operator_type expected_operator_type,
    void* workspace, const float* query, const float* key, const float* value,
    const float* scale, const float* mask, float* output) {
  if (attention_op->type != expected_operator_type) {
    xnn_log_error(
  ...
  attention_op->context.gemm.packw_gemm_goi.packed_weights =
      (void*)((uintptr_t)workspace +
              attention_op->context.gemm.gemm.attention.packed_k_offset);
  ...
  attention_op->context.gemm.gemm.attention.scaled_query =
      (void*)((uintptr_t)workspace +
              attention_op->context.gemm.gemm.attention.scaled_query_offset);
  attention_op->context.gemm.gemm.attention.logits_buffer =
      (void*)((uintptr_t)workspace +
              attention_op->context.gemm.gemm.attention.logits_offset);
  attention_op->context.gemm.gemm.attention.query = query;
  attention_op->context.gemm.gemm.attention.key =
      attention_op->context.gemm.packw_gemm_goi.packed_weights;
  attention_op->context.gemm.gemm.attention.value =
      attention_op->context.gemm.packw_gemm_gio.packed_weights;
...
enum xnn_status xnn_setup_scaled_dot_product_attention_nhtc_f16(
  ...
  return setup_scaled_dot_product_attention_nhtc(
      attention_op, xnn_operator_type_scaled_dot_product_attention_nhtc_f16,
      workspace, query, key, value, scale, mask, output);
}
