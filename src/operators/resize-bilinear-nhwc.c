...
static const struct xnn_ibilinear_config* get_ibilinear_nhwc_config(
...
enum xnn_status xnn_create_resize_bilinear2d_nhwc(
  ...
  const struct xnn_ibilinear_config* ibilinear_config =
      get_ibilinear_nhwc_config(datatype);
...
enum xnn_status xnn_reshape_resize_bilinear2d_nhwc(
  ...
  const size_t log2_data_element_size =
      resize_op->ibilinear_config->log2_data_element_size;
  const size_t log2_weight_element_size =
      resize_op->ibilinear_config->log2_weight_element_size;
  const size_t output_height = resize_op->output_height;
  const size_t output_width = resize_op->output_width;
  const bool enable_transient_indirection =
      !!(resize_op->flags & XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER);
  const size_t input_pixel_stride_in_bytes = input_pixel_stride
  ...
  if (enable_transient_indirection) {
    ...
    const size_t indirect_input_offset =
        (packed_weights_size + sizeof(void*) - 1) & ~(sizeof(void*) - 1);
    ...
    *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
    ...
    resize_op->context.resize_nhwc_indirection_init =
        (struct resize_bilinear_nhwc_indirection_init_context){
            ...
            .tensorflow_legacy_mode =
                !!(resize_op->flags & XNN_FLAG_TENSORFLOW_LEGACY_MODE),
    ...
    resize_op->compute[0].context_offset =
        offsetof(struct xnn_operator, context.resize_nhwc_indirection_init) -
        offsetof(struct xnn_operator, context);
    resize_op->compute[0].task_1d_tile_1d =
        (pthreadpool_task_1d_tile_1d_t)xnn_compute_resize_bilinear_indirection;
    resize_op->compute[0].range[0] = output_height;

    if (num_threads > 1) {
      const size_t target_tiles_per_thread = 5;
      resize_op->compute[0].tile[0] =
  ...
  const size_t output_pixel_stride_in_bytes = output_pixel_stride
                                              << log2_data_element_size;
  ...
  resize_op->context.resize_bilinear = (struct resize_bilinear_context){
      ...
      .input_batch_stride =
          input_pixel_stride_in_bytes * input_height * input_width,
      .packed_weights = resize_op->packed_weights.pointer,
      .output_pixel_stride = output_pixel_stride_in_bytes,
      .output_batch_stride =
          output_pixel_stride_in_bytes * output_height * output_width,
  ...
  if (num_threads > 1) {
    const size_t target_tiles_per_thread = 5;
    const size_t max_output_size_tile =
        divide_round_up(output_size, num_threads * target_tiles_per_thread);
    if (max_output_size_tile < output_size_tile) {
      const uint32_t output_size_subtile = ibilinear->pixel_tile;
      output_size_tile =
          min(output_size_tile,
              divide_round_up(output_size_tile,
                              max_output_size_tile * output_size_subtile) *
                  output_size_subtile);
    }
  }

  resize_op->compute[resize_bilinear_compute_index].type =
      xnn_parallelization_type_2d_tile_1d;
  resize_op->compute[resize_bilinear_compute_index].task_2d_tile_1d =
      (pthreadpool_task_2d_tile_1d_t)xnn_compute_resize_bilinear;
...
enum xnn_status xnn_setup_resize_bilinear2d_nhwc(xnn_operator_t resize_op,
  ...
  const size_t output_height =
      resize_op->context.resize_nhwc_indirection_init.output_height;
  const size_t output_width =
      resize_op->context.resize_nhwc_indirection_init.output_width;
  const size_t packed_weights_size = (output_height * output_width * 2)
                                     << log2_weight_element_size;
  if (resize_op->flags & XNN_FLAG_TRANSIENT_INDIRECTION_BUFFER) {
    // indirect_input should start at a multiple of pointer size to avoid ubsan
    // failures
    const size_t indirect_input_offset =
        (packed_weights_size + sizeof(void*) - 1) & ~(sizeof(void*) - 1);
    resize_op->context.resize_bilinear.packed_weights = (const void*)workspace;
    resize_op->context.resize_bilinear.indirect_input =
        (const void**)((uintptr_t)workspace + indirect_input_offset);
    resize_op->context.resize_nhwc_indirection_init.buffer =
        (const void**)workspace;
    resize_op->context.resize_nhwc_indirection_init.input = input;
  } else {
    resize_op->context.resize_bilinear.input_offset =
