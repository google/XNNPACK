    ...
    xnn_operator_t* softmax_op_out)
{
  ...
  const struct xnn_reduce_config* rmax_config = xnn_init_u8_rminmax_config();
    ...
    pthreadpool_t threadpool)
{
  ...
  softmax_op->context.u8_softmax = (struct u8_softmax_context) {
    ...
    .rmax_ukernel = (xnn_u8_rmax_ukernel_fn) softmax_op->rmax_config->rmax_ukernel,
    ...
    xnn_operator_t* softmax_op_out)
{
  ...
  const struct xnn_reduce_config* rmax_config = xnn_init_f16_rminmax_config();
  if (rmax_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                  xnn_operator_type_to_string(xnn_operator_type_softmax_nc_f16));
    return xnn_status_unsupported_hardware;
    ...
    xnn_operator_t* softmax_op_out)
{
  ...
  const struct xnn_reduce_config* rmax_config = xnn_init_f32_rminmax_config();
  if (rmax_config == NULL) {
    xnn_log_error(
      "failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_softmax_nc_f32));
    ...
    pthreadpool_t threadpool)
{
  ...
  return reshape_softmax_nc_floating_point(
    ...
    softmax_op->rmax_config->rmax_ukernel,
