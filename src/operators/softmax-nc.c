...
enum xnn_status xnn_create_softmax_nc_qu8(
  float input_scale, uint8_t output_zero_point, float output_scale, uint32_t flags, xnn_operator_t* softmax_op_out)
{
  ...
  if (softmax_op == NULL) {
    xnn_log_error(
      "failed to allocate %zu bytes for %s operator descriptor", sizeof(struct xnn_operator),
      xnn_operator_type_to_string(xnn_operator_type_sigmoid_nc_qu8));
    goto error;
  }
  ...
  pthreadpool_t threadpool)
{
  if (softmax_op->type != xnn_operator_type_softmax_nc_qu8) {
    xnn_log_error(
      ...
      xnn_operator_type_to_string(xnn_operator_type_softmax_nc_qu8), xnn_operator_type_to_string(softmax_op->type));
...
enum xnn_status xnn_setup_softmax_nc_qu8(xnn_operator_t softmax_op, const uint8_t* input, uint8_t* output)
{
  if (softmax_op->type != xnn_operator_type_softmax_nc_qu8) {
    xnn_log_error(
      "failed to setup operator: operator type mismatch (expected %s, got %s)",
...
enum xnn_status xnn_create_softmax_nc_f16(uint32_t flags, xnn_operator_t* softmax_op_out)
{
  const struct xnn_raddstoreexpminusmax_config* raddstoreexpminusmax_config =
  ...
  return create_softmax_nc_floating_point(
    flags, raddstoreexpminusmax_config, rmax_config, vmul_config, xnn_operator_type_softmax_nc_f16, softmax_op_out);
}

enum xnn_status xnn_create_softmax_nc_f32(uint32_t flags, xnn_operator_t* softmax_op_out)
{
  const struct xnn_raddstoreexpminusmax_config* raddstoreexpminusmax_config =
  ...
  return create_softmax_nc_floating_point(
    flags, raddstoreexpminusmax_config, rmax_config, vmul_config, xnn_operator_type_softmax_nc_f32, softmax_op_out);
}
  ...
  size_t minmax_params_size)
{
  ...
  if (softmax_op->type != expected_operator_type) {
    xnn_log_error(
      "failed to reshape operator: operator type mismatch (expected %s, got %s)",
      xnn_operator_type_to_string(expected_operator_type), xnn_operator_type_to_string(softmax_op->type));
    return xnn_status_invalid_parameter;
  }
  ...
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
    xnn_log_error(
      "failed to reshape %s operator: XNNPACK is not initialized", xnn_operator_type_to_string(expected_operator_type));
    return xnn_status_uninitialized;
  }
...
static enum xnn_status setup_softmax_nc_floating_point(
  xnn_operator_t softmax_op, enum xnn_operator_type expected_operator_type, const void* input, void* output)
{
  if (softmax_op->type != expected_operator_type) {
    xnn_log_error(
      "failed to setup operator: operator type mismatch (expected %s, got %s)",
...
static void compute_reciprocal_f16(const uint16_t input[XNN_MIN_ELEMENTS(1)], uint16_t output[XNN_MIN_ELEMENTS(1)])
{
  *output = fp16_ieee_from_fp32_value(1.0f / fp16_ieee_to_fp32_value(*input));
}

enum xnn_status xnn_setup_softmax_nc_f16(xnn_operator_t softmax_op, const void* input, void* output)
{
  return setup_softmax_nc_floating_point(softmax_op, xnn_operator_type_softmax_nc_f16, input, output);
}

static void compute_reciprocal_f32(const float input[XNN_MIN_ELEMENTS(1)], float output[XNN_MIN_ELEMENTS(1)])
{
  *output = 1.0f / *input;
}

enum xnn_status xnn_setup_softmax_nc_f32(xnn_operator_t softmax_op, const float* input, float* output)
{
  return setup_softmax_nc_floating_point(softmax_op, xnn_operator_type_softmax_nc_f32, input, output);
}
  ...
  pthreadpool_t threadpool)
{
  ...
  return reshape_softmax_nc_floating_point(
    softmax_op, xnn_operator_type_softmax_nc_f16, channels, input_stride, output_stride, batch_size,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_HALF, softmax_op->rmax_config->ukernel,
    softmax_op->raddstoreexpminusmax_config, f16_vmul_config, (xnn_compute_reciprocal_fn) compute_reciprocal_f16,
    &rmax_params, sizeof(rmax_params), &expminus_params, sizeof(expminus_params), &minmax_params,
    sizeof(minmax_params));
}
  ...
  pthreadpool_t threadpool)
{
  ...
  return reshape_softmax_nc_floating_point(
    softmax_op, xnn_operator_type_softmax_nc_f32, channels, input_stride, output_stride, batch_size,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_FLOAT, softmax_op->rmax_config->ukernel,
    softmax_op->raddstoreexpminusmax_config, f32_vmul_config, (xnn_compute_reciprocal_fn) compute_reciprocal_f32,
    &rmax_params, sizeof(rmax_params), &expminus_params, sizeof(expminus_params), &minmax_params,
    sizeof(minmax_params));
}
