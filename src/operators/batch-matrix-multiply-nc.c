...
static bool batch_matrix_safe_round_up(size_t n, size_t q, size_t* result) {
  if (q == 0) {
    return false;
  }
  return xnn_safe_mul(divide_round_up(n, q), q, result);
}

static bool batch_matrix_safe_round_up_po2(size_t n, size_t q,
                                           size_t* result) {
  if (!is_po2(q) || n > SIZE_MAX - (q - 1)) {
    return false;
  }
  *result = round_up_po2(n, q);
  return true;
}
...
XNN_NO_SANITIZE_FUNCTION enum xnn_status
create_batch_matrix_multiply_nc_const_weights(
  ...
  size_t extra_bytes;
  if (!xnn_safe_add(scale_b_size, extra_weights_bytes, &extra_bytes)) {
    xnn_log_error(
        "failed to create %s operator: extra weights size overflows size_t",
        xnn_operator_type_to_string_v2(batch_matrix_multiply_op));
    return xnn_status_out_of_memory;
  }
  size_t k_block;
  size_t k_stride;
  if (!xnn_safe_mul((size_t)kr, (size_t)sr, &k_block) ||
      !batch_matrix_safe_round_up_po2(k, k_block, &k_stride)) {
    xnn_log_error(
        "failed to create %s operator: k stride overflows size_t",
        xnn_operator_type_to_string_v2(batch_matrix_multiply_op));
    return xnn_status_out_of_memory;
  }
  ...
  if (gemm_config->packed_stride_weights_and_biases) {
    ...
    size_t scaled_k;
    if (!xnn_safe_mul(k_stride, (size_t) 1 << log2_kernel_element_size,
                      &scaled_k) ||
        !xnn_safe_add(scaled_k, bias_element_size, &weights_stride) ||
  ...
  if (cache_offset == XNN_CACHE_NOT_FOUND) {
    ...
    size_t n_stride;
    if (!batch_matrix_safe_round_up(n, nr, &n_stride)) {
      xnn_log_error(
          "failed to create %s operator: n stride overflows size_t",
          xnn_operator_type_to_string_v2(batch_matrix_multiply_op));
      return xnn_status_out_of_memory;
    }
    size_t packed_size;
    ...
    size_t aligned_size;
    if (!batch_matrix_safe_round_up_po2(packed_size, XNN_ALLOCATION_ALIGNMENT,
                                         &aligned_size)) {
      xnn_log_error(
          "failed to create %s operator: aligned weights size overflows size_t",
          xnn_operator_type_to_string_v2(batch_matrix_multiply_op));
      return xnn_status_out_of_memory;
    }
...
enum xnn_status create_batch_matrix_multiply_nc_helper(
  ...
  XNN_IF_ERROR_GOTO(error,
                    create_batch_matrix_multiply_nc(
                        context->flags, &context->params, context->params_size,
                        context->gemm_config, context->gemm_ukernels,
...
enum xnn_status xnn_create_batch_matrix_multiply_nc_f32_qc8w(
  ...
  return create_batch_matrix_multiply_nc_helper(&f32_qc8w_variant, &context);
}
...
enum xnn_status xnn_create_batch_matrix_multiply_nc_qs8(
  ...
  return create_batch_matrix_multiply_nc_helper(&qs8_variant, &context);
}
...
reshape_batch_matrix_multiply_nc(
  ...
  if (!gemm_context->const_weights) {
    ...
    size_t k_stride;
    if (!batch_matrix_safe_round_up(n, nr, &n_stride) ||
        !xnn_safe_mul((size_t)kr, (size_t)sr, &k_block) ||
        !batch_matrix_safe_round_up_po2(k, k_block, &k_stride)) {
      xnn_log_error(
    ...
    if (gemm_config->packed_stride_weights_and_biases) {
      ...
      size_t scaled_k;
      if (!xnn_safe_mul(k_stride, (size_t) 1 << log2_input_b_element_size,
                        &scaled_k) ||
          !xnn_safe_add(scaled_k, bias_element_size, &weights_stride) ||
          !xnn_safe_add(weights_stride, extra_weights_bytes,
                        &weights_stride)) {
        xnn_log_error(
  ...
  if (!xnn_safe_mul(m, k, &mk) ||
      !xnn_safe_mul(mk, (size_t)1u << log2_input_a_element_size, &ga_stride)) {
    xnn_log_error("failed to reshape %s operator: ga_stride overflows size_t",
                  xnn_operator_type_to_string_v2(batch_matrix_multiply_op));
    return xnn_status_out_of_memory;
  }
  ...
  if (packed_lh_config) {
    ...
    if (inline_lhs_packing) {
      ...
      size_t thread_rows;
      if (!batch_matrix_safe_round_up(batch_size_a, mr, &rounded_batch_size) ||
      ...
      if (packed_lh_config->gemv_noop && mr == 1) {
        xnn_log_debug(
            ...
            batch_size_c, m, n, k);
                 ...
                 thread_rows > rounded_batch_size) {
        xnn_log_debug(
        ...
        if (!batch_matrix_safe_round_up_po2(
                *workspace_size, XNN_ALLOCATION_ALIGNMENT, &workspace_offset)) {
          xnn_log_error(
        ...
        size_t gi_stride = 0;
        if (!xnn_safe_mul(
                k, (size_t)1u << packed_lh_config->log2_input_element_size,
                &lhs_stride) ||
  ...
  if (!xnn_safe_mul(k, (size_t) 1u << log2_input_a_element_size, &a_stride) ||
      !batch_matrix_safe_round_up(n, nr, &n_stride_final) ||
      !xnn_safe_mul(batch_matrix_multiply_op->weights_stride, n_stride_final,
                    &gw_stride) ||
...
enum xnn_status xnn_reshape_batch_matrix_multiply_nc_qs8(
    ...
    size_t n, size_t* workspace_size, pthreadpool_t threadpool) {
  return reshape_batch_matrix_multiply_nc(
      batch_matrix_multiply_op, xnn_operator_type_batch_matrix_multiply_nc_qs8,
      num_batch_dims, batch_dims_a, batch_dims_b, m, k, n, workspace_size,
