  ...
  if (normalized_reduction_axes[num_reduction_axes - 1] == num_input_dims - 1) {
    ...
    switch (reduce_op->type) {
      case xnn_operator_type_reduce_min_nd:
        reduce_op->context.reduce.ukernel.contiguous_reduce = reduce_op->contiguous_reduce_config->rmin_ukernel;
        break;
      case xnn_operator_type_reduce_max_nd:
        reduce_op->context.reduce.ukernel.contiguous_reduce = reduce_op->contiguous_reduce_config->rmax_ukernel;
        break;
      default:
        reduce_op->context.reduce.ukernel.contiguous_reduce = reduce_op->contiguous_reduce_config->ukernel;
    }
    ...
    switch (reduce_op->type) {
      case xnn_operator_type_reduce_min_nd:
        reduce_op->context.reduce.ukernel.discontiguous_reduce =
            reduce_op->discontiguous_reduce_config->rdmin_ukernel;
        break;
      case xnn_operator_type_reduce_max_nd:
        reduce_op->context.reduce.ukernel.discontiguous_reduce =
            reduce_op->discontiguous_reduce_config->rdmax_ukernel;
        break;
      default:
        reduce_op->context.reduce.ukernel.rdsum =
    ...
    xnn_operator_t* reduce_op_out)
{
  ...
  switch(datatype) {
    case xnn_datatype_fp16: {
      if (is_minmax) {
        ...
        contiguous_config = xnn_init_f16_rminmax_config();
        discontiguous_config = xnn_init_f16_rdminmax_config();
        cvt_config = unused;
      } else {
    ...
    case xnn_datatype_fp32: {
      if (is_minmax) {
        contiguous_config = xnn_init_f32_rminmax_config();
        discontiguous_config = xnn_init_f32_rdminmax_config();
      } else {
    ...
    case xnn_datatype_qint8: { // qs8
      if (is_minmax) {
        ...
        contiguous_config = xnn_init_s8_rminmax_config();
        discontiguous_config = xnn_init_s8_rdminmax_config();
        cvt_config = unused;
      } else {
        log2_accumulator_element_size = 2;
    ...
    case xnn_datatype_quint8: { // qu8
      if (is_minmax) {
        ...
        assert(
          input_quantization->zero_point == output_quantization->zero_point);
        log2_accumulator_element_size = 0;
        contiguous_config = xnn_init_u8_rminmax_config();
        discontiguous_config = xnn_init_u8_rdminmax_config();
        cvt_config = unused;
      } else {
        log2_accumulator_element_size = 2;
  ...
  return create_reduce_nd(
    ...
    contiguous_config, discontiguous_config, cvt_config, &params, params_size,
    &cvt_params, cvt_params_size, reduce_op_out);
}
