...
static enum xnn_status create_reduce_nd(
    uint32_t flags, uint32_t log2_data_element_size,
    ...
    const struct xnn_unary_elementwise_config* cvt_config, const void* params,
    size_t params_size, const void* cvt_params, size_t cvt_params_size,
...
static enum xnn_status reshape_reduce_nd(
    ...
    size_t* workspace_alignment, enum xnn_operator_type expected_operator_type,
  ...
  xnn_normalize_reduction(&num_reduction_axes, normalized_reduction_axes,
  ...
  if (normalized_reduction_axes[num_reduction_axes - 1] == num_input_dims - 1) {
    if (workspace_size != NULL) {
      const size_t num_output_elements = normalized_input_shape[0] *
                                         normalized_input_shape[2] *
                                         normalized_input_shape[4];
      *workspace_size = (num_output_elements << log2_accumulator_element_size) +
                        XNN_EXTRA_BYTES;
    }
    num_reduction_elements = normalized_input_shape[1] *
                             normalized_input_shape[3] *
                             normalized_input_shape[5];
    ...
    reduce_op->context.reduce = (struct reduce_context){
        ...
        .accumulation_element_size = UINT32_C(1)
                                     << log2_accumulator_element_size,
        .output_element_size = UINT32_C(1) << log2_data_element_size,
        .identity_value = reduce_op->reduce.identity_value,
        .ukernel.contiguous_reduce =
            reduce_op->contiguous_reduce_config->ukernel,
    };
    ...
    reduce_op->compute[0].task_3d_tile_2d =
        (pthreadpool_task_3d_tile_2d_t)xnn_compute_contiguous_reduce;
    ...
    for (int i = XNN_MAX_TENSOR_DIMS / 2 - 2; i >= 0; --i) {
      reduce_op->context.reduce.output_stride[i] =
          (reduce_op->context.reduce.output_stride[i + 1] *
           normalized_input_shape[(i + 1) * 2]);
    }
  } else {
    // Reduction along the non-innermost dimension
    const size_t channel_like_dim =
        normalized_input_shape[XNN_MAX_TENSOR_DIMS - 1];
    if (workspace_size != NULL) {
      const size_t num_output_elements = normalized_input_shape[1] *
                                         normalized_input_shape[3] *
                                         normalized_input_shape[5];
      *workspace_size = (num_output_elements << log2_accumulator_element_size) +
                        XNN_EXTRA_BYTES;
    }
    num_reduction_elements = normalized_input_shape[0] *
                             normalized_input_shape[2] *
                             normalized_input_shape[4];
    ...
    if (reduce_op->channels != channel_like_dim) {
      ...
      // Note: zero buffer must be SIMD-aligned, so we can't use
    ...
    reduce_op->compute[0].task_3d_tile_2d =
        (pthreadpool_task_3d_tile_2d_t)xnn_compute_discontiguous_reduce;
  ...
  memcpy(&reduce_op->context.reduce.params, &reduce_op->params.reduce,
         sizeof(reduce_op->params.reduce));
  memcpy(&reduce_op->context.reduce.cvt_params, &reduce_op->params2.unary,
         sizeof(reduce_op->params2.unary));
  reduce_op->context.reduce.input_stride[XNN_MAX_TENSOR_DIMS - 1] =
      (1 << log2_data_element_size);
  if (reduce_op->cvt_config) {
    ...
    //
    //   y = x * x_scale * inv_y_scale - x_zero_point * x_scale * inv_y_scale +
    //   y_zero_point
    //
    // Now we can say:
    //
  ...
  for (int i = XNN_MAX_TENSOR_DIMS - 2; i >= 0; --i) {
    reduce_op->context.reduce.input_stride[i] =
        (reduce_op->context.reduce.input_stride[i + 1] *
         normalized_input_shape[i + 1]);
  }
  memcpy(reduce_op->context.reduce.input_shape, normalized_input_shape,
         XNN_MAX_TENSOR_DIMS * sizeof(size_t));
...
static enum xnn_status setup_reduce_nd(
    xnn_operator_t reduce_op, void* workspace, const float* input,
    float* output, enum xnn_operator_type expected_operator_type) {
  if (reduce_op->type != expected_operator_type) {
    xnn_log_error(
...
enum xnn_status xnn_create_reduce_nd(
    ...
    xnn_operator_t* reduce_op_out) {
  enum xnn_operator_type operator_type =
      xnn_reduce_operator_to_operator_type(reduce_operator_type);
  ...
  switch (datatype) {
    ...
    case xnn_datatype_qint8: {  // qs8
      if (is_minmax) {
        ...
        assert(input_quantization->zero_point ==
    ...
    default:
      xnn_log_error(
          "failed to create Sum (ND) operator: unsupported data type: %s",
  ...
  if (cvt_config && cvt_config->init) {
    cvt_params_size = cvt_config->init(&cvt_params, NULL, input_quantization,
                                       output_quantization);
  }

  return create_reduce_nd(flags, log2_data_element_size,
                          log2_accumulator_element_size, operator_type,
                          contiguous_config, discontiguous_config, fill_config,
                          cvt_config, &params, params_size, &cvt_params,
                          cvt_params_size, reduce_op_out);
}
...
enum xnn_status xnn_reshape_reduce_nd(
  ...
  return reshape_reduce_nd(reduce_op, num_reduction_axes, reduction_axes,
                           num_input_dims, input_shape, workspace_size,
                           workspace_alignment, reduce_op->type, threadpool);
}

enum xnn_status xnn_setup_reduce_nd(xnn_operator_t reduce_op, void* workspace,
                                    const void* input, void* output) {
  return setup_reduce_nd(reduce_op, workspace, input, output, reduce_op->type);
}
