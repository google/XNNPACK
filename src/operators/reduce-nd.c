...
enum xnn_status xnn_create_reduce_nd(
  ...
  switch (datatype) {
    ...
    case xnn_datatype_quint8: {  // qu8
      ...
      // have a 31-bit accumulator instead of 32-bit, but that seems
      // insignificant.
      cvt_config = xnn_init_unary_reference_config(
  ...
  if (rsum_config->init) {
    params_size =
        rsum_config->init(&params, input_quantization, output_quantization);
  }
  ...
  return create_reduce_nd(
      ...
      operator_type, rdsum_config, rsum_config, cvt_config, &params,
