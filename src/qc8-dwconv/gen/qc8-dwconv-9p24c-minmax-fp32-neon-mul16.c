...
void xnn_qc8_dwconv_minmax_fp32_ukernel_9p24c__neon_mul16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
  ...
  do {
    ...
    if XNN_UNLIKELY(c != 0) {
      ...
      do {
        ...
        const float32x4_t vscale0123 = vld1q_f32((const float*) ((uintptr_t) w + 16 * sizeof(int32_t) + 216 * sizeof(int8_t)));
        const float32x4_t vscale4567 = vld1q_f32((const float*) ((uintptr_t) w + 16 * sizeof(int32_t) + 216 * sizeof(int8_t) + 4 * sizeof(float)));
        vfpacc0123 = vmulq_f32(vfpacc0123, vscale0123);
        vfpacc4567 = vmulq_f32(vfpacc4567, vscale4567);
