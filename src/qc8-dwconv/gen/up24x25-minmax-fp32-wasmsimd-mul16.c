...
void xnn_qc8_dwconv_minmax_fp32_ukernel_up24x25__wasmsimd_mul16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
    ...
    const union xnn_qs8_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN XNN_DISABLE_MSAN
{
  ...
  do {
    ...
    if XNN_UNLIKELY(c != 0) {
      ...
      const v128_t vscale0123 = wasm_v128_load((const float*) ((uintptr_t) w + 24 * sizeof(int32_t) + 600 * sizeof(int8_t)));
      const v128_t vscale4567 = wasm_v128_load((const float*) ((uintptr_t) w + 24 * sizeof(int32_t) + 600 * sizeof(int8_t) + 4 * sizeof(float)));
