...
void xnn_pack_qs8_f32_gemm_goi_w(
  size_t g,
  size_t nc,
  size_t kc,
  size_t nr,
  ...
  const struct xnn_qs8_packing_params* params)
{
  ...
  do {
    for (size_t nr_block_start = 0; nr_block_start < nc; nr_block_start += nr) {
      ...
      if XNN_LIKELY(b != NULL) {
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size; nr_block_offset++) {
          unaligned_store_f32(packed_weights, b[nr_block_start + nr_block_offset]);
          packed_weights = (float*) packed_weights + 1;
        }
      } else {
        n = nr_block_size;
        do {
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr); kr_block_start += kr) {
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size; nr_block_offset++) {
          ...
          unaligned_indexed_store_u32(packed_b, nr_block_offset, unaligned_indexed_load_u32(packed_b, nr_block_offset) - ksum);
