...
void xnn_xx_fill_ukernel__rvv_u8v(
    size_t rows,
    size_t channels,
    void* output,
    ...
    const uint32_t fill_pattern) 
{
  ...
  const vuint8m8_t vfill = __riscv_vreinterpret_v_u32m8_u8m8(
      __riscv_vmv_v_x_u32m8(fill_pattern, __riscv_vsetvlmax_e32m8()));

  do {
    size_t c = channels;
    for (size_t vl; c > 0; c -= vl, output += vl) {
      vl = __riscv_vsetvl_e8m8(c);
      __riscv_vse8_v_u8m8(output, vfill, vl);
    }
