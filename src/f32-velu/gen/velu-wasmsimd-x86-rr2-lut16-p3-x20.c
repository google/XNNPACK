    ...
    const union xnn_f32_elu_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 20 * sizeof(float); n -= 20 * sizeof(float)) {
    ...
    vs0123 = wasm_v128_andnot(vs0123, wasm_f32x4_le(vz0123, vsat_cutoff));
    vs4567 = wasm_v128_andnot(vs4567, wasm_f32x4_le(vz4567, vsat_cutoff));
    vs89AB = wasm_v128_andnot(vs89AB, wasm_f32x4_le(vz89AB, vsat_cutoff));
    vsCDEF = wasm_v128_andnot(vsCDEF, wasm_f32x4_le(vzCDEF, vsat_cutoff));
    vsGHIJ = wasm_v128_andnot(vsGHIJ, wasm_f32x4_le(vzGHIJ, vsat_cutoff));
