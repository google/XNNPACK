...
void xnn_f16_vsigmoid_ukernel__rvvfp16arith_rr2_p2_u1v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* restrict params)
{
  ...
  do {
    ...
    vfloat16m1_t vt = __riscv_vfmacc(vz, vln2_hi, vn, n);
    vt = __riscv_vfmacc(vt, vln2_lo, vn, n);

    // Polynomial: p = c1 + c2 * t.
    vfloat16m1_t vp = __riscv_vfmv_v_f_f16m1(vc1, n);
    ...
    // sigmoid = f / (1 + f).
    vfloat16m1_t vy = __riscv_vfdiv(vf, __riscv_vfadd(vf, 1.0f, n), n);
