    ...
    const struct xnn_f16_default_params* restrict params)
{
  ...
  do {
    ...
    // Flush denorms.
    vbool4_t vdenorm_mask = __riscv_vmflt(vz, vdenorm_cutoff, n);
    ...
    vfloat16m4_t vy_compl = __riscv_vfsub(
        __riscv_vfdiv(vf, __riscv_vfadd(vf, 1.0f, n), n), 1.0f, n);
    // Actually cleaner: for x>=0, sig = 1 - f/(1+f) = 1/(1+f)
    vfloat16m4_t vsig_pos = __riscv_vfdiv(
        __riscv_vfmv_v_f_f16m4(1.0f, n),
        __riscv_vfadd(vf, 1.0f, n), n);
    vfloat16m4_t vsig_neg = __riscv_vfdiv(vf, __riscv_vfadd(vf, 1.0f, n), n);
