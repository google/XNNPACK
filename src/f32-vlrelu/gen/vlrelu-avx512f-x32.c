    ...
    const union xnn_f32_lrelu_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; n >= 32 * sizeof(float); n -= 32 * sizeof(float)) {
    ...
    const __mmask16 vsign0123456789ABCDEF = _mm512_movepi32_mask(_mm512_castps_si512(vacc0123456789ABCDEF));
    const __mmask16 vsignGHIJKLMNOPQRSTUV = _mm512_movepi32_mask(_mm512_castps_si512(vaccGHIJKLMNOPQRSTUV));
  ...
  for (; n >= 16 * sizeof(float); n -= 16 * sizeof(float)) {
    ...
    x += 16;
    const __mmask16 vsign = _mm512_movepi32_mask(_mm512_castps_si512(vacc));
