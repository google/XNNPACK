...
void xnn_f32_vlrelu_ukernel__wasmrelaxedsimd_laneselect_u8(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_lrelu_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 8 * sizeof(float); batch -= 8 * sizeof(float)) {
    ...
    vacc0123 = wasm_i32x4_relaxed_laneselect(vacc0123, vx0123, vmask0123);
    vacc4567 = wasm_i32x4_relaxed_laneselect(vacc4567, vx4567, vmask4567);
