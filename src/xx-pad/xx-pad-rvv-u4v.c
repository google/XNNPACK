...
void xnn_xx_pad_ukernel__rvv_u4v(size_t rows, size_t channels,
  ...
  do {
    ...
    if XNN_LIKELY (l != 0) {
      for (size_t vl; l > 0; l -= vl, output = (uint8_t*)output + vl) {
    ...
    // Copy input channels.
    size_t c = channels;
    for (size_t vl; c > 0; c -= vl, output = (uint8_t*)output + vl,
    ...
    if XNN_LIKELY (r != 0) {
      for (size_t vl; r > 0; r -= vl, output = (uint8_t*)output + vl) {
