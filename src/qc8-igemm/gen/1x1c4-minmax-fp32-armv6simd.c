...
//   Template: src/qs8-igemm/c4-armv6simd.c.in
...
void xnn_qc8_igemm_minmax_fp32_ukernel_1x1c4__armv6simd(
    size_t mr,
    size_t nc,
    size_t kc,
    size_t ks,
    ...
    const union xnn_qs8_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  const float vmagic_bias = params->armv6simd.magic_bias;
  do {
    ...
    const int32_t vmagic_bias_less_zero_point = params->armv6simd.magic_bias_less_zero_point;
    ...
    const int8x4_t voutput_min = (int8x4_t) params->armv6simd.output_min;
    __ssub8((int8x4_t) vout, voutput_min);
    vout = (uint32_t) __sel((uint8x4_t) vout, (uint8x4_t) voutput_min);

    const int8x4_t voutput_max = (int8x4_t) params->armv6simd.output_max;
