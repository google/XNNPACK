...
void xnn_qs8_vhswish_ukernel__rvv_u1v(
    size_t batch,
    const int8_t* input,
    int8_t* output,
    const union xnn_qs8_hswish_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  vint8m1_t inzp_i8v = __riscv_vmv_v_x_i8m1(input_zero_point, __riscv_vsetvl_e8m1(batch));

  do {
    int32_t n = __riscv_vsetvl_e8m1(batch); batch -= n;

    vint8m1_t in_i8v = __riscv_vle8_v_i8m1(input, n); input += n;
    vint16m2_t insub_i16v = __riscv_vwsub_vv_i16m2(inzp_i8v, in_i8v, n);
    ...
    vint8m1_t out_i8v = __riscv_vnclip_wx_i8m1(add_i16v, 0, 0, n);
