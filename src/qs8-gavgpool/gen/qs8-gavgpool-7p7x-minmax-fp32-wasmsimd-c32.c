...
void xnn_qs8_gavgpool_minmax_fp32_ukernel_7p7x__wasmsimd_c32(
    size_t rows,
    size_t channels,
    const int8_t* input,
    size_t input_stride,
    ...
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; c >= 32; c -= 32) {
    ...
    const v128_t vaccOPQR = wasm_i32x4_add(vinit_bias, wasm_i32x4_extend_low_i16x8(vaccOPQRSTUV));
    const v128_t vaccSTUV = wasm_i32x4_add(vinit_bias, wasm_i32x4_extend_high_i16x8(vaccOPQRSTUV));
