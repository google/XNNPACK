...
void xnn_qs8_gavgpool_minmax_fp32_ukernel_7p7x__sse41_c24_acc2(
    size_t rows,
    size_t channels,
    const int8_t* input,
    size_t input_stride,
    ...
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; c >= 24; c -= 24) {
    ...
    const __m128i vaccGHIJ = _mm_add_epi32(_mm_cvtepi16_epi32(vacc0xGHIJKLMN), vinit_bias);
    const __m128i vaccKLMN = _mm_add_epi32(_mm_srai_epi32(_mm_unpackhi_epi16(vacc0xGHIJKLMN, vacc0xGHIJKLMN), 16), vinit_bias);
  ...
  for (rows -= 7; rows > 7; rows -= 7) {
    ...
    for (; c >= 24; c -= 24) {
      ...
      const __m128i vaccKLMN = _mm_add_epi32(_mm_srai_epi32(_mm_unpackhi_epi16(vacc0xGHIJKLMN, vacc0xGHIJKLMN), 16), _mm_load_si128((const __m128i*) (b + 20)));
