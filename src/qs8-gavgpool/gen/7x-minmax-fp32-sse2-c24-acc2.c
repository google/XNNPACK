...
void xnn_qs8_gavgpool_minmax_fp32_ukernel_7x__sse2_c24_acc2(
    size_t rows,
    size_t channels,
    const int8_t* input,
    size_t input_stride,
    ...
    const union xnn_qs8_avgpool_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; channels >= 24; channels -= 24) {
    ...
    __m128i vaccGHIJ = _mm_add_epi32(vinit_bias, _mm_unpacklo_epi16(vacc0xGHIJKLMN, vsgnacc0xGHIJKLMN));
    __m128i vaccKLMN = _mm_add_epi32(vinit_bias, _mm_unpackhi_epi16(vacc0xGHIJKLMN, vsgnacc0xGHIJKLMN));
