...
void xnn_f32_dwconv_minmax_ukernel_2f2m2l32c8s4r__avx(
    size_t channels,
    size_t output_width,
    const float** input,
    const float* weights,
  ...
  do {
    ...
    // First pass to process 2 inputs.
    {
      ...
      for (; c >= 32; c -= 32) {
        ...
        vaccGHIJKLMNp0 = _mm256_add_ps(vaccGHIJKLMNp0, _mm256_mul_ps(vi1xGHIJKLMN, vk1xGHIJKLMN));
        vaccOPQRSTUVp0 = _mm256_add_ps(vaccOPQRSTUVp0, _mm256_mul_ps(vi1xOPQRSTUV, vk1xOPQRSTUV));
