...
void xnn_f32_dwconv_minmax_ukernel_2f2m2l16c4s4r__wasmrelaxedsimd_fma_acc2(
    size_t channels,
    size_t output_width,
    const float** input,
    const float* weights,
  ...
  do {
    ...
    // First pass to process 2 inputs.
    {
      ...
      for (; c >= 16; c -= 16) {
        ...
        v128_t vacc89ABp1 = wasm_f32x4_mul(vi1x89AB, vk1x89AB);
        v128_t vaccCDEFp1 = wasm_f32x4_mul(vi1xCDEF, vk1xCDEF);
        ...
        vacc89ABp0 = wasm_f32x4_add(vacc89ABp0, vacc89ABp1);
        vaccCDEFp0 = wasm_f32x4_add(vaccCDEFp0, vaccCDEFp1);
    ...
    // Last pass to process up to 2 inputs.
    {
      ...
      for (; c >= 16; c -= 16) {
        ...
        v128_t vacc89AB = __builtin_wasm_relaxed_max_f32x4(vacc89ABp0, vmin);
        v128_t vaccCDEF = __builtin_wasm_relaxed_max_f32x4(vaccCDEFp0, vmin);

        vacc0123 = __builtin_wasm_relaxed_min_f32x4(vacc0123, vmax);
        vacc4567 = __builtin_wasm_relaxed_min_f32x4(vacc4567, vmax);
        vacc89AB = __builtin_wasm_relaxed_min_f32x4(vacc89AB, vmax);
        vaccCDEF = __builtin_wasm_relaxed_min_f32x4(vaccCDEF, vmax);
