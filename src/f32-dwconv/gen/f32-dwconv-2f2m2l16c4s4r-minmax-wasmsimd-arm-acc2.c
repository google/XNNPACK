...
void xnn_f32_dwconv_minmax_ukernel_2f2m2l16c4s4r__wasmsimd_arm_acc2(
    size_t channels,
    size_t output_width,
    const float** input,
    const float* weights,
  ...
  do {
    ...
    // Last pass to process up to 2 inputs.
    {
      ...
      for (; c >= 16; c -= 16) {
        ...
        v128_t vacc89AB = wasm_f32x4_max(vacc89ABp0, vmin);
        v128_t vaccCDEF = wasm_f32x4_max(vaccCDEFp0, vmin);

        vacc0123 = wasm_f32x4_min(vacc0123, vmax);
        vacc4567 = wasm_f32x4_min(vacc4567, vmax);
        vacc89AB = wasm_f32x4_min(vacc89AB, vmax);
        vaccCDEF = wasm_f32x4_min(vaccCDEF, vmax);
