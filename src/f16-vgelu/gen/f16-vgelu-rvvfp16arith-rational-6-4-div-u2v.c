...
void xnn_f16_vgelu_ukernel__rvvfp16arith_rational_6_4_div_u2v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m2_t vx_orig = __riscv_vle16_v_f16m2(input, n);
    input += n;

    // Clamp.
    vfloat16m2_t vx = __riscv_vfmin(vx_orig, vmax_abs_x, n);
    vx = __riscv_vfmax(vx, -vmax_abs_x, n);

    vfloat16m2_t vx2 = __riscv_vfmul(vx, vx, n);

    // Numerator p (odd polynomial).
    vfloat16m2_t vp = __riscv_vfmv_v_f_f16m2(valpha_3, n);
    vp = __riscv_vfmacc(vp, valpha_5, vx2, n);
    vp = __riscv_vfmadd(vp, vx2, __riscv_vfmv_v_f_f16m2(valpha_1, n), n);
    vp = __riscv_vfmul(vp, vx, n);

    // Denominator q (even polynomial).
    vfloat16m2_t vq = __riscv_vfmv_v_f_f16m2(vbeta_2, n);
    vq = __riscv_vfmacc(vq, vbeta_4, vx2, n);
    vq = __riscv_vfmadd(vq, vx2, __riscv_vfmv_v_f_f16m2(1.0f, n), n);

    // erf = p / q.
    vfloat16m2_t verf = __riscv_vfdiv(vp, vq, n);

    // y = x_orig * 0.5 * (1 + erf).
    vfloat16m2_t vy = __riscv_vfadd(verf, 1.0f, n);
