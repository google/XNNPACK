...
void xnn_u8_rdmax_ukernel_2p2x__scalar_u2(
    size_t rows,
    size_t channels,
    const uint8_t* input,
    size_t input_stride,
    ...
    const void* params)
{
  ...
  for (; channels >= 2; channels -= 2) {
    ...
    xnn_simd_u8_t vmax1 = xnn_loadu_u8(output + 1 * sizeof(uint8_t));;
    ...
    for (int r = rows; r > 0; r -= 2) {
      ...
      vmax1 = xnn_max_u8(vmax1, vin_0_1);
      vmax0 = xnn_max_u8(vmax0, vin_1_0);
      vmax1 = xnn_max_u8(vmax1, vin_1_1);
