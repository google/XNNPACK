...
#else
// This define needs to come first because errno include features.h and would
// have defined macros that lead to sys/mman.h not having mremap.
...
// Helpers to allocate/mmap and release memory used by both code and weights
// cache.
...
static void* allocate_buffer(size_t size) {
  ...
  if (p == NULL) {
    xnn_log_error(
        "failed to allocate %zu bytes for code/weights buffer, error code: "
        "%" PRIu32,
  ...
  if (p == NULL) {
    xnn_log_error(
        "failed to allocate %zu bytes for code/weights buffer, error code: %d",
        size, errno);
    return NULL;
  }
#elif !XNN_HAS_MMAP
...
// Releases memory previously mapped by `allocate_buffer`, returns
// xnn_status_success on success.
static enum xnn_status release_memory(void* start, size_t capacity) {
  ...
  if (!VirtualFree(start, 0, MEM_RELEASE)) {
    xnn_log_error("failed to release code/weights buffer, error code: %" PRIu32,
  ...
  if (munmap(start, capacity) == -1) {
    xnn_log_error("failed to release code/weights buffer, error code: %d",
                  errno);
    return xnn_status_invalid_state;
  }
#endif
...
// Resize a buffer at old_pointer of size old_bytes to new_size. The actual new
// size of the resized buffer is written to new_capacity_out, which can be >=
// new_size due to page alignment requirements. Returns a pointer to a buffer
// which might be the same as old_pointer if we can remap virtual memory,
// otherwise we allocate a new buffer and copy contents of old_buffer over.
static void* resize_buffer(void* old_pointer, size_t old_size,
                           size_t old_capacity, size_t new_size,
                           size_t* new_capacity_out) {
  const size_t new_capacity = round_up_po2(new_size, get_page_size());
#if XNN_PLATFORM_LINUX
  void* new_pointer =
      mremap(old_pointer, old_capacity, new_capacity, MREMAP_MAYMOVE, NULL);
  if (new_pointer == MAP_FAILED) {
    xnn_log_error("mremap failed with errno: %d", errno);
    return NULL;
  }
  xnn_log_debug("resize_buffer: remap, old capacity %zu to new capacity %zu",
  ...
  if (status != xnn_status_success) {
    xnn_log_error(
        "releasing old buffer failed, this could be a leak of %zu bytes",
        old_capacity);
    // Log but proceed as per normal since we successfully allocated a new
    // memory that can be used by the caller.
  }
  xnn_log_debug(
      "resize_buffer: allocate memory, old capacity %zu to new capacity %zu",
      old_capacity, new_capacity);
#endif
...
static enum xnn_status release_unused_memory(size_t size, void* start,
                                             size_t* capacity) {
  ...
  xnn_log_debug(
      "releasing memory, start %p, used: %zu, capacity: %zu, unused %zu",
      mem_start, size, *capacity, unused_capacity);

  if (unused_capacity != 0) {
// Free unused pages.
#if XNN_PLATFORM_WINDOWS
    // We cannot selectively release pages inside the region of pages, so just
    // decommit them.
    if (!VirtualFree((void*)unused_start, unused_capacity, MEM_DECOMMIT)) {
      xnn_log_error("failed to unmap weights buffer, error code: %" PRIu32,
                    (uint32_t)GetLastError());
      return xnn_status_invalid_state;
    }
...
static enum xnn_status set_memory_permission(
    void* start, size_t size, enum xnn_memory_permission permission) {
#if XNN_PLATFORM_WINDOWS
  DWORD old = 0, prot = 0;
  switch (permission) {
  ...
  if (!VirtualProtect(start, size, prot, &old)) {
    xnn_log_error("failed to set memory permission (%d), error code: %" PRIu32,
                  permission, (uint32_t)GetLastError());
    return xnn_status_invalid_state;
  }
  ...
  if (mprotect(start, size, prot) == -1) {
    xnn_log_error("failed to set memory permission (%d), error code: %d",
                  permission, errno);
    return xnn_status_invalid_state;
  }
#endif
...
enum xnn_status xnn_allocate_weights_memory(struct xnn_weights_buffer* buffer,
                                            size_t size) {
...
enum xnn_status xnn_release_weights_memory(struct xnn_weights_buffer* buffer) {
  ...
  const enum xnn_status status =
      release_memory(buffer->start, buffer->capacity);
  if (status != xnn_status_success) {
    return status;
  }
  memset(buffer, 0, sizeof(struct xnn_weights_buffer));
...
enum xnn_status xnn_reserve_weights_memory(struct xnn_weights_buffer* buffer,
                                           size_t min_available_size) {
  if (buffer->size + min_available_size <= buffer->capacity) {
    xnn_log_debug("reserving weights memory of size %zu without growing buffer",
                  min_available_size);
    return xnn_status_success;
  }
  ...
  void* new_start =
      resize_buffer(buffer->start, buffer->size, buffer->capacity,
                    buffer->size + min_available_size, &new_capacity);
  if (new_start == NULL) {
...
enum xnn_status xnn_finalize_weights_memory(struct xnn_weights_buffer* buffer) {
  const enum xnn_status status =
      release_unused_memory(buffer->size, buffer->start, &buffer->capacity);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  return set_memory_permission(buffer->start, buffer->size,
                               xnn_memory_permission_read_only);
}
