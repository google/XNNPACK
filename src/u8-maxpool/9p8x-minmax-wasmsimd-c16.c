  ...
  do {
    uint8_t* o = output;
    {
      ...
      for (; c >= 16; c -= 16) {
        ...
        const v128_t vmax = wasm_u8x16_max(vmax2345, vmax01678);
        const v128_t vout = wasm_u8x16_max(wasm_u8x16_min(vmax, voutput_max), voutput_min);
      ...
      if (c != 0) {
        ...
        const v128_t vmax01678 = wasm_u8x16_max(vmax018, vmax67);
        const v128_t vmax = wasm_u8x16_max(vmax2345, vmax01678);
        v128_t vout = wasm_u8x16_max(wasm_u8x16_min(vmax, voutput_max), voutput_min);
        ...
        if (c & 2) {
          *((uint16_t*) o) = (uint16_t) wasm_i16x8_extract_lane(vout, 0);
          vout = wasm_u32x4_shr(vout, 16);
          o += 2;
        }
        if (c & 1) {
          *((uint8_t*) o) = (uint8_t) wasm_i8x16_extract_lane(vout, 0);
          o += 1;
        }
      }
    }
    ...
    for (ptrdiff_t k = (ptrdiff_t) kernel_elements - 9; k > 0; k -= 8) {
      ...
      for (; c >= 16; c -= 16) {
        ...
        const v128_t vmax = wasm_u8x16_max(vmax2345, vmax0167);
        const v128_t vout = wasm_u8x16_max(wasm_u8x16_min(vmax, voutput_max), voutput_min);
