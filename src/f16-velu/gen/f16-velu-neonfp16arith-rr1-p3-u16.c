    ...
    const union xnn_f16_elu_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  const float16x8_t vsat_cutoff = vmovq_n_u16(UINT16_C(0xC829));  // -0x1.0A4p+3h
  const float16x8_t vmagic_bias = vmovq_n_u16(UINT16_C(0x660F));  // 0x1.83Cp+10h
  const float16x8_t vlog2e = vmovq_n_u16(UINT16_C(0x3DC5));  // 0x1.714p+0h
  const float16x8_t vminus_ln2 = vmovq_n_u16(UINT16_C(0xB98C));  // -0x1.62E430p-1h
  const float16x8_t vc3 = vmovq_n_u16(UINT16_C(0x315B));  // 0x1.56Cp-3h
  const float16x8_t vc2 = vmovq_n_u16(UINT16_C(0x3808));  // 0x1.020p-1h
