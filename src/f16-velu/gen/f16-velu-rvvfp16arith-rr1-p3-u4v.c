...
void xnn_f16_velu_ukernel__rvvfp16arith_rr1_p3_u4v(
    size_t batch,
    ...
    const struct xnn_f16_elu_params* restrict params)
{
  ...
  do {
    ...
    vfloat16m4_t vz = __riscv_vfmul(vx, vprescale, n);
    ...
    vfloat16m4_t vt = __riscv_vfmacc(vz, vminus_ln2, vn, n);
    ...
    vfloat16m4_t ve = __riscv_vfmul(
        __riscv_vfadd(vp, vs, n), valpha, n);

    // Select: y = x < 0 ? e : x * beta
    vfloat16m4_t vy = __riscv_vfmul(vx, vbeta, n);
    vbool4_t mask = __riscv_vmflt(vx, 0.0f, n);
