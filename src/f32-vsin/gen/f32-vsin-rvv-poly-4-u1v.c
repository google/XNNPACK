...
void xnn_f32_vsin_ukernel__rvv_poly_4_u1v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vx = __riscv_vfmacc_vf_f32m1(vx, vpi_0, vk_float, vl);
    vx = __riscv_vfmacc_vf_f32m1(vx, vpi_1, vk_float, vl);
    vx = __riscv_vfmacc_vf_f32m1(vx, vpi_2, vk_float, vl);

    vuint32m1_t vu_k = __riscv_vreinterpret_u32m1(vk);
    vuint32m1_t vsign_flip = __riscv_vsll(vu_k, 31u, vl);
