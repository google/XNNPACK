...
static XNN_INLINE vfloat32m8_t xnn_round_f32(vfloat32m8_t a, size_t vl) {
  // preserve NaN
  vbool4_t nan_bv = __riscv_vmfeq(a, a, vl);
  // magnitude < (1 << FLT_MANT_DIG)
  vfloat32m8_t mag = __riscv_vfabs(a, vl);
  vbool4_t mag_bv = __riscv_vmflt(mag, (1 << __FLT_MANT_DIG__), vl);
  vbool4_t mask_bv = __riscv_vmnand(nan_bv, mag_bv, vl);

  vint32m8_t a_rnd = __riscv_vfcvt_x(a, __RISCV_FRM_RNE, vl);
  vfloat32m8_t result = __riscv_vfcvt_f(a_rnd, vl);
  result = __riscv_vmerge(result, a, mask_bv, vl);
  return result;
}


void xnn_f32_vsin_ukernel__rvv_rational_5_4_div_u8v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat32m8_t vx_div_2pi = __riscv_vfmul(vx, v2pi_inv, vl);
    ...
    const vfloat32m8_t vx2 = __riscv_vfmul(vx, vx, vl);

    // Evaluate the numerator polynomial p.
    vfloat32m8_t vp = __riscv_vfadd(__riscv_vfmul(vx2, valpha_5, vl), valpha_3, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx2, vp, vl), vone, vl);
    vp = __riscv_vfmul(vx, vp, vl);

    // Evaluate the denominator polynomial q.
    vfloat32m8_t vq = __riscv_vfadd(__riscv_vfmul(vx2, vbeta_4, vl), vbeta_2, vl);
