...
static void xnn_qs8_qc4w_packw_gemm_goi_internal_x8c8__scalar_oddkc(
  size_t g,
  size_t nc,
  size_t kc,
  size_t nr,
  ...
  const struct xnn_qs8_qc4w_packing_params* params)
{
  ...
  do {
    ...
    do {
      const size_t nr_block_size = min(nc - nr_block_start, 8);
      ...
      packed_weights = (int32_t*) packed_weights + 8;
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr * 2); kr_block_start += 8 * 2) {
        ...
        packed_weights = (uint8_t*) packed_weights + (8 - nr_block_size) * 8;  // skip NR remainder
      }
  ...
  const struct xnn_qs8_qc4w_packing_params* params)
{
  ...
  if (kc & 1) {
    xnn_qs8_qc4w_packw_gemm_goi_internal_x8c8__scalar_oddkc(
      g, nc, kc, nr, kr, sr,
