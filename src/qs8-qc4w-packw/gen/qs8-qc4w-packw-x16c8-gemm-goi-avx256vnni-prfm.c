...
// Load 'n' bytes from 'address' into a uint64_t, padding the remaining bytes with 'pad_value'.
// We initialize 'value' by replicating 'pad_value' across all 8 bytes.
// Then we overwrite the first 'n' bytes with the actual data.
// This is used for safe loading at the end of a row, ensuring correct padding (e.g. zero point).
...
// For 4-bit quantization (QS4), each byte contains two 4-bit weights.
// When padding, we must replicate the 4-bit kernel_zero_point (0-15) into both
// the low and high nibbles of the pad byte.
// Multiplying by 0x11 (17) achieves this: ZP * 0x11 = (ZP << 4) | ZP.
  ...
  do {
    ...
    for (;n >= 16; n -= 16) {
      ...
      for (; k >= 32; k -= 32) {
        ...
        v8_0 = xnn_packed2planar(&vacc8, v8_0, vmask, vone);
        v8_1 = xnn_packed2planar(&vacc8, v8_1, vmask, vone);
        v8_2 = xnn_packed2planar(&vacc8, v8_2, vmask, vone);
        v8_3 = xnn_packed2planar(&vacc8, v8_3, vmask, vone);
        v12_0 = xnn_packed2planar(&vacc12, v12_0, vmask, vone);
        v12_1 = xnn_packed2planar(&vacc12, v12_1, vmask, vone);
        v12_2 = xnn_packed2planar(&vacc12, v12_2, vmask, vone);
        v12_3 = xnn_packed2planar(&vacc12, v12_3, vmask, vone);
      ...
      for (; k >= 8; k -= 8) {
        ...
        v8 = xnn_packed2planar(&vacc8, v8, vmask, vone);
        v12 = _mm256_xor_si256(v12, vkernel_zero_point);    // uint4 -> int4
        v12 = xnn_packed2planar(&vacc12, v12, vmask, vone);
      ...
      if (k != 0) {
        ...
        __m256i v8 = _mm256_set1_epi64x((int64_t) safe_load_u64(w8, k, params->kernel_zero_point * 0x11));
        v8 = _mm256_blend_epi32(v8, _mm256_set1_epi64x((int64_t) safe_load_u64(w9, k, params->kernel_zero_point * 0x11)), 0x0C);
        v8 = _mm256_blend_epi32(v8, _mm256_set1_epi64x((int64_t) safe_load_u64(w10, k, params->kernel_zero_point * 0x11)), 0x30);
        v8 = _mm256_blend_epi32(v8, _mm256_set1_epi64x((int64_t) safe_load_u64(w11, k, params->kernel_zero_point * 0x11)), 0xC0);
        __m256i v12 = _mm256_set1_epi64x((int64_t) safe_load_u64(w12, k, params->kernel_zero_point * 0x11));
        v12 = _mm256_blend_epi32(v12, _mm256_set1_epi64x((int64_t) safe_load_u64(w13, k, params->kernel_zero_point * 0x11)), 0x0C);
        v12 = _mm256_blend_epi32(v12, _mm256_set1_epi64x((int64_t) safe_load_u64(w14, k, params->kernel_zero_point * 0x11)), 0x30);
        v12 = _mm256_blend_epi32(v12, _mm256_set1_epi64x((int64_t) safe_load_u64(w15, k, params->kernel_zero_point * 0x11)), 0xC0);
