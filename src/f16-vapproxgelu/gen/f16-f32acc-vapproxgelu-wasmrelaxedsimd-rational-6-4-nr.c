// clang-format off
// Auto-generated file. Do not edit!
//   Template: src/f16-vapproxgelu/f16-f32acc.c.in
//   Generator: tools/xngen
//
// Copyright 2026 Google LLC
//
// This source code is licensed under the BSD-style license found in the
// LICENSE file in the root directory of this source tree.

#include <assert.h>
#include <stddef.h>
#include <stdint.h>

#include "src/xnnpack/common.h"
#include "src/xnnpack/microparams.h"
#include "src/xnnpack/simd/f32-wasmrelaxedsimd.h"

#undef XNN_SIMD_HAS_NATIVE_FMA
#include "src/xnnpack/simd/f16-wasmrelaxedsimd.h"
#include "src/xnnpack/vunary.h"

// Helper functions for f16 <-> f32 conversion using xnn_simd_f32_t.
static XNN_INLINE xnn_simd_f32_t xnn_simd_load_f16_f32(const xnn_float16* ptr) {
  return xnn_cvt_f32_f16(xnn_loadu_f16(ptr));
}

static XNN_INLINE void xnn_simd_store_f32_f16(xnn_float16* ptr, xnn_simd_f32_t v) {
  xnn_store_tail_f16(ptr, xnn_cvt_f16_f32(v), xnn_simd_size_f32);
}

static XNN_INLINE xnn_simd_f32_t xnn_simd_load_tail_f16_f32(const xnn_float16* ptr, size_t elements) {
  return xnn_cvt_f32_f16(xnn_load_tail_f16(ptr, elements));
}

static XNN_INLINE void xnn_simd_store_tail_f32_f16(xnn_float16* ptr, xnn_simd_f32_t v, size_t elements) {
  xnn_store_tail_f16(ptr, xnn_cvt_f16_f32(v), elements);
}


void xnn_f16_f32acc_vapproxgelu_ukernel__wasmrelaxedsimd_rational_6_4_nr_u4(
    size_t batch,
    const xnn_float16* input,
    xnn_float16* output,
    const struct xnn_f16_default_params* unused_params)
{
  assert(batch != 0);
  assert(batch % sizeof(xnn_float16) == 0);
  assert(input != NULL);
  assert(output != NULL);
  assert(xnn_simd_size_f32 == 4);

  const xnn_float16* i = input;
  xnn_float16* o = output;

  // Cap the inputs to this value as `erf(x/sqrt(2))` will always be `+/-1.0f`
  // beyond this point. This value is chosen as the first floating point
  // number as of which the interpolation returns +/-1.0f.
#if XNN_SIMD_HAS_NATIVE_FMA
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26367188e+00f);
#else
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26953125e+00f);
#endif  // XNN_SIMD_HAS_NATIVE_FMA

  // The monomial coefficients of the numerator polynomial (odd).
  XNN_SIMD_CONST_F32(valpha_1, 7.9763704538e-01f);
  XNN_SIMD_CONST_F32(valpha_3, 1.4811584353e-01f);
  XNN_SIMD_CONST_F32(valpha_5, 4.3507730588e-03f);

  // The monomial coefficients of the denominator polynomial (even).
  XNN_SIMD_CONST_F32(vbeta_2, 3.5130375624e-01f);
  XNN_SIMD_CONST_F32(vbeta_4, 4.0821321309e-02f);

  XNN_SIMD_CONST_F32(vone, 1.0f);
  XNN_SIMD_CONST_F32(vhalf, 0.5f);

  for (; batch >= xnn_simd_size_f32 * sizeof(xnn_float16); batch -= xnn_simd_size_f32 * sizeof(xnn_float16)) {
    const xnn_simd_f32_t vx_orig = xnn_simd_load_f16_f32(i);
    i += xnn_simd_size_f32;

    // Clamp the inputs to the interpolation range.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);

    // Since the polynomials are odd/even, we need x^2.
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);

    // Divide the numerator by the denominator.
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));

    // Add one to the rational interpolant, and multiply by 0.5 times the
    // original input.
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_f32_f16(o, vy);
    o += xnn_simd_size_f32;
  }
  if XNN_UNLIKELY(batch != 0) {
    xnn_simd_f32_t vx_orig = xnn_simd_load_tail_f16_f32(i, batch >> XNN_LOG2_SIZEOF_FLOAT16);

    // See above for comments.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_tail_f32_f16(o, vy, batch >> XNN_LOG2_SIZEOF_FLOAT16);
  }
}

void xnn_f16_f32acc_vapproxgelu_ukernel__wasmrelaxedsimd_rational_6_4_nr_u8(
    size_t batch,
    const xnn_float16* input,
    xnn_float16* output,
    const struct xnn_f16_default_params* unused_params)
{
  assert(batch != 0);
  assert(batch % sizeof(xnn_float16) == 0);
  assert(input != NULL);
  assert(output != NULL);
  assert(xnn_simd_size_f32 == 4);

  const xnn_float16* i = input;
  xnn_float16* o = output;

  // Cap the inputs to this value as `erf(x/sqrt(2))` will always be `+/-1.0f`
  // beyond this point. This value is chosen as the first floating point
  // number as of which the interpolation returns +/-1.0f.
#if XNN_SIMD_HAS_NATIVE_FMA
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26367188e+00f);
#else
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26953125e+00f);
#endif  // XNN_SIMD_HAS_NATIVE_FMA

  // The monomial coefficients of the numerator polynomial (odd).
  XNN_SIMD_CONST_F32(valpha_1, 7.9763704538e-01f);
  XNN_SIMD_CONST_F32(valpha_3, 1.4811584353e-01f);
  XNN_SIMD_CONST_F32(valpha_5, 4.3507730588e-03f);

  // The monomial coefficients of the denominator polynomial (even).
  XNN_SIMD_CONST_F32(vbeta_2, 3.5130375624e-01f);
  XNN_SIMD_CONST_F32(vbeta_4, 4.0821321309e-02f);

  XNN_SIMD_CONST_F32(vone, 1.0f);
  XNN_SIMD_CONST_F32(vhalf, 0.5f);

  for (; batch >= 8 * sizeof(xnn_float16); batch -= 8 * sizeof(xnn_float16)) {
    const xnn_simd_f32_t vx_orig_0 = xnn_simd_load_f16_f32(i + 0 * xnn_simd_size_f32);
    const xnn_simd_f32_t vx_orig_1 = xnn_simd_load_f16_f32(i + 1 * xnn_simd_size_f32);
    i += 8;

    // Clamp the inputs to the interpolation range.
    xnn_simd_f32_t vx_0 = xnn_min_f32(vmax_abs_x, vx_orig_0);
    xnn_simd_f32_t vx_1 = xnn_min_f32(vmax_abs_x, vx_orig_1);
    vx_0 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_0);
    vx_1 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_1);

    // Since the polynomials are odd/even, we need x^2.
    const xnn_simd_f32_t vx2_0 = xnn_mul_f32(vx_0, vx_0);
    const xnn_simd_f32_t vx2_1 = xnn_mul_f32(vx_1, vx_1);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp_0 = xnn_fmadd_f32(vx2_0, valpha_5, valpha_3);
    xnn_simd_f32_t vp_1 = xnn_fmadd_f32(vx2_1, valpha_5, valpha_3);
    vp_0 = xnn_fmadd_f32(vx2_0, vp_0, valpha_1);
    vp_1 = xnn_fmadd_f32(vx2_1, vp_1, valpha_1);
    vp_0 = xnn_mul_f32(vx_0, vp_0);
    vp_1 = xnn_mul_f32(vx_1, vp_1);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq_0 = xnn_fmadd_f32(vx2_0, vbeta_4, vbeta_2);
    xnn_simd_f32_t vq_1 = xnn_fmadd_f32(vx2_1, vbeta_4, vbeta_2);
    vq_0 = xnn_fmadd_f32(vx2_0, vq_0, vone);
    vq_1 = xnn_fmadd_f32(vx2_1, vq_1, vone);

    // Divide the numerator by the denominator.
    const xnn_simd_f32_t verf_0 = xnn_mul_f32(vp_0, xnn_rcp_f32(vq_0));
    const xnn_simd_f32_t verf_1 = xnn_mul_f32(vp_1, xnn_rcp_f32(vq_1));

    // Add one to the rational interpolant, and multiply by 0.5 times the
    // original input.
    const xnn_simd_f32_t vy_0 = xnn_mul_f32(xnn_mul_f32(vx_orig_0, vhalf),
                                        xnn_add_f32(verf_0, vone));
    const xnn_simd_f32_t vy_1 = xnn_mul_f32(xnn_mul_f32(vx_orig_1, vhalf),
                                        xnn_add_f32(verf_1, vone));

    xnn_simd_store_f32_f16(o + 0 * xnn_simd_size_f32, vy_0);
    xnn_simd_store_f32_f16(o + 1 * xnn_simd_size_f32, vy_1);
    o += 8;
  }
  for (; batch >= xnn_simd_size_f32 * sizeof(xnn_float16); batch -= xnn_simd_size_f32 * sizeof(xnn_float16)) {
    const xnn_simd_f32_t vx_orig = xnn_simd_load_f16_f32(i);
    i += xnn_simd_size_f32;

    // Clamp the inputs to the interpolation range.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);

    // Since the polynomials are odd/even, we need x^2.
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);

    // Divide the numerator by the denominator.
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));

    // Add one to the rational interpolant, and multiply by 0.5 times the
    // original input.
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_f32_f16(o, vy);
    o += xnn_simd_size_f32;
  }
  if XNN_UNLIKELY(batch != 0) {
    xnn_simd_f32_t vx_orig = xnn_simd_load_tail_f16_f32(i, batch >> XNN_LOG2_SIZEOF_FLOAT16);

    // See above for comments.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_tail_f32_f16(o, vy, batch >> XNN_LOG2_SIZEOF_FLOAT16);
  }
}

void xnn_f16_f32acc_vapproxgelu_ukernel__wasmrelaxedsimd_rational_6_4_nr_u16(
    size_t batch,
    const xnn_float16* input,
    xnn_float16* output,
    const struct xnn_f16_default_params* unused_params)
{
  assert(batch != 0);
  assert(batch % sizeof(xnn_float16) == 0);
  assert(input != NULL);
  assert(output != NULL);
  assert(xnn_simd_size_f32 == 4);

  const xnn_float16* i = input;
  xnn_float16* o = output;

  // Cap the inputs to this value as `erf(x/sqrt(2))` will always be `+/-1.0f`
  // beyond this point. This value is chosen as the first floating point
  // number as of which the interpolation returns +/-1.0f.
#if XNN_SIMD_HAS_NATIVE_FMA
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26367188e+00f);
#else
  XNN_SIMD_CONST_F32(vmax_abs_x, 3.26953125e+00f);
#endif  // XNN_SIMD_HAS_NATIVE_FMA

  // The monomial coefficients of the numerator polynomial (odd).
  XNN_SIMD_CONST_F32(valpha_1, 7.9763704538e-01f);
  XNN_SIMD_CONST_F32(valpha_3, 1.4811584353e-01f);
  XNN_SIMD_CONST_F32(valpha_5, 4.3507730588e-03f);

  // The monomial coefficients of the denominator polynomial (even).
  XNN_SIMD_CONST_F32(vbeta_2, 3.5130375624e-01f);
  XNN_SIMD_CONST_F32(vbeta_4, 4.0821321309e-02f);

  XNN_SIMD_CONST_F32(vone, 1.0f);
  XNN_SIMD_CONST_F32(vhalf, 0.5f);

  for (; batch >= 16 * sizeof(xnn_float16); batch -= 16 * sizeof(xnn_float16)) {
    const xnn_simd_f32_t vx_orig_0 = xnn_simd_load_f16_f32(i + 0 * xnn_simd_size_f32);
    const xnn_simd_f32_t vx_orig_1 = xnn_simd_load_f16_f32(i + 1 * xnn_simd_size_f32);
    const xnn_simd_f32_t vx_orig_2 = xnn_simd_load_f16_f32(i + 2 * xnn_simd_size_f32);
    const xnn_simd_f32_t vx_orig_3 = xnn_simd_load_f16_f32(i + 3 * xnn_simd_size_f32);
    i += 16;

    // Clamp the inputs to the interpolation range.
    xnn_simd_f32_t vx_0 = xnn_min_f32(vmax_abs_x, vx_orig_0);
    xnn_simd_f32_t vx_1 = xnn_min_f32(vmax_abs_x, vx_orig_1);
    xnn_simd_f32_t vx_2 = xnn_min_f32(vmax_abs_x, vx_orig_2);
    xnn_simd_f32_t vx_3 = xnn_min_f32(vmax_abs_x, vx_orig_3);
    vx_0 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_0);
    vx_1 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_1);
    vx_2 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_2);
    vx_3 = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx_3);

    // Since the polynomials are odd/even, we need x^2.
    const xnn_simd_f32_t vx2_0 = xnn_mul_f32(vx_0, vx_0);
    const xnn_simd_f32_t vx2_1 = xnn_mul_f32(vx_1, vx_1);
    const xnn_simd_f32_t vx2_2 = xnn_mul_f32(vx_2, vx_2);
    const xnn_simd_f32_t vx2_3 = xnn_mul_f32(vx_3, vx_3);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp_0 = xnn_fmadd_f32(vx2_0, valpha_5, valpha_3);
    xnn_simd_f32_t vp_1 = xnn_fmadd_f32(vx2_1, valpha_5, valpha_3);
    xnn_simd_f32_t vp_2 = xnn_fmadd_f32(vx2_2, valpha_5, valpha_3);
    xnn_simd_f32_t vp_3 = xnn_fmadd_f32(vx2_3, valpha_5, valpha_3);
    vp_0 = xnn_fmadd_f32(vx2_0, vp_0, valpha_1);
    vp_1 = xnn_fmadd_f32(vx2_1, vp_1, valpha_1);
    vp_2 = xnn_fmadd_f32(vx2_2, vp_2, valpha_1);
    vp_3 = xnn_fmadd_f32(vx2_3, vp_3, valpha_1);
    vp_0 = xnn_mul_f32(vx_0, vp_0);
    vp_1 = xnn_mul_f32(vx_1, vp_1);
    vp_2 = xnn_mul_f32(vx_2, vp_2);
    vp_3 = xnn_mul_f32(vx_3, vp_3);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq_0 = xnn_fmadd_f32(vx2_0, vbeta_4, vbeta_2);
    xnn_simd_f32_t vq_1 = xnn_fmadd_f32(vx2_1, vbeta_4, vbeta_2);
    xnn_simd_f32_t vq_2 = xnn_fmadd_f32(vx2_2, vbeta_4, vbeta_2);
    xnn_simd_f32_t vq_3 = xnn_fmadd_f32(vx2_3, vbeta_4, vbeta_2);
    vq_0 = xnn_fmadd_f32(vx2_0, vq_0, vone);
    vq_1 = xnn_fmadd_f32(vx2_1, vq_1, vone);
    vq_2 = xnn_fmadd_f32(vx2_2, vq_2, vone);
    vq_3 = xnn_fmadd_f32(vx2_3, vq_3, vone);

    // Divide the numerator by the denominator.
    const xnn_simd_f32_t verf_0 = xnn_mul_f32(vp_0, xnn_rcp_f32(vq_0));
    const xnn_simd_f32_t verf_1 = xnn_mul_f32(vp_1, xnn_rcp_f32(vq_1));
    const xnn_simd_f32_t verf_2 = xnn_mul_f32(vp_2, xnn_rcp_f32(vq_2));
    const xnn_simd_f32_t verf_3 = xnn_mul_f32(vp_3, xnn_rcp_f32(vq_3));

    // Add one to the rational interpolant, and multiply by 0.5 times the
    // original input.
    const xnn_simd_f32_t vy_0 = xnn_mul_f32(xnn_mul_f32(vx_orig_0, vhalf),
                                        xnn_add_f32(verf_0, vone));
    const xnn_simd_f32_t vy_1 = xnn_mul_f32(xnn_mul_f32(vx_orig_1, vhalf),
                                        xnn_add_f32(verf_1, vone));
    const xnn_simd_f32_t vy_2 = xnn_mul_f32(xnn_mul_f32(vx_orig_2, vhalf),
                                        xnn_add_f32(verf_2, vone));
    const xnn_simd_f32_t vy_3 = xnn_mul_f32(xnn_mul_f32(vx_orig_3, vhalf),
                                        xnn_add_f32(verf_3, vone));

    xnn_simd_store_f32_f16(o + 0 * xnn_simd_size_f32, vy_0);
    xnn_simd_store_f32_f16(o + 1 * xnn_simd_size_f32, vy_1);
    xnn_simd_store_f32_f16(o + 2 * xnn_simd_size_f32, vy_2);
    xnn_simd_store_f32_f16(o + 3 * xnn_simd_size_f32, vy_3);
    o += 16;
  }
  for (; batch >= xnn_simd_size_f32 * sizeof(xnn_float16); batch -= xnn_simd_size_f32 * sizeof(xnn_float16)) {
    const xnn_simd_f32_t vx_orig = xnn_simd_load_f16_f32(i);
    i += xnn_simd_size_f32;

    // Clamp the inputs to the interpolation range.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);

    // Since the polynomials are odd/even, we need x^2.
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);

    // Divide the numerator by the denominator.
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));

    // Add one to the rational interpolant, and multiply by 0.5 times the
    // original input.
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_f32_f16(o, vy);
    o += xnn_simd_size_f32;
  }
  if XNN_UNLIKELY(batch != 0) {
    xnn_simd_f32_t vx_orig = xnn_simd_load_tail_f16_f32(i, batch >> XNN_LOG2_SIZEOF_FLOAT16);

    // See above for comments.
    xnn_simd_f32_t vx = xnn_min_f32(vmax_abs_x, vx_orig);
    vx = xnn_max_f32(xnn_neg_f32(vmax_abs_x), vx);
    const xnn_simd_f32_t vx2 = xnn_mul_f32(vx, vx);
    xnn_simd_f32_t vp = xnn_fmadd_f32(vx2, valpha_5, valpha_3);
    vp = xnn_fmadd_f32(vx2, vp, valpha_1);
    vp = xnn_mul_f32(vx, vp);
    xnn_simd_f32_t vq = xnn_fmadd_f32(vx2, vbeta_4, vbeta_2);
    vq = xnn_fmadd_f32(vx2, vq, vone);
    const xnn_simd_f32_t verf = xnn_mul_f32(vp, xnn_rcp_f32(vq));
    const xnn_simd_f32_t vy = xnn_mul_f32(xnn_mul_f32(vx_orig, vhalf),
                                          xnn_add_f32(verf, vone));

    xnn_simd_store_tail_f32_f16(o, vy, batch >> XNN_LOG2_SIZEOF_FLOAT16);
  }
}
