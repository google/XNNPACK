    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    const __m128i vinput_zero_point0 = _mm_castps_si128(_mm_broadcast_ss((const float*) &qp0->zero_point));
    const __m128i vinput_zero_point1 = _mm_castps_si128(_mm_broadcast_ss((const float*) &qp1->zero_point));
    const __m128i vinput_zero_point2 = _mm_castps_si128(_mm_broadcast_ss((const float*) &qp2->zero_point));
    const __m128i vinput_zero_point3 = _mm_castps_si128(_mm_broadcast_ss((const float*) &qp3->zero_point));
    ...
    const __m128 vinput_scale0 = _mm_broadcast_ss(&qp0->inv_scale);
    const __m128 vinput_scale1 = _mm_broadcast_ss(&qp1->inv_scale);
    const __m128 vinput_scale2 = _mm_broadcast_ss(&qp2->inv_scale);
    const __m128 vinput_scale3 = _mm_broadcast_ss(&qp3->inv_scale);
