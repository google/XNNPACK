...
void xnn_qd8_f32_qc4w_gemm_minmax_ukernel_3x16c4__avx512vnni_vpshufb_prfm(
    size_t mr,
    size_t nc,
    size_t kc,
    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  const __m512i vmaskhi = _mm512_set1_epi8(params->avx512vnni.mask);  // 0xF0
  const __m512i vmasklo = _mm512_set1_epi8(params->avx512vnni.masklo);  // 0x0F
  const __m512i vshufflelo = _mm512_load_si512(params->avx512vnni.shufflelo);  // shuffle low to hi
  do {
    ...
    while (k >= 8 * sizeof(int8_t)) {
      ...
      const __m512i vbs0123456789ABCDEFx0123 = _mm512_and_si512(vbb0123456789ABCDEFx01234567, vmasklo);
      const __m512i vb0123456789ABCDEFx4567 = _mm512_and_si512(vbb0123456789ABCDEFx01234567, vmaskhi);
      const __m512i vb0123456789ABCDEFx0123 = _mm512_shuffle_epi8(vshufflelo, vbs0123456789ABCDEFx0123);
    ...
    if (k != 0) {
      ...
      const __m512i vbs123456789ABCDEF = _mm512_and_si512(vbb0123456789ABCDEF, vmasklo);
      const __m512i vb0123456789ABCDEF = _mm512_shuffle_epi8(vshufflelo, vbs123456789ABCDEF);
      xnn_prefetch_to_l1((const int8_t*) w + 960);
