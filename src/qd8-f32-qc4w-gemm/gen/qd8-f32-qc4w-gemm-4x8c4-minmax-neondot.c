...
void xnn_qd8_f32_qc4w_gemm_minmax_ukernel_4x8c4__neondot(
    size_t mr,
    size_t nc,
    size_t kc,
...
void xnn_qd8_f32_qc4w_bl_gemm_minmax_ukernel_4x8c4__neondot(
    size_t mr,
    size_t nc,
    size_t kc,
    size_t bl,
    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  // Loop over groups of 8 columns.
  do {
    // TODO - convert the izp dtype to float
    const int32x4_t vinput_zero_point01_int = vld1q_s32(&quantization_params[0].zero_point);
    const int32x4_t vinput_zero_point23_int = vld1q_s32(&quantization_params[2].zero_point);

    const float32x4_t vinput_zero_point01 = vcvtq_f32_s32(vinput_zero_point01_int);
    const float32x4_t vinput_zero_point23 = vcvtq_f32_s32(vinput_zero_point23_int);

    const float32x4_t vksum0123 = vld1q_f32(w); w = (const float32_t*) w + 4;
    const float32x4_t vksum4567 = vld1q_f32(w); w = (const float32_t*) w + 4;

    float32x4_t vout0x0123 = vmulq_lane_f32(vksum0123, vget_low_s32(vinput_zero_point01), 0);
    float32x4_t vout1x0123 = vmulq_lane_f32(vksum0123, vget_high_s32(vinput_zero_point01), 0);
    float32x4_t vout2x0123 = vmulq_lane_f32(vksum0123, vget_low_s32(vinput_zero_point23), 0);
    float32x4_t vout3x0123 = vmulq_lane_f32(vksum0123, vget_high_s32(vinput_zero_point23), 0);

    float32x4_t vout0x4567 = vmulq_lane_f32(vksum4567, vget_low_s32(vinput_zero_point01), 0);
    float32x4_t vout1x4567 = vmulq_lane_f32(vksum4567, vget_high_s32(vinput_zero_point01), 0);
    float32x4_t vout2x4567 = vmulq_lane_f32(vksum4567, vget_low_s32(vinput_zero_point23), 0);
    float32x4_t vout3x4567 = vmulq_lane_f32(vksum4567, vget_high_s32(vinput_zero_point23), 0);
