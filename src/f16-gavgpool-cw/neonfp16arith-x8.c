...
#if XNN_ARCH_ARM64
    const float16x8_t vout01 = vpaddq_f16(vsum0, vsum1);
    const float16x8_t vout23 = vpaddq_f16(vsum2, vsum3);
    const float16x8_t vout0123 = vpaddq_f16(vout01, vout23);
    float16x4_t vout = vget_low_f16(vpaddq_f16(vout0123, vout0123));
#else  // XNN_ARCH_ARM
    const float16x4_t vout0 = vadd_f16(vget_low_f16(vsum0), vget_high_f16(vsum0));
    const float16x4_t vout1 = vadd_f16(vget_low_f16(vsum1), vget_high_f16(vsum1));
    const float16x4_t vout2 = vadd_f16(vget_low_f16(vsum2), vget_high_f16(vsum2));
    const float16x4_t vout3 = vadd_f16(vget_low_f16(vsum3), vget_high_f16(vsum3));
    const float16x4_t vout01 = vpadd_f16(vout0, vout1);
    const float16x4_t vout23 = vpadd_f16(vout2, vout3);
    float16x4_t vout = vpadd_f16(vout01, vout23);
#endif

    vout = vmul_f16(vout, vmultiplier);
