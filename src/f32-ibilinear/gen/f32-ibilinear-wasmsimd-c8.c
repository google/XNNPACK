    ...
    size_t output_increment) XNN_OOB_READS
{
  ...
  do {
    ...
    for (; c >= 8 * sizeof(float); c -= 8 * sizeof(float)) {
      ...
      const v128_t vt0123 = wasm_f32x4_add(wasm_f32x4_mul(valphah, vtd0123), vtl0123);
      const v128_t vb0123 = wasm_f32x4_add(wasm_f32x4_mul(valphah, vbd0123), vbl0123);
      const v128_t vt4567 = wasm_f32x4_add(wasm_f32x4_mul(valphah, vtd4567), vtl4567);
      const v128_t vb4567 = wasm_f32x4_add(wasm_f32x4_mul(valphah, vbd4567), vbl4567);

      const v128_t vd0123 = wasm_f32x4_sub(vb0123, vt0123);
      const v128_t vd4567 = wasm_f32x4_sub(vb4567, vt4567);

      const v128_t vo0123 = wasm_f32x4_add(wasm_f32x4_mul(valphav, vd0123), vt0123);
      const v128_t vo4567 = wasm_f32x4_add(wasm_f32x4_mul(valphav, vd4567), vt4567);
    ...
    for (; c >= 4 * sizeof(float); c -= 4 * sizeof(float)) {
      ...
      const v128_t vt = wasm_f32x4_add(wasm_f32x4_mul(valphah, vtd), vtl);
      const v128_t vb = wasm_f32x4_add(wasm_f32x4_mul(valphah, vbd), vbl);
      const v128_t vd = wasm_f32x4_sub(vb, vt);
      const v128_t vo = wasm_f32x4_add(wasm_f32x4_mul(valphav, vd), vt);
    ...
    if XNN_UNLIKELY(c != 0) {
      ...
      const v128_t vb = wasm_f32x4_add(wasm_f32x4_mul(valphah, vbd), vbl);
      const v128_t vd = wasm_f32x4_sub(vb, vt);
      v128_t vo = wasm_f32x4_add(wasm_f32x4_mul(valphav, vd), vt);
