    ...
    const union xnn_qu8_conv_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  const int32_t vmultiplier = params->rndnu16_scalar.multiplier;
  const int64_t vrounding = params->rndnu16_scalar.rounding;
  const uint32_t vshift = params->rndnu16_scalar.shift;
  const int32_t voutput_min_less_zero_point = (int32_t) params->rndnu16_scalar.output_min - (int32_t) params->rndnu16_scalar.output_zero_point;
  const int32_t voutput_max_less_zero_point = (int32_t) params->rndnu16_scalar.output_max - (int32_t) params->rndnu16_scalar.output_zero_point;
  const int32_t voutput_zero_point = params->rndnu16_scalar.output_zero_point;
