...
void xnn_f32_vrsqrt_ukernel__avx512f_sqrt_u16(
...
void xnn_f32_vrsqrt_ukernel__avx512f_sqrt_u32(
    size_t batch, const float* input, float* output,
    const struct xnn_f32_default_params unused_params[restrict XNN_MIN_ELEMENTS(1)]) {
  assert(batch != 0);
  assert(batch % sizeof(float) == 0);
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    const xnn_simd_f32_t vx1 = xnn_loadu_f32(input + 16);
    input += 32;
...
void xnn_f32_vrsqrt_ukernel__avx512f_sqrt_u48(
    size_t batch, const float* input, float* output,
    const struct xnn_f32_default_params unused_params[restrict XNN_MIN_ELEMENTS(1)]) {
  assert(batch != 0);
  assert(batch % sizeof(float) == 0);
  ...
  for (; batch >= 48 * sizeof(float); batch -= 48 * sizeof(float)) {
    const xnn_simd_f32_t vx0 = xnn_loadu_f32(input);
    const xnn_simd_f32_t vx1 = xnn_loadu_f32(input + 16);
    const xnn_simd_f32_t vx2 = xnn_loadu_f32(input + 32);
    input += 48;
