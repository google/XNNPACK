...
approx_reciprocal_sqrt_f32(xnn_simd_f32_t a) {
  ...
  const __m256i vmagic = _mm256_set1_epi32(magic);
  return _mm256_castsi256_ps(
      _mm256_sub_epi32(vmagic, _mm256_srai_epi32(_mm256_castps_si256(a), 1)));
}


void xnn_f32_vrsqrt_ukernel__avx2_approx_rsqrt_u8(
    size_t batch,
    const float* input,
    float* output,
...
void xnn_f32_vrsqrt_ukernel__avx2_approx_rsqrt_u16(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params unused_params[restrict XNN_MIN_ELEMENTS(1)])
...
void xnn_f32_vrsqrt_ukernel__avx2_approx_rsqrt_u32(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params unused_params[restrict XNN_MIN_ELEMENTS(1)])
