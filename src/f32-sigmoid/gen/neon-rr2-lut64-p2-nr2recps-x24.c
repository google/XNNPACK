    ...
    const void* params)
{
  ...
  for (; n >= 24 * sizeof(float); n -= 24 * sizeof(float)) {
    ...
    float32x4_t vn0123 = vmlaq_f32(vmagic_bias, vz0123, vminus_log2e_x64);
    float32x4_t vn4567 = vmlaq_f32(vmagic_bias, vz4567, vminus_log2e_x64);
    float32x4_t vn89AB = vmlaq_f32(vmagic_bias, vz89AB, vminus_log2e_x64);
    float32x4_t vnCDEF = vmlaq_f32(vmagic_bias, vzCDEF, vminus_log2e_x64);
    float32x4_t vnGHIJ = vmlaq_f32(vmagic_bias, vzGHIJ, vminus_log2e_x64);
    float32x4_t vnKLMN = vmlaq_f32(vmagic_bias, vzKLMN, vminus_log2e_x64);
    ...
    const int32x4_t veKLMN = vshlq_n_s32(vbicq_s32(vreinterpretq_s32_f32(vnKLMN), vmovq_n_s32(INT32_C(0x3F))), 17);
    ...
    float32x2_t vlKL = vld1_dup_f32(&xnn_table_exp2_k_over_64[(uint32_t) vidxKL]);
    float32x2_t vlMN = vld1_dup_f32(&xnn_table_exp2_k_over_64[(uint32_t) vidxMN]);
    ...
    vlKL = vld1_lane_f32(&xnn_table_exp2_k_over_64[(uint32_t) (vidxKL >> 32)], vlKL, 1);
    vlMN = vld1_lane_f32(&xnn_table_exp2_k_over_64[(uint32_t) (vidxMN >> 32)], vlMN, 1);
    ...
    float32x4_t vt0123 = vmlaq_f32(vz0123, vn0123, vln2_o64_hi);
    float32x4_t vt4567 = vmlaq_f32(vz4567, vn4567, vln2_o64_hi);
    float32x4_t vt89AB = vmlaq_f32(vz89AB, vn89AB, vln2_o64_hi);
    float32x4_t vtCDEF = vmlaq_f32(vzCDEF, vnCDEF, vln2_o64_hi);
    float32x4_t vtGHIJ = vmlaq_f32(vzGHIJ, vnGHIJ, vln2_o64_hi);
    float32x4_t vtKLMN = vmlaq_f32(vzKLMN, vnKLMN, vln2_o64_hi);

    vt0123 = vmlaq_f32(vt0123, vn0123, vln2_o64_lo);
    vt4567 = vmlaq_f32(vt4567, vn4567, vln2_o64_lo);
    vt89AB = vmlaq_f32(vt89AB, vn89AB, vln2_o64_lo);
    vtCDEF = vmlaq_f32(vtCDEF, vnCDEF, vln2_o64_lo);
    vtGHIJ = vmlaq_f32(vtGHIJ, vnGHIJ, vln2_o64_lo);
    vtKLMN = vmlaq_f32(vtKLMN, vnKLMN, vln2_o64_lo);
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    // Compute reduced argument t := (z + n * log(2) / 64). Note that -t = -z - n * log(2) / 64.
    // Use Cody-Waite range reduction method (note two constants to represent log(2) / 64) to improve accuracy.
    float32x4_t vt = vmlaq_f32(vz, vn, vln2_o64_hi);
    vt = vmlaq_f32(vt, vn, vln2_o64_lo);
