...
enum xnn_status xnn_init_cache_with_size(struct xnn_cache* cache,
                                         size_t num_buckets,
                                         enum xnn_cache_type cache_type) {
  memset(cache, 0, sizeof(struct xnn_cache));
  cache->buckets = (struct xnn_cache_bucket*)xnn_allocate_zero_memory(
      num_buckets * sizeof(struct xnn_cache_bucket));
...
static inline bool bytes_equal(struct xnn_cache* cache, void* ptr, size_t size,
                               size_t offset) {
  return memcmp(ptr, (void*)((uintptr_t)cache_start(cache) + offset), size) ==
         0;
}

static bool lookup(struct xnn_cache* cache, void* ptr, size_t size,
  ...
  while (buckets[idx].size != 0 &&
         !(buckets[idx].hash == hash && size == buckets[idx].size &&
...
static bool insert(struct xnn_cache* cache, void* ptr, size_t size) {
  ...
  if (cache->num_entries * XNN_CACHE_MAX_LOAD_ENTRIES_MULTIPLIER >
    ...
    // If the cache grew, idx is stale, since that is based on the old cache's
    // num_buckets.
...
size_t xnn_get_or_insert_cache(struct xnn_cache* cache, void* ptr,
                               size_t size) {
...
enum xnn_status xnn_internal_init_weights_cache(
    struct xnn_internal_weights_cache* cache, size_t num_buckets,
    size_t buffer_size) {
...
enum xnn_status xnn_internal_init_weights_cache_with_size(
  ...
  return xnn_internal_init_weights_cache(cache, XNN_CACHE_INITIAL_BUCKETS,
                                         size);
}
  ...
  switch (cache->finalization_state) {
    ...
    case xnn_cache_state_not_finalized: {
      ...
      if (finalization_kind == xnn_weights_cache_finalization_kind_hard) {
        ...
        // Also release the memory used by hash table (but not the weights
        // memory).
        ...
        // Finalize weights cache by reserving sufficient space for the
        // insertion of the largest cached weights. This ensures that we have
        // space to write packed weights to check for cache hits without growing
        // and moving the memory. This has some memory overhead, which can be as
        // large as the size of the largest cached weights, rounded up to page
        // size.
        status = xnn_reserve_weights_memory(&cache->cache.weights,
                                            cache->max_weights_size);
...
static inline bool cache_has_space(struct xnn_internal_weights_cache* cache,
                                   size_t n) {
...
void* xnn_internal_reserve_space_in_weights_cache(
  ...
  switch (cache->finalization_state) {
    case xnn_cache_state_hard_finalized:
      xnn_log_error(
          "cannot reserve additional space in a finalized compact weights "
          "cache");
      return NULL;
    case xnn_cache_state_soft_finalized:
      if (!cache_has_space(cache, n)) {
        xnn_log_error(
            "cannot reserve additional space in a finalized weights cache");
        return NULL;
      }
      // If the cache is finalized, and has space for `n` bytes, we still want
      // to lock the mutex, because we can have multiple writers attempting to
      // write to this space.
      break;
    default:
      break;
...
size_t xnn_internal_get_or_insert_weights_cache(
  ...
  switch (cache->finalization_state) {
    ...
    case xnn_cache_state_soft_finalized: {
      ...
      // 1. there is sufficient space in the memory (to write the incoming
      // packed weights), or
      // 2. incoming packed weights is already in cache
      if (!cache_has_space(cache, size)) {
        xnn_log_error(
            "insufficient extra space in finalized weights cache buffer");
        return XNN_CACHE_NOT_FOUND;
      }

      // We need to release the mutex from this point onwards, because
      // xnn_reserve_space_in_weights would have returned non-NULL (which means
      // that it locked the mutex).
    ...
    case xnn_cache_state_not_finalized: {
      ...
      if (offset != XNN_CACHE_NOT_FOUND) {
        // Found or inserted packed weights, update the largest size seen so
        // far, this will be used when finalizing the weights cache, to ensure
        // there is an extra space at the end for future cache checks.
  ...
  // Mutex is locked in xnn_reserve_space_in_weights_cache when it returns
  // non-NULL, i.e. when cache is not finalized, or if it is
  // xnn_cache_state_soft_finalized and has sufficient space.
...
enum xnn_status xnn_finalize_weights_cache(
    ...
    enum xnn_weights_cache_finalization_kind finalization_kind) {
  return xnn_internal_finalize_weights_cache(weights_cache->context,
                                             finalization_kind);
}
