...
void xnn_f16_spmm_minmax_ukernel_2vx1__rvvfp16arith(
    size_t mc,
    size_t nc,
    ...
    const struct xnn_f16_minmax_params* restrict params)
{
  ...
  while (mc > 0) {
    size_t vl = __riscv_vsetvl_e16m2(mc);
    const xnn_float16* w = weights;
    const int32_t* dmap = widx_dmap;
    const uint32_t* nnzmap = nidx_nnzmap;
    size_t n = nc;
    do {
      vfloat16m2_t acc_f16v = __riscv_vfmv_v_f_f16m2(0.0f, vl);
      acc_f16v = __riscv_vfadd_vf_f16m2(acc_f16v, *w++, vl);
      uint32_t nnz = *nnzmap++;

      if XNN_LIKELY(nnz != 0) {
        do {
          const intptr_t diff = *dmap++;
          vfloat16m2_t input_f16v = __riscv_vle16_v_f16m2(input, vl);
          acc_f16v = __riscv_vfmacc_vf_f16m2(acc_f16v, *w++, input_f16v, vl);
          input = (const xnn_float16*) ((uintptr_t) input + (uintptr_t) diff);
        } while (--nnz != 0);
      }

      acc_f16v = __riscv_vfmin_vf_f16m2(acc_f16v, max, vl);
      acc_f16v = __riscv_vfmax_vf_f16m2(acc_f16v, min, vl);
      __riscv_vse16_v_f16m2(output, acc_f16v, vl);
