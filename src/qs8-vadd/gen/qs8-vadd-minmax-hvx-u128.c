    ...
    const union xnn_qs8_add_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 128 * sizeof(int8_t); batch -= 128 * sizeof(int8_t)) {
    ...
    HVX_Vector va0_hi_mul_even = Q6_Vw_vmpyie_VwVh(va_multiplier_lo, va_multiplier_hi, va0_hi);
    HVX_Vector va0_hi_mul_odd = Q6_Vw_vmpyio_VwVh(va_multiplier, va0_hi);

    HVX_Vector vb0_hi_mul_even = Q6_Vw_vmpyie_VwVh(vb_multiplier_lo, vb_multiplier_hi, vb0_hi);
    HVX_Vector vb0_hi_mul_odd = Q6_Vw_vmpyio_VwVh(vb_multiplier, vb0_hi);

    HVX_Vector vacc0_hi_even = Q6_Vw_vadd_VwVw(va0_hi_mul_even, vb0_hi_mul_even);
    HVX_Vector vacc0_hi_odd = Q6_Vw_vadd_VwVw(va0_hi_mul_odd, vb0_hi_mul_odd);

    // vacc = vacc + vbias
    vacc0_lo_even = Q6_Vw_vadd_VwVw(vbias, vacc0_lo_even);
    vacc0_lo_odd = Q6_Vw_vadd_VwVw(vbias, vacc0_lo_odd);
    vacc0_hi_even = Q6_Vw_vadd_VwVw(vbias, vacc0_hi_even);
    vacc0_hi_odd = Q6_Vw_vadd_VwVw(vbias, vacc0_hi_odd);
