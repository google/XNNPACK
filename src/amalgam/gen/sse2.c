...
void xnn_qc8_dwconv_minmax_fp32_ukernel_25p8c__sse2_mul16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
...
void xnn_qc8_dwconv_minmax_fp32_ukernel_3p8c__sse2_mul16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
...
void xnn_qc8_dwconv_minmax_fp32_ukernel_9p8c__sse2_mul16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
    ...
    const union xnn_qs8_cvt_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 32 * sizeof(int8_t); batch -= 32 * sizeof(int8_t)) {
    ...
    const __m128i vprod2hi = _mm_mulhi_epi16(vextx2, vmultiplier);
    const __m128i vprod3lo = _mm_mullo_epi16(vextx3, vmultiplier);
    const __m128i vprod3hi = _mm_mulhi_epi16(vextx3, vmultiplier);
