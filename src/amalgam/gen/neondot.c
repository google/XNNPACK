...
void kai_matmul_clamp_f32_qai8dxp1x8_qsu4cxp8x8_1x8x32_neon_dotprod(
    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  const size_t kai_mr = 1;
  const size_t kai_nr = 8;
  const size_t kai_k0 = 32;
  ...
  const size_t lhs_packed_stride = kai_mr * (k + sizeof(float) + sizeof(float));
  ...
  for (size_t row_idx = 0; row_idx < num_rows; row_idx += kai_mr) {
      ...
      for (size_t col_idx = 0; col_idx < num_cols; col_idx += kai_nr) {
          ...
          vst1q_f32((float*)((uint8_t*)dst + (col_idx + 0) * sizeof(float) + row_idx * dst_stride_row), main_acc0);
          vst1q_f32((float*)((uint8_t*)dst + (col_idx + 4) * sizeof(float) + row_idx * dst_stride_row), main_acc1);
      }
