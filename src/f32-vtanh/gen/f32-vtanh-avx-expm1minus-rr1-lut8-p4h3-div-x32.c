...
void xnn_f32_vtanh_ukernel__avx_expm1minus_rr1_lut8_p4h3_div_x32(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_tanh_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    __m256 vnOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vzOPQRSTUV, vlog2e), vmagic_bias);
    ...
    const __m256 vtOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vnOPQRSTUV, vminus_ln2), vzOPQRSTUV);
    ...
    __m256 vpOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vc4, vtOPQRSTUV), vc3);
    ...
    vpOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vpOPQRSTUV, vtOPQRSTUV), vc2);
    ...
    vpOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vpOPQRSTUV, vtOPQRSTUV), vtwo);
    ...
    const __m256 vemoOPQRSTUV = _mm256_add_ps(_mm256_mul_ps(vpOPQRSTUV, vtsOPQRSTUV), vsmoOPQRSTUV);
