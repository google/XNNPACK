...
void xnn_f32_vtanh_ukernel__rvv_rational_9_8_div_u8v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    // Preserve NaN
    vbool4_t nan_mask = __riscv_vmfne(vx, vx, vl);
    ...
    vfloat32m8_t vp = __riscv_vfadd(__riscv_vfmul(vx2, valpha_9, vl), valpha_7, vl);
    ...
    vfloat32m8_t vq = __riscv_vfadd(__riscv_vfmul(vx2, vbeta_8, vl), vbeta_6, vl);
