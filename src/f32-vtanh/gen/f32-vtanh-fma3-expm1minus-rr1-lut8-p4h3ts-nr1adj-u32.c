...
void xnn_f32_vtanh_ukernel__fma3_expm1minus_rr1_lut8_p4h3ts_nr1adj_u32(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_tanh_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    __m256 vrepo01234567 = _mm256_rcp_ps(vepo01234567);
    __m256 vrepo89ABCDEF = _mm256_rcp_ps(vepo89ABCDEF);
    __m256 vrepoGHIJKLMN = _mm256_rcp_ps(vepoGHIJKLMN);
    __m256 vrepoOPQRSTUV = _mm256_rcp_ps(vepoOPQRSTUV);

    const __m256 verepo01234567 = _mm256_fnmsub_ps(vrepo01234567, vepo01234567, vminus_one);
    const __m256 verepo89ABCDEF = _mm256_fnmsub_ps(vrepo89ABCDEF, vepo89ABCDEF, vminus_one);
    const __m256 verepoGHIJKLMN = _mm256_fnmsub_ps(vrepoGHIJKLMN, vepoGHIJKLMN, vminus_one);
    const __m256 verepoOPQRSTUV = _mm256_fnmsub_ps(vrepoOPQRSTUV, vepoOPQRSTUV, vminus_one);
    vrepo01234567 = _mm256_fmadd_ps(verepo01234567, vrepo01234567, vrepo01234567);
    vrepo89ABCDEF = _mm256_fmadd_ps(verepo89ABCDEF, vrepo89ABCDEF, vrepo89ABCDEF);
    vrepoGHIJKLMN = _mm256_fmadd_ps(verepoGHIJKLMN, vrepoGHIJKLMN, vrepoGHIJKLMN);
    vrepoOPQRSTUV = _mm256_fmadd_ps(verepoOPQRSTUV, vrepoOPQRSTUV, vrepoOPQRSTUV);

    __m256 vy01234567 = _mm256_mul_ps(vemo01234567, vrepo01234567);
    __m256 vy89ABCDEF = _mm256_mul_ps(vemo89ABCDEF, vrepo89ABCDEF);
    __m256 vyGHIJKLMN = _mm256_mul_ps(vemoGHIJKLMN, vrepoGHIJKLMN);
    __m256 vyOPQRSTUV = _mm256_mul_ps(vemoOPQRSTUV, vrepoOPQRSTUV);

    const __m256 vey01234567 = _mm256_fnmadd_ps(vy01234567, vepo01234567, vemo01234567);
    const __m256 vey89ABCDEF = _mm256_fnmadd_ps(vy89ABCDEF, vepo89ABCDEF, vemo89ABCDEF);
    const __m256 veyGHIJKLMN = _mm256_fnmadd_ps(vyGHIJKLMN, vepoGHIJKLMN, vemoGHIJKLMN);
    const __m256 veyOPQRSTUV = _mm256_fnmadd_ps(vyOPQRSTUV, vepoOPQRSTUV, vemoOPQRSTUV);
    vy01234567 = _mm256_fmadd_ps(vey01234567, vrepo01234567, vy01234567);
    vy89ABCDEF = _mm256_fmadd_ps(vey89ABCDEF, vrepo89ABCDEF, vy89ABCDEF);
    vyGHIJKLMN = _mm256_fmadd_ps(veyGHIJKLMN, vrepoGHIJKLMN, vyGHIJKLMN);
    vyOPQRSTUV = _mm256_fmadd_ps(veyOPQRSTUV, vrepoOPQRSTUV, vyOPQRSTUV);
