    ...
    const union xnn_f32_tanh_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 16 * sizeof(float); batch -= 16 * sizeof(float)) {
    ...
    const __m128 vt0_0 = _mm_rcp_ps(vq_0);
    const __m128 vt0_1 = _mm_rcp_ps(vq_1);
    const __m128 vt0_2 = _mm_rcp_ps(vq_2);
    const __m128 vt0_3 = _mm_rcp_ps(vq_3);
    const __m128 vt1_0 = _mm_mul_ps(vt0_0, _mm_sub_ps(vtwo, _mm_mul_ps(vt0_0, vq_0)));
    const __m128 vt1_1 = _mm_mul_ps(vt0_1, _mm_sub_ps(vtwo, _mm_mul_ps(vt0_1, vq_1)));
    const __m128 vt1_2 = _mm_mul_ps(vt0_2, _mm_sub_ps(vtwo, _mm_mul_ps(vt0_2, vq_2)));
    const __m128 vt1_3 = _mm_mul_ps(vt0_3, _mm_sub_ps(vtwo, _mm_mul_ps(vt0_3, vq_3)));
    const __m128 vy_0 =  _mm_mul_ps(vp_0, vt1_0);
    const __m128 vy_1 =  _mm_mul_ps(vp_1, vt1_1);
    const __m128 vy_2 =  _mm_mul_ps(vp_2, vt1_2);
    const __m128 vy_3 =  _mm_mul_ps(vp_3, vt1_3);
