...
void xnn_f16_vtanh_ukernel__avx512fp16_expm1minus_rr1_p3h2ts_div_u32(
    size_t batch,
    ...
    const struct xnn_f16_default_params* restrict params) XNN_OOB_READS
{
  ...
  for (; batch >= 1 * xnn_simd_bytes_f16; batch -= 1 * xnn_simd_bytes_f16) {
    const xnn_simd_f16_t vx = xnn_loadu_f16(i); i += 32;
...
void xnn_f16_vtanh_ukernel__avx512fp16_expm1minus_rr1_p3h2ts_div_u64(
    size_t batch,
    const xnn_float16* input,
    xnn_float16* output,
    const struct xnn_f16_default_params* restrict params) XNN_OOB_READS
{
  ...
  for (; batch >= 2 * xnn_simd_bytes_f16; batch -= 2 * xnn_simd_bytes_f16) {
    const xnn_simd_f16_t vx0 = xnn_loadu_f16(i); i += 32;
    const xnn_simd_f16_t vx1 = xnn_loadu_f16(i); i += 32;
    ...
    xnn_storeu_f16(o, vy0); o += 32;
    xnn_storeu_f16(o, vy1); o += 32;
  }
  for (; batch >= 1 * xnn_simd_bytes_f16; batch -= 1 * xnn_simd_bytes_f16) {
    const xnn_simd_f16_t vx = xnn_loadu_f16(i); i += 32;
