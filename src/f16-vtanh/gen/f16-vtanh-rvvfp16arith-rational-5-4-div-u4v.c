...
void xnn_f16_vtanh_ukernel__rvvfp16arith_rational_5_4_div_u4v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m4_t vp = __riscv_vfadd(__riscv_vfmul(vx2, valpha_5, vl), valpha_3, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx2, vp, vl), vone, vl);
    vp = __riscv_vfmul(vx, vp, vl);

    // Evaluate the denominator polynomial q.
    vfloat16m4_t vq = __riscv_vfadd(__riscv_vfmul(vx2, vbeta_4, vl), vbeta_2, vl);
