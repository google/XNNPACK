...
void xnn_f16_vtanh_ukernel__rvvfp16arith_expm1minus_rr1_p3h2ts_div_u1v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* restrict params)
{
  ...
  do {
    ...
    vuint16m1_t vux = __riscv_vreinterpret_u16m1(vx);
    vuint16m1_t vsign = __riscv_vand(vux, 0x8000, n);
    ...
    vfloat16m1_t vt = __riscv_vfmacc(vz, vln2, vn, n);
    ...
    vfloat16m1_t vexpm1 = __riscv_vfadd(vp, vs, n);

    // tanh = expm1 / (expm1 + 2).
    vfloat16m1_t vy = __riscv_vfdiv(vexpm1,
        __riscv_vfadd(vexpm1, 2.0f, n), n);

    // Restore sign.
    vuint16m1_t vuy = __riscv_vreinterpret_u16m1(vy);
    vuy = __riscv_vxor(vuy, vsign, n);
    vy = __riscv_vreinterpret_f16m1(vuy);
