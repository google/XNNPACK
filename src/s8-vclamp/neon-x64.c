    ...
    const union xnn_s8_minmax_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 64; n -= 64) {
    int8x16_t vacc0 = vld1q_s8(x); x += 16;
    int8x16_t vacc1 = vld1q_s8(x); x += 16;
    int8x16_t vacc2 = vld1q_s8(x); x += 16;
    int8x16_t vacc3 = vld1q_s8(x); x += 16;
    ...
    vst1q_s8(y, vacc0); y += 16;
    vst1q_s8(y, vacc1); y += 16;
    vst1q_s8(y, vacc2); y += 16;
    vst1q_s8(y, vacc3); y += 16;
  }
  for (; n >= 8; n -= 8) {
    int8x8_t vacc = vld1_s8(x); x += 8;
  ...
  if XNN_UNLIKELY(n != 0) {
    ...
    if (n & 4) {
      vst1_lane_u32(__builtin_assume_aligned(y, 1), vreinterpret_u32_s8(vacc), 0); y += 4;
      vacc = vext_s8(vacc, vacc, 4);
    }
    if (n & 2) {
      vst1_lane_u16(__builtin_assume_aligned(y, 1), vreinterpret_u16_s8(vacc), 0); y += 2;
      vacc = vext_s8(vacc, vacc, 2);
    }
    if (n & 1) {
      vst1_lane_s8(y, vacc, 0);
    }
  }
}
