...
void xnn_qu8_vpreluc_ukernel__rvv_u1v(
    size_t batch,
    ...
    const union xnn_qs8_vprelu_scalar_params* restrict params)
{
  ...
  for (size_t vl; batch > 0; batch -= vl, input_a += vl, output += vl) {
    ...
    const vuint8mf4_t va8 = __riscv_vle8_v_u8mf4(input_a, vl);
    vint32m1_t va32 = __riscv_vreinterpret_v_u32m1_i32m1(
        __riscv_vzext_vf4_u32m1(va8, vl));
