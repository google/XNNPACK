...
void xnn_qu8_vpreluc_ukernel__rvv_u8v(
    size_t batch,
    ...
    const union xnn_qs8_vprelu_scalar_params* restrict params)
{
  ...
  for (size_t vl; batch > 0; batch -= vl, input_a += vl, output += vl) {
    ...
    const vbool4_t ma =
        __riscv_vmslt_vx_i32m8_b4(va32, 0, vl);

    vint32m8_t vacc = __riscv_vmul_vx_i32m8_mu(ma, va32, va32, slope, vl);
    const vfloat32m8_t vaccf = __riscv_vfcvt_f_x_v_f32m8(vacc, vl);

    vfloat32m8_t vfpacc =
        __riscv_vfmul_vf_f32m8(vaccf, positive_multiplier, vl);
    vfpacc = __riscv_vfmul_vf_f32m8_mu(
