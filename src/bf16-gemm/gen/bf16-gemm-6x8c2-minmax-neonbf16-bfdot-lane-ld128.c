    ...
    const union xnn_bf16_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    if XNN_UNLIKELY(k != 0) {
      ...
      const uint32x4_t va5c01 = vdupq_lane_u32(vget_low_u32(vreinterpretq_u32_bf16(va5)), 0);
      ...
      if (k > 2 * sizeof(bfloat16_t)) {
        ...
        const uint32x4_t va5c23 = vdupq_lane_u32(vget_low_u32(vreinterpretq_u32_bf16(va5)), 1);
        ...
        if (k > 4 * sizeof(bfloat16_t)) {
          ...
          const uint32x4_t va5c45 = vdupq_lane_u32(vget_high_u32(vreinterpretq_u32_bf16(va5)), 0);
          ...
          if (k > 6 * sizeof(bfloat16_t)) {
            ...
            const uint32x4_t va5c67 = vdupq_lane_u32(vget_high_u32(vreinterpretq_u32_bf16(va5)), 1);
    ...
    if XNN_LIKELY(nc >= 8) {
      ...
      if (nc & 2) {
        ...
        vout5x0123 = vdup_lane_bf16(vout5x0123, 2);
      }
      if (nc & 1) {
