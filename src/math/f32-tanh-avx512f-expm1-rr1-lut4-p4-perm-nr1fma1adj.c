...
void xnn_math_f32_tanh__avx512f_expm1_rr1_lut4_p4_perm_nr1fma1adj(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= 16 * sizeof(float)) {
    ...
    // Reconstruct tanh(-z) := expm1(-2z) / (2.0 + expm1(-2z)) with adjustment to match IEEE division result
    __m512 vabsy = _mm512_mul_ps(vem1, vrep1);
    vabsy = _mm512_fmadd_ps(_mm512_fnmadd_ps(vabsy, vep1, vem1), vrep1, vabsy);
