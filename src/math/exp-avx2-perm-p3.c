...
void xnn_math_f32_exp__avx2_perm_p3(
    size_t n,
    const float* input,
    float* output)
{
  ...
  const __m256 c2 = _mm256_set1_ps(0x1.00021Ep-1f);
  const __m256 c3 = _mm256_set1_ps(0x1.55559Ap-3f);
  ...
  for (; n != 0; n -= 8 * sizeof(float)) {
    ...
    __m256 rf = _mm256_fmadd_ps(rx, c3, c2);
