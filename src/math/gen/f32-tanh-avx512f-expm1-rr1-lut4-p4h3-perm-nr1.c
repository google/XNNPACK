...
//   Template: src/math/f32-tanh-avx512f-expm1.c.in
...
void xnn_math_f32_tanh__avx512f_expm1_rr1_lut4_p4h3_perm_nr1(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= sizeof(__m512)) {
    ...
    __m512 vrep1 = _mm512_rcp14_ps(vep1);
    const __m512 verep1 = _mm512_fnmadd_ps(vrep1, vep1, vone);
    vrep1 = _mm512_fmadd_ps(verep1, vrep1, vrep1);

    // Reconstruct y = expm1(-2z) / (expm1(-2z) + 2)
    __m512 vy = _mm512_mul_ps(vem1, vrep1);
