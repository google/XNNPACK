...
//   Template: src/math/f32-tanh-neon-expm1.c.in
...
void xnn_math_f32_tanh__neonfma_expm1_rr1_p6h5_nr2fmaadj(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= sizeof(float32x4_t)) {
    ...
    const float32x4_t vem1 = vfmsq_f32(vsm1, vp, vts);
    ...
    float32x4_t verep1 = vfmsq_f32(vone, vrep1, vep1);
    vrep1 = vfmaq_f32(vrep1, vrep1, verep1);
    verep1 = vfmsq_f32(vone, vrep1, vep1);
    vrep1 = vfmaq_f32(vrep1, vrep1, verep1);

    // Reconstruct y = expm1(-2z) / (expm1(-2z) + 2)
    float32x4_t vy = vmulq_f32(vem1, vrep1);

    // Adjust reconstructred expm1(-2z) / (2 + expm1(-2z)) to match the correctly rounded division result
    const float32x4_t vey = vfmsq_f32(vem1, vy, vep1);
    vy = vfmaq_f32(vy, vey, vrep1);
