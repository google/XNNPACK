...
void xnn_math_f32_tanh__scalar_expm1_rr1_lut32_p3h1_div(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= sizeof(float)) {
    ...
    const uint32_t ven = float_as_uint32(vn) << 18;

    // Use bits 0:5 bits of n, as integer, as an index for table lookup of l := 2**frac(2n).
    const uint32_t vidx = float_as_uint32(vn) & vindex_mask;
    // Adjust exponent of the value l fetched from the table to get the final s value.
    const float vs = uint32_as_float(xnn_table_exp2minus_k_over_32[vidx] + ven);
