...
void xnn_math_f32_tanh__sse2_expm1_rr1_lut8_p4h3_nr2(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= 4 * sizeof(float)) {
    ...
    const __m128 vep1 = _mm_sub_ps(vem1, vminus_two);
    ...
    vrep1 = _mm_mul_ps(vrep1, _mm_add_ps(_mm_mul_ps(vrep1, vep1), vminus_two));
    vrep1 = _mm_mul_ps(vrep1, _mm_sub_ps(vminus_two, _mm_mul_ps(vrep1, vep1)));

    // Reconstruct tanh(z) := expm1(2z) / (2 + expm1(2z))
    __m128 vabsy = _mm_mul_ps(vem1, vrep1);

    // Saturate tanh(z) at -1 for large inputs.
    vabsy = _mm_or_ps(_mm_andnot_ps(vm, vabsy), _mm_and_ps(vminus_one, vm));
