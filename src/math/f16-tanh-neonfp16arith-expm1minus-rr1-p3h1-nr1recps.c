...
void xnn_math_f16_tanh__neonfp16arith_expm1minus_rr1_p3h1_nr1recps(
    size_t n,
    const void* input,
    void* output)
{
  ...
  for (; n != 0; n -= 8 * sizeof(uint16_t)) {
    ...
    // (|-z / log(2)| <= 2**8, i.e. |z| <= 0x1.62E43p+7 = 177.45), but that is acceptable, because inputs x
    // outside of [-4.5078125, 4.5078125] (i.e. z outsize [0, 4.5078125]) saturate tanhf(x). We fixup the result for such
    ...
    float16x8_t vrep1 = vrecpeq_f16(vep1);
    vrep1 = vmulq_f16(vrep1, vrecpsq_f16(vrep1, vep1));
