...
void xnn_math_f32_tanh__avx512f_expm1_rr1_lut4_p4h3_perm_nr1fma(
    size_t n,
    const float* input,
    float* output)
{
  ...
  for (; n != 0; n -= 16 * sizeof(float)) {
    ...
    vrep1 = _mm512_fmadd_ps(_mm512_fnmadd_ps(vrep1, vep1, vone), vrep1, vrep1);

    // Reconstruct tanh(-z) := expm1(-2z) / (2.0 + expm1(-2z))
    const __m512 vabsy = _mm512_mul_ps(vem1, vrep1);
