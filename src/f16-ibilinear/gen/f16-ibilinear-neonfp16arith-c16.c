    ...
    size_t output_increment) XNN_OOB_READS
{
  ...
  do {
    ...
    const float16x8_t valphah = vld1q_dup_f16(weights); weights = (const uint16_t*) weights + 1;
    const float16x8_t valphav = vld1q_dup_f16(weights); weights = (const uint16_t*) weights + 1;
