...
void xnn_f32_rminmaxsum_ukernel__avx512f_u64_acc2(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 64 * sizeof(float); batch -= 64 * sizeof(float)) {
    ...
    vmin0 = _mm512_min_ps(vmin0, vt2);
    vmax0 = _mm512_max_ps(vmax0, vt2);
    vsum0 = _mm512_add_ps(vsum0, vt2);
    vmin1 = _mm512_min_ps(vmin1, vt3);
    vmax1 = _mm512_max_ps(vmax1, vt3);
    vsum1 = _mm512_add_ps(vsum1, vt3);
  }
