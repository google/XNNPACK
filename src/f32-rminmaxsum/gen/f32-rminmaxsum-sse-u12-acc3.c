...
void xnn_f32_rminmaxsum_ukernel__sse_u12_acc3(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  vsum0 = _mm_add_ss(vsum0, _mm_shuffle_ps(vsum0, vsum0, _MM_SHUFFLE(1, 1, 1, 1)));

  _mm_store_ss(output, vmin0);
  _mm_store_ss(output + 1, vmax0);
  _mm_store_ss(output + 2, vsum0);
}
