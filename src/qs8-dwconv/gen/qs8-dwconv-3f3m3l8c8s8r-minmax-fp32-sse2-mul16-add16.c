...
void xnn_qs8_dwconv_minmax_fp32_ukernel_3f3m3l8c8s8r__sse2_mul16_add16(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
  ...
  do {
    ...
    // First pass to process 3 inputs.
    {
      ...
      for (; c >= 8; c -= 8) {
        ...
        const __m128i vsignprod2x01234567 = _mm_cmpgt_epi16(_mm_setzero_si128(), vprod01234567);
        vacc0123 = _mm_add_epi32(vacc0123, _mm_unpacklo_epi16(vprod01234567, vsignprod2x01234567));
        vacc4567 = _mm_add_epi32(vacc4567, _mm_unpackhi_epi16(vprod01234567, vsignprod2x01234567));

        w = (const void*) ((uintptr_t) w + 8 * sizeof(int32_t) + 24 * sizeof(int8_t));

        _mm_storeu_si128((__m128i*) ((int32_t*) b), vacc0123);
        _mm_storeu_si128((__m128i*) ((int32_t*) b + 4), vacc4567);
        b += 8;
      }
      ...
      if XNN_UNLIKELY(c != 0) {
        {
          ...
          // _mm_storel_epi64((__m128i*) b, vacc0123);
          _mm_storeu_si128((__m128i*) b, vacc0123);
          _mm_storeu_si128((__m128i*) b + 1, vacc4567);
          b += 8;
          c -= 8;
        }
      }
    ...
    for (size_t ks = kernel_size - 3; ks > 3; ks -= 3) {
      ...
      for (; c >= 8; c -= 8) {
        ...
        __m128i vacc4567 = _mm_loadu_si128((const __m128i*) ((const int32_t*) b + 4));
        ...
        // TODO(zhin): remove this, won't ever hit this i think.
        _mm_storeu_si128((__m128i*) b, vacc0123);
        _mm_storeu_si128((__m128i*) b + 1, vacc4567);
        b += 8;
      }
