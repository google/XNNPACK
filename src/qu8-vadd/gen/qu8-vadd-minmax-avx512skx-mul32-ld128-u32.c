...
void xnn_qu8_vadd_minmax_ukernel__avx512skx_mul32_ld128_u32(
    size_t batch,
    ...
    const union xnn_qu8_add_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= 32 * sizeof(uint8_t); batch -= 32 * sizeof(uint8_t)) {
    ...
    const __m512i vaGHIJKLMNOPQRSTUV = _mm512_cvtepu8_epi32(_mm_loadu_si128((const __m128i*) (input_a + 16)));
    const __m512i vbGHIJKLMNOPQRSTUV = _mm512_cvtepu8_epi32(_mm_loadu_si128((const __m128i*) (input_b + 16)));
    input_a += 32;
    input_b += 32;
    ...
    __m256i vout0123456789ABCDEFGHIJKLMNOPQRSTUV = _mm256_permutevar8x32_epi32(_mm256_packus_epi16(_mm512_castsi512_si256(vout0123GHIJ4567KLMN89ABOPQRCDEFSTUV), _mm512_extracti32x8_epi32(vout0123GHIJ4567KLMN89ABOPQRCDEFSTUV, 1)), _mm256_set_epi32(7, 3, 5, 1, 6, 2, 4, 0));
  ...
  if XNN_UNLIKELY(batch != 0) {
    do {
      ...
      if XNN_LIKELY(batch >= (16 * sizeof(uint8_t))) {
        _mm_storeu_si128((__m128i*) output, vout0123456789ABCDEF);
        output += 16;
        batch -= 16 * sizeof(uint8_t);
      } else {
