...
void xnn_qs8_qc8w_dwconv_minmax_fp32_ukernel_6f6m7l8c4s4r__sse41_mul32(
    size_t channels,
    size_t output_width,
    const int8_t** input,
    const void* weights,
    ...
    // Last pass to process up to 7 inputs.
    {
      ...
      if XNN_UNLIKELY(c != 0) {
        do {
          ...
          __m128 vscaled0123 = _mm_cvtepi32_ps(vacc0123);
          const __m128 vscale0123 = _mm_loadu_ps((const float*) ((uintptr_t) w + 28 * sizeof(int8_t)));
          vscaled0123 = _mm_mul_ps(vscaled0123, vscale0123);
          vscaled0123 = _mm_min_ps(vscaled0123, _mm_load_ps(params->fp32_sse4.output_max_less_zero_point));
          vacc0123 = _mm_cvtps_epi32(vscaled0123);

          w = (void*) ((uintptr_t) w + 28 + 4 * sizeof(float));
