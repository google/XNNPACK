// clang-format off
// Auto-generated file. Do not edit!
//   Template: src/bf16-f32-dwconv/unipass-scalar.c.in
//   Generator: tools/xngen
//
// Copyright 2026 Google LLC
//
// This source code is licensed under the BSD-style license found in the
// LICENSE file in the root directory of this source tree.

#include <assert.h>
#include <stddef.h>
#include <stdint.h>

#include "src/xnnpack/common.h"
#include "src/xnnpack/dwconv.h"
#include "src/xnnpack/math.h"
#include "src/xnnpack/microparams.h"


// Packed weights layout, per block of 2 channels:
//   float bias[2];
//   uint16_t kernel[25][2];  // bf16
void xnn_bf16_f32_dwconv_minmax_ukernel_25p2c__scalar_acc2(
    size_t channels,
    size_t output_width,
    const xnn_bfloat16** input,
    const void* weights,
    float* output,
    intptr_t input_stride,
    size_t output_increment,
    size_t input_offset,
    size_t input_pixel_stride,
    const xnn_bfloat16* zero,
    const struct xnn_f32_minmax_params* restrict params)
{
  assert(channels != 0);
  assert(output_width != 0);

  const float vmin = params->scalar.min;
  const float vmax = params->scalar.max;
  do {
    const uint16_t* i0 = (const uint16_t*) input[0];
    assert(i0 != NULL);
    if XNN_UNPREDICTABLE(i0 != (const uint16_t*) zero) {
      i0 = (const uint16_t*) ((uintptr_t) i0 + input_offset);
    }
    const uint16_t* i1 = (const uint16_t*) input[1];
    assert(i1 != NULL);
    if XNN_UNPREDICTABLE(i1 != (const uint16_t*) zero) {
      i1 = (const uint16_t*) ((uintptr_t) i1 + input_offset);
    }
    const uint16_t* i2 = (const uint16_t*) input[2];
    assert(i2 != NULL);
    if XNN_UNPREDICTABLE(i2 != (const uint16_t*) zero) {
      i2 = (const uint16_t*) ((uintptr_t) i2 + input_offset);
    }
    const uint16_t* i3 = (const uint16_t*) input[3];
    assert(i3 != NULL);
    if XNN_UNPREDICTABLE(i3 != (const uint16_t*) zero) {
      i3 = (const uint16_t*) ((uintptr_t) i3 + input_offset);
    }
    const uint16_t* i4 = (const uint16_t*) input[4];
    assert(i4 != NULL);
    if XNN_UNPREDICTABLE(i4 != (const uint16_t*) zero) {
      i4 = (const uint16_t*) ((uintptr_t) i4 + input_offset);
    }
    const uint16_t* i5 = (const uint16_t*) input[5];
    assert(i5 != NULL);
    if XNN_UNPREDICTABLE(i5 != (const uint16_t*) zero) {
      i5 = (const uint16_t*) ((uintptr_t) i5 + input_offset);
    }
    const uint16_t* i6 = (const uint16_t*) input[6];
    assert(i6 != NULL);
    if XNN_UNPREDICTABLE(i6 != (const uint16_t*) zero) {
      i6 = (const uint16_t*) ((uintptr_t) i6 + input_offset);
    }
    const uint16_t* i7 = (const uint16_t*) input[7];
    assert(i7 != NULL);
    if XNN_UNPREDICTABLE(i7 != (const uint16_t*) zero) {
      i7 = (const uint16_t*) ((uintptr_t) i7 + input_offset);
    }
    const uint16_t* i8 = (const uint16_t*) input[8];
    assert(i8 != NULL);
    if XNN_UNPREDICTABLE(i8 != (const uint16_t*) zero) {
      i8 = (const uint16_t*) ((uintptr_t) i8 + input_offset);
    }
    const uint16_t* i9 = (const uint16_t*) input[9];
    assert(i9 != NULL);
    if XNN_UNPREDICTABLE(i9 != (const uint16_t*) zero) {
      i9 = (const uint16_t*) ((uintptr_t) i9 + input_offset);
    }
    const uint16_t* i10 = (const uint16_t*) input[10];
    assert(i10 != NULL);
    if XNN_UNPREDICTABLE(i10 != (const uint16_t*) zero) {
      i10 = (const uint16_t*) ((uintptr_t) i10 + input_offset);
    }
    const uint16_t* i11 = (const uint16_t*) input[11];
    assert(i11 != NULL);
    if XNN_UNPREDICTABLE(i11 != (const uint16_t*) zero) {
      i11 = (const uint16_t*) ((uintptr_t) i11 + input_offset);
    }
    const uint16_t* i12 = (const uint16_t*) input[12];
    assert(i12 != NULL);
    if XNN_UNPREDICTABLE(i12 != (const uint16_t*) zero) {
      i12 = (const uint16_t*) ((uintptr_t) i12 + input_offset);
    }
    const uint16_t* i13 = (const uint16_t*) input[13];
    assert(i13 != NULL);
    if XNN_UNPREDICTABLE(i13 != (const uint16_t*) zero) {
      i13 = (const uint16_t*) ((uintptr_t) i13 + input_offset);
    }
    const uint16_t* i14 = (const uint16_t*) input[14];
    assert(i14 != NULL);
    if XNN_UNPREDICTABLE(i14 != (const uint16_t*) zero) {
      i14 = (const uint16_t*) ((uintptr_t) i14 + input_offset);
    }
    const uint16_t* i15 = (const uint16_t*) input[15];
    assert(i15 != NULL);
    if XNN_UNPREDICTABLE(i15 != (const uint16_t*) zero) {
      i15 = (const uint16_t*) ((uintptr_t) i15 + input_offset);
    }
    const uint16_t* i16 = (const uint16_t*) input[16];
    assert(i16 != NULL);
    if XNN_UNPREDICTABLE(i16 != (const uint16_t*) zero) {
      i16 = (const uint16_t*) ((uintptr_t) i16 + input_offset);
    }
    const uint16_t* i17 = (const uint16_t*) input[17];
    assert(i17 != NULL);
    if XNN_UNPREDICTABLE(i17 != (const uint16_t*) zero) {
      i17 = (const uint16_t*) ((uintptr_t) i17 + input_offset);
    }
    const uint16_t* i18 = (const uint16_t*) input[18];
    assert(i18 != NULL);
    if XNN_UNPREDICTABLE(i18 != (const uint16_t*) zero) {
      i18 = (const uint16_t*) ((uintptr_t) i18 + input_offset);
    }
    const uint16_t* i19 = (const uint16_t*) input[19];
    assert(i19 != NULL);
    if XNN_UNPREDICTABLE(i19 != (const uint16_t*) zero) {
      i19 = (const uint16_t*) ((uintptr_t) i19 + input_offset);
    }
    const uint16_t* i20 = (const uint16_t*) input[20];
    assert(i20 != NULL);
    if XNN_UNPREDICTABLE(i20 != (const uint16_t*) zero) {
      i20 = (const uint16_t*) ((uintptr_t) i20 + input_offset);
    }
    const uint16_t* i21 = (const uint16_t*) input[21];
    assert(i21 != NULL);
    if XNN_UNPREDICTABLE(i21 != (const uint16_t*) zero) {
      i21 = (const uint16_t*) ((uintptr_t) i21 + input_offset);
    }
    const uint16_t* i22 = (const uint16_t*) input[22];
    assert(i22 != NULL);
    if XNN_UNPREDICTABLE(i22 != (const uint16_t*) zero) {
      i22 = (const uint16_t*) ((uintptr_t) i22 + input_offset);
    }
    const uint16_t* i23 = (const uint16_t*) input[23];
    assert(i23 != NULL);
    if XNN_UNPREDICTABLE(i23 != (const uint16_t*) zero) {
      i23 = (const uint16_t*) ((uintptr_t) i23 + input_offset);
    }
    const uint16_t* i24 = (const uint16_t*) input[24];
    assert(i24 != NULL);
    if XNN_UNPREDICTABLE(i24 != (const uint16_t*) zero) {
      i24 = (const uint16_t*) ((uintptr_t) i24 + input_offset);
    }
    input = (const xnn_bfloat16**) ((uintptr_t) input + input_stride);

    size_t c = channels;
    const float* wb = (const float*) weights;
    for (; c >= 2; c -= 2) {
      const uint16_t* wk = (const uint16_t*) (wb + 2);
      float vacc0p0 = wb[0];
      float vacc1p0 = wb[1];


      const float vi0x0 = math_cvt_fp32_bf16(i0[0]);
      const float vi0x1 = math_cvt_fp32_bf16(i0[1]);
      i0 += 2;

      const float vk0x0 = math_cvt_fp32_bf16(wk[0]);
      vacc0p0 = math_muladd_f32(vi0x0, vk0x0, vacc0p0);
      const float vk0x1 = math_cvt_fp32_bf16(wk[1]);
      vacc1p0 = math_muladd_f32(vi0x1, vk0x1, vacc1p0);

      const float vi1x0 = math_cvt_fp32_bf16(i1[0]);
      const float vi1x1 = math_cvt_fp32_bf16(i1[1]);
      i1 += 2;

      const float vk1x0 = math_cvt_fp32_bf16(wk[2]);
      float vacc0p1 = vi1x0 * vk1x0;
      const float vk1x1 = math_cvt_fp32_bf16(wk[3]);
      float vacc1p1 = vi1x1 * vk1x1;

      const float vi2x0 = math_cvt_fp32_bf16(i2[0]);
      const float vi2x1 = math_cvt_fp32_bf16(i2[1]);
      i2 += 2;

      const float vk2x0 = math_cvt_fp32_bf16(wk[4]);
      vacc0p0 = math_muladd_f32(vi2x0, vk2x0, vacc0p0);
      const float vk2x1 = math_cvt_fp32_bf16(wk[5]);
      vacc1p0 = math_muladd_f32(vi2x1, vk2x1, vacc1p0);

      const float vi3x0 = math_cvt_fp32_bf16(i3[0]);
      const float vi3x1 = math_cvt_fp32_bf16(i3[1]);
      i3 += 2;

      const float vk3x0 = math_cvt_fp32_bf16(wk[6]);
      vacc0p1 = math_muladd_f32(vi3x0, vk3x0, vacc0p1);
      const float vk3x1 = math_cvt_fp32_bf16(wk[7]);
      vacc1p1 = math_muladd_f32(vi3x1, vk3x1, vacc1p1);

      const float vi4x0 = math_cvt_fp32_bf16(i4[0]);
      const float vi4x1 = math_cvt_fp32_bf16(i4[1]);
      i4 += 2;

      const float vk4x0 = math_cvt_fp32_bf16(wk[8]);
      vacc0p0 = math_muladd_f32(vi4x0, vk4x0, vacc0p0);
      const float vk4x1 = math_cvt_fp32_bf16(wk[9]);
      vacc1p0 = math_muladd_f32(vi4x1, vk4x1, vacc1p0);

      const float vi5x0 = math_cvt_fp32_bf16(i5[0]);
      const float vi5x1 = math_cvt_fp32_bf16(i5[1]);
      i5 += 2;

      const float vk5x0 = math_cvt_fp32_bf16(wk[10]);
      vacc0p1 = math_muladd_f32(vi5x0, vk5x0, vacc0p1);
      const float vk5x1 = math_cvt_fp32_bf16(wk[11]);
      vacc1p1 = math_muladd_f32(vi5x1, vk5x1, vacc1p1);

      const float vi6x0 = math_cvt_fp32_bf16(i6[0]);
      const float vi6x1 = math_cvt_fp32_bf16(i6[1]);
      i6 += 2;

      const float vk6x0 = math_cvt_fp32_bf16(wk[12]);
      vacc0p0 = math_muladd_f32(vi6x0, vk6x0, vacc0p0);
      const float vk6x1 = math_cvt_fp32_bf16(wk[13]);
      vacc1p0 = math_muladd_f32(vi6x1, vk6x1, vacc1p0);

      const float vi7x0 = math_cvt_fp32_bf16(i7[0]);
      const float vi7x1 = math_cvt_fp32_bf16(i7[1]);
      i7 += 2;

      const float vk7x0 = math_cvt_fp32_bf16(wk[14]);
      vacc0p1 = math_muladd_f32(vi7x0, vk7x0, vacc0p1);
      const float vk7x1 = math_cvt_fp32_bf16(wk[15]);
      vacc1p1 = math_muladd_f32(vi7x1, vk7x1, vacc1p1);

      const float vi8x0 = math_cvt_fp32_bf16(i8[0]);
      const float vi8x1 = math_cvt_fp32_bf16(i8[1]);
      i8 += 2;

      const float vk8x0 = math_cvt_fp32_bf16(wk[16]);
      vacc0p0 = math_muladd_f32(vi8x0, vk8x0, vacc0p0);
      const float vk8x1 = math_cvt_fp32_bf16(wk[17]);
      vacc1p0 = math_muladd_f32(vi8x1, vk8x1, vacc1p0);

      const float vi9x0 = math_cvt_fp32_bf16(i9[0]);
      const float vi9x1 = math_cvt_fp32_bf16(i9[1]);
      i9 += 2;

      const float vk9x0 = math_cvt_fp32_bf16(wk[18]);
      vacc0p1 = math_muladd_f32(vi9x0, vk9x0, vacc0p1);
      const float vk9x1 = math_cvt_fp32_bf16(wk[19]);
      vacc1p1 = math_muladd_f32(vi9x1, vk9x1, vacc1p1);

      const float vi10x0 = math_cvt_fp32_bf16(i10[0]);
      const float vi10x1 = math_cvt_fp32_bf16(i10[1]);
      i10 += 2;

      const float vk10x0 = math_cvt_fp32_bf16(wk[20]);
      vacc0p0 = math_muladd_f32(vi10x0, vk10x0, vacc0p0);
      const float vk10x1 = math_cvt_fp32_bf16(wk[21]);
      vacc1p0 = math_muladd_f32(vi10x1, vk10x1, vacc1p0);

      const float vi11x0 = math_cvt_fp32_bf16(i11[0]);
      const float vi11x1 = math_cvt_fp32_bf16(i11[1]);
      i11 += 2;

      const float vk11x0 = math_cvt_fp32_bf16(wk[22]);
      vacc0p1 = math_muladd_f32(vi11x0, vk11x0, vacc0p1);
      const float vk11x1 = math_cvt_fp32_bf16(wk[23]);
      vacc1p1 = math_muladd_f32(vi11x1, vk11x1, vacc1p1);

      const float vi12x0 = math_cvt_fp32_bf16(i12[0]);
      const float vi12x1 = math_cvt_fp32_bf16(i12[1]);
      i12 += 2;

      const float vk12x0 = math_cvt_fp32_bf16(wk[24]);
      vacc0p0 = math_muladd_f32(vi12x0, vk12x0, vacc0p0);
      const float vk12x1 = math_cvt_fp32_bf16(wk[25]);
      vacc1p0 = math_muladd_f32(vi12x1, vk12x1, vacc1p0);

      const float vi13x0 = math_cvt_fp32_bf16(i13[0]);
      const float vi13x1 = math_cvt_fp32_bf16(i13[1]);
      i13 += 2;

      const float vk13x0 = math_cvt_fp32_bf16(wk[26]);
      vacc0p1 = math_muladd_f32(vi13x0, vk13x0, vacc0p1);
      const float vk13x1 = math_cvt_fp32_bf16(wk[27]);
      vacc1p1 = math_muladd_f32(vi13x1, vk13x1, vacc1p1);

      const float vi14x0 = math_cvt_fp32_bf16(i14[0]);
      const float vi14x1 = math_cvt_fp32_bf16(i14[1]);
      i14 += 2;

      const float vk14x0 = math_cvt_fp32_bf16(wk[28]);
      vacc0p0 = math_muladd_f32(vi14x0, vk14x0, vacc0p0);
      const float vk14x1 = math_cvt_fp32_bf16(wk[29]);
      vacc1p0 = math_muladd_f32(vi14x1, vk14x1, vacc1p0);

      const float vi15x0 = math_cvt_fp32_bf16(i15[0]);
      const float vi15x1 = math_cvt_fp32_bf16(i15[1]);
      i15 += 2;

      const float vk15x0 = math_cvt_fp32_bf16(wk[30]);
      vacc0p1 = math_muladd_f32(vi15x0, vk15x0, vacc0p1);
      const float vk15x1 = math_cvt_fp32_bf16(wk[31]);
      vacc1p1 = math_muladd_f32(vi15x1, vk15x1, vacc1p1);

      const float vi16x0 = math_cvt_fp32_bf16(i16[0]);
      const float vi16x1 = math_cvt_fp32_bf16(i16[1]);
      i16 += 2;

      const float vk16x0 = math_cvt_fp32_bf16(wk[32]);
      vacc0p0 = math_muladd_f32(vi16x0, vk16x0, vacc0p0);
      const float vk16x1 = math_cvt_fp32_bf16(wk[33]);
      vacc1p0 = math_muladd_f32(vi16x1, vk16x1, vacc1p0);

      const float vi17x0 = math_cvt_fp32_bf16(i17[0]);
      const float vi17x1 = math_cvt_fp32_bf16(i17[1]);
      i17 += 2;

      const float vk17x0 = math_cvt_fp32_bf16(wk[34]);
      vacc0p1 = math_muladd_f32(vi17x0, vk17x0, vacc0p1);
      const float vk17x1 = math_cvt_fp32_bf16(wk[35]);
      vacc1p1 = math_muladd_f32(vi17x1, vk17x1, vacc1p1);

      const float vi18x0 = math_cvt_fp32_bf16(i18[0]);
      const float vi18x1 = math_cvt_fp32_bf16(i18[1]);
      i18 += 2;

      const float vk18x0 = math_cvt_fp32_bf16(wk[36]);
      vacc0p0 = math_muladd_f32(vi18x0, vk18x0, vacc0p0);
      const float vk18x1 = math_cvt_fp32_bf16(wk[37]);
      vacc1p0 = math_muladd_f32(vi18x1, vk18x1, vacc1p0);

      const float vi19x0 = math_cvt_fp32_bf16(i19[0]);
      const float vi19x1 = math_cvt_fp32_bf16(i19[1]);
      i19 += 2;

      const float vk19x0 = math_cvt_fp32_bf16(wk[38]);
      vacc0p1 = math_muladd_f32(vi19x0, vk19x0, vacc0p1);
      const float vk19x1 = math_cvt_fp32_bf16(wk[39]);
      vacc1p1 = math_muladd_f32(vi19x1, vk19x1, vacc1p1);

      const float vi20x0 = math_cvt_fp32_bf16(i20[0]);
      const float vi20x1 = math_cvt_fp32_bf16(i20[1]);
      i20 += 2;

      const float vk20x0 = math_cvt_fp32_bf16(wk[40]);
      vacc0p0 = math_muladd_f32(vi20x0, vk20x0, vacc0p0);
      const float vk20x1 = math_cvt_fp32_bf16(wk[41]);
      vacc1p0 = math_muladd_f32(vi20x1, vk20x1, vacc1p0);

      const float vi21x0 = math_cvt_fp32_bf16(i21[0]);
      const float vi21x1 = math_cvt_fp32_bf16(i21[1]);
      i21 += 2;

      const float vk21x0 = math_cvt_fp32_bf16(wk[42]);
      vacc0p1 = math_muladd_f32(vi21x0, vk21x0, vacc0p1);
      const float vk21x1 = math_cvt_fp32_bf16(wk[43]);
      vacc1p1 = math_muladd_f32(vi21x1, vk21x1, vacc1p1);

      const float vi22x0 = math_cvt_fp32_bf16(i22[0]);
      const float vi22x1 = math_cvt_fp32_bf16(i22[1]);
      i22 += 2;

      const float vk22x0 = math_cvt_fp32_bf16(wk[44]);
      vacc0p0 = math_muladd_f32(vi22x0, vk22x0, vacc0p0);
      const float vk22x1 = math_cvt_fp32_bf16(wk[45]);
      vacc1p0 = math_muladd_f32(vi22x1, vk22x1, vacc1p0);

      const float vi23x0 = math_cvt_fp32_bf16(i23[0]);
      const float vi23x1 = math_cvt_fp32_bf16(i23[1]);
      i23 += 2;

      const float vk23x0 = math_cvt_fp32_bf16(wk[46]);
      vacc0p1 = math_muladd_f32(vi23x0, vk23x0, vacc0p1);
      const float vk23x1 = math_cvt_fp32_bf16(wk[47]);
      vacc1p1 = math_muladd_f32(vi23x1, vk23x1, vacc1p1);

      const float vi24x0 = math_cvt_fp32_bf16(i24[0]);
      const float vi24x1 = math_cvt_fp32_bf16(i24[1]);
      i24 += 2;

      const float vk24x0 = math_cvt_fp32_bf16(wk[48]);
      vacc0p0 = math_muladd_f32(vi24x0, vk24x0, vacc0p0);
      const float vk24x1 = math_cvt_fp32_bf16(wk[49]);
      vacc1p0 = math_muladd_f32(vi24x1, vk24x1, vacc1p0);

      wb = (const float*) (wk + 50);

      vacc0p0 = vacc0p0 + vacc0p1;
      vacc1p0 = vacc1p0 + vacc1p1;

      float vacc0 = math_max_f32(vacc0p0, vmin);
      float vacc1 = math_max_f32(vacc1p0, vmin);

      vacc0 = math_min_f32(vacc0, vmax);
      vacc1 = math_min_f32(vacc1, vmax);

      output[0] = vacc0;
      output[1] = vacc1;
      output += 2;
    }
    if XNN_UNLIKELY(c != 0) {
      const uint16_t* wk = (const uint16_t*) (wb + 2);
      do {
        float vacc0p0 = *wb++;

        const float vi0 = math_cvt_fp32_bf16(*i0++);
        const float vk0 = math_cvt_fp32_bf16(wk[0]);
        vacc0p0 = math_muladd_f32(vi0, vk0, vacc0p0);
        const float vi1 = math_cvt_fp32_bf16(*i1++);
        const float vk1 = math_cvt_fp32_bf16(wk[2]);
        float vacc0p1 = vi1 * vk1;
        const float vi2 = math_cvt_fp32_bf16(*i2++);
        const float vk2 = math_cvt_fp32_bf16(wk[4]);
        vacc0p0 = math_muladd_f32(vi2, vk2, vacc0p0);
        const float vi3 = math_cvt_fp32_bf16(*i3++);
        const float vk3 = math_cvt_fp32_bf16(wk[6]);
        vacc0p1 = math_muladd_f32(vi3, vk3, vacc0p1);
        const float vi4 = math_cvt_fp32_bf16(*i4++);
        const float vk4 = math_cvt_fp32_bf16(wk[8]);
        vacc0p0 = math_muladd_f32(vi4, vk4, vacc0p0);
        const float vi5 = math_cvt_fp32_bf16(*i5++);
        const float vk5 = math_cvt_fp32_bf16(wk[10]);
        vacc0p1 = math_muladd_f32(vi5, vk5, vacc0p1);
        const float vi6 = math_cvt_fp32_bf16(*i6++);
        const float vk6 = math_cvt_fp32_bf16(wk[12]);
        vacc0p0 = math_muladd_f32(vi6, vk6, vacc0p0);
        const float vi7 = math_cvt_fp32_bf16(*i7++);
        const float vk7 = math_cvt_fp32_bf16(wk[14]);
        vacc0p1 = math_muladd_f32(vi7, vk7, vacc0p1);
        const float vi8 = math_cvt_fp32_bf16(*i8++);
        const float vk8 = math_cvt_fp32_bf16(wk[16]);
        vacc0p0 = math_muladd_f32(vi8, vk8, vacc0p0);
        const float vi9 = math_cvt_fp32_bf16(*i9++);
        const float vk9 = math_cvt_fp32_bf16(wk[18]);
        vacc0p1 = math_muladd_f32(vi9, vk9, vacc0p1);
        const float vi10 = math_cvt_fp32_bf16(*i10++);
        const float vk10 = math_cvt_fp32_bf16(wk[20]);
        vacc0p0 = math_muladd_f32(vi10, vk10, vacc0p0);
        const float vi11 = math_cvt_fp32_bf16(*i11++);
        const float vk11 = math_cvt_fp32_bf16(wk[22]);
        vacc0p1 = math_muladd_f32(vi11, vk11, vacc0p1);
        const float vi12 = math_cvt_fp32_bf16(*i12++);
        const float vk12 = math_cvt_fp32_bf16(wk[24]);
        vacc0p0 = math_muladd_f32(vi12, vk12, vacc0p0);
        const float vi13 = math_cvt_fp32_bf16(*i13++);
        const float vk13 = math_cvt_fp32_bf16(wk[26]);
        vacc0p1 = math_muladd_f32(vi13, vk13, vacc0p1);
        const float vi14 = math_cvt_fp32_bf16(*i14++);
        const float vk14 = math_cvt_fp32_bf16(wk[28]);
        vacc0p0 = math_muladd_f32(vi14, vk14, vacc0p0);
        const float vi15 = math_cvt_fp32_bf16(*i15++);
        const float vk15 = math_cvt_fp32_bf16(wk[30]);
        vacc0p1 = math_muladd_f32(vi15, vk15, vacc0p1);
        const float vi16 = math_cvt_fp32_bf16(*i16++);
        const float vk16 = math_cvt_fp32_bf16(wk[32]);
        vacc0p0 = math_muladd_f32(vi16, vk16, vacc0p0);
        const float vi17 = math_cvt_fp32_bf16(*i17++);
        const float vk17 = math_cvt_fp32_bf16(wk[34]);
        vacc0p1 = math_muladd_f32(vi17, vk17, vacc0p1);
        const float vi18 = math_cvt_fp32_bf16(*i18++);
        const float vk18 = math_cvt_fp32_bf16(wk[36]);
        vacc0p0 = math_muladd_f32(vi18, vk18, vacc0p0);
        const float vi19 = math_cvt_fp32_bf16(*i19++);
        const float vk19 = math_cvt_fp32_bf16(wk[38]);
        vacc0p1 = math_muladd_f32(vi19, vk19, vacc0p1);
        const float vi20 = math_cvt_fp32_bf16(*i20++);
        const float vk20 = math_cvt_fp32_bf16(wk[40]);
        vacc0p0 = math_muladd_f32(vi20, vk20, vacc0p0);
        const float vi21 = math_cvt_fp32_bf16(*i21++);
        const float vk21 = math_cvt_fp32_bf16(wk[42]);
        vacc0p1 = math_muladd_f32(vi21, vk21, vacc0p1);
        const float vi22 = math_cvt_fp32_bf16(*i22++);
        const float vk22 = math_cvt_fp32_bf16(wk[44]);
        vacc0p0 = math_muladd_f32(vi22, vk22, vacc0p0);
        const float vi23 = math_cvt_fp32_bf16(*i23++);
        const float vk23 = math_cvt_fp32_bf16(wk[46]);
        vacc0p1 = math_muladd_f32(vi23, vk23, vacc0p1);
        const float vi24 = math_cvt_fp32_bf16(*i24++);
        const float vk24 = math_cvt_fp32_bf16(wk[48]);
        vacc0p0 = math_muladd_f32(vi24, vk24, vacc0p0);
        wk += 1;

        vacc0p0 = vacc0p0 + vacc0p1;

        float vacc0 = math_max_f32(vacc0p0, vmin);
        vacc0 = math_min_f32(vacc0, vmax);
        *output++ = vacc0;
      } while (--c != 0);
    }

    input_offset += input_pixel_stride;
    output = (float*) ((uintptr_t) output + output_increment);
  } while (--output_width != 0);
}
