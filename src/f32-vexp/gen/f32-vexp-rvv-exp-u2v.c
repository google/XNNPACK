...
forceinline vfloat32m2_t calc_polynom_deg_2_f32m2(vfloat32m2_t x, float a0, float a1, float a2, size_t vl)
{ 
    return __riscv_vfmadd_vv_f32m2(x, __riscv_vfmadd_vf_f32m2(x, a2,
        __riscv_vfmv_v_f_f32m2(a1, vl), vl), __riscv_vfmv_v_f_f32m2(a0, vl), vl);
}

forceinline void fast_2_sum_vv_f32m2(vfloat32m2_t a, vfloat32m2_t b, vfloat32m2_t* sh, vfloat32m2_t* sl, size_t vl)
{
    *sh = __riscv_vfadd_vv_f32m2(a, b, vl);
    *sl = __riscv_vfsub_vv_f32m2(b, __riscv_vfsub_vv_f32m2(*sh, a, vl), vl);
}
    
forceinline void fast_2_sum_fv_f32m2(float a, vfloat32m2_t b, vfloat32m2_t* sh, vfloat32m2_t* sl, size_t vl)
{
    *sh = __riscv_vfadd_vf_f32m2(b, a, vl);
    *sl = __riscv_vfsub_vv_f32m2(b, __riscv_vfsub_vf_f32m2(*sh, a, vl), vl);
}
        
forceinline void mul21_vv_f32m2(vfloat32m2_t ah, vfloat32m2_t al, vfloat32m2_t bh, vfloat32m2_t bl, vfloat32m2_t* rh, size_t vl)
{
    vfloat32m2_t zh, zl;
    zh = __riscv_vfmul_vv_f32m2(ah, bh, vl);
    zl = __riscv_vfmsub_vv_f32m2(ah, bh, zh, vl);
    zl = __riscv_vfadd_vv_f32m2(zl, __riscv_vfmadd_vv_f32m2(ah, bl, __riscv_vfmul_vv_f32m2(al, bh, vl), vl), vl);
    *rh = __riscv_vfadd_vv_f32m2(zh, zl, vl);
}

forceinline void fma12_vv_f32m2(vfloat32m2_t ah, vfloat32m2_t bh, vfloat32m2_t ch, vfloat32m2_t* zh, vfloat32m2_t* zl, size_t vl)
{ 
    *zh = __riscv_vfmadd_vv_f32m2(ah, bh, ch, vl);
    *zl = __riscv_vfmadd_vv_f32m2(ah, bh, __riscv_vfsub_vv_f32m2(ch, *zh, vl), vl);
}

forceinline void fma12_vf_f32m2(vfloat32m2_t ah, float bh, vfloat32m2_t ch, vfloat32m2_t* zh, vfloat32m2_t* zl, size_t vl)
{ 
    *zh = __riscv_vfmadd_vf_f32m2(ah, bh, ch, vl);
    *zl = __riscv_vfmadd_vf_f32m2(ah, bh, __riscv_vfsub_vv_f32m2(ch, *zh, vl), vl);
}
    ...
    float overflowThreshold, size_t vl)
{
    ...
    *special = __riscv_vfmerge_vfm_f32m1(*x, RVVMF_EXP_AS_FP32(pinf), *specialMask, vl);
    // check overflow
    mask = __riscv_vmand_mm_b32(__riscv_vmfgt_vf_f32m2_b32(*x, overflowThreshold, vl),
...
forceinline void do_exp_argument_reduction_hl_f32m2(vfloat32m2_t x,
    vfloat32m2_t* yh, vfloat32m2_t* yl, vuint32m2_t* ei, vuint32m2_t* fi, size_t vl)
{
    vfloat32m2_t vmagicConst1 = __riscv_vfmv_v_f_f32m2(RVVMF_EXP_AS_FP32(MAGIC_CONST_1_F32), vl);
    vfloat32m2_t h = __riscv_vfmadd_vf_f32m2(x, RVVMF_EXP_AS_FP32(INV_LOG2_2K_F32), vmagicConst1, vl);
    ...
    *ei = __riscv_vsrl_vx_u32m2(hi, TABLE_SIZE_DEG_F32, vl);
    ...
    fma12_vf_f32m2(h, RVVMF_EXP_AS_FP32(M_LOG2_2K_L_F32),
        __riscv_vfmadd_vf_f32m2(h, RVVMF_EXP_AS_FP32(M_LOG2_2K_H_F32), x, vl), yh, yl, vl);
    *yl = __riscv_vfmadd_vf_f32m2(h, RVVMF_EXP_AS_FP32(M_LOG2_2K_LL_F32), *yl, vl);
    ...
    vuint32m2_t* index, vfloat32m2_t* th, vfloat32m2_t* tl, size_t vl)
{
    ...
    *th = __riscv_vloxei32_v_f32m2((float*)RVVMF_EXP_LOOK_UP_TABLE_HIGH_F32, *index, vl);
    *tl = __riscv_vloxei32_v_f32m2((float*)RVVMF_EXP_LOOK_UP_TABLE_LOW_F32, *index, vl);
}

forceinline void calculate_exp_polynom_hl_f32m2(vfloat32m2_t yh, vfloat32m2_t yl, vfloat32m2_t* ph, vfloat32m2_t* pl, size_t vl)
{
    vfloat32m2_t sqryh = __riscv_vfmul_vv_f32m2(yh, yh, vl);
    vfloat32m2_t r = calc_polynom_deg_2_f32m2(yh, RVVMF_EXP_AS_FP32(EXP_POL_COEFF_2_F32),
...
forceinline void update_exponent_f32m2(vuint32m2_t ei, vfloat32m2_t* res, size_t vl)
{
...
forceinline void update_exponent_with_subnormal_f32m2(float subnormalThreshold, vfloat32m2_t x,
    vuint32m2_t ei, vfloat32m2_t* res, size_t vl)
{
    ...
    RVV_BOOL subnormalMask = __riscv_vmand_mm_b32(__riscv_vmfgt_vf_f32m1_b32(x, RVVMF_EXP_AS_FP32(ninf), vl),
        __riscv_vmflt_vf_f32m2_b32(x, subnormalThreshold, vl), vl);
#endif
#if 2 == 2
    ...
    subnormalRes = __riscv_vfadd_vv_f32m2(*res, __riscv_vreinterpret_v_u32m2_f32m2(shiftNum), vl);
...
forceinline void reconstruct_exp_hl_hl_f32m2(vfloat32m2_t x, vuint32m2_t ei, vfloat32m2_t th, vfloat32m2_t tl,
    vfloat32m2_t pm2h, vfloat32m2_t pm2l, vfloat32m2_t* res, float subnormalThreshold, size_t vl)
{
    vfloat32m2_t sh, sl;
    fast_2_sum_fv_f32m2(RVVMF_EXP_AS_FP32(ONE_F32), pm2h, &sh, &sl, vl);
...
forceinline void update_underflow_f32m2(vfloat32m2_t x, vfloat32m2_t* res,
    float underflowThreshold, float underflowValue, size_t vl)
{
#if 2 == 1
    RVV_BOOL underflowMask = __riscv_vmflt_vf_f32m2_b32(x, underflowThreshold, vl);
#endif
#if 2 == 2
    RVV_BOOL underflowMask = __riscv_vmflt_vf_f32m2_b16(x, underflowThreshold, vl);
#endif
#if 2 == 4
    RVV_BOOL underflowMask = __riscv_vmflt_vf_f32m2_b8(x, underflowThreshold, vl);
#endif
#if 2 == 8
    RVV_BOOL underflowMask = __riscv_vmflt_vf_f32m2_b4(x, underflowThreshold, vl);
#endif
...
vfloat32m2_t __riscv_vexp_f32m2(vfloat32m2_t x, size_t avl)
{
    ...
    vfloat32m2_t res, yh, yl, th, tl, pm2h, pm2l;
    vuint32m2_t ei, fi;
    ...
    check_special_cases_f32m2(&x, &special, &specialMask, RVVMF_EXP_AS_FP32(EXP_EXPM1_OVERFLOW_THRESHOLD_F32), vl);
#else
    ...
    calculate_exp_polynom_hl_f32m2(yh, yl, &pm2h, &pm2l, vl);
    reconstruct_exp_hl_hl_f32m2(x, ei, th, tl, pm2h, pm2l, &res, RVVMF_EXP_AS_FP32(EXP_SUBNORMAL_THRESHOLD_F32), vl);
    update_underflow_f32m2(x, &res, zeroThreshold, RVVMF_EXP_AS_FP32(EXP_UNDERFLOW_VALUE_F32), vl);
...
void xnn_f32_vexp_ukernel__rvv_exp_u2v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* params)
{
  ...
  do {
    ...
    input += n;
    vfloat32m2_t vacc = __riscv_vexp_f32m2(vx, n);
