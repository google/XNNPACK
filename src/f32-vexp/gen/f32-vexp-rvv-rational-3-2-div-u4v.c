...
static XNN_INLINE vfloat32m4_t xnn_setexp_f32(vfloat32m4_t vx, size_t vl) {
  ...
  return __riscv_vreinterpret_f32m4(__riscv_vsll(__riscv_vreinterpret_u32m4(__riscv_vfadd(vx, vmagic, vl)), 23, vl));
}

// Quick-and-dirty round to nearest, only works for floats in the range
// `[2^-22, 2^22)`.
static XNN_INLINE vfloat32m4_t xnn_qd_round_f32(vfloat32m4_t vx, size_t vl) {
...
void xnn_f32_vexp_ukernel__rvv_rational_3_2_div_u4v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat32m4_t vz_prime = __riscv_vfmul(vx, vlog2e, vl);
    vz_prime = __riscv_vfmin(__riscv_vfmax(vz_prime, vm127, vl), v128, vl);

    // Decompose x * log2e into `z` (integer part) and `r` (remainder).
    const vfloat32m4_t vz = xnn_qd_round_f32(vz_prime, vl);
    const vfloat32m4_t vr = __riscv_vfsub(vz_prime, vz, vl);

    // Compute 2^z.
    vfloat32m4_t v2z = xnn_setexp_f32(vz, vl);

    // Evaluate the numerator polynomial p(r).
    vfloat32m4_t vp = __riscv_vfadd(__riscv_vfmul(vr, valpha_3, vl), valpha_2, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vr, vp, vl), valpha_1, vl);
    vp = __riscv_vfmul(vr, vp, vl);

    // Evaluate the denominator polynomial q(r).
    vfloat32m4_t vq = __riscv_vfadd(__riscv_vfmul(vr, vbeta_2, vl), vbeta_1, vl);
    ...
    const vfloat32m4_t v2r = __riscv_vfadd(__riscv_vfdiv(vp, vq, vl), vone, vl);

    // Compute 2^z * 2^r.
    const vfloat32m4_t vy = __riscv_vfmul(v2z, v2r, vl);
