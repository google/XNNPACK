...
inline static bool value_lifecycle_overlap(const struct xnn_usage_record* a,
                                           const struct xnn_usage_record* b) {
...
static inline int cmp_value_usage_tensor_size(const void* a, const void* b) {
  const size_t tensor_size_a =
      (*(struct xnn_usage_record* const*)a)->tensor_size;
  const size_t tensor_size_b =
      (*(struct xnn_usage_record* const*)b)->tensor_size;
  return (tensor_size_b > tensor_size_a) - (tensor_size_b < tensor_size_a);
}

static void populate_value_lifecycle(const struct xnn_runtime* runtime,
                                     struct xnn_usage_record* usage) {
  assert(runtime != NULL);
  if (runtime->num_ops == 0) {
    return;
  }
  // As we initialized first/last_node in each xnn_usage_record to 0 as in
  // 'xnn_init_value_mem_allocation_tracker', we start with the second node to
  // tell whether first/last_node have been set or not, and check the first node
  // last.
  ...
  // Separate loop over all values to make sure we have usage records properly
  // initialized with invalid reuse_value_id. Some usage records are not
  // associated with any nodes, and they will not be visited by the loops over
  // nodes above.
...
// Given the current live memory blocks, return the offset in a memory arena for
// a to-be-allocated value of size 'to_alloc_size'.
static size_t find_value_alloc_offset(struct memory_block* live_mem_blocks,
  ...
  qsort(live_mem_blocks, num_mem_blocks, sizeof(struct memory_block),
        cmp_memory_block);

  // Coalesce overlapping or immediate adjacent memory blocks to form a list of
  // non-overlapping memory blocks in order to find the smallest gap.
  ...
  for (size_t i = 1; i < num_mem_blocks; ++i) {
    ...
    if (live_mem_blocks[i].end > current_coalesced_end) {
      live_mem_blocks[num_coalesced_mem_blocks - 1].end =
          live_mem_blocks[i].end;
    }
  }

  size_t smallest_gap_size = SIZE_MAX;
  // The first index to live_mem_blocks that the 'to_alloc_size' should be
  // allocated after.
...
void xnn_init_value_allocation_tracker(
  ...
  tracker->usage =
      xnn_allocate_zero_memory(sizeof(struct xnn_usage_record) *
                               (runtime->num_values + runtime->num_ops));
...
void xnn_mark_tensor_as_reuse(struct xnn_value_allocation_tracker* tracker,
                              uint32_t value_id, uint32_t reuse_value_id,
                              uint32_t new_last_node) {
  // Set tensor_size to 0 so memory planner will not try to find memory for
  // these tensors.
...
void xnn_add_value_allocation_tracker(
...
void xnn_plan_value_allocation_tracker(
    struct xnn_value_allocation_tracker* tracker) {
#if XNN_ENABLE_MEMOPT
  if (tracker->min_value_id == XNN_INVALID_VALUE_ID) {
  ...
  struct xnn_usage_record** sorted_usage =
      xnn_allocate_zero_memory(sizeof(struct xnn_usage_record*) * num_values);
  ...
  // Walk through all tensors that are reusing memory, and update their usage
  // records.
  for (size_t i = tracker->min_value_id; i <= tracker->max_value_id; ++i) {
