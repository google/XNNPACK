...
void xnn_qu8_rsum_ukernel__avx2_u128_acc4(
    size_t batch,
    const uint8_t* input,
    uint32_t* output,
    const struct xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 128; batch -= 128) {
    ...
    const __m256i vin2 = _mm256_loadu_si256((const __m256i*) (input + 64));
    const __m256i vin3 = _mm256_loadu_si256((const __m256i*) (input + 96));
    input += 128;
    const __m256i vt0 = _mm256_sad_epu8(vin0, vzero);
    const __m256i vt1 = _mm256_sad_epu8(vin1, vzero);
    const __m256i vt2 = _mm256_sad_epu8(vin2, vzero);
    const __m256i vt3 = _mm256_sad_epu8(vin3, vzero);
     vacc0 = _mm256_add_epi32(vacc0, vt0);
     vacc1 = _mm256_add_epi32(vacc1, vt1);
     vacc2 = _mm256_add_epi32(vacc2, vt2);
     vacc3 = _mm256_add_epi32(vacc3, vt3);
  }
