...
void xnn_qu8_rsum_ukernel__wasmsimd_u32_acc2(
    size_t batch,
    const uint8_t* input,
    uint32_t* output,
    const struct xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 4096; batch -= 4096) {
    ...
    for (size_t current_batch = 4096; current_batch > 0; current_batch -= 32) {
      ...
      const v128_t vt1 = wasm_u16x8_load8x8(input); input += 8;
      const v128_t vt2 = wasm_u16x8_load8x8(input); input += 8;
      const v128_t vt3 = wasm_u16x8_load8x8(input); input += 8;
    ...
    vacc1 = wasm_i32x4_add(vacc1, wasm_u32x4_extend_low_u16x8(vacc16_1));
    vacc1 = wasm_i32x4_add(vacc1, wasm_u32x4_extend_high_u16x8(vacc16_1));
  }
