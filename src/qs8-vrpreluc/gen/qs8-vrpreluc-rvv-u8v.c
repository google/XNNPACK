...
void xnn_qs8_vrpreluc_ukernel__rvv_u8v(
    size_t batch,
    ...
    const union xnn_qs8_vprelu_scalar_params* restrict params)
{
  ...
  if (slope >= 0) {
    ...
    const vint32m8_t vacc = __riscv_vadd_vx_i32m8(
        ...
        vlmax);
    const vint32m8_t vout32 = vacc;
    const vint8m2_t vout8 =
  ...
  for (size_t vl; batch > 0; batch -= vl, input_a += vl, output += vl) {
    ...
    const vint8m2_t va8 = __riscv_vle8_v_i8m2(input_a, vl);
    vint32m8_t va32 = __riscv_vsext_vf4_i32m8(va8, vl);
