...
void xnn_f32_qc4w_gemm_minmax_ukernel_6x8__sse41_dup(
    size_t mr,
    size_t nc,
    size_t kc,
    ...
    const union xnn_f32_qc4w_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    for (; k >= 4 * sizeof(float); k -= 4 * sizeof(float)) {
      ...
      const __m128 va5c0000 = _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(va5), _MM_SHUFFLE(0, 0, 0, 0)));
    ...
    if XNN_UNLIKELY(k >= 2 * sizeof(float)) {
      ...
      a4 += 2;
      const __m128 va5 = _mm_castsi128_ps(_mm_loadl_epi64((const __m128i *) a5));
      a5 += 2;
    ...
    vacc5x0123 = _mm_mul_ps(vacc5x0123, vscale0123);
    ...
    vacc5x4567 = _mm_mul_ps(vacc5x4567, vscale4567);
    w = (const float*) w + 8;
