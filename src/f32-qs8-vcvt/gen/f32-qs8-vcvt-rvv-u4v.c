...
void xnn_f32_qs8_vcvt_ukernel__rvv_u4v(
    size_t batch,
    const float* input,
    int8_t* output,
    const union xnn_f32_qs8_cvt_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  const vfloat32m4_t magic_bias_f32v = __riscv_vfmv_v_f_f32m4(params->rvv.magic_bias, __riscv_vsetvl_e32m4(batch));
  ...
  for (; batch > 0; ) {
    ...
    __riscv_vse8_v_i8m1(output, __riscv_vncvt_x_x_w_i8m1(__riscv_vncvt_x_x_w_i16m2(y_i32v, n), n), n); output += n;
  }
}
