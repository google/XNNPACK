...
template <typename InputT, typename OutputT, typename qs8_cvt_params_t,
          const struct xnn_unary_elementwise_config*(init_cvt_config_fn)(void),
          const struct xnn_reduce_config*(init_rminmax_config_fn)(void),
          struct xnn_qd8_quantization_params(quantization_params_fn)(
              InputT min, InputT max, InputT* scale)>
void pack_lh_fx_qd(size_t m, size_t k, size_t mr_packed, size_t kr, size_t sr,
  ...
  // Initialize a static pointer to the convert and minmax configs. Not doing
  // this atomically since the `xnn_init_*_config` calls are synced.
  static const struct xnn_unary_elementwise_config* convert_config = nullptr;
  if (!convert_config) {
    convert_config = init_cvt_config_fn();
    assert(convert_config);
  }
  static const struct xnn_reduce_config* minmax_config = nullptr;
  if (!minmax_config) {
    minmax_config = init_rminmax_config_fn();
