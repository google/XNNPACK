...
void xnn_f32_vsigmoid_ukernel__rvv_rr2_p5_div_u1v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* restrict params)
{
  ...
  do {
    ...
    vfloat32m1_t vz = __riscv_vfabs(vx, vl);

    vfloat32m1_t vn = __riscv_vfadd(__riscv_vfmul(vz, vminus_log2e, vl), vmagic_bias, vl);

    vfloat32m1_t vs = __riscv_vreinterpret_f32m1(__riscv_vsll(__riscv_vreinterpret_u32m1(vn), 23, vl));
    vn = __riscv_vfsub(vn, vmagic_bias, vl);

    vfloat32m1_t vt = __riscv_vfadd(__riscv_vfmul(vn, vln2_hi, vl), vz, vl);
    vt = __riscv_vfmacc(vt, vln2_lo, vn, vl);

    vfloat32m1_t vp = __riscv_vfadd(__riscv_vfmul(vt, vc5, vl), vc4, vl);
    ...
    vfloat32m1_t ve = __riscv_vfadd(__riscv_vfmul(vt, vp, vl), vs, vl);
    vfloat32m1_t vd = __riscv_vfadd(ve, vone, vl);

    vfloat32m1_t vf = __riscv_vfdiv(ve, vd, vl);

    vbool32_t vmask = __riscv_vmfgt(vz, vdenorm_cutoff, vl);
