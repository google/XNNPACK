    ...
    const union xnn_f32_sigmoid_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 24 * sizeof(float); batch -= 24 * sizeof(float)) {
    ...
    vpCDEF = __builtin_wasm_fms_f32x4(vtCDEF, vpCDEF, vtCDEF);
    vpGHIJ = __builtin_wasm_fms_f32x4(vtGHIJ, vpGHIJ, vtGHIJ);
    vpKLMN = __builtin_wasm_fms_f32x4(vtKLMN, vpKLMN, vtKLMN);
    ...
    const v128_t vyCDEF = __builtin_wasm_fms_f32x4(vsCDEF, vsCDEF, vpCDEF);
    const v128_t vyGHIJ = __builtin_wasm_fms_f32x4(vsGHIJ, vsGHIJ, vpGHIJ);
    const v128_t vyKLMN = __builtin_wasm_fms_f32x4(vsKLMN, vsKLMN, vpKLMN);
