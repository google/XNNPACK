    ...
    const union xnn_f32_sigmoid_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 12 * sizeof(float); batch -= 12 * sizeof(float)) {
    ...
    vf0123 = __builtin_wasm_laneselect_i32x4(vf0123, vcf0123, vx0123);
    vf4567 = __builtin_wasm_laneselect_i32x4(vf4567, vcf4567, vx4567);
    vf89AB = __builtin_wasm_laneselect_i32x4(vf89AB, vcf89AB, vx89AB);
  ...
  for (; batch >= 4 * sizeof(float); batch -= 4 * sizeof(float)) {
    ...
    vf = __builtin_wasm_laneselect_i32x4(vf, vcf, vx);
