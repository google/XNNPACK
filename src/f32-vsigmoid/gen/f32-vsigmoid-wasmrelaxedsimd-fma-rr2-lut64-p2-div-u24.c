...
void xnn_f32_vsigmoid_ukernel__wasmrelaxedsimd_fma_rr2_lut64_p2_div_u24(
    size_t batch,
    const float* input,
    float* output,
    const union xnn_f32_sigmoid_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 24 * sizeof(float); batch -= 24 * sizeof(float)) {
    ...
    vpGHIJ = wasm_f32x4_relaxed_nmadd(vpGHIJ, vtGHIJ, vtGHIJ);
    vpKLMN = wasm_f32x4_relaxed_nmadd(vpKLMN, vtKLMN, vtKLMN);
    ...
    const v128_t vyGHIJ = wasm_f32x4_relaxed_nmadd(vsGHIJ, vpGHIJ, vsGHIJ);
    const v128_t vyKLMN = wasm_f32x4_relaxed_nmadd(vsKLMN, vpKLMN, vsKLMN);
