...
void xnn_qu8_prelu_ukernel__avx2_2x16(
    size_t rows,
    size_t channels,
    ...
    const struct xnn_qu8_prelu_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    for (; c >= 16 * sizeof(uint8_t); c -= 16 * sizeof(uint8_t)) {
        __m256i vacc0 = _mm256_cvtepu8_epi16(_mm_loadu_si128((const __m128i*) i0));
        i0 += 16;
        __m256i vacc1 = _mm256_cvtepu8_epi16(_mm_loadu_si128((const __m128i*) i1));
        i1 += 16;
        ...
        const __m128i vy0 = _mm_packus_epi16(_mm256_castsi256_si128(vacc0), vacc0_hi);
        _mm_storeu_si128((__m128i*) o0, vy0);
        o0 += 16;
        const __m128i vacc1_hi = _mm256_extracti128_si256(vacc1, 1);
        const __m128i vy1 = _mm_packus_epi16(_mm256_castsi256_si128(vacc1), vacc1_hi);
        _mm_storeu_si128((__m128i*) o1, vy1);
        o1 += 16;
    }
    if XNN_UNLIKELY(c != 0) {
        assert(c >= 1 * sizeof(uint8_t));
        assert(c <= 15 * sizeof(uint8_t));
        ...
        const __m128i vacc0_hi = _mm256_extracti128_si256(vacc0, 1);
        __m128i vy0 = _mm_packus_epi16(_mm256_castsi256_si128(vacc0), vacc0_hi);
        const __m128i vacc1_hi = _mm256_extracti128_si256(vacc1, 1);
        __m128i vy1 = _mm_packus_epi16(_mm256_castsi256_si128(vacc1), vacc1_hi);
        ...
        if (c & (1 * sizeof(uint8_t))) {
            *o0 = (uint8_t) _mm_extract_epi8(vy0, 0);
            o0 += 1;
            *o1 = (uint8_t) _mm_extract_epi8(vy1, 0);
            o1 += 1;
        }
    }
