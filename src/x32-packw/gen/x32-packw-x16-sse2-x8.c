  ...
  do {
    ...
    for (; n >= 16; n -= 16) {
      ...
      for (; k >= 8; k -= 8) {
        ...
        const __m128 vAx4567 = _mm_loadu_ps(wA + 4);
        wA += 8;
        const __m128 vBx0123 = _mm_loadu_ps(wB);
        const __m128 vBx4567 = _mm_loadu_ps(wB + 4);
        wB += 8;
        const __m128 vCx0123 = _mm_loadu_ps(wC);
        const __m128 vCx4567 = _mm_loadu_ps(wC + 4);
        wC += 8;
        const __m128 vDx0123 = _mm_loadu_ps(wD);
        const __m128 vDx4567 = _mm_loadu_ps(wD + 4);
        wD += 8;
        const __m128 vEx0123 = _mm_loadu_ps(wE);
        const __m128 vEx4567 = _mm_loadu_ps(wE + 4);
        wE += 8;
        const __m128 vFx0123 = _mm_loadu_ps(wF);
        const __m128 vFx4567 = _mm_loadu_ps(wF + 4);
        wF += 8;
