...
static XNN_INLINE vfloat32m1_t xnn_round_f32(vfloat32m1_t a, size_t vl) {
  // preserve NaN
  vbool32_t nan_bv = __riscv_vmfeq(a, a, vl);
  // magnitude < (1 << FLT_MANT_DIG)
  vfloat32m1_t mag = __riscv_vfabs(a, vl);
  vbool32_t mag_bv = __riscv_vmflt(mag, (1 << __FLT_MANT_DIG__), vl);
  vbool32_t mask_bv = __riscv_vmnand(nan_bv, mag_bv, vl);

  vint32m1_t a_rnd = __riscv_vfcvt_x(a, __RISCV_FRM_RNE, vl);
  vfloat32m1_t result = __riscv_vfcvt_f(a_rnd, vl);
  result = __riscv_vmerge(result, a, mask_bv, vl);
  return result;
}


void xnn_f32_vcos_ukernel__rvv_rational_5_4_div_u1v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat32m1_t vx_div_2pi = __riscv_vfmul(vx, v2pi_inv, vl);
    ...
    vfloat32m1_t vp = __riscv_vfadd(__riscv_vfmul(vx2, valpha_5, vl), valpha_3, vl);
