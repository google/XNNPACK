...
void xnn_f32_vcos_ukernel__rvv_poly_4_u4v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vx = __riscv_vfmacc_vf_f32m4(vx, vhalf_pi_0, vk_float, vl);
    vx = __riscv_vfmacc_vf_f32m4(vx, vhalf_pi_1, vk_float, vl);
    vx = __riscv_vfmacc_vf_f32m4(vx, vhalf_pi_2, vk_float, vl);

    vfloat32m4_t vk_quad = __riscv_vfadd(vk, 1.0f, vl);
    vuint32m4_t vu_quad = __riscv_vreinterpret_u32m4(vk_quad);
    vbool8_t vmask_odd = __riscv_vmseq(__riscv_vand(vu_quad, 1u, vl), 1u, vl);
    vuint32m4_t vsign_flip = __riscv_vsll(__riscv_vsrl(vu_quad, 1u, vl), 31u, vl);
