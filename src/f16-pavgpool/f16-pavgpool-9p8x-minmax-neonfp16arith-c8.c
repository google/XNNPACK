  ...
  do {
    {
      ...
      const __fp16* i8 = (const __fp16*) *input++;
      assert(i8 != NULL);
...

    {
      ...
      const float16x8_t vmultiplier = vld1q_dup_f16(multiplier); multiplier = (const __fp16*) multiplier + 1;

      size_t c = channels;
      const __fp16* b = (const __fp16*) buffer;
      while (c >= 8) {
        const float16x8_t vi0 = vld1q_f16(i0); i0 += 8;
        const float16x8_t vi1 = vld1q_f16(i1); i1 += 8;
        const float16x8_t vi2 = vld1q_f16(i2); i2 += 8;
