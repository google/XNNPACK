...
void xnn_f32_gavgpool_cw_ukernel__rvv_u8v(
    size_t elements,
    size_t channels,
    const float* input,
    float* output,
    const union xnn_f32_gavgpool_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  while (channels != 0) {
    int32_t fin = __riscv_vsetvl_e32m8(elements);
    vfloat32m8_t sum_f32v = __riscv_vfmv_v_f_f32m8(0.f, __riscv_vsetvl_e32m8(elements));

    size_t ele = elements;
    for( ; ele > 0; ){
      int32_t n =  __riscv_vsetvl_e32m8(ele); ele -= n;
      vfloat32m8_t i0_f32v = __riscv_vle32_v_f32m8(i0, n); i0 += n;
      sum_f32v = __riscv_vfadd_vv_f32m8_tu(sum_f32v, i0_f32v, sum_f32v, n);
    }
    vfloat32m1_t scl_f32v = __riscv_vfmv_s_f_f32m1(0.f, 1);
    scl_f32v = __riscv_vfredusum_vs_f32m8_f32m1(sum_f32v, scl_f32v, fin);
