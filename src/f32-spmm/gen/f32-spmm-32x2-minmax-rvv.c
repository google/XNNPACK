...
void xnn_f32_spmm_minmax_ukernel_32x2__rvv(
    size_t mc,
    size_t nc,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_minmax_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  size_t vector_length = max_vector_length < 32 ? max_vector_length : 32;
  ...
  while (mc >= 32) {
    ...
    while (n >= 2) {
      ...
      for (int i = 0; i < 32; i += vector_length) {
        input_ptr = input + i;
        w_ptr = w;
        dmap_ptr = dmap;
        uint32_t nnz = *nnzmap;
      ...
      dmap = dmap_ptr;
      nnzmap++;
      input = input_ptr - (32 - vector_length);
  ...
  if XNN_UNLIKELY(mc != 0) {
    vector_length = max_vector_length < 16 ? max_vector_length : 16;
    ...
    if (mc & 16) {
      ...
      while(n >= 2) {
        ...
        const float *w_ptr;
        const int32_t *dmap_ptr;
        for (int i = 0; i < 16; i += vector_length) {
          input_ptr = input + i;
          w_ptr = w;
          dmap_ptr = dmap;
          uint32_t nnz = *nnzmap;
        ...
        dmap = dmap_ptr;
        nnzmap++;
        input = input_ptr - (16 - vector_length);
        output = (float*) ((uintptr_t) output + output_stride * 2);
        n -= 2;
      }
      // fall back loop with nr = 1
    ...
    vector_length = max_vector_length < 8 ? max_vector_length : 8;
    ...
    if (mc & 8) {
      ...
      while(n >= 2) {
        ...
        const float *w_ptr;
        const int32_t *dmap_ptr;
        for (int i = 0; i < 8; i += vector_length) {
          input_ptr = input + i;
          w_ptr = w;
          dmap_ptr = dmap;
          uint32_t nnz = *nnzmap;
        ...
        dmap = dmap_ptr;
        nnzmap++;
        input = input_ptr - (8 - vector_length);
        output = (float*) ((uintptr_t) output + output_stride * 2);
        n -= 2;
      }
      // fall back loop with nr = 1
    ...
    vector_length = max_vector_length < 4 ? max_vector_length : 4;
