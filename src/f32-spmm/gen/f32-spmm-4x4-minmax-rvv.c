...
void xnn_f32_spmm_minmax_ukernel_4x4__rvv(
    size_t mc,
    size_t nc,
    const float* input,
    const float* weights,
  ...
  while (mc >= 4) {
    ...
    while (n >= 4) {
      ...
      for (int i = 0; i < 4; i += vector_length) {
        ...
        vfloat32m1_t acc2_f32v = __riscv_vfmv_v_f_f32m1(*w_ptr++, vector_length);
        vfloat32m1_t acc3_f32v = __riscv_vfmv_v_f_f32m1(*w_ptr++, vector_length);
        if XNN_LIKELY(nnz != 0) {
          do {
            ...
            acc2_f32v = __riscv_vfmacc_vf_f32m1(acc2_f32v, *w_ptr++, input_f32v, vector_length);
            acc3_f32v = __riscv_vfmacc_vf_f32m1(acc3_f32v, *w_ptr++, input_f32v, vector_length);
        ...
        vfloat32m1_t out2_f32v = __riscv_vfmin_vf_f32m1(acc2_f32v, max, vector_length);
        out2_f32v = __riscv_vfmax_vf_f32m1(out2_f32v, min, vector_length);
        __riscv_vse32_v_f32m1((float*) ((uintptr_t) output + output_stride * 2) + i, out2_f32v, vector_length);
        vfloat32m1_t out3_f32v = __riscv_vfmin_vf_f32m1(acc3_f32v, max, vector_length);
        out3_f32v = __riscv_vfmax_vf_f32m1(out3_f32v, min, vector_length);
        __riscv_vse32_v_f32m1((float*) ((uintptr_t) output + output_stride * 3) + i, out3_f32v, vector_length);
      }
      w = w_ptr;
      dmap = dmap_ptr;
      nnzmap++;
      input = input_ptr - (4 - vector_length);
      output = (float*) ((uintptr_t) output + output_stride * 4);
      n -= 4;
    }
