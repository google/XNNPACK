...
void xnn_indirection_init_conv2d(
  ...
  for (size_t output_tile_start = output_start; output_tile_start < output_end;
       output_tile_start += output_tile_size) {
    for (size_t output_tile_offset = 0; output_tile_offset < output_tile_size;
         output_tile_offset++) {
      const size_t output_index =
          min(output_tile_start + output_tile_offset, output_size - 1);
      const struct fxdiv_result_size_t output_y_x =
      ...
      for (size_t kernel_y = 0; kernel_y < kernel_height; kernel_y++) {
        ...
        if (input_y < input_height) {
          for (size_t kernel_x = 0; kernel_x < kernel_width; kernel_x++) {
            ...
            const size_t index = output_tile_start * kernel_size +
                                 kernel_index * output_tile_size +
                                 output_tile_offset;
...
void xnn_indirection_init_deconv2d(
    ...
    const size_t input_pixel_stride, const void* zero,
    ...
    const size_t padding_top, const size_t padding_left) {
  const size_t output_size = output_height * output_width;
  ...
  const struct fxdiv_divisor_size_t output_width_divisor =
      fxdiv_init_size_t(output_width);
  const struct fxdiv_divisor_size_t stride_height_divisor =
      fxdiv_init_size_t(stride_height);
  const struct fxdiv_divisor_size_t stride_width_divisor =
      fxdiv_init_size_t(stride_width);
...
void xnn_indirection_init_subconv2d(
  ...
  for (size_t offset_y = 0; offset_y < stride_height; offset_y++) {
    ...
    for (size_t offset_x = 0; offset_x < stride_width; offset_x++) {
      ...
      const size_t sliced_output_width =
      ...
      subconvolution_params->indirection_y_stride =
          ...
          round_up(sliced_output_width, output_tile_size);
      ++subconvolution_params;

      for (size_t output_y = output_y_start; output_y < output_height;
           output_y += stride_height) {
        for (size_t output_tile_start = 0;
             output_tile_start < sliced_output_width;
             output_tile_start += output_tile_size) {
          for (size_t kernel_y = offset_y; kernel_y < kernel_height;
               kernel_y += stride_height) {
            ...
            for (size_t kernel_x = offset_x; kernel_x < kernel_width;
                 kernel_x += stride_width) {
              for (size_t output_tile_offset = 0;
                   output_tile_offset < output_tile_size;
                   output_tile_offset++) {
                const size_t sliced_output_x =
                    min(output_tile_start + output_tile_offset,
                        sliced_output_width - 1);
                const size_t output_x =
                    output_x_start + sliced_output_x * stride_width;

                assert(doz(output_x + padding_left, kernel_x) % stride_width ==
                       0);
...
void xnn_indirection_init_dwconv2d_compressed(
  ...
  // to write any rows here, hence the additional check that output_y <
  // output_y_end. This allows callers to call this function to write
  ...
  for (size_t output_y = output_y_end - indirect_bot_height;
       output_y < output_y_end; output_y++, indirection_y++) {
    for (size_t kernel_y = 0; kernel_y < kernel_height; kernel_y++) {
      const size_t input_y = output_y * stride_height +
                             kernel_y * dilation_height - input_padding_top;
      if (input_y < input_height) {
  ...
  if (output_y_end == output_height) {
    const void* last_output_pixel =
        indirection_buffer[indirection_y * step_height - 1];
    const size_t last_kernel_index =
        indirection_y * step_height - kernel_height * kernel_width;
    for (size_t tile_index = kernel_height * kernel_width;
         tile_index < primary_tile; tile_index++) {
...
void xnn_indirection_init_dwconv2d(
  ...
  xnn_indirection_init_dwconv2d_compressed(
      output_y_start, output_y_end, indirection_buffer, input,
      input_pixel_stride, zero_buffer, input_height, input_width, output_height,
      output_width, kernel_height, kernel_width, stride_height, stride_width,
      dilation_height, dilation_width, input_padding_top, input_padding_left,
      step_height, step_width,
      /*indirect_top_height=*/output_y_end,
      /*indirect_bot_height=*/0, primary_tile);
}
  ...
  if (any_dilation) {
    ...
    for (size_t output_y = 0; output_y < output_height; output_y++) {
      for (size_t pooling_y = 0; pooling_y < kernel_height; pooling_y++) {
        ...
        size_t input_y = output_y * stride_height +
                         pooling_y * dilation_height - input_padding_top;
        ...
        for (size_t output_x = 0; output_x < output_width; output_x++) {
          for (size_t pooling_x = 0; pooling_x < kernel_width; pooling_x++) {
            ...
            size_t input_x = output_x * stride_width +
                             pooling_x * dilation_width - input_padding_left;
    ...
    for (size_t output_y = 0; output_y < output_height; output_y++) {
      for (size_t pooling_y = 0; pooling_y < kernel_height; pooling_y++) {
        const size_t input_y =
            min(doz(output_y * stride_height + pooling_y * dilation_height,
                    input_padding_top),
                input_y_max);
        for (size_t output_x = 0; output_x < output_width; output_x++) {
          for (size_t pooling_x = 0; pooling_x < kernel_width; pooling_x++) {
            const size_t input_x =
                min(doz(output_x * stride_width + pooling_x * dilation_width,
                        input_padding_left),
                    input_x_max);
            const size_t index = output_y * step_height +
                                 output_x * step_width * kernel_height +
...
void xnn_indirection_init_resize_bilinear2d_hwc_f16(
  ...
  const int32_t width_adjustment =
      (int32_t)(align_corners && output_width != 1);
  const int32_t height_adjustment =
      (int32_t)(align_corners && output_height != 1);
  const float width_scale = (float)((int32_t)input_width - width_adjustment) /
                            (float)((int32_t)output_width - width_adjustment);
  const float height_scale =
      (float)((int32_t)input_height - height_adjustment) /
      (float)((int32_t)output_height - height_adjustment);
  ...
  if (tensorflow_legacy || align_corners) {
    for (size_t output_y = output_y_start; output_y < output_y_end;
      ...
      const uint32_t input_y_bottom =
          math_min_u32(input_y_top + 1, input_y_max);
      ...
      for (size_t output_x = 0; output_x < output_width; output_x++) {
        ...
        const uint32_t input_x_right =
            math_min_u32(input_x_left + 1, input_x_max);
        const float alpha_x = input_x - (float)input_x_left;
        indirection_buffer[0] =
            (void*)((uintptr_t)input +
                    (input_y_top * input_width + input_x_left) *
                        input_pixel_stride);
        indirection_buffer[1] =
            (void*)((uintptr_t)input +
                    (input_y_top * input_width + input_x_right) *
                        input_pixel_stride);
        indirection_buffer[2] =
            (void*)((uintptr_t)input +
                    (input_y_bottom * input_width + input_x_left) *
                        input_pixel_stride);
        indirection_buffer[3] =
            (void*)((uintptr_t)input +
                    (input_y_bottom * input_width + input_x_right) *
                        input_pixel_stride);
    ...
    for (size_t input_y = 0; input_y < input_height; input_y++) {
      for (size_t pooling_y = 0; pooling_y < kernel_height; pooling_y++) {
        const size_t output_y =
            min(doz(input_y * kernel_height + pooling_y, output_padding_top),
                output_height - 1);
        for (size_t input_x = 0; input_x < input_width; input_x++) {
          for (size_t pooling_x = 0; pooling_x < kernel_width; pooling_x++) {
            const size_t output_x = min(
                doz(input_x * kernel_width + pooling_x, output_padding_left),
                output_width - 1);
            const size_t index =
...
void xnn_indirection_init_pavgpool2d_f16(
  ...
  for (size_t output_y = 0; output_y < output_height; output_y++) {
    ...
    const size_t input_y_end =
        min(doz(output_y * stride_height + pooling_height, padding_top),
            input_height);
    ...
    for (size_t output_x = 0; output_x < output_width; output_x++) {
      ...
      const size_t input_x_end =
          min(doz(output_x * stride_width + pooling_width, padding_left),
              input_width);
      const uint32_t input_x_range = (uint32_t)(input_x_end - input_x_start);
      *pixelwise_buffer++ = xnn_float16_from_float(
          1.0f / ((float)(int32_t)(input_y_range * input_x_range)));
    }
  }
}
