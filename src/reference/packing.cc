...
struct unaligned_int32_t {
  ...
  XNN_INLINE unaligned_int32_t(int32_t value)
      : value(value) {
  }
...
void transpose_weights_x8(const int8_t* in, int8_t* out,
...
void xnn_pack_kai_qs8_qc8w_weights_and_biases_sme2(
  ...
  const uint32_t kr = UINT32_C(1) << gemm_config->log2_kr;
  const uint32_t sr = UINT32_C(1) << gemm_config->log2_sr;
  const size_t rhs_stride = output_channels * sizeof(int32_t);
  ...
  if (flags & XNN_FLAG_TRANSPOSE_WEIGHTS) {
    ...
    printf("rhs_stride %zu\n", rhs_stride);
    printf("transpose nxk weights \n");
    for (int i = 0; i < output_channels; ++i) {
      for (int j = 0; j < input_channels; ++j) {
        printf("%d ", ((int8_t*)weights)[j + i * input_channels]);
      }
      printf("\n");
    }
    int8_t* tmp_data = (int8_t*)malloc(
        input_channels * output_channels * sizeof(int8_t));
    transpose_weights_x8((const int8_t*)weights, tmp_data,
                          output_channels, input_channels);
    printf("unpacked weights \n");
    for (int j = 0; j < input_channels; ++j) {
      for (int i = 0; i < output_channels; ++i) {
        printf("%d ", tmp_data[i + j * output_channels]);
      }
      printf("\n");
    }
    printf("baises \n");
      for (int i = 0; i < output_channels; ++i) {
        printf("%d ", ((int*)accumulator_init)[i]);
      }
      printf("\n");
    printf("scale \n");
      for (int i = 0; i < output_channels; ++i) {
        printf("%f ", ((float*)extra_data0)[i]);
      }
      printf("\n");
    kai_run_rhs_pack_kxn_qsi8cxp2vlx4sb_qs8cx_f32_i32_sme(
        groups, output_channels, input_channels, nr, kr, sr, rhs_stride/4,
        /*rhs=*/tmp_data,
        /*bias=*/accumulator_init,
        /*scale=*/extra_data0,
        /*rhs_packed=*/packed_weights_ptr,
        /*extra_bytes=*/0,
        &kai_params);
    int8_t *pw = (int8_t*)packed_weights_ptr;
    int32_t *pwb = (int32_t*)packed_weights_ptr;
    printf("packed weights \n");
    for (int i = 0; i < kai_get_rhs_packed_size_rhs_pack_kxn_qsi8cxp2vlx4sb_qs8cx_f32_i32_sme(output_channels, input_channels); ++i) {
      printf("%d ", (int)pw[i]);
    }
      printf("\n");
    printf("packed biases \n");
    for (int i = 0; i < kai_get_rhs_packed_size_rhs_pack_kxn_qsi8cxp2vlx4sb_qs8cx_f32_i32_sme(output_channels, input_channels)/4; ++i) {
      printf("%d ", (int)pwb[i]);
    }
      printf("\n");
    free(tmp_data);
