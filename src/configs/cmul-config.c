...
static void init_f16_cmul_config(void) {
  f16_cmul_config.ukernel =
      (xnn_vbinary_ukernel_fn)xnn_f16_vcmul_ukernel__neonfp16arith_u16;
}
#endif
...
static void init_f32_cmul_config(void) {
  ...
  if (hardware_config->use_arm_neon) {
    f32_cmul_config.ukernel =
        (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__neon_u8;
  } else if (!XNN_PLATFORM_MOBILE) {
    f32_cmul_config.ukernel =
        (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__scalar_u4;
  }
#elif XNN_ARCH_ARM64
  f32_cmul_config.ukernel =
      (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__neon_u8;
  ...
  if (!XNN_PLATFORM_MOBILE && hardware_config->use_x86_avx512f) {
    f32_cmul_config.ukernel =
        (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__avx512f_u32;
  } else
#endif
      if (hardware_config->use_x86_fma3) {
    f32_cmul_config.ukernel =
        (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__fma3_u16;
  } else {
    f32_cmul_config.ukernel =
        (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__sse_u8;
  }
#elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
  f32_cmul_config.ukernel =
      (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__wasmsimd_u8;
#elif XNN_ARCH_RISCV && XNN_ENABLE_RISCV_VECTOR
  const struct xnn_hardware_config* hardware_config =
      xnn_init_hardware_config();
  f32_cmul_config.ukernel =
      (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__rvv_u2v;
#else
  f32_cmul_config.ukernel =
      (xnn_vbinary_ukernel_fn)xnn_f32_vcmul_ukernel__scalar_u4;
#endif
