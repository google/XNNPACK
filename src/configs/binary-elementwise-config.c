...
static struct xnn_binary_elementwise_config s32_vmultiply_config = {0};
...
static struct xnn_binary_elementwise_config qu8_vmul_config = {0};
  ...
  static INIT_ONCE init_guard_s32_vmultiply = INIT_ONCE_STATIC_INIT;
  ...
  static pthread_once_t init_guard_s32_vmultiply = PTHREAD_ONCE_INIT;
...
static void init_s32_vmultiply_config(void) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon){
...
static void init_qu8_vmul_config(void) {
  ...
  static BOOL CALLBACK init_s32_vmultiply_config_windows(PINIT_ONCE init_once, PVOID parameter, PVOID* context) {
    init_s32_vmultiply_config();
    return TRUE;
  }
...
const struct xnn_binary_elementwise_config* xnn_init_s32_vmultiply_config() {
  const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
  if (hardware_config == NULL) {
    return NULL;
  }
  #if XNN_PLATFORM_WINDOWS
    InitOnceExecuteOnce(&init_guard_s32_vmultiply, &init_s32_vmultiply_config_windows, NULL, NULL);
  #else
    pthread_once(&init_guard_s32_vmultiply, &init_s32_vmultiply_config);
  #endif
  return &s32_vmultiply_config;
}
