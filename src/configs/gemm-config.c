...
static void init_pf32_gemm_config(void) {
  ...
  assert(hardware_config != NULL);
  //if (XNN_ENABLE_ARM_SME && hardware_config->use_arm_sme2) {
  if (hardware_config->use_arm_sme2) {
//#if XNN_ENABLE_ARM_SME
    printf("SME 2 FC\n");
    const size_t reg = 32;
    //pf32_gemm_config.minmax.gemm[XNN_MR_TO_INDEX(1)] = xnn_init_hmp_gemm_ukernel((xnn_gemm_ukernel_fn) xnn_f32_gemm_minmax_ukernel_32x32__neonsme2);
    pf32_gemm_config.minmax.gemm[XNN_MR_TO_INDEX(reg)] = xnn_init_hmp_gemm_ukernel((xnn_gemm_ukernel_fn) xnn_f32_gemm_minmax_ukernel_32x32__neonsme2);
    ...
    pf32_gemm_config.mr_packed = reg;
    pf32_gemm_config.mr = reg;
    pf32_gemm_config.nr = reg;
