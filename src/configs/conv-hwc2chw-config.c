...
static void init_f16_conv_hwc2chw_3x3c3s2_config(void) {
  ...
  if (hardware_config->use_arm_neon_fp16_arith) {
    ...
    f16_conv_hwc2chw_3x3c3s2_config.init.f16 =
...
const struct xnn_conv_hwc2chw_config*
xnn_init_f16_conv_hwc2chw_3x3c3s2_config() {
  const struct xnn_hardware_config* hardware_config =
      xnn_init_hardware_config();
  if (hardware_config == NULL ||
      !xnn_is_f16_chw_compatible_config(hardware_config)) {
    return NULL;
  }
...
const struct xnn_conv_hwc2chw_config*
xnn_init_f32_conv_hwc2chw_3x3c3s2_config() {
  const struct xnn_hardware_config* hardware_config =
      xnn_init_hardware_config();
  if (hardware_config == NULL ||
