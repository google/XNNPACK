...
static void init_f32_approxgelu_config_impl(struct xnn_unary_elementwise_config* f32_approxgelu_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_approxgelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vapproxgelu_ukernel__sse2fma_rational_12_10_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    f32_approxgelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vapproxgelu_ukernel__wasmsimd_rational_12_10_div_u12;
  #elif XNN_ARCH_HEXAGON && XNN_ENABLE_HVX
    f32_approxgelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vapproxgelu_ukernel__hvx_rational_12_10_div_u128;
  #else
...
static void init_f32_approxgelu_config() {
  init_f32_approxgelu_config_impl(&f32_approxgelu_config[0], false);
...
static void init_f32_cosine_config_impl(struct xnn_unary_elementwise_config* f32_cosine_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_cosine_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vcos_ukernel__sse2fma_rational_5_4_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
  ...
  #elif XNN_ARCH_HEXAGON && XNN_ENABLE_HVX
    f32_cosine_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vcos_ukernel__hvx_rational_5_4_div_u128;
  #else
...
static void init_f32_cosine_config() {
  init_f32_cosine_config_impl(&f32_cosine_config[0], false);
  init_f32_cosine_config_impl(&f32_cosine_config[CONSISTENT_CONFIG], true);
}

static void init_f32_elu_config() {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
...
static void init_f32_gelu_config_impl(struct xnn_unary_elementwise_config* f32_gelu_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_gelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vgelu_ukernel__sse2fma_rational_12_10_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    f32_gelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vgelu_ukernel__wasmsimd_rational_12_10_div_u12;
  #elif XNN_ARCH_HEXAGON && XNN_ENABLE_HVX
    f32_gelu_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vgelu_ukernel__hvx_rational_12_10_div_u128;
  #else
...
static void init_f32_gelu_config() {
  init_f32_gelu_config_impl(&f32_gelu_config[0], false);
  init_f32_gelu_config_impl(&f32_gelu_config[CONSISTENT_CONFIG], true);
}

static void init_f32_hswish_config_impl(struct xnn_unary_elementwise_config* f32_hswish_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_hswish_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vhswish_ukernel__sse2fma_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
...
static void init_f32_hswish_config() {
  init_f32_hswish_config_impl(&f32_hswish_config[0], false);
  init_f32_hswish_config_impl(&f32_hswish_config[CONSISTENT_CONFIG], true);
}

static void init_f32_exp_config_impl(struct xnn_unary_elementwise_config* f32_exp_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
  const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
  assert(hardware_config != NULL);
  if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_exp_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vexp_ukernel__sse2fma_rational_3_2_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    f32_exp_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vexp_ukernel__wasmsimd_rational_3_2_div_u12;
  #elif XNN_ARCH_HEXAGON && XNN_ENABLE_HVX
    f32_exp_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vexp_ukernel__hvx_rational_3_2_div_u128;
  #else
    f32_exp_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vexp_ukernel__scalar_rational_3_2_div_u4;
  #endif
}
...
static void init_f32_exp_config() {
  init_f32_exp_config_impl(&f32_exp_config[0], false);
  init_f32_exp_config_impl(&f32_exp_config[CONSISTENT_CONFIG], true);
}

static void init_f32_log_config_impl(struct xnn_unary_elementwise_config* f32_log_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_log_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vlog_ukernel__sse2fma_rational_3_3_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
...
static void init_f32_log_config() {
  init_f32_log_config_impl(&f32_log_config[0], false);
...
static void init_f32_sqrt_config_impl(struct xnn_unary_elementwise_config* f32_sqrt_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM64
    f32_sqrt_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vsqrt_ukernel__aarch64_neon_sqrt_u4;
      ...
      if (hardware_config->use_x86_avx) {
        ...
        f32_sqrt_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vsqrt_ukernel__sse2_sqrt_u8;
      }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
...
static void init_f32_sqrt_config() {
  init_f32_sqrt_config_impl(&f32_sqrt_config[0], false);
  init_f32_sqrt_config_impl(&f32_sqrt_config[CONSISTENT_CONFIG], true);
}

static void init_f32_rsqrt_config_impl(struct xnn_unary_elementwise_config* f32_rsqrt_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
...
static void init_f32_rsqrt_config() {
  init_f32_rsqrt_config_impl(&f32_rsqrt_config[0], false);
  init_f32_rsqrt_config_impl(&f32_rsqrt_config[CONSISTENT_CONFIG], true);
}

static void init_f32_sine_config_impl(struct xnn_unary_elementwise_config* f32_sine_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_sine_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vsin_ukernel__sse2fma_rational_5_4_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
...
static void init_f32_sine_config() {
  init_f32_sine_config_impl(&f32_sine_config[0], false);
  init_f32_sine_config_impl(&f32_sine_config[CONSISTENT_CONFIG], true);
}

static void init_f32_tanh_config_impl(struct xnn_unary_elementwise_config* f32_tanh_config, bool consistent_arithmetic) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
      f32_tanh_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vtanh_ukernel__neon_rational_9_8_div_u16;
    } else {
    ...
    if (hardware_config->use_x86_fma3) {
      ...
      f32_tanh_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vtanh_ukernel__sse2_rational_9_8_div_u8;
    } else {
      f32_tanh_config->ukernel = (xnn_vunary_ukernel_fn) xnn_f32_vtanh_ukernel__sse2fma_rational_9_8_div_u8;
    }
  #elif XNN_ARCH_WASMSIMD || XNN_ARCH_WASMRELAXEDSIMD
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
...
static void init_f32_tanh_config() {
  init_f32_tanh_config_impl(&f32_tanh_config[0], false);
...
const struct xnn_unary_elementwise_config* xnn_init_f32_approxgelu_config(uint32_t flags) {
  ...
  if (flags & XNN_FLAG_SLOW_CONSISTENT_ARITHMETIC) {
    ...
    return &f32_approxgelu_config[0];
  }
}
