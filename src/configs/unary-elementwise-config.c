...
static struct xnn_unary_elementwise_config s32_popcnt_config = {0};
...
static void init_s32_popcnt_config(void) {
  #if XNN_ARCH_ARM
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    if (hardware_config->use_arm_neon) {
      s32_popcnt_config.ukernel = (xnn_vunary_ukernel_fn) xnn_s32_vpopcnt_ukernel__neon_u4;
      s32_popcnt_config.element_tile = 1;
    } else if (!XNN_PLATFORM_MOBILE) {
      s32_popcnt_config.ukernel = (xnn_vunary_ukernel_fn) xnn_s32_vpopcnt_ukernel__scalar_u2;
      s32_popcnt_config.element_tile = 2;
    }
  #elif XNN_ARCH_ARM64
    s32_popcnt_config.ukernel = (xnn_vunary_ukernel_fn) xnn_s32_vpopcnt_ukernel__neon_u8;
