...
ExecutionPlan FP16SparseMobileNetV1(float sparsity, pthreadpool_t threadpool) {
  ...
  auto f32rng = std::bind(std::uniform_real_distribution<uint16_t>(-1.0f, +1.0f), std::ref(rng));
  ...
  status = xnn_create_convolution2d_nhwc_f16(
    ...
    w84.data(), w85.data(),
    -std::numeric_limits<uint16_t>::infinity() /* output min */, std::numeric_limits<uint16_t>::infinity() /* output max */,
    0 /* flags */,
    &caches,
    &op28);
  if (status != xnn_status_success) {
