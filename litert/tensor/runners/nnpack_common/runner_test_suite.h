...
TYPED_TEST_P(NnpackRunnerTest, ComputesRuntimeInputFromTensorHandle) {
  ...
  TensorType external_a(
      {.name = "external_a",
       .type = Type::kFP32,
       .shape = {2},
       .buffer = OwningCpuBuffer::Copy<Type::kFP32>({1.f, 2.f})});
  ...
  auto floats = std::move(bytes).template As<const float>();
  ASSERT_EQ(floats.size(), 2);
  EXPECT_FLOAT_EQ(floats.data()[0], 4.f);
  EXPECT_FLOAT_EQ(floats.data()[1], 6.f);
}
...
TYPED_TEST_P(NnpackRunnerTest, ReallocatesOwningCpuBufferOnReshape) {
  ...
  std::vector<float> b_data = {3.f, 4.f};
  ASSERT_THAT(runner.SetInputAsCopy(a, a_data), IsOk());
  ASSERT_THAT(runner.SetInputAsCopy(b, b_data), IsOk());
  ASSERT_THAT(runner.Run(), IsOk());
  ...
  std::vector<float> new_a = {1.f, 2.f, 3.f, 4.f};
  std::vector<float> new_b = {5.f, 6.f, 7.f, 8.f};
  ASSERT_THAT(runner.WriteInput(a, 0, new_a), IsOk());
  ASSERT_THAT(runner.WriteInput(b, 0, new_b), IsOk());
  ASSERT_THAT(runner.Run(), IsOk());

  LRT_TENSOR_ASSERT_OK_AND_ASSIGN(auto bytes, runner.ReadOutput(c));
  auto floats = std::move(bytes).template As<const float>();
  ASSERT_EQ(floats.size(), 4);
  EXPECT_THAT(floats, ::testing::ElementsAre(6.f, 8.f, 10.f, 12.f));
}
...
TYPED_TEST_P(NnpackRunnerTest, FailsWhenNonOwningViewTooSmall) {
  ...
  // Reshaping to 4 elements requires 16 bytes, but view only has 8 bytes. Run() must fail!
  ASSERT_THAT(runner.ReshapeInput(a, {4}), IsOk());
